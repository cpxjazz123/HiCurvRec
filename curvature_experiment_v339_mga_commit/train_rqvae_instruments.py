...
# v339 NOVEL: Möbius Gyrovector ADDITION commit (MGA) — 在 v337 MGC 基础上完全替换 commit path
# v337 commit (MGC): emb_out = α ⊗ (x ⊕ (emb ⊖ x))  (Möbius scalar mul)
# v339 commit (MGA): emb_out = (x ⊕ (emb ⊖ x)) ⊕ (γ ⊗ emb)  (Möbius ADD vector add)
# 数学差异:
#   MGC: commit 是标量乘 (只调 magnitude, 保留 direction)
#   MGA: commit 是向量加 (调 magnitude + direction, γ-scaled emb 加到 residual 上)
# 论文支撑: Ungar A.A. (2008) "Thomas precession: a key to hyperbolic geometry" / Ungar 2009/2010 Ch.4 Gyrovector 群论.
# 关闭 MGC (与 MGA 互斥), 启用 MGA, c=1 固定. R36n (e) 几何变换全新子方向. R36h ceiling 第 68 次验证.
USE_MGC = False                          # v339: 关 MGC (与 MGA 互斥)
USE_MGA = True                           # v339: enable MGA (Möbius ADD commit)
MOBIUS_ADDITION_GAMMA = 0.5              # v339: γ Möbius ADD weight (类似 MGC α=0.5 平衡点)
MOBIUS_GYROVECTOR_ALPHA = 0.5            # 保留基线参数 (MGC 关闭后无效, 但避免 type check fail)
USE_CYCLIC_CURVATURE_OVERRIDE = False    # v339: 关 cyclic (继承 v337)
...
def main():
    ...
    model = RqVae(
        ...
        use_mobius_addition=USE_MGA,  # v339: MGA (Möbius ADD commit) — 与 MGC 互斥
        mobius_addition_gamma=MOBIUS_ADDITION_GAMMA,  # v339: γ=0.5 Möbius ADD weight
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                # v339: MGA γ per-layer log (R36r #3 验证 mechanism 非 silent no-op)
                if USE_MGA:
                    try:
                        mga_gammas = [float(l.mobius_addition_gamma) for l in model.module.layers]
                        mga_gamma_str = "/".join(f"{g:.3f}" for g in mga_gammas)
                        mga_emb_norms = [float(l._last_mobius_emb_norm) for l in model.module.layers if hasattr(l, "_last_mobius_emb_norm")]
                        mga_out_norms = [float(l._last_mobius_out_norm) for l in model.module.layers if hasattr(l, "_last_mobius_out_norm")]
                        print(
                            f"  [v339 MGA] γ={mga_gamma_str} | "
                            f"emb_norm={[f'{n:.3f}' for n in mga_emb_norms]} | "
                            f"out_norm={[f'{n:.3f}' for n in mga_out_norms]}",
                            flush=True,
                        )
                    except Exception as e:
                        print(f"  [v339 MGA] log error: {e}", flush=True)
                if low_usage:
                    print(
