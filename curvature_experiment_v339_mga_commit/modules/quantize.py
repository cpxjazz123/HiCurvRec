...
class Quantize(nn.Module):
    def __init__(
        ...
        # v339 NOVEL: Möbius Gyrovector ADDITION commit (MGA) — Ungar 2008 gyrovector ⊕ 群运算
        # 替代 v337 MGC (α ⊗ res scalar mul) → MGA (res ⊕ (γ ⊗ emb) vector add).
        # 数学差异: MGC 只调 magnitude (标量乘), MGA 保留 direction (向量加, γ-scaled emb 加到 residual 上).
        # 论文支撑: Ungar A.A. (2008) "Thomas precession: a key to hyperbolic geometry" / 他 2009/2010 "Hyperbolic Geometry" Ch.4 Gyrovector 群论.
        # R36n (e) 几何变换全新子方向, R36h ceiling 第 68 次验证.
        use_mobius_addition: bool = False,  # v339: enable MGA commit
        mobius_addition_gamma: float = 0.5,  # v339: γ Möbius ADD weight (∈ (0, 1), γ=0 → residual-only, γ=1 → gyrocommutative)
    ) -> None:
        super().__init__()
        ...
        # v339: Möbius Gyrovector ADDITION commit (MGA) 配置
        self.use_mobius_addition = bool(use_mobius_addition)
        self.mobius_addition_gamma = float(mobius_addition_gamma)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        if self.training:
            if self.forward_mode == QuantizeForwardMode.GUMBEL_SOFTMAX:
                ...
                elif self.use_mobius_addition:
                    # v339 NOVEL: Möbius Gyrovector ADDITION commit (MGA) — Ungar 2008 ⊕ 群运算
                    # 数学:
                    #   Step 1: res = x ⊖ emb (Möbius subtraction, 同 v337)
                    #   Step 2: commit = res ⊕ (γ ⊗ emb) (Möbius ADD γ-scaled emb)
                    # 关键差异 vs MGC (v337):
                    #   MGC (v337): commit = α ⊗ res → 标量乘 (只调 magnitude, 保留 direction)
                    #   MGA (v339): commit = res ⊕ (γ ⊗ emb) → 向量加 (调 magnitude + direction)
                    # γ 含义:
                    #   γ=0 → commit = res = x ⊖ emb (only residual)
                    #   γ=1 → commit = res ⊕ emb (gyrocommutative product)
                    #   γ=0.5 → 平衡 (类似 MGC α=0.5)
                    # 期望突破 R36h ceiling: 不同的 commit path 可能产生不同的 SID 子空间.
                    from modules.hyperbolic import _mobius_add_t, _mobius_scalar_mul_t
                    c = self.get_c()
                    c_scalar = float(c.item()) if c.dim() == 0 else float(c.mean().item())
                    c_t = torch.tensor(c_scalar, device=emb.device, dtype=emb.dtype)
                    # Step 1: res = x ⊖ emb = x ⊕ (-emb)
                    minus_emb = -emb
                    res = _mobius_add_t(x, minus_emb, c_t)  # x ⊕ (-emb)
                    # Step 2: γ ⊗ emb (Möbius scalar mul)
                    gamma = self.mobius_addition_gamma
                    gamma_emb = _mobius_scalar_mul_t(emb, gamma, c_t)
                    # Step 3: commit = res ⊕ (γ ⊗ emb) (Möbius ADD)
                    emb_out = _mobius_add_t(res, gamma_emb, c_t)
                    # Step 4: STE backward (同 v337 模式)
                    emb_out = emb_out.detach() + x - x.detach()
                    # log 验证
                    self._last_mobius_alpha = gamma  # 复用 _last_mobius_alpha 字段
                    self._last_mobius_c = c_scalar
                    self._last_mobius_emb_norm = float(emb.norm(dim=-1).mean().item())
                    self._last_mobius_out_norm = float(emb_out.norm(dim=-1).mean().item())
                else:
