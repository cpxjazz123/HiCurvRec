...
\usepackage{times}            % Task #100: cleaner Times Roman font for submission
...
\usepackage{listings}
\usepackage{xcolor}
\usepackage{fancyhdr}        % Task #100: page header/footer
% authblk omitted: not installed in current TeX Live. Author block stays inline \author{}.
\setlength{\headheight}{14pt}  % fancyhdr default requires at least 13.6pt
\fancyhf{}
\fancyhead[L]{HG-Rec Reproduction --- Musical\_Instruments}
\fancyhead[R]{\thepage}
\pagestyle{fancy}
...
\title{Hyperbolic RQ-VAE enhanced Generative Recommendation with Differential-Length Codebook: An Independent Reproduction on Amazon Musical\_Instruments}
...
\subsection{1.1 Background: Generative Recommendation and Codebook-based Tokenization}
...
\subsection{1.2 The HG-Rec Approach}
...
\subsection{1.3 Our Reproduction Motivation}
...
**Our reproduction setting** is single-dataset, single-seed (seed = 42), single embedding (sentence-T5-base, 768d), single codebook size ($[64, 128, 256]$): **Amazon Musical\_Instruments** (9922 items, 511,836 interactions).


\subsection{1.4 Our Key Findings Preview}

Five independent lines of evidence show that **on Amazon Musical\_Instruments, the hyperbolic geometry hypothesis of HG-Rec is only marginally beneficial — and a simpler baseline (vanilla RQ-VAE + Sinkhorn, which we call *phonism*) matches or exceeds HG-Rec variants on test Recall@10**.
...
**Bottom line:** on Musical\_Instruments, the **mechanism of codebook utilization** (Sinkhorn-balanced post-processing) is more impactful than **the geometry of distance computation** (hyperbolic vs Euclidean).


\subsection{1.5 Contributions}

1. **Independent reproduction on Amazon Musical\_Instruments.** We verify the relative ranking HG-Rec > Letter > TIGER > SASRec, but document that all 8 paper-reported baselines exceed our reproduced numbers by 18\%–61\%, indicating a systematic dataset/protocol gap.
...
\[d_{\mathbb{B}}(\mathbf{x}, \mathbf{y}) = \frac{2}{\sqrt{c}} \arctanh\!\left(\sqrt{c}\,\|(- \mathbf{x}) \oplus_c \mathbf{y}\|\right),\quad\text{(1)}\]
where the Möbius addition is
\[\mathbf{x} \oplus_c \mathbf{y} = \frac{(1 + 2c\langle \mathbf{x}, \mathbf{y}\rangle + c\|\mathbf{y}\|^2)\,\mathbf{x} + (1 - c\|\mathbf{x}\|^2)\,\mathbf{y}}{1 + 2c\langle \mathbf{x}, \mathbf{y}\rangle + c^2\|\mathbf{x}\|^2 \|\mathbf{y}\|^2}.\quad\text{(2)}\]
...
\[\log_o^c(\mathbf{h}) = \frac{1}{\sqrt{c}} \operatorname{arctanh}\!\left(\sqrt{c}\,\|\mathbf{h}\|\right) \frac{\mathbf{h}}{\|\mathbf{h}\|}.\quad\text{(3,4)}\]
...
\subsection{3.1 Hyperbolic RQ-VAE}


\subsubsection{3.1.1 Latent semantic embedding extraction}
...
\subsubsection{3.1.2 Hyperbolic residual quantization}
...
\[\mathbf{r}_{\ell-1}^{\mathbb{B}} = \exp^{\,c}_o(\mathbf{r}_{\ell-1}), \quad \mathbf{e}^{\mathbb{B}}_{\ell,i} = \exp^{\,c}_o(\mathbf{e}_{\ell,i}),\quad\text{(5)}\]
and select the codeword index that minimizes the hyperbolic distance:
\[c_\ell = \arg\min_{i \in \{1,\ldots,K_\ell\}} d_{\mathbb{B}}\bigl(\mathbf{r}_{\ell-1}^{\mathbb{B}}, \mathbf{e}^{\mathbb{B}}_{\ell,i}\bigr).\quad\text{(6)}\]


\subsubsection{3.1.3 Residual updates in the tangent space (stability fix)}
...
\[\mathbf{r}_{\ell-1} = \log^{\,c}_o(\mathbf{r}_{\ell-1}^{\mathbb{B}}), \quad \mathbf{e}_{\ell, c_\ell} = \log^{\,c}_o(\mathbf{e}^{\mathbb{B}}_{\ell, c_\ell}),\quad\text{(7)}\]
and obtain the next-layer residual through simple Euclidean subtraction in the tangent space:
\[\mathbf{r}_\ell = \mathbf{r}_{\ell-1} - \mathbf{e}_{\ell, c_\ell}.\quad\text{(8)}\]
...
\subsection{3.2 Differential-Length Codebook Strategy}


\subsubsection{3.2.1 Motivation}
...
\subsubsection{3.2.2 Volume growth in the Poincaré ball (Theorem 3.4)}
...
\[Vol_{\mathbb{B}}(\rho) = \omega_{n-1} \int_0^\rho \bigl(\sinh(\sqrt{c}\, t)\bigr)^{n-1} dt,\quad\text{(10)}\]
...
\subsection{3.3 Model Training and Inference}


\subsubsection{3.3.1 Tokenization (offline)}
...
\subsubsection{3.3.2 Generative model training}
...
\[\mathcal{L} = - \sum_{u=1}^{|\mathcal{U}|}\;\sum_{t=1}^{|C^{\mathrm{out}}_u|} \log\, p\bigl(C^{\mathrm{out}}_{u,t} \,\big|\, C^{\mathrm{out}}_{u,<t},\, C^{\mathrm{in}}_u\bigr).\quad\text{(13)}\]


\subsubsection{3.3.3 Inference via beam search}

At inference time, the decoder autoregressively generates the target token tuple using **beam search** (beam=20, max\_len=20), then resolves the generated code back to the recommended item via a fixed item-code lookup.


\subsection{3.4 Discussion}

The two components operate on orthogonal axes: **Hyperbolic RQ-VAE** modifies the *latent space* (Euclidean → Poincaré), while **Differential-Length Codebook** modifies the *capacity allocation* ($K_\ell = K_1 \gamma^\ell$). On Musical\_Instruments, both components show only marginal benefit, with vanilla + Sinkhorn post-processing matching or exceeding their combined effect.
...
\subsection{4.1 Sequential Recommendation}
...
\subsection{4.2 Generative Recommendation}
...
\subsection{4.3 Hyperbolic Methods in Recommendation}
...
\subsection{4.4 RQ-VAE and Codebook Techniques}
...
\subsection{4.5 Reproduction Methodology for Geometric Priors}
...
**Amazon Musical\_Instruments** (5-core filtering):
...
\subsection{5.3 Codebook Architecture Ablation (Table 3)}
...
\subsection{5.4 Per-Layer Curvature Ablation (Table 4)}
...
\subsection{5.5 Codebook Decomposition (Tables 5-6)}
...
\subsection{5.6 Training Dynamics (Table 7)}
...
\subsubsection{5.7.1 Is Hyperbolic Geometry Necessary?}

Five independent lines of evidence consistently indicate that **Musical\_Instruments is effectively Euclidean**:
...
\subsubsection{5.7.2 Mechanism vs Geometry}
...
\subsubsection{5.7.3 Generalization Beats Fitting}
...
\subsection{6.1 Theoretical Implications}

Five lines of evidence (§5.7) consistently indicate **Amazon Musical\_Instruments is effectively Euclidean**. The strongest is free-curvature learning: 18/18 (layer, $\kappa_m$) converge to exactly 0.000000 after 1000 epochs despite $\kappa_{\max}=2$ exploration space. This is not optimization failure (loss converges normally to \textasciitilde{}1.10) but a gradient signal of zero.
...
1. **Explicit hierarchical structure** (taxonomy-based catalogs). Musical\_Instruments lacks this.
...
For datasets satisfying all three (DBpedia, Yelp with taxonomy), hyperbolic geometry remains valuable. For datasets satisfying none (Musical\_Instruments), simpler architectures are not only sufficient but preferable.


\subsection{6.2 When Does Hyperbolic Geometry Actually Help?}
...
| Short-history sequential | Musical\_Instruments | ❌ | ❌ Weak | ❌ (our finding) |
...
\subsection{6.3 Mechanism vs Geometry}
...
2. Single-dataset evaluation (Musical\_Instruments) — transferability uncertain.
...
\subsection{6.6 Concluding Thoughts}

Our work demonstrates that **simpler is often better** for generative recommendation on flat datasets. Vanilla RQ-VAE + Sinkhorn achieves best test R@10 (0.1058) on Musical\_Instruments, statistically tied with HG-Rec c555 (0.1051) but with **better generalization** (+0.0204 gap). For practitioners building recommendation systems on similar datasets, we recommend:
...
This reproduction was carried out independently. We thank the authors of HG-Rec (Zhang et al., 2026) for releasing the original paper and methodology, which made this reproduction possible. The Stage 1--4 pipeline uses the open-source framework \texttt{snap-research/GRID} (TIGER pipeline backbone, Apache 2.0 license). Computational resources were provided by the institutional GPU cluster (4$\times$ NVIDIA L40S). No code was shared with the original HG-Rec authors.
...
% BibTeX entries are defined in papers/refs.bib (Task #100).
% Inline references are kept below for self-contained submission
% (no bibtex / xelatex auto-resolution required).
% To switch to BibTeX mode: \bibliographystyle{plainnat} + \bibliography{refs} + run bibtex.
...
*This paper was produced via independent reproduction. No code was shared with the original HG-Rec authors. All experiments reproducible with seed=42 on Amazon Musical\_Instruments (5-core filter, leave-one-out protocol).*
