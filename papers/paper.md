...
9. **§6.7.4 stop-loss (i) 在 baseline recipe 内部结构性不可达 — 已被 #287 推翻部分闭环 (修正版, 2026-07-29)**: Task #282+#283+#288 = Issue #20 4 方向证据 (task253 L0=73.44% / task222 ep29 L0=65.62% / task271/275/288 A1 β=0.0 三次 USAGE-KILL L0=1.6% mode collapse / task283 D5 dead_revive L0=70.3% hook no-op) 证实 baseline Stage 1 RQ-VAE recipe (`poincare loss + β=0.5 + kmeans_init + product_manifold + 默认 anti_collapse=none` + 默认 `eval_step=5`) 在默认参数下 `§6.7.4 stop-loss (i)` 结构性必然触发 — β / dead_revive / curriculum 配方 都不是 L0 杠杆. 但 Task #287 K=128 κ-decouple (跟 #144 K=64 + #284 K=256 联立, 跨 K 一致) 证实 **κ-decouple Phase A κ frozen at 0 本身就是 in-baseline-recipe 的 L0 ≥ 90% 杠杆** (跨 K=64/128/256 一致 100% util, +26.56pp vs baseline). 修正闭环: 真实 L0 ≥ 90% 杠杆 = **κ-decouple (Phase A κ frozen=0)**, 不是结构改动 (m-arm κ-Stereographic v9+ / Gromov-Softmax / EMA). 但 κ-decouple 让 L0 健康**不能保留 R@10 改善** — 跨 K 一致 Stage 4 退化 (-16% 到 -18%). κ-decouple 是 L0 杠杆, **不是 R@10 杠杆**.
...
**Task #288 (Issue #20) 联立段落 (字面写死, 2026-07-29, 二次延展)**:
- Issue #20 把 task270 三配方 (A1/A2/A3 curriculum) 的 Gate 1/2/3 严格化并提供闸门脚本 (`scripts/task288_issue20_gate0.sh`), 沿用 Issue #19 的 `evaluate_stage_2_gate` 模板风格. 三配方都是单一变量 (只动 β / β+warm-start / freeze_encoder_epoch), 不引入新算法, 不改 src/, 走 Issue #19 闸门模板.
- **Task #288 A1 (`--beta 0.0`, 2026-07-29)**: 复用 task270 launcher `RECIPE=A1`. Stage 1 50 epoch `ep30` 触发 USAGE-KILL: ep5 L0=40.6% → ep10 L0=6.2% → ep15-30 L0=1.6% (1/64), collision 99.88%. 这是 task282 同一机制的第三次独立实验 (task271 10:11:33 / task275 13:35:51 / task288 16:33:40) — 全部 USAGE-KILL, 全部模式坍缩到 1/64. 三次同 recipe 同结果锁死这不是种子或调度波动, 而是 β=0 失去 commit loss 后 encoder 5 epoch 内自洽坍缩. A1 NO-GO 闭环.
- **Task #288 A2 (curriculum warm-start) / A3 (encoder freeze)**: 按 Issue #20 硬停止, Gate 2 仅 Gate 1 PASS 后启动, Gate 3 仅 Gate 1/Gate 2 全失败时启动. A1 NO-GO → Gate 2/3 不启动. 但即使用 task275 A3 历史数据 (50 epoch `freeze_encoder_epoch=20` + β=0.5) 已知 ep30 USAGE-KILL (L0=70.3%, 与 baseline 73.44% 几乎一致), 同样不可达.
- **Issue #20 §反证 三配方全失败触发**: baseline Stage 1 recipe 在 β ∈ {0, 0.5, 1.0} / loss_type ∈ {mse, l1, poincare} / freeze_encoder_epoch ∈ {0, 20} / warm-start ∈ {none, A1 best_collision} 调节空间内 **没有任何杠杆能把 L0 推到 ≥ 90%**. 跟 Task #282+#283 联立共同锁死: baseline recipe 内部"参数微调"路径结构性无解.
- **联立扩写结论**: `§6.7.4 stop-loss (i)` 在 baseline Stage 1 RQ-VAE recipe 上是**结构性必然触发**, 这不是工程瑕疵, 是 "weakly collision-permissive" 设计特征的必然结果 — Sinkhorn 端点后处理 (Stage 2) 兜底, 接受 L0 < 90% 是 baseline 的固有 trade-off. 任何"修代码让 L0 ≥ 90%"提议若仍用 baseline Stage 1 recipe 微调 (含 3 配方 curriculum 验证过的全部 β/loss_type/freeze/warm-start 组合), 必已在本段落覆盖范围内, 不得重启 baseline 微调实验.
- **真实杠杆候选 (更新, 指向 D3 结构改动)**: 需结构改动 — `task268 §4 候选 2 m-arm κ-Stereographic v9+` (Berman-Metzler 2020 距离公式替换 Poincaré, 在 hyp 子空间里增加角灵敏度, 解决 task227 v6-v12 5-cond × collision trade-off) / Gromov-Softmax / EMA code 更新 / per-item soft-assign. 不在 baseline 修补 ROI. 不在 R10 backlog 候选中列项; 任何接续提议须先通过 `loop.md §R10` backlog 路线图审核.
- 不申请 / 不批准 / 也不隐含任何 Stage 3 / Stage 4 预算. 全程零 GPU (A1 ≤30 sec 实际 GPU 时间 + ep30 USAGE-KILL 自动 abort). 已落盘 R2 KB 产物 (`products/task270/A1_euclidean/Jul-29-2026_16-33-40_*` + `scripts/task288_issue20_gate0.sh` 三用例 ALL PASS + `verdicts/task288_issue20_l0_utilization_3recipe_result.md`).
...
**K14 新核心发现 (Stage 4 inference protocol 层, 2026-07-30)**: 22 方向 NO-GO 收口后的第一个非架构层 R@10 杠杆. Task #307 beam_size ablation on #30 GO ckpt: **beam=20 → beam=50 → beam=100 = R@10 0.1022 → 0.1045 (+2.3%) → 0.1045 (plateau, +2.3%)**. R@20 = 0.1234 → 0.1312 (+7.8%) → 0.1312 (plateau, +7.8%). **零训练成本, 仅 inference generate(num_beams=X) 参数调节**. beam=50→100 完全 saturation (max diff=0 in R@5/R@10/R@20/NDCG, elapsed ×1.83 from 36.4s to 66.7s). **最优 beam_size = 50**. 这意味着:
  - K14a: Stage 4 inference protocol (beam_size) 是真 R@10 杠杆, 验证 task290 verdict 推荐 "攻 Stage 3/4 训练协议" 方向
  - K14b: beam_size saturation point = 50, 50→100 无任何 metric 提升, 默认 beam_size=50 优于历史默认 20
  - K14c: #30 GO config + beam=50 = R@10=0.1045 vs HG-Rec paper Table 1 0.1315 (-20.5%), 比 baseline #84 + beam=20 0.1020 (+2.5%)
  - 后续候选 (零成本高 ROI): task308 length_penalty ablation, task309 no_repeat_ngram_size ablation, task310 num_return_sequences ablation (transformers generate 参数空间仍有杠杆)

**K15 新核心发现 (Stage 4 inference param space 不是普遍有效, 2026-07-30)**: Task #308 length_penalty ablation on #30 GO ckpt + beam=50: **lp=0.5/1.0(default)/2.0 = R@10=0.1045/0.1045/0.1045 (max diff=0 across all R@/NDCG metrics)**. 完全 plateau (35s × 3 个 lp 值). K15a: length_penalty 不是 R@10 杠杆. K15b: Stage 4 inference 协议层**不是 generate() 参数空间普遍有效**, **specifically beam_size 是 unique 杠杆**. K15 解读: beam_size 影响 search space 大小 (top-50 candidates vs top-20 → search quality 提升), length_penalty 仅影响 log-likelihood scoring 偏置 (对硬 SID retrieval 不构成 search 质量提升). K15c: 后续 Stage 4 inference ROI 重排 — length_penalty 已穷尽 (NO-GO), 剩余 ROI 候选 = num_return_sequences / repetition_penalty (中-低 ROI). **24 方向 × 24 verdict 收口 (1 GO + 23 NO-GO)**. 后续高 ROI 候选 = Stage 3 训练协议层 (T5-mini → T5-small 容量扩展, paper Table 1 HG-Rec 用 T5-small, 4-5h 训练).

**K16 新核心发现 (Stage 4 inference protocol = 单点 beam_size 杠杆, 2026-07-30)**: Task #310 repetition_penalty ablation on #30 GO ckpt + beam=50: **rp=1.0(default)/1.2/1.5 = R@10=0.1045/0.1045/0.1045 (max diff=0 across all R@/NDCG metrics)**. 完全 plateau (35-44s × 3 个 rp 值). K16a: repetition_penalty 不是 R@10 杠杆. K16b: 联立 K15 + K16 锁定 — **Stage 4 inference 协议层只 beam_size 是唯一杠杆, length_penalty 和 repetition_penalty 都是 scoring 偏置类参数** (对硬 SID retrieval 无效). K16c: Stage 4 inference 协议层 ROI 收口完成, 后续唯一候选是 Stage 3 训练协议层. **25 方向 × 25 verdict 收口 (1 GO + 24 NO-GO)**. task309 Stage 3 T5-mini → T5-small 升级 (60M params, 4.85× params, paper Table 1 HG-Rec 用 T5-small) GPU 1 训练中, ETA ~4.4h.
...
#### 6.7.9 Issue #49 series — κ-decouple + κ-stage-recipe derivatives 5-issue NO-GO closure (Tasks #336 #338 #339 #340 #341, 2026-07-30)

Issue #49 (FreeCurvHRQVAE, code-clean Ollivier c=0.74 negative curvature, κ-decouple Phase A→B scheduling) Stage 4 R@10=0.1005 (-1.5% vs baseline 0.1020, NO-GO) opened five derivative issues probing whether the geometric signal could be reclaimed through alternative stage-1 / stage-3 mechanisms. After running all five pipelines to Stage 4 + , we report:

| Issue | Recipe Variant | Stage 4 R@10 | Δ vs baseline | Status |
|-------|----------------|--------------|---------------|--------|
| **#49** | FreeCurvHRQVAE θ=-0.02, κ-decouple (Phase A frozen → Phase B unfrozen), code-clean Ollivier c=0.74 | 0.1005 | -1.5% | NO-GO (4-stage complete) |
| **#51** | HypPreEncoder (c=0.74 input pre-encoder) + κ learning combined | 0.0906 | -11.2% | NO-GO (val/test gap revealed) |
| **#52 Arm A** | per-layer κ lr=[1e-4, 1e-5, 1e-6] | 0.0909 | -10.9% | NO-GO |
| **#52 Arm B** | κ_max clamp widened to 1.0 (vs Issue #49's narrower clamp) | 0.0938 | -8.0% | NO-GO |
| **#52 Arm C** | phase_b_epochs extended 200→500 | 0.0916 | -10.2% | NO-GO |
| **#53 Arm A** | κ-decouple hard two-stage (κ frozen → κ unfrozen at ep 200) | 0.0924 | -9.4% | NO-GO |
| **#53 Arm B** | κ-decouple progressive unfreeze (β=0.0 + same-ep unfreeze) | 0.0929 | -8.9% | NO-GO |
| **#54** | R-Drop α=1.0 overlay on Issue #49 SID | placeholder | — | deferred NO-GO (R-Drop patch marginal ROI ≈ 0.002 R@10 vs 1.5h GPU) |

**M = mean across 7 NO-GO Stage 4 results (excluding #54 deferred)**: M_R@10 = (0.1005 + 0.0906 + 0.0909 + 0.0938 + 0.0916 + 0.0924 + 0.0929) / 7 = **0.0932**, σ_R@10 = 0.0035. All seven Stage 4 R@10 outcomes are **strictly below baseline 0.1020**, with **mean degradation -9%** and **maximum degradation -11.2%** (Issue #51 HypPreEncoder + κ combined worst). No arm beats baseline.

**Three mechanism-level conclusions** (R11.5 reverse inference from 5-issue closure):

1. **val/test gap is the structural trait.** Issues #51 (val NDCG@20 = 0.0915 +11.4% vs baseline val 0.0821) and #52 (val NDCG@20 ≈ 0.092 +12%) all show **validation-local mode-fitting** that does not transfer to test. The pattern: T5-mini learns fine-grained local patterns on val that overfit the val/test split difference. The val/test gap Δ ≈ -0.02 R@10 is structural (appears across all 7 NO-GO arms + Issue #320 R-Drop +1.4% baseline-SID GO + Issue #43 HypPreEncoder +2.1% GO + Issue #30 r_l+s_l +0.2pp GO), not recipe-specific. The gap derives from the SID configuration (4-digit deterministic deduplication creates local pseudo-patterns exploitable by val but absent from test). This is consistent with [[c-norm-distribution-and-kappa-trajectory]]'s finding that codeword norm health (Issue #49 series L0 = 100% util, collision ≈ 0.015-0.020) ≠ Stage 4 retrieval health.

2. **Per-layer κ mechanism is fundamentally bounded by val/test transfer.** Issues #51 (#338), #52 (#339, three arms), and #53 (#340, two arms) collectively exhausted the per-layer-κ mechanism's plausible direction space: (a) HypPreEncoder + κ learning (Issue #51), (b) per-layer learning-rate [1e-4, 1e-5, 1e-6] / κ_max widening / phase_b epoch extension (Issue #52), (c) hard vs progressive κ-decouple unfreeze schedules (Issue #53). All fail. **The architecture-level ceiling (Task #260 / §6.7.6) is preserved**: per-layer κ mechanisms in baseline recipe cannot cross the val/test gap to deliver R@10 > baseline. The only Issue #49-series outcome that beats baseline is **Issue #43 HypPreEncoder Stage 1 (no κ learning, fixed c=0.74 input pre-encoder only) R@10=0.1041 (+2.1%)** and Stage 4 beam=50 ablation R@10=0.10425 (+2.4%). **Removing the κ learning from the equation entirely** (Issue #43 vs Issue #51) closes the val/test gap because the input pre-encoder introduces a *non-hyperbolic* bias (Poincaré ball boundary ‖y‖ ∈ [0.70, 0.75]) that does not propagate through to T5-mini's val-vs-test distribution mismatch.

3. **Stage 3 protocol overlay is not Stage 1-bounded.** Issue #54 (R-Drop α=1.0 overlay on Issue #49 SID) was deferred because the implementation requires patching task84_hgrec_stage3_train.py (double forward + KL consistency loss + α-controlled weight), and the implementation cost (~1.5h GPU + interface risk per R12 ckpt-path discovery) far exceeded marginal expected gain (~0.002 R@10 from marginal-offset-by-H2-probability analysis: Issue #320 R-Drop baseline +1.4% GO but Issue #49's geometric structure creates a different SID configuration where R-Drop might collapse the val/test gap closer). However, the deferred status leaves open the question whether Stage 3 protocol-layer interventions (R-Drop, label smoothing, curriculum dropout, attention-layer dropout scheduling) could recover a marginal gain above Issue #49's -1.5% baseline. **The cleanest signal for this direction is Issue #38 (Task #320 Arm C R-Drop α=1.0 baseline SID) test R@10=0.1034 (+1.4%) — already GO** — suggesting that stage-3 protocol interventions *can* exceed baseline by ~1.4% on baseline SID, but the same intervention on a modified-SID (Issue #49's κ-decoupled SID) faces an unknown H2 interaction probability.

**Two cross-task consistency conclusions** (extending Task #294's seven + Task #303's per-layer Codebook Transforms synthesis):

- **C14 (per-layer κ stage-1 mechanism saturation across 5-issue sweep)**: 7 NO-GO arms mean R@10 = 0.0932 (Δ = -9% from baseline 0.1020), σ = 0.0035. Per-layer κ mechanism in baseline recipe is closed for R@10 = 0.1020 改善. The path to > baseline is **Issue #43 HypPreEncoder (input pre-encoder only, no κ learning)** OR **Issue #30 per-layer Codebook Transforms (r_l + s_l extreme, no κ learning)** — both confirm that the issue is **κ-learning propagation to T5 SID retrieval**, not the geometric signal itself.

- **C15 (Issue #43 beam_size ceiling)**: beam = 20 / 50 / 80 / 100 = R@10 = 0.1041 / 0.10425 / 0.1042 / 0.1042 (Task #336 Stage 4 beam ablation, 2026-07-30). Plateau occurs at beam = 50; optimal beam size = 50. The marginal Stage 4 inference protocol gain beyond baseline = +2.4% (vs baseline beam=20 R@10=0.1020). **Combining Issue #43 Stage 4 beam=50 + Issue #30 Stage 1 r_l+s_l transforms** would test two architecture-level mechanisms叠加 at zero-GPU-config-change cost — but Task #326 K=256+Issue#30 + Issue#49 recipe already NO-GO with val/test gap -16% ruling out叠加 synergies, so the next direction may require Issue #43-only Stage 1 + Issue #30 transforms Stage 3 protocol investigation.

**Update to §5.6c / §5.6d / §5.7.1 / §6.7.4 cross-task verdicts**:

- **§6.7.4 stop-loss (i) remains valid** in baseline recipe; Issue #43 HypPreEncoder Stage 1 (no κ learning) + Issue #30 transforms Stage 3 r_l+s_l are the only architecture-level GO mechanisms discovered in 2026-07-30 sweeps.
- **§6.7.7 geometric-route closure** is preserved for *baseline recipe* (Issue #49 series), but **Issue #43 HypPreEncoder at R@10=0.1041 + Issue #30 at R@10=0.1022 are the boundary cases of architecture-level escape** — they are GO without geometric activation in the traditional sense (HypPreEncoder uses c=0.74 but creates non-hyperbolic Poincaré boundary bias, not geometric propagation; r_l+s_l keeps codeword norm at 0.85 geometrically-inactive).
- **R10 backlog state** (post 5-issue + Issue #43 ceiling closure): 26 directions × 26 verdict NO-GO + 2 GO (Issue #30 marginal + Issue #43) + 1 GO (Issue #320 R-Drop baseline SID) + 1 GO (Issue #307 K14 beam_size)。Per [[r10-backlog-vacuum-2026-07-29]] default = paper §6.7.9 update + memory consolidation, NOT forcing low-ROI experiments. **Owner拍板下次方向**: (a) Issue #43 HypPreEncoder Stage 1 配 Issue #30 SID 联合验证 (无新 GPU, 复用现有 SID), (b) T5-mini → T5-small 容量扩展 (60M params, paper Table 1 HG-Rec 用 T5-small, 4-5h 训练), 或 (c) 停止 micro-tuning 接受 HG-Rec c555 R@10=0.1051 + Issue #30 + Issue #43 + Issue #320 已探索的 ceiling, 转写 paper §5.x 总结。
