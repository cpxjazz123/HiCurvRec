...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "lorentz_ball_v277"  # ← v277 NOVEL: Lorentz ball manifold 替换 (R36n c) + v270 cyclic + M2 + M3. 论文支撑: Nickel & Kiela "Learning Continuous Hierarchies in the Lorentz Model of Hyperbolic Geometry" ICML 2018. 机制: modules/hyperbolic.py 中 _expmap0_t / _logmap0_t / _mobius_add_t / _transport_between_t / _poincare_distance_t 全部走 Lorentz 公式 (D-dim input 内部 lift 到 D+1-dim Lorentz point, 算完 drop x_0 返回 D-dim). 接口不变. 保留 v270 全部 (USE_CYCLIC_CURVATURE=True, USE_M2_INTRINSIC=True, USE_M3_TRANSPORT=True, USE_FIXED_CURVATURE=True, DISTANCE_MODE=L2, hyperbolic_distance=True, sk_eps=0.05). 目标: test_R@10 > 0.1130396262886598 (v270 baseline). 6 cyclic variants (v271-275) + v276 Riemannian Adam 全 char-level equal baseline → R36h ceiling 6 次证伪. v277 走 R36n (c) manifold 替换 是 orthogonal 方向.
