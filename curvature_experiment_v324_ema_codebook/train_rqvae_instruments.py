...
# v324 NOVEL: EMA codebook update (van den Oord NeurIPS 2017 VQ-VAE 原始 Section 3)
# 机制: codebook embedding 用 exponential moving average 替代 gradient descent 更新
#   N_i ← γ·N_i + (1-γ)·count_i
#   m_i ← γ·m_i + (1-γ)·Σ_{x assigned to i} x
#   e_i ← m_i / (N_i + ε)
# **关键**: encoder 仍 gradient descent; 仅 codebook 走 EMA (commitment_loss 仍反向到 encoder)
# **论文支撑**: van den Oord et al. "Neural Discrete Representation Learning" NeurIPS 2017 (VQ-VAE 原始论文)
# **R36v3.6**: Stage 1 端非曲率架构变更, 字面 PASS
# **R36n 灰色地带**: 不属 R36n 6 大方向, 但 R36v3.6 允许 Stage 1 任意变更
# **R36r 预防**: EMA N_i/m_i 是 buffer (非 gradient), commitment_loss 正常反向, ckpt MD5 ≠ baseline
USE_EMA_CODEBOOK = True          # v324: 启用 EMA codebook update
EMA_GAMMA_START = 0.99           # v324: 起始 γ (训练初期慢更新)
EMA_GAMMA_END = 0.999            # v324: 终止 γ (训练后期慢更新)
EMA_DECAY_STEPS = 100_000        # v324: γ 从 start → end 所需 step 数 (覆盖全 100k 训练)
EMA_EPS = 1e-5                   # v324: 除零保护
...
def main():
    ...
    model = RqVae(
        ...
        # v324 NOVEL: EMA codebook update (van den Oord NeurIPS 2017 VQ-VAE Section 3)
        use_ema_codebook=USE_EMA_CODEBOOK,
        ema_gamma=EMA_GAMMA_START,  # 初始 γ (training loop 内按 EMA_DECAY_STEPS 调度)
        ema_eps=EMA_EPS,
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # v324: 更新 EMA γ (γ: EMA_GAMMA_START → EMA_GAMMA_END over EMA_DECAY_STEPS)
            ema_progress = min(1.0, global_step_sync / EMA_DECAY_STEPS)
            ema_gamma_t = EMA_GAMMA_START + (EMA_GAMMA_END - EMA_GAMMA_START) * ema_progress
            model.module.set_ema_gamma(ema_gamma_t)
