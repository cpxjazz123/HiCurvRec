...
class Quantize(nn.Module):
    def __init__(
        ...
        # v324 NOVEL: EMA codebook update 参数
        use_ema_codebook: bool = False,    # v324: 用 EMA 替代 gradient descent 更新 codebook
        ema_gamma: float = 0.99,           # v324: EMA decay (γ_start → γ_end 由 train 端 schedule)
        ema_eps: float = 1e-5,             # v324: EMA 更新中的 ε 防止除零
    ) -> None:
        super().__init__()
        ...
        # v324 NOVEL: EMA codebook update (van den Oord NeurIPS 2017 VQ-VAE 原始论文 Section 3)
        # 机制: codebook embedding 不参与 gradient descent, 用 exponential moving average 更新
        #   N_i ← γ·N_i + (1-γ)·count_i  (per-code 激活次数)
        #   m_i ← γ·m_i + (1-γ)·Σ_{x assigned to i} x  (per-code sum of assigned latents)
        #   e_i ← m_i / (N_i + ε)
        # γ decay schedule: γ_start (0.99) → γ_end (0.999) over EMA_DECAY_STEPS
        # **关键**: commitment_loss 仍反向到 encoder (encoder 仍 gradient descent), 仅 codebook 走 EMA
        # **论文支撑**: van den Oord et al. "Neural Discrete Representation Learning" NeurIPS 2017
        # **R36v3.6**: Stage 1 端非曲率架构变更, 字面 PASS; **R36n 灰色地带**: 非 6 大方向
        # **R36r 预防**: EMA 更新必须非 silent no-op (e_i 用 N_i, m_i 反向; 不能 detach commitment_loss)
        self.use_ema_codebook = bool(use_ema_codebook)
        self.ema_gamma = float(ema_gamma)
        self.ema_eps = float(ema_eps)
        # EMA N_i, m_i buffers (注册到 state_dict, 不参与 grad)
        # 注意: buffers 用 register_buffer 让 DDP 跨 rank 同步 (R51+ 6 确定性约束)
        self.register_buffer('_ema_N', torch.zeros(n_embed))
        self.register_buffer('_ema_m', torch.zeros(n_embed, embed_dim))
    ...
    def set_ema_gamma(self, gamma: float) -> None:
        """v324: 更新 EMA decay γ (rank 0 在每个 training step 调用).
        γ schedule: γ_start (0.99) → γ_end (0.999) over EMA_DECAY_STEPS.
        同步到所有 rank 需要 dist.broadcast — 由调用方负责.
        """
        self.ema_gamma = float(gamma)

    def _ema_codebook_update(self, x_flat: Tensor, ids: Tensor) -> None:
        """v324: EMA codebook update (van den Oord NeurIPS 2017 VQ-VAE 原始 Section 3).
        ...
        **关键**:
          - EMA 更新在 self.training=True 时触发
          - x_flat 必须 detach (EMA 更新不应流入 encoder gradient)
          - encoding (ids) 必须 detach (argmin 已 detach)
          - 不与 TCU 共存 (use_ema_codebook=True 时 use_tcu 必须 False, 防止双重更新)
        **R36r 预防**: commitment_loss 仍正常反向到 encoder, 不被 detach (silent no-op 风险)
        """
        if not self.use_ema_codebook or self.hypervq:
            return
        # x_flat: (B, D), ids: (B,) 都已经 detach
        K = self.n_embed
        D = self.embed_dim
        # 1. count_i: per-code activation count
        count = torch.zeros(K, dtype=x_flat.dtype, device=x_flat.device)
        count.scatter_add_(0, ids.long(), torch.ones_like(ids, dtype=x_flat.dtype))
        # 2. sum_i: per-code sum of assigned latents (B, K, D) → (K, D)
        # one-hot (B, K) @ x (B, D) → (K, D)
        one_hot = torch.zeros(x_flat.shape[0], K, dtype=x_flat.dtype, device=x_flat.device)
        one_hot.scatter_(1, ids.long().unsqueeze(1), 1.0)
        sum_x = one_hot.t() @ x_flat  # (K, D)
        # 3. EMA 更新 N_i, m_i (in-place, 但不通过 autograd — EMA 不应流入 gradient)
        with torch.no_grad():
            gamma = self.ema_gamma
            # 首次初始化 (从 KMeans init 后): 直接覆盖, 不用 EMA
            if not bool(self._ema_initialized.item()):
                self._ema_N.copy_(count)
                self._ema_m.copy_(sum_x)
                self._ema_initialized.fill_(True)
            else:
                self._ema_N.mul_(gamma).add_(count, alpha=1.0 - gamma)
                self._ema_m.mul_(gamma).add_(sum_x, alpha=1.0 - gamma)
            # 4. 更新 embedding: e_i = m_i / (N_i + ε)
            new_emb = self._ema_m / (self._ema_N.unsqueeze(1) + self.ema_eps)
            # 5. 写回 self.embedding.weight (in-place)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v324: EMA codebook update (use_ema_codebook) — 与 TCU 互斥 (use_ema_codebook=True 时 use_tcu 必须 False)
        if self.training and self.use_ema_codebook and not self.hypervq:
            self._ema_codebook_update(x.detach(), ids.detach())
