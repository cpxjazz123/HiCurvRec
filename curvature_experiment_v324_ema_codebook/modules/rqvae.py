...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        use_ema_codebook: bool = False,    # v324: 用 EMA 替代 gradient descent 更新 codebook (非曲率变更)
        ema_gamma: float = 0.99,            # v324: EMA decay (γ_start → γ_end schedule)
        ema_eps: float = 1e-5,              # v324: EMA 除零保护
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    use_ema_codebook=use_ema_codebook,  # v324: EMA codebook update
                    ema_gamma=ema_gamma,
                    ema_eps=ema_eps,
                )
    ...
    def set_ema_gamma(self, gamma: float) -> None:
        """v324: 把 EMA decay γ 广播到所有 Quantize 层.
        γ schedule: γ_start (0.99) → γ_end (0.999) over EMA_DECAY_STEPS.
        训练脚本每个 step 调用一次 (rank 0 计算, 然后 dist.broadcast 或各 rank 同步算).
        """
        for layer in self.layers:
            layer.set_ema_gamma(gamma)
