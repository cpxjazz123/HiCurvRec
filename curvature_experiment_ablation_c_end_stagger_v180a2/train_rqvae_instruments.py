...
C_START = 0.05                            # 初始 c (近欧氏, 训练稳定) — v180: 所有层统一 c_start
C_END = 0.7                               # v161 baseline default (v180 per-layer 覆盖)
CURRICULUM_STEPS = 50_000                 # 50k 步 ramp up (总 100k 步, 后半段稳定)

# === v180: Per-Layer c_end curriculum stagger (R36n (b)) ===
# c_start 全局统一 0.05; c_end per-layer: L0=0.5 (浅, 信息容量小, KMeans 主导)
# L1=0.6 (中), L2=0.7 (深, 信息容量最大, 双曲几何充分). 三层 c_end 阶梯递增.
...
def main():
    ...
    model = RqVae(
        ...
        hyperbolic_distance=True,   # v161 启用 Poincaré 距离 (让 Sinkhorn-OT 在双曲空间运作)
        sk_eps=0.05,               # v161 Sinkhorn-Knopp 温度 0.05 (10x 平滑 vs v159 0.005, 更均匀分配)
    ...
    # v180: Per-Layer c_end curriculum stagger (c_start 统一, c_end per-layer 阶梯)
    if USE_PER_LAYER_C_END:
        per_layer_c_end = [L0_C_END, L1_C_END, L2_C_END]
        for li, layer in enumerate(model.module.layers):
            layer.c_end = float(per_layer_c_end[li])
            if rank == 0:
                print(f"  [v180] L{li} override: c_end={per_layer_c_end[li]:.3f}", flush=True)
