...
# === v211 NOVEL: per-layer c_end ladder [0.5, 0.6, 0.7] + per-layer codebook size [320, 256, 192] 双机制 ===
# v207 ladder (c 容量差异化) + v210 size (code 容量差异化) 组合. R36c Round 7.
# 期望: 双维度 per-layer 异质 capacity 协同突破 L0 死码 (历史 v207 ladder 单独 L0=17/256, v210 size 单独 L0=28/320).
# 论文支撑: R36n (b) per-item / per-layer / per-codebook 异质 capacity. c 维度 + size 维度双 ladder.
USE_PER_LAYER_C_END = True
PER_LAYER_C_END = [0.5, 0.6, 0.7]  # L0 低 (近欧氏, 易扩展) → L1 中 → L2 高 (双曲, 信息密)
USE_PER_LAYER_CODEBOOK_SIZE = True
PER_LAYER_CODEBOOK_SIZE = [320, 256, 192]  # L0 +25%, L2 -25%, KMeans 应可行
...
USE_FIXED_CURVATURE = False               # v211: 关固定曲率, 让 c_end_per_laye r ladder 生效
C_FIXED = 1.0                             # 固定曲率值 (HG-Rec, 仅当 USE_FIXED=True 时生效)
...
C_END = 0.7                               # v211 fallback (per-layer 启用时此项被覆盖)
...
def main():
    ...
    model = RqVae(
        ...
        # v211: per-layer c_end ladder + per-layer codebook size ladder (双机制组合)
        c_end_per_layer=PER_LAYER_C_END if USE_PER_LAYER_C_END else None,
        codebook_size_per_layer=PER_LAYER_CODEBOOK_SIZE if USE_PER_LAYER_CODEBOOK_SIZE else None,
    ).to(device)
