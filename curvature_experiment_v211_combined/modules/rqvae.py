...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v211 NOVEL: per-layer codebook size ladder + per-layer c_end ladder
        # 双机制组合 (v207 ladder + v210 size): c + size 双维度 per-layer 异质 capacity
        codebook_size_per_layer: Optional[List[int]] = None,  # v211: 各层独立 codebook size
        c_end_per_layer: Optional[List[float]] = None,       # v211: 各层独立 c_end (curriculum ramp 终点)
    ) -> None:
        self._config = locals()
        ...
        # v211: per-layer codebook size ladder + per-layer c_end ladder
        if codebook_size_per_layer is not None:
            assert len(codebook_size_per_layer) == n_layers, f"codebook_size_per_layer length {len(codebook_size_per_layer)} != n_layers {n_layers}"
        ...
        self.codebook_size_per_layer = codebook_size_per_layer
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    embed_dim=embed_dim,
                    n_embed=(codebook_size_per_layer[i] if codebook_size_per_layer is not None else codebook_size),  # v211: per-layer size ladder
                    ...
                    c_start=c_start,
                    c_end=(c_end_per_layer[i] if c_end_per_layer is not None else c_end),  # v211: per-layer c_end
