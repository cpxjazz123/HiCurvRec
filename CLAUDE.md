...
> HG-Rec (Hyperbolic RQ-VAE + Differential-Length Codebook + T5) 流水线复现工作目录, **Musical_Instruments 9922 items, 4 阶段流水线, seed=42**, **当前 curvature_base baseline = v282 Geodesic Midpoint Commit (test_R@10=0.1135631443298969, R36h ceiling 三次突破 +0.46% vs v270)**.
...
|---|---|---|---|---|---|---|
| **v282 Geodesic Midpoint Commit** (新 baseline, 2026-08-28 promoted from curvature_experiment_v282_geodesic_midpoint_commit, R36n e 几何变换 variant #3 — commit 路径走测地线中点 mid = exp_0(0.5*log_0(res) + 0.5*log_0(emb), c), 替代 v262 Möbius 减法, 保留 v270 全部配置 cyclic+M2+M3, R36h ceiling 三次突破 +0.46% vs v270 0.11304, 2 RUN 字符级完全一致 0.1135631443298969, utility 247/249/249 健康, Stage 3 best ckpt E111/E112, n_eval=24832, R51+ 6 确定性约束全开) | ✅ R37 PASS | **0.0966** | **0.1135631443298969** | **0.14316204896907217** | **0.08940618062576725** | `curvature_base/out/decoder/instruments_hgrec_configs/hgrec_geodesic_midpoint_v282/best_ckpt.pt` |
...
**R5** — 任务硬约束 = v282 Geodesic Midpoint Commit baseline (test_R@10=0.1135631443298969), 4 阶段流水线 seed=42, R51+ 6 确定性约束全开 (旧 v270/v262 baseline 已备份到 scratch, 数值不再视为当前基线).
