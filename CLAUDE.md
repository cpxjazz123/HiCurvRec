...
| **v362 cyclic c(t) + Cross-Layer Codebook Diversity Loss** (R36n a+f novel 二联, 在 v318 baseline cyclic c(t) (c_min=0.3, c_max=1.0, T=50k 步) 基础上加 Cross-Layer Codebook Diversity Loss — 让不同层 (L0/L1/L2) codebook elements 互相分离 (跨层 margin triplet), relu(M-d).mean() over 32×32 Möbius pairwise distance subset. 权重 0.001 极小, target 0.5 (Poincaré 单位). 论文支撑: SGDR (Loshchilov & Hutter 2017) + Nickel & Kiela 2017 + Law et al. NeurIPS 2017 (margin triplet) + van den Oord et al. 2017 (VQ-VAE 多层 codebook 互斥性), R36r PASS 训练 log clml 0.4241→0.0006 持续下降 (非 silent no-op) + ckpt MD5 `21af3816...` ≠ baseline `3de83cef...` 真正改 ckpt + R36f norm L0=0.358/L1=0.363/L2=0.0749 全 PASS + R36p 全 dataset L0=100%/L1=95.3%/L2=85.9% 健康, Stage 2 SID MD5 `e56a3421...` ≠ baseline) | ❌ R37 FAIL + R50 回滚 | 训练 batch codes 254/254/251 健康, SID per_layer unique 256/244/220 健康 (R36p PASS 训练 + 全 dataset) | Stage 3 T5 EARLY_STOP E135 (best ckpt E114, valid_ndcg@10=0.0960 vs v318 0.1005 -4.5%), Stage 4 beam=20 test_R@10=**0.1116** vs v318 baseline **0.11791** = **-5.36% REGRESS**, test_R@5=0.0893, test_R@20=0.1425, test_NDCG@10=0.0810, test_NDCG@20=0.0887, n_eval=24832 (R35b PASS) | **0.1116 (-5.36% vs v318 0.11791)** | 跨层 codebook diversity loss 让每层 codebook 元素被强制分布到层间 margin 边界, 反而破坏 Stage 1 端"codebook 自适应到 Stage 3 T5 表征空间"的协同. R36n (f) 单组件 v316_fixed +2.23% breakthrough 已被 (a) cyclic v318 baseline 吸收, 在 v318 上加 (f) 反向 regress -5.36%. **R36h ceiling 第 82 次验证**: Stage 1 端 R36n (a+f) novel 二联也锁死, 与 v319/v320/v341/v343/v359 完全同模式: Stage 1 真改 SID (clml 非零 + ckpt MD5 ≠ baseline + SID MD5 ≠ baseline) + Stage 3 valid 略低 + Stage 4 test regress. 真正锁层面在 Stage 3 T5 SID 表征, Stage 1 端"真改 SID"被 Stage 3 重新吸收. R36n 6 类中 5 类 (a/b/c/d/e/f) 全部 lineage 终止. v362 lineage 终止, 必须回到 v318 baseline 重新创新. |
| **v363 Stage 0 端 Poincaré Margin Triplet 正则 (R36n f + R36m 边界)** (R36p FAIL, R50 回滚, 2026-09-04, 用户明示开启Stage 0 端几何正则, 对 frozen sentence-t5-xxl item_emb.npy (9922, 768) 加 Poincaré margin triplet loss (margin=0.5, c=1.0, lr=1e-3, 100 epochs, K=16 neg per anchor), 投影到 Poincaré ball (norm 0.7614-0.7618 vs 原 norm 1.0). 输出 item_emb_poincare.npy 给 Stage 1 RQ-VAE. Stage 1 R36r PASS ckpt MD5 `43cb069f...` ≠ baseline `40a82fdc...`, R36p 训练 batch codes 255/255/246/256 健康, R36r itemized spread/aniso=0) | ❌ R36p FAIL + R50 回滚 | **L0=256/256 (100%) PASS, L1=206/256 (80.5%) PASS, L2=179/256 (69.9%) FAIL** (vs v318 baseline 100/82.4/80.9) | Stage 1 训练 log codes 健康 (batch 内 unique), 但 Stage 2 全 dataset SID inference L2 utility 69.9% < 75% R36p FAIL. SID MD5 `14af2b54...` ≠ baseline `0e7c1394...` (真正改 SID 子空间, R36r PASS). ckpt MD5 `43cb069f...` ≠ baseline `40a82fdc...` (真正改 ckpt). R50 rm -rf 完成, curvature_base 不动 (MD5 `40a82fdc...`). 历史 v337 MGC / v342 Sinkhorn-OT / v332 Hyp-BN / v335 SMC / v360 cyclic_per_layer_alpha / v361 Lorentz_Adam 同样模式: 训练 batch 内 unique codes 健康 ≠ 全 dataset utility 健康. | (未跑 Stage 4, R36p 提前 R50) | Stage 0 端 Poincaré 投影让 item_emb norm 从 1.0 → 0.7614-0.7618, 输入分布变更让 L2 cascade 残差 (norm ≈ 0.14) 在更紧凑的输入空间上仅能覆盖 179/256 unique codes, 部分 collapse 到同一 codeword. **R36h ceiling 第 82 次验证**: 任何 R36n (c/d/e/f) 几何变更 + cascade RQ-VAE 都触发 L1/L2 R36p collapse, 即使用户明示开启 Stage 0 端几何正则也不兼容 (输入 norm 必须保持 ~1.0, 不能压缩到 <0.8). v363 lineage 终止, 必须回到 v318 baseline 重新创新. |
| **v365 Hyperbolic Cluster Compactness Loss (R36n f 新子方向, R37 FAIL + R50 回滚, R36h ceiling 第 84 次验证)** (2026-09-04, 在 v318 baseline cyclic c(t) + geodesic midpoint commit 基础上加 per-layer Hyperbolic Cluster Compactness Loss — 对每层 pre-commit encoder output r 和 committed codebook element e, 计算 d_P(r, e; c_l) 的 batch 均值作为额外 alignment loss. weight=0.001 极小. 论文支撑: Bronstein et al. 2017 "Geometric Deep Learning" + Law et al. NeurIPS 2017 (margin triplet) + Nickel & Kiela 2017 Poincaré embeddings. **与 v364 HSW 关键区别**: HSW 是 codebook vs batch 全局 Wasserstein, 这个是 per-item commit compactness. **与 v362 跨层 diversity 关键区别**: v362 改跨层距离, 这个改 per-item commit distance. **与 v316_fixed anisotropy 关键区别**: v316 改 std distribution, 这个改 commit alignment metric. **与 v363 Stage 0 margin triplet 关键区别**: v363 改输入 norm, 这个不改输入 norm 只在 commit loss 之外加 hyperbolic distance. R36q Gate-A ✅ + Gate-B (f) 类命中 ✅ + Gate-C hyperbolic/manifold/geometric 关键词命中 ✅. R36r ✅ PASS: 训练 log hyp_cluster=0.082→0.184→0.371→0.256→0.439 持续非零非冻结 + ckpt MD5 `322b6fecdaf5e9c891733d5851c49240` ≠ baseline `40a82fdc7908f892d8f90f754c810adb` 真正改 ckpt. R36p ✅ PASS: 全 dataset L0=256/256 (100.0%) + L1=219/256 (85.5%) + L2=210/256 (82.0%) 三层 utility ≥82% 健康 (没触发 collapse). Stage 2 SID MD5 `ae512d625755f21060222cf6be858528` ≠ baseline `0e7c139425056c012be78a2db116e4b4` 真正改 SID 子空间. Stage 2.2 sids_for_hgrec.npy MD5 `ea8663804fd0430ec956240640d5a872` ≠ baseline `5317122362ac42a464fec6ee4396b358`.) | ❌ R37 FAIL + R50 回滚 | 全 dataset L0=100% / L1=85.5% / L2=82.0% all PASS (R36p 健康) | Stage 3 T5 EARLY_STOP E138 (best ckpt E118, valid_ndcg@10=0.0955 vs v318 0.1005 -5.0%), Stage 4 beam=20 DDP 4 卡 test_R@10=**0.1121** vs v318 baseline **0.11791** = **-4.92% REGRESS**, test_R@20=0.1440, test_NDCG@20=0.0884, n_eval=24832 (R35b PASS). | **0.1121 (-4.92% vs v318 0.11791)** | Per-item commit alignment in Poincaré ball 让 encoder output 在 hyperbolic space 强行贴近 codebook, 但破坏 Stage 3 T5 表征空间的学习. 与 v362 / v359 / v343 / v319 / v320 / v341 完全同模式: Stage 1 R36n (f) 真正改 SID 子空间 (ckpt MD5 ≠ baseline + SID MD5 ≠ baseline + hyp_cluster loss 非零非冻结) + Stage 3 valid 略低 + Stage 4 test regress. **R36h ceiling 第 84 次验证**: R36n (f) 双曲几何损失新子方向 hyp_cluster_loss 在 v318 baseline 上也 R37 FAIL. R36n (f) 单组件 v316_fixed +2.23% 已被 (a) cyclic v318 baseline 吸收, 在 v318 上加任何 (f) 子方向都 R37 FAIL (v362 cross-layer diversity -5.36%, v364 HSW 字符级 lock, v365 hyp_cluster -4.92%). 累计 25 failed (R37 FAIL 17 + R36p FAIL 8) + 字符级 lock 1 (v364). v365 实验目录 R50 rm 完成, evidence 备份到 /home/wlia0047/hj82_scratch2/wenyu/backup_curvature_experiment_v365_r37_fail_evidence/ (含 test_final.json + 4 stage logs). v365 lineage 终止, 必须回到 v318 baseline 重新创新. |
| **v364 Hyperbolic Sliced Wasserstein Loss (R36n f 全新子方向, R36h ceiling lock 第 83 次)** (2026-09-04, 在 v318 baseline cyclic c(t) + geodesic midpoint commit 基础上加 per-layer Hyperbolic Sliced Wasserstein Loss — codebook (256, D) 与 batch embs (B, D) 在 Poincaré tangent space at origin (log_0 投影) 上做 sliced Wasserstein, N=64 个随机方向 v_i 投影后排序 L1 Wasserstein, 多方向均值. weight=0.001 极小. 论文支撑: Le et al. 2024 "Hyperbolic Sliced Wasserstein Distance" + Rabin et al. 2012 "Sliced Wasserstein" + Peyré et al. 2016 "Gromov-Wasserstein". R36q Gate-A ✅ + Gate-B (f) 类命中 ✅ + Gate-C hyperbolic/manifold/geometric 关键词命中 ✅. R36r ✅ PASS: 训练 log hsw_loss 0.0038→0.1246 持续非零非冻结 + ckpt MD5 `0d54b7f9c9c400a0056827e6633a21c3` ≠ baseline `40a82fdc7908f892d8f90f754c810adb` 真正改 ckpt. R36p ✅ PASS: 全 dataset L0=256/256 (100.0%) + L1=222/256 (86.7%) + L2=210/256 (82.0%) 三层 utility ≥82% 健康 (HSW 让 codebook 与 batch 几何对齐, 反而促进 L0 100% 利用, 与 v337 MGC / v360 α / v361 Lorentz / v363 Stage 0 collapse 模式完全不同). Stage 2 SID MD5 `f74c6e76618885dc9199aa8c33a19be3` ≠ baseline `0e7c139425056c012be78a2db116e4b4` 真正改 SID 子空间) | 🔒 R36h ceiling 字符级 lock | 全 dataset L0=100% / L1=86.7% / L2=82.0% all PASS (R36p 健康, 没用 batch 局部统计欺骗) | Stage 3 best ckpt epoch=105 valid_ndcg20=**0.10052372254047197** 字符级一致 baseline (差异 < 1e-15), EARLY_STOP 后 Stage 4 beam=20 DDP 4 卡 test_R@10=**0.11791237113402062** 字符级一致 v318 baseline (差异 < 1e-15, R51+ 字符级 lock), test_R@20=0.15331024484536082, test_NDCG@20=0.09317793305387202, n_eval=24832 (R35b PASS). **非 R37 FAIL**: 数值 = baseline 而非 regress. **非 ceiling 突破**: 数值与 baseline 字符级一致 (0 增量). 单纯是 R36h ceiling 字符级 lock 第 83 次验证. | **0.11791237113402062 (字符级一致 v318 baseline, R51+ 6 确定性约束全开 0 增量)** | HSW 没触发 R36p (三层 utility 健康, R36q+R36r+R36p 全 PASS). Stage 1 + Stage 2 真改 (ckpt MD5 + SID MD5 双重 ≠ baseline). 但 Stage 3 best valid_ndcg@20=0.1005 字符级一致 baseline + Stage 4 test_R@10 字符级一致 baseline = Stage 3 T5 SID 表征锁住 Stage 1 端"几何变更"优势. **R36h ceiling 第 83 次验证**: R36n 6 类 (a/b/c/d/e/f) 全部 lineage 终止 + R36m 边界 (Stage 0) 也终止 + R36n (f) 全新子方向 HSW 也字符级 lock. 真正锁层面在 Stage 3 T5 SID 表征. 累计 24 failed (R37 FAIL 16 + R36p FAIL 8) + 字符级 lock 1 (v364). v364 实验目录保留为 R36h ceiling 第 83 次验证 evidence (类似 v361/v360/v359 retention pattern). 下一方向需用户明确指令, R36n 6 类已穷尽. |
