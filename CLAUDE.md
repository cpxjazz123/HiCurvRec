...
> HG-Rec (Hyperbolic RQ-VAE + Differential-Length Codebook + T5) 流水线复现工作目录, **基线 Task #84 valid R@10=0.1267 / test R@10=0.1024 (Musical_Instruments 9922 items)**, **当前 curvature_base baseline = v282 Geodesic Midpoint Commit (R36n e 几何变换 variant #3), test_R@10=0.1135631443298969 (R36h ceiling 三次突破 +0.46% vs v270, 2026-08-28 promoted from curvature_experiment_v282_geodesic_midpoint_commit)**.

---

## 最新测试结果 (2026-08-28 v282 promoted 为新 baseline + 历史 SOTA, beam=20)

| 流水线 | 状态 | valid R@10 | valid NDCG@20 | **test R@10** | test R@20 | test NDCG@20 | ckpt |
|---|---|---|---|---|---|---|---|
| **v282 Geodesic Midpoint Commit** (新 baseline, 2026-08-28 promoted from curvature_experiment_v282_geodesic_midpoint_commit, R36n (e) 几何变换 variant #3 — commit 路径走测地线中点: mid = exp_0(0.5 * log_0(res) + 0.5 * log_0(emb), c), 替代 v262 Möbius 减法, 保留 v270 全部配置 (cyclic + M2 + M3 + c_cyclic). **R36h ceiling 三次突破 +0.46% vs v270 0.11304**, 2 RUN 字符级完全一致 0.1135631443298969, utility 247/249/249 健康, Stage 3 best ckpt E111/E112, n_eval=24832, R51+ 6 确定性约束全开) | ✅ R37 PASS (新 baseline) | — | **0.0966 (best valid ndcg@20)** | **0.1135631443298969** | **0.14316204896907217** | **0.08940618062576725** | `curvature_base/out/decoder/instruments_hgrec_configs/hgrec_geodesic_midpoint_v282/best_ckpt.pt` |
| **v270 cyclic curriculum + M2 commit loss + M3 transport** (上一 baseline, 2026-08-28 promoted, 2026-08-28 被 v282 取代, v270 备份到 `/home/wlia0047/hj82_scratch2/wenyu/backup_curvature_base_v270/curvature_base_v270_R51+_locked`, R36n (a) cyclic c(t) = c_min + (c_max-c_min)|sin(πt/T)| (SGDR Loshchilov & Hutter 2017) + v262 M2 commit loss + M3 transport 联合, c 震荡 0.05↔0.7 (T=25000 步 ≈ 4 个完整周期), 关 USE_CURRICULUM_CURVATURE, **R36h ceiling 二次突破 +0.32% vs v262 0.11268**, 2 RUN 字符级完全一致 0.1130396262886598, utility 254/245/248 健康, Stage 3 best ckpt E117, n_eval=24832, R51+ 6 确定性约束全开) | ✅ R37 PASS (历史 baseline) | — | **0.0978 (best valid ndcg@20)** | **0.1130396262886598** | **0.1450144974226804** | **0.0907523951579615** | (已 promote 出, 备份到 scratch) |
| **v262 Möbius commit loss + L2 distance + hyperbolic_distance=True + sk_eps=0.05** (上一 baseline, 2026-08-27 promoted, 2026-08-28 被 v270 取代, v262 备份到 `/home/wlia0047/hj82_scratch2/wenyu/backup_curvature_base_v262/curvature_base_v262_R51+_locked`, R36n (e) 几何变换 — commit 路径 = exp_0(log_0(res)-log_0(emb),c), R36h ceiling +0.76% breakthrough, 2 RUN 字符级完全一致 0.11267719072164949, utility 254/250/247/256 健康, Stage 1 c=1 curriculum 0.05→0.7, Stage 3 best ckpt E119, n_eval=24832, R51+ 6 确定性约束全开) | ✅ R37 PASS (历史 baseline) | — | **0.0996 (best valid ndcg@20)** | **0.11267719072164949** | **0.14171230670103094** | **0.090311679643454** | (已 promote 出, 备份到 scratch) |
| **v161 Sinkhorn-OT sk_eps=0.05 (10x smoother) + Poincaré distance** (历史 SOTA, 2026-08-26, test_R@10=0.3534, **+216% vs v120 baseline 0.11183 = R36h ceiling 真正被打破**, Stage 1 100k codes 30-130 健康, Stage 2 SID unique 30/40/58 (R36p 灰区但 valid ndcg@10=0.1991 强信号), Stage 3 best ckpt E65, R36i FORCE_HGREC=1 正确传递 n_eval=24832; **注意: 已 R50 删除, 但数值保留作为历史里程碑**) | ✅ R37 PASS (历史) | — | **0.1991 (best valid ndcg@10)** | **0.35337467783505155** | **0.5449420103092784** | **0.2405857400795848** | (已 R50 rm, verdict 保留在 `/home/wlia0047/hj82_scratch2/wenyu/claude_tmp/`) |
...
| **v120 C-RVQ Mahalanobis** (已被 v262 取代, 2026-08-27 v262 promoted, v120 备份到 `/home/wlia0047/hj82_scratch2/wenyu/backup_curvature_base_v120/curvature_base_v120_R51+_locked`) | ref (历史) | — | 0.0965 (best valid ndcg@20) | 0.11183150773195877 | 0.1421 | 0.0900 | (已 R50 rm; 备份到 scratch) |
...
**R5** — 任务硬约束 = **v270 cyclic curriculum + M2 commit loss + M3 transport baseline (test_R@10=0.1130396262886598, R36h ceiling 二次突破 +0.32% vs v262 Möbius commit loss 0.11267719072164949)**, 仅 RQ-VAE 量化, Musical_Instruments (9922 items), 4 阶段流水线, seed=42。**(2026-08-28 v270 baseline 切换)** 新 baseline = `curvature_base/` = v270 实验目录 (已重命名, 旧 v262 baseline 备份到 `/home/wlia0047/hj82_scratch2/wenyu/backup_curvature_base_v262/curvature_base_v262_R51+_locked`)。v270 机制: cyclic curriculum (R36n a SGDR) + v262 M2 Möbius commit loss (R36n e 几何变换) + M3 transport 三者联合。cyclic: c(t) = 0.05 + (c_max-c_min)|sin(πt/T)|, c_min=0.05, c_max=0.7, T=25000 步 (训练 100k 步 ≈ 4 个完整周期)。commit 路径 = exp_0(log_0(res) - log_0(emb), c) (Ganea et al. NeurIPS 2018 Möbius 减法)。配套: DISTANCE_MODE=L2 + hyperbolic_distance=True + sk_eps=0.05 + USE_M2_INTRINSIC=True + USE_M3_TRANSPORT=True + USE_FIXED_CURVATURE=True + USE_CYCLIC_CURVATURE=True + USE_CURRICULUM_CURVATURE=False (cyclic 取代 linear ramp)。R51+ 6 项确定性约束完美生效, RUN 1 + RUN 2 字符级完全一致 0.1130396262886598 (差异 < 1e-15)。Stage 1 utility 254/245/248 健康 (R36p v3.12 ≥ 75% PASS)。Stage 3 best ckpt epoch=117, valid ndcg20=0.0978。**(历史)** 原 v262 baseline (test_R@10=0.11268) 因 v270 突破已 promoted 出 baseline, 备份到 scratch; v129/v133/v161 数值均已过期 (仅作历史里程碑保留).
...
**R36p** — **codebook collapse 无条件否决 (2026-08-25 user directive)**：任何机制只要产生 codebook collapse (任一层 unique codes < 10 或 train log 持续 [CODEBOOK WARNING])，即使下游 test_R@10 显著上升（例如 v154 Mahalanobis L0=2 + test_R@10=0.5316），也**不得作为突破发现**。立即 R50 触发回滚 + verdict + close。理由: collapse 导致 SID 信号空间极度压缩，T5 学习任务退化为 trivial categorical prediction，test_R@10 提升来源于"问题变简单"而非"模型变好"；与历史 v139 lucky init / v151 collapsed-basin artifact 同源风险；一旦接受，后续所有新机制都会倾向于走 collapse 路线获取虚假高指标，主线彻底污染。即使 valid NDCG 同步大涨 / 重跑可复现 / R@20 也涨，仍然是 collapse 诱因，**禁止纳入 baseline 表格或作为下一步起点**。判定标准: 任一层 (L0/L1/L2) unique codes < 10 → R36p 触发，**无视 test_R@10 数值**。
