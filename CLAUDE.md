...
> HG-Rec (Hyperbolic RQ-VAE + Differential-Length Codebook + T5) 流水线复现工作目录, **基线 Task #84 valid R@10=0.1267 / test R@10=0.1024 (Musical_Instruments 9922 items)**, **当前 curvature_base baseline = v262 Möbius commit loss, test_R@10=0.11267719072164949 (R36h ceiling +0.76% breakthrough, 2026-08-27 promoted)**.

---

## 最新测试结果 (2026-08-27 v262 promoted 为新 baseline + v161 旧 SOTA + 历史, beam=20)

| 流水线 | 状态 | valid R@10 | valid NDCG@20 | **test R@10** | test R@20 | test NDCG@20 | ckpt |
|---|---|---|---|---|---|---|---|
| **v262 Möbius commit loss + L2 distance + hyperbolic_distance=True + sk_eps=0.05** (新 baseline, 2026-08-27 promoted from curvature_experiment_v262_mobius_commit_loss, R36n (e) 几何变换 — commit 路径 = exp_0(log_0(res)-log_0(emb),c), R36h ceiling +0.76% breakthrough, 2 RUN 字符级完全一致 0.11267719072164949, utility 254/250/247/256 健康, Stage 1 c=1 curriculum 0.05→0.7, Stage 3 best ckpt E119, n_eval=24832, R51+ 6 确定性约束全开) | ✅ R37 PASS (新 baseline) | — | **0.0996 (best valid ndcg@20)** | **0.11267719072164949** | **0.14171230670103094** | **0.090311679643454** | `curvature_base/out/decoder/instruments_hgrec_configs/hgrec_mobius_commit_loss_v262_baseline/best_ckpt.pt` |
...
**R5** — 任务硬约束 = **v262 Möbius commit loss baseline (test_R@10=0.11267719072164949, R36h ceiling +0.76% breakthrough vs 旧 v120 C-RVQ Mahalanobis 0.11183150773195877)**, 仅 RQ-VAE 量化, Musical_Instruments (9922 items), 4 阶段流水线, seed=42。**(2026-08-27 v262 baseline 切换)** 新 baseline = `curvature_base/` = v262 实验目录 (已重命名, 旧 v120 baseline 备份到 `/home/wlia0047/hj82_scratch2/wenyu/backup_curvature_base_v120/`)。v262 机制: Möbius commit loss (R36n e 几何变换) — commit 路径 = 欧氏 `res = res - emb` 替换为 Poincaré ball 上测地线 `res = exp_0(log_0(res) - log_0(emb), c)` (Ganea et al. "Hyperbolic Neural Networks" NeurIPS 2018 Möbius 减法公式)。配套: DISTANCE_MODE=L2 + hyperbolic_distance=True + sk_eps=0.05。R51+ 6 项确定性约束完美生效, RUN 1 + RUN 2 字符级完全一致 0.11267719072164949 (差异 < 1e-15)。Stage 1 utility 254/250/247/256 健康 (R36p v3.12 ≥ 75% PASS)。Stage 3 best ckpt epoch=119, valid ndcg20=0.0996。**(历史)** 原 v129 C-RVQ Mahalanobis baseline (test_R@10=0.14030) 因 R36p v3.12 utility < 75% 触发 codebook collapse 限制 (虽然 v129 是 2026-08-24 切换的临时 baseline), 现已被 v262 取代。v19/v69/v87/v120/v129/v133/v161 数值均已过期 (仅作历史里程碑保留).
