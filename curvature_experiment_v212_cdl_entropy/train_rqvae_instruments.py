...
USE_FIXED_CURVATURE = False               # v212: 关固定曲率, 让 per-layer c_end ladder 生效
# === v212 NOVEL: per-layer c_end ladder [0.5, 0.6, 0.7] + Codebook Diversity Loss (CDL) ===
# 7 rounds ladder/size R36p FAIL 后, 改走 loss-side 显式 diversity bonus. R36c Round 8.
# 机制: c_end_per_layer = [0.5, 0.6, 0.7] (per-layer 异质 capacity, R36n b) + CDL 软分配熵 (R36n f)
# 软分配 p[i,k] = softmax(-dist[i,:]/τ)[k], q[k] = mean_i p[i,k], H = -Σ q·log(q), loss_cdl = -α·H
# 期望: 强制 utility ≥ 90% (R36p v3.10), 解决 7 rounds 死码 (v205/v206/v207/v208/v209/v210/v211)
USE_PER_LAYER_C_END = True
PER_LAYER_C_END = [0.5, 0.6, 0.7]    # L0 浅+扩张 → L1 中 → L2 深
USE_CDL_ENTROPY = True               # v212: 启用 CDL 软分配熵 bonus
CDL_ENTROPY_WEIGHT = 0.01            # 软分配熵损失权重 (低, 仅引导 codebook 均匀利用)
CDL_TEMPERATURE = 1.0                # softmax 温度 (1.0 = 标准 softmax; 越小越 sharp, 越大越 uniform)
CDL_DEAD_CODE_PENALTY = 0.05         # dead-code penalty 权重 (强制未用 code 接受梯度)
...
def main():
    ...
    model = RqVae(
        ...
        # v212: per-layer c_end ladder + CDL entropy bonus
