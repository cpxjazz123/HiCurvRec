...
class RqVaeOutput(NamedTuple):
    ...
    cdl_losses: List[Tensor]  # v212: 各层 CDL entropy loss (autograd 可流), list of scalar
...
class RqVaeComputedLosses(NamedTuple):
    ...
    cdl_loss: Tensor  # v212: CDL entropy bonus (鼓励 codebook 均匀利用) + dead-code penalty 聚合
...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v212 NOVEL: per-layer c_end ladder + Codebook Diversity Loss (CDL)
        c_end_per_layer: Optional[List[float]] = None,       # v212: 各层独立 c_end (curriculum ramp 终点)
        use_cdl_entropy: bool = False,     # v212: enable CDL entropy bonus
        cdl_entropy_weight: float = 0.01,  # v212: CDL loss 权重
        cdl_temperature: float = 1.0,      # v212: softmax 温度
        cdl_dead_code_penalty: float = 0.0, # v212: dead-code penalty 权重
    ) -> None:
        self._config = locals()
        ...
        # v212: per-layer c_end ladder
        if c_end_per_layer is not None:
            assert len(c_end_per_layer) == n_layers, f"c_end_per_layer length {len(c_end_per_layer)} != n_layers {n_layers}"
            from modules.hyperbolic import C_MAX as _C_MAX
            for li, ce in enumerate(c_end_per_layer):
                assert 0.0 < ce <= _C_MAX, f"c_end_per_layer[{li}]={ce} out of (0, {_C_MAX}]"
        self.c_end_per_layer = c_end_per_layer
        # v212: CDL entropy bonus (保存 self attr 供 forward 调用)
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    c_start=c_start,
                    c_end=(c_end_per_layer[i] if c_end_per_layer is not None else c_end),  # v212: per-layer c_end ladder
                    curriculum_steps=curriculum_steps,
                    c_fixed=c_fixed,
                    use_spread_loss=spread_loss_weight > 0,  # F3 v83: Spread Loss 开关
                    spread_loss_margin=spread_loss_margin,
                    # v212: CDL entropy bonus (per-layer 共享, 鼓励各层均匀利用 codebook)
                    use_cdl_entropy=use_cdl_entropy,
                    cdl_entropy_weight=cdl_entropy_weight,
                    cdl_temperature=cdl_temperature,
                    cdl_dead_code_penalty=cdl_dead_code_penalty,
                )
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001) -> RqVaeOutput:
        ...
        cdl_losses = []  # v212: 各层 CDL entropy loss (autograd 可流, 累加到总损失)
        ...
        for li, layer in enumerate(self.layers):
            ...
            cdl_losses.append(quantized.cdl_loss)  # v212: 收集各层 CDL entropy loss (autograd)
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v212: CDL entropy bonus — 聚合各层 cdl_loss (autograd 可流, 直接累加到 total loss)
        cdl_loss_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.use_cdl_entropy and quantized.cdl_losses:
            # 注意: cdl_loss 已经在各 Quantize.forward 中计算 = -α·H (权重已包含)
            # 所以这里直接 sum 即可, 无需再乘以 self.cdl_entropy_weight
            cdl_loss_total = torch.stack(quantized.cdl_losses).sum()
        loss = (reconstuction_loss + rqvae_loss).mean() + self.margin_reg_weight * margin_loss + self.spread_loss_weight * spread_loss_total + cdl_loss_total
