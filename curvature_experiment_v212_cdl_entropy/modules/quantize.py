...
class QuantizeOutput(NamedTuple):
    ...
    cdl_loss: Tensor  # v212: Codebook Diversity Loss (CDL) — 软分配熵 (鼓励均匀利用 codebook) + dead-code penalty
...
class Quantize(nn.Module):
    def __init__(
        ...
        # v212 NOVEL: Codebook Diversity Loss (CDL) — soft assignment entropy bonus
        # R36n (f) 双曲度量: 用 Mahalanobis 距离算 soft assignment (L2/Mahalanobis 都是双曲度量的退化/参数化)
        # 机制: p[i,k] = softmax(-dist[i,k] / τ)[k], q[k] = mean_i p[i,k]
        #       H = -Σ_k q[k] * log(q[k]) → loss_cdl = -α * H (最大化熵, 即鼓励 codebook 全利用)
        # 期望: 强制 layer utility ≥ 90% (R36p v3.10 阈值), 解决 ladder+size KMeans init 死码 (7 rounds FAIL)
        use_cdl_entropy: bool = False,     # v212: enable CDL entropy bonus (强制 codebook 全利用)
        cdl_entropy_weight: float = 0.01,  # v212: CDL loss 权重 (低权重, 仅引导)
        cdl_temperature: float = 1.0,      # v212: softmax 温度 (越小越 sharp 鼓励单 code, 越大越 uniform)
        cdl_dead_code_penalty: float = 0.0, # v212: dead-code penalty 权重 (0=关闭, >0 强制未用 code 接受梯度)
    ) -> None:
        super().__init__()
        ...
        # v212: CDL entropy bonus
        self.use_cdl_entropy = bool(use_cdl_entropy)
        self.cdl_entropy_weight = float(cdl_entropy_weight)
        self.cdl_temperature = float(cdl_temperature)
        self.cdl_dead_code_penalty = float(cdl_dead_code_penalty)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v212 NOVEL: Codebook Diversity Loss (CDL) — soft assignment entropy bonus
        # 机制: 用 dist (Mahalanobis/L2/Poincaré/RBF 任意双曲度量) 算 soft assignment
        #       p[i, k] = softmax(-dist[i, :] / τ)[k] (B, K)
        #       q[k] = mean_i p[i, k]  (K,) 该层 batch 内经验分布
        #       H = -Σ_k q[k] * log(q[k] + ε)  (越高 = codebook 利用越均匀)
        #       loss_cdl = -α * H (鼓励均匀, 即最大化熵)
        # 梯度通路: H → q → p → softmax(-dist/τ) → dist → codebook/encoder
        # R36n (f) 双曲度量: Mahalanobis/L2 都是双曲度量的退化, R36n 合规
        # R36p v3.10 期望: 强制 utility ≥ 90% (即 q 接近 uniform, H 接近 log(K))
        cdl_loss = torch.zeros((), device=dist.device, dtype=dist.dtype)
        if self.use_cdl_entropy and self.training:
            tau = max(self.cdl_temperature, 1e-3)
            # 数值稳定: 减去 per-row min (避免 softmax overflow)
            dist_for_softmax = -dist / tau  # (B, K) 负距离 = logits
            p = torch.softmax(dist_for_softmax, dim=-1)  # (B, K)
            q = p.mean(dim=0)  # (K,) 该层 batch 内 code 经验分布
            # 熵: H = -Σ q log(q), uniform q → H = log(K)
            H = -(q * torch.log(q.clamp_min(1e-10))).sum()
            cdl_loss = -self.cdl_entropy_weight * H  # 最大化 H = 最小化 -H
            # Dead-code penalty: 惩罚 q[k] = 0 的 code (强制未用 code 接受梯度)
            if self.cdl_dead_code_penalty > 0:
                dead_mask = (q < 1e-6).float()  # (K,) 0 经验用的 code
                dead_frac = dead_mask.mean()
                cdl_loss = cdl_loss + self.cdl_dead_code_penalty * dead_frac
            self._last_cdl_loss = cdl_loss.detach()
        else:
            self._last_cdl_loss = cdl_loss.detach()
        ...
        return QuantizeOutput(embeddings=emb_out, ids=ids, loss=loss, margin=margin, spread_loss=spread_loss, cdl_loss=cdl_loss)
