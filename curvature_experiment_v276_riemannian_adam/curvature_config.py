...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "riemannian_adam_v276"  # ← v276 NOVEL: Riemannian Adam for codebook embedding (R36n d) + v270 cyclic + M2 + M3. 论文支撑: Bécigneul & Ganea "Riemannian Adaptive Optimization Methods" ICLR 2019. 机制: codebook embedding.weight 在 Poincaré ball 上, 用 Riemannian Adam (Riemannian gradient λ(x)=(1-||x||²)²/4 + Adam m/v + projection ||x||≤0.99). 其他参数 (encoder/decoder/router) 仍 AdamW. 保留 v270 全部 (USE_CYCLIC_CURVATURE=True, USE_M2_INTRINSIC=True, USE_M3_TRANSPORT=True, USE_FIXED_CURVATURE=True, DISTANCE_MODE=L2, hyperbolic_distance=True, sk_eps=0.05). 目标: test_R@10 > 0.1130396262886598 (v270 baseline). 5 cyclic variants (v271-275) 全 R37 FAIL → R36h ceiling 强证. v276 走 R36n (d) Riemannian optimizer 是 orthogonal 方向.
