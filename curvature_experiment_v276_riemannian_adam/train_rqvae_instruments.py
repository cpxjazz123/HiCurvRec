...
# v276 NOVEL: Riemannian Adam optimizer for codebook embedding (Bécigneul & Ganea 2019)
# 论文支撑: "Riemannian Adaptive Optimization Methods" ICLR 2019
# 机制: 对 codebook embedding (各 Quantize 层的 embedding.weight) 用 Poincaré ball metric
# Riemannian gradient scaling: λ(x) = (1 - ||x||²)² / 4; Adam update + projection to ball
# 其他参数 (encoder/decoder/router) 仍用 AdamW
USE_RIEMANNIAN_ADAM = True                # v276 启用 Riemannian Adam (仅对 embedding.weight)
RIEMANN_ADAM_LR = 1e-3                     # Riemannian Adam lr (与 AdamW 同步)
RIEMANN_ADAM_WD = 1e-4                     # Riemannian Adam weight_decay
RIEMANN_ADAM_MAX_NORM = 0.99               # Poincaré ball ||x|| 上限 (避免 boundary issue)
...
def main():
    ...
    # v276 NOVEL: Riemannian Adam for codebook embedding on Poincaré ball
    # 仅对各 Quantize 层的 embedding.weight 应用 (其他参数仍 AdamW)
    riem_optimizer = None
    if USE_RIEMANNIAN_ADAM:
        from modules.riemannian_adam import RiemannianAdam
        embedding_params = [
            p for n, p in model.module.named_parameters()
            if "layers." in n and n.endswith(".embedding.weight")
        ]
        assert len(embedding_params) == N_LAYERS, f"期望 {N_LAYERS} 个 codebook embedding, 实际 {len(embedding_params)}"
        riem_optimizer = RiemannianAdam(
            embedding_params,
            lr=RIEMANN_ADAM_LR,
            weight_decay=RIEMANN_ADAM_WD,
            max_norm=RIEMANN_ADAM_MAX_NORM,
        )
        if rank == 0:
            print(f"[v276] RiemannianAdam enabled for {len(embedding_params)} codebook embeddings "
                  f"(lr={RIEMANN_ADAM_LR}, wd={RIEMANN_ADAM_WD}, max_norm={RIEMANN_ADAM_MAX_NORM})", flush=True)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v276 NOVEL: Riemannian Adam step for codebook embedding (Poincaré ball metric)
            if riem_optimizer is not None:
                riem_optimizer.step()
            global_step += 1
