...
SPREAD_LOSS_MARGIN = 2.5         # v316 default scalar (per-layer list 优先)
# v329 NOVEL: per-layer异质 SPREAD_LOSS_MARGIN (R36n b+f). L0 输入 norm 最大 (~1.0) → 最大 margin 推最 spread.
# L2 输入 norm 最小 (~0.01) → 最小 margin. 与 anisotropy_target_std_per_layer 协同.
SPREAD_LOSS_MARGIN_PER_LAYER = [3.0, 2.5, 2.0]  # L0/L1/L2
...
ANISOTROPY_TARGET_STD = 2.0      # v316 default scalar (per-layer list 优先)
ANISOTROPY_TEMP = 1.0            # v316: 温度缩放 ((cur_std - target)/temp)²
# v329 NOVEL: per-layer异质 ANISOTROPY_TARGET_STD (R36n b+f). L0 spread 最远, L2 最紧凑.
ANISOTROPY_TARGET_STD_PER_LAYER = [2.5, 2.0, 1.5]  # L0/L1/L2
...
def main():
    ...
    model = RqVae(
        ...
        spread_loss_margin=SPREAD_LOSS_MARGIN,  # F3 v83 scalar (per-layer list override)
        spread_loss_margin_per_layer=SPREAD_LOSS_MARGIN_PER_LAYER,  # v329: per-layer list
        ...
        anisotropy_target_std=ANISOTROPY_TARGET_STD,  # v316 scalar (per-layer list override)
        anisotropy_target_std_per_layer=ANISOTROPY_TARGET_STD_PER_LAYER,  # v329: per-layer list
