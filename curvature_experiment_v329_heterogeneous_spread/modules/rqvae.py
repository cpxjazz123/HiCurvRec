...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        spread_loss_margin: float = 2.0,    # F3 v83: pairwise 距离阈值 (Poincaré 单位, scalar)
        # v329 NOVEL: per-layer spread_loss_margin list (R36n b+f). 若 list 则 per-layer 应用, scalar 则 uniform.
        spread_loss_margin_per_layer: Optional[List[float]] = None,
        ...
        anisotropy_target_std: float = 2.0,   # v316: target std (Poincaré 单位, scalar)
        anisotropy_temp: float = 1.0,         # v316: 温度缩放
        # v329 NOVEL: per-layer anisotropy_target_std list (R36n b+f). 若 list 则 per-layer 应用.
        anisotropy_target_std_per_layer: Optional[List[float]] = None,
        ...
        # v329: per-layer margin (None → 全部层用 spread_loss_margin scalar)
        self.spread_loss_margin_per_layer = spread_loss_margin_per_layer
        ...
        # v329: per-layer target_std (None → 全部层用 scalar)
        self.anisotropy_target_std_per_layer = anisotropy_target_std_per_layer
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    # v329: per-layer margin (priority: per-layer list > scalar)
                    spread_loss_margin=(
                        spread_loss_margin_per_layer[i]
                        if spread_loss_margin_per_layer is not None
                        else spread_loss_margin
                    ),
        ...
        for i, layer in enumerate(self.layers):
            ...
            # v329: per-layer target_std (priority: per-layer list > scalar)
            layer.anisotropy_target_std = (
                anisotropy_target_std_per_layer[i]
                if anisotropy_target_std_per_layer is not None
                else self.anisotropy_target_std
            )
