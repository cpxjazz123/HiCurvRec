...
# 攻击 v213-v221 DDP L0 极早期坍缩通路. 论文支撑: Absil et al. 2008 retraction + Nickel & Kiela 2017 ball 约束.
...
def main():
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v222: Riemannian Codebook Retraction — 强制 codebook 回到 Poincaré 球内
            # 必须在 DDP all-reduce 之后 (gradient 已 sync, 各 rank 独立做 retraction 即可)
            if USE_CODEBOOK_RETRACTION:
                model.module.apply_retraction()
            global_step += 1
