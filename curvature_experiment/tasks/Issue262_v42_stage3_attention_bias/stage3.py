"""v42 Stage 3 wrapper — HG-Rec T5 训练 + Q/K 双曲距离 attention bias (DDP 4 卡).
...
Stage 3 启用 v42: use_hyp_attn_bias=True → 注入 Q/K Poincaré 距离 bias 到 T5 encoder attention.
"""
import os
import subprocess
import sys
...
LOG_PATH = os.path.join(LOG_DIR, "curvature_exp_v42_stage3.log")
...
cmd = [
    ...
    os.path.join(MAIN_DIR, "configs/decoder_instruments_hgrec_v42.gin"),
]
...
env = os.environ.copy()
env["FORCE_HGREC"] = "1"
env["SAVE_DIR_ROOT"] = "/home/wlia0047/ar57/wenyu/GeneRec/curvature_experiment/out/decoder/instruments_hgrec_v42/"
env["USE_HYP_ATTN_BIAS"] = "1"  # v42 创新开关 (Issue262): Stage 3 attention Q/K 双曲距离 bias
