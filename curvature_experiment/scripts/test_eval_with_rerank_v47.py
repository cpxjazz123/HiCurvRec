...
def _test_eval_hgrec(accelerator, device, BEST_CKPT_PATH):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # preds.sequences: (B*beam, max_len) — 重塑为 (B, beam, max_len)
            seqs = preds.sequences  # (B*beam, max_len)
            scores = preds.sequences_scores  # (B*beam,) beam log-prob sum
            B = input_ids.shape[0]
            seqs = seqs.view(B, beam_size, -1)  # (B, beam, max_len)
            seq_logp = scores.view(B, beam_size)  # (B, beam)

            # === v47 Stage 4 端曲率机制: Poincaré distance rerank (R36 新曲率机制) ===
            # 对每 beam 输出 candidate SID (sid0, sid1, sid2) → 查 codebook embedding → Poincaré 距离
            # 重新排序 score = log_prob + λ * (-poinc_dist_centroid)
            seqs_sid = seqs[:, :, 1:].reshape(B * beam_size, -1)  # (B*beam, max_len-1) raw token ids
            # v19 SID vocab: 0=PAD, 1..256=layer0, 257..512=layer1, 513..768=layer2, 768=END (n_layers=3)
            # 但 generate 输出 (seq_ids) 是 flat token ids in [0, 768], hgrec_vocab_size=769
            # 截断到 sid0/1/2 (前 3 个非 PAD token), 重新映射到 layer-level SID index [0, 255]
            # 因为 vocab 已经是 flat 的 (1..256 = layer0, 257..512 = layer1, 513..768 = layer2)
            # 取每层 token = floor((token-1)/256), codebook_idx = (token-1) % 256
            # RQ-VAE codebook 加载 (从 RQ-VAE ckpt, 不能用 T5 shared embedding)
            import torch as _torch
            RQVAE_CKPT = "/home/wlia0047/hj82_scratch2/wenyu/rqvae_dataset/instruments/rqvae_out_v19_cend_07/rqvae_final.pt"
            _rqvae_state = _torch.load(RQVAE_CKPT, map_location="cpu", weights_only=False)
            # RqVae state_dict keys: "layers.0.embedding.weight", "layers.1.embedding.weight", "layers.2.embedding.weight"
            cb_l0 = _rqvae_state["model"]["layers.0.embedding.weight"].to(device)  # (256, 32)
            cb_l1 = _rqvae_state["model"]["layers.1.embedding.weight"].to(device)
            cb_l2 = _rqvae_state["model"]["layers.2.embedding.weight"].to(device)
            # HG-Rec flat vocab mapping: token 1..256 → layer0 idx 0..255, 257..512 → layer1, 513..768 → layer2
            seqs_sid_3layer = seqs_sid[:, :3]  # (B*beam, 3) 取前 3 个非 PAD token 当 SID
            # codebook idx per layer
            cb_idx_l0 = ((seqs_sid_3layer[:, 0] - 1) % 256).clamp(0, 255)  # (B*beam,)
            cb_idx_l1 = ((seqs_sid_3layer[:, 1] - 1) % 256).clamp(0, 255)
            cb_idx_l2 = ((seqs_sid_3layer[:, 2] - 1) % 256).clamp(0, 255)
            # 取 codebook embeddings
            code_emb_l0 = cb_l0[cb_idx_l0]  # (B*beam, 32)
            code_emb_l1 = cb_l1[cb_idx_l1]
            code_emb_l2 = cb_l2[cb_idx_l2]
            # Poincaré 距离: d_P(u, v) = arccosh(1 + 2 * ||u-v||^2 / ((1-||u||^2)(1-||v||^2)))
            # 取 candidate SID 与 codebook centroid (mean over all 256 codes) 的距离
            cb0_centroid = cb_l0.mean(0, keepdim=True)  # (1, 32)
            cb1_centroid = cb_l1.mean(0, keepdim=True)
            cb2_centroid = cb_l2.mean(0, keepdim=True)
            c = 1.0  # RQ-VAE 默认 c=1.0 (v19 baseline 锁定)

            def _poinc_dist(u, v, c=1.0):
                u_norm2 = (u * u).sum(-1)
                v_norm2 = (v * v).sum(-1)
                diff2 = ((u - v) ** 2).sum(-1)
                arg = 1.0 + 2.0 * c * diff2 / ((1.0 - c * u_norm2) * (1.0 - c * v_norm2) + 1e-9)
                arg = arg.clamp(min=1.0)
                return torch.acosh(arg) / (c ** 0.5)

            dist_l0 = _poinc_dist(code_emb_l0, cb0_centroid.expand_as(code_emb_l0), c)
            dist_l1 = _poinc_dist(code_emb_l1, cb1_centroid.expand_as(code_emb_l1), c)
            dist_l2 = _poinc_dist(code_emb_l2, cb2_centroid.expand_as(code_emb_l2), c)
            poinc_dist_total = (dist_l0 + dist_l1 + dist_l2).view(B, beam_size)  # (B, beam)

            # === Rerank: score = log_prob + λ * (-poinc_dist) ===
            # λ=0.1 (R36 硬编码, 不调参): 让更"双曲集中" (距 centroid 近) 的 candidate 排名靠前
            LAMBDA_RERANK = 0.1
            rerank_score = seq_logp + LAMBDA_RERANK * (-poinc_dist_total)  # (B, beam)
            # 按 rerank_score 降序排, 取新的 top-20 顺序
            sorted_idx = rerank_score.argsort(dim=-1, descending=True)  # (B, beam)
            seqs = seqs.gather(1, sorted_idx.unsqueeze(-1).expand(-1, -1, seqs.shape[-1]))  # (B, beam, max_len)
            # 重新切片取前 n_layers=3 token 作为 SID 输出 (去掉 PAD)
            preds = seqs[:, :, 1:].reshape(B, beam_size, -1)
    ...
    if accelerator.is_main_process:
        ...
        print(f"  n_eval:    {int(global_total)}", flush=True)
        out_path = "/home/wlia0047/ar57/wenyu/GeneRec/curvature_experiment/out/decoder/instruments_hgrec_v47/test_final.json"
