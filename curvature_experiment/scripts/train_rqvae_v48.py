...
# 改 distance = α·d_Poincaré + (1-α)·d_Lorentz, α=0.5 (R36 硬编码)
...
OUT_DIR = "/home/wlia0047/hj82_scratch2/wenyu/rqvae_dataset/instruments/rqvae_out_v48_lorentz_mix"
...
# 每层 distance = lorentz_alpha * d_Poincaré + (1-lorentz_alpha) * d_Lorentz
# α=0.5 (硬编码, R36 不调参). Lorentz distance 在 hyperboloid manifold 上算,
# 把 expmap0 (Poincaré D 维) 投影到 hyperboloid (D+1 维) 后 arccosh(-<u,v>_L) / sqrt(c).
USE_LORENTZ_MIX = True
LORENTZ_ALPHA = 0.5
...
def main():
    ...
    model = RqVae(
        ...
        # v39: Poincaré embedding regularization (默认 0=关闭, v19 baseline 不变)
        hyp_emb_reg_weight=HYP_EMB_REG_WEIGHT if USE_HYP_EMB_REG else 0.0,
        hyp_emb_c=HYP_EMB_C,
        # v48: Poincaré-Lorentz Product Manifold Codebook (R36 曲率机制变更)
        use_lorentz_mix=USE_LORENTZ_MIX,
        ...
        for batch in train_loader:
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                if USE_SCS:
                    ...
                    scs_str = "off"
                # v39: hyp_emb per-layer log
                if USE_HYP_EMB_REG and out.per_layer_hyp_emb is not None:
                    hyp_str = "/".join(f"{float(h):.4f}" for h in out.per_layer_hyp_emb.tolist())
                else:
                    hyp_str = "off"
                print(
                    ...
                    f"| hl={float(out.hyp_emb_loss.item() if hasattr(out.hyp_emb_loss, 'item') else out.hyp_emb_loss):.4f} "
