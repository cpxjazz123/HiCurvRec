...
for userID, item_sequence in json.load(data).items():
    if int(userID) > num_user:
        num_user = int(userID)
    if max(item_sequence) > num_item:
        num_item = max(item_sequence)
    interaction_count = interaction_count + len(item_sequence)
    all_len_seq = all_len_seq + len(item_sequence)

print(num_item, num_user, interaction_count, 1 - interaction_count/(num_item*num_user), all_len_seq/num_user)