...
def make_sample_collator(dataset: GenRecDataset):
...
def evaluate(
    ...
    *,
    ...
    """Evaluate by direct SID-token matching, aligned with curvature_base.

    The generated beams and target are compared directly as ``(B, beam,
    n_digit)`` tensors.  This intentionally does not perform item conversion,
    duplicate removal, seen-history filtering, or fallback filling; those are
    the CPU-side operations used by the former RecBole item-level evaluator.
    """
    ...
    with torch.inference_mode():
        for batch in tqdm(
            ...
            input_ids = batch["input_ids"].to(device, non_blocking=True)
            attention_mask = batch["attention_mask"].to(device, non_blocking=True)
            target_sid_tokens = batch["labels"][:, : eval_dataset.n_digit].to(
            ...
            # Same semantics as curvature_base/evaluate/metrics.py:
            # compare the complete SID tuple and keep the first beam hit.
            matches = (sid_tokens == target_sid_tokens.unsqueeze(1)).all(dim=-1)
            found, rank = matches.max(dim=1)
            for k in topk_list:
                hit = found & (rank < k)
                recall_sums[k] += hit.sum().item()
                if hit.any():
                    ndcg_sums[k] += (
                        1.0 / torch.log2(rank[hit].float() + 2.0)
                    ).sum().item()
            n_eval += batch_size
...
def _parse_args():
    parser = argparse.ArgumentParser(description="RecBole-aligned HG-Rec TIGER")
    # Fast single-A100 defaults (validated on the currently available 80-GiB A100).
    # Keep the protocol/model defaults unchanged; only throughput-safe settings
    # are selected here.  Explicit CLI values still override these defaults.
    parser.add_argument("--batch_size", type=int, default=10240)
    parser.add_argument("--infer_size", type=int, default=64)
    ...
    parser.add_argument("--dataset_name", type=str, default="Amazon_2023_Instruments")
    parser.add_argument("--dataset_path", type=str, default="./dataset/")
    parser.add_argument("--codebook_size", type=int, nargs="+", default=[256, 256, 256, 1])
    parser.add_argument("--code_path", type=str, default="item_sids_recbole.json")
    parser.add_argument("--train_file", type=str, default="train_recbole.parquet")
    parser.add_argument("--valid_file", type=str, default="valid_recbole.parquet")
    parser.add_argument("--test_file", type=str, default="test_recbole.parquet")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Validate every N epochs when --eval is enabled.",
    )
    parser.add_argument(
        "--no_eval",
        action="store_true",
        default=False,
        help="Skip validation entirely and select/early-stop checkpoints by train loss.",
    )
    parser.add_argument(
        "--eval",
        dest="no_eval",
        action="store_false",
        help="Keep validation enabled (the default; retained as a compatibility alias).",
    )
    parser.add_argument(
        "--skip_test",
        action="store_true",
        help="Skip the final test (benchmark-only; the default always tests after training)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="CPU DataLoader workers used to overlap collation with GPU work",
    )
    parser.add_argument(
        "--prefetch_factor",
        type=int,
    ...
    parser.add_argument(
        "--no_pin_memory",
        action="store_true",
        help="Disable page-locked host memory (slower for CUDA transfers)",
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        default=True,
        help="Allow nondeterministic/cuDNN fast paths for throughput",
    )
    parser.add_argument(
        "--no_fast",
        dest="fast",
        action="store_false",
        help="Disable throughput-oriented fast paths",
    )
    parser.add_argument(
        "--compile",
        action="store_true",
        default=True,
        help="Compile the training forward/backward with torch.compile; evaluation stays uncompiled",
    )
    parser.add_argument(
        "--no_compile",
        dest="compile",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use BF16 autocast for training and generation on supported CUDA GPUs",
    )
    parser.add_argument(
        "--no_bf16",
        dest="bf16",
...
def main():
    ...
    config["exclude_history"] = not config.pop("no_exclude_history")
    config["pin_memory"] = device.type == "cuda" and not config.pop("no_pin_memory")
    ...
    eval_collator = (
        make_sample_collator(valid_dataset)
    ...
    valid_loader = (
        ...
        if rank == 0 and valid_dataset is not None
        else None
    )
    ...
    if config["compile"]:
        if is_ddp:
            raise ValueError("--compile benchmark currently supports single-GPU training only")
    ...
    for epoch in range(1, config["num_epochs"] + 1):
        ...
        if config["no_eval"] and rank == 0:
            ...
            should_validate = (epoch % config["eval_interval"] == 0) or (epoch == config["num_epochs"])
        if not config["no_eval"] and rank == 0 and should_validate:
            ...
            monitor = ndcgs["ndcg@10"]
            if monitor > best_ndcg:
                best_ndcg = monitor
                early_stop_counter = 0
                best_checkpoint = os.path.join(ckpt_path, "HG_Rec_best.pth")
                raw_model = model.module if hasattr(model, "module") else model
                torch.save(raw_model.state_dict(), best_checkpoint)
                logging.info("Best NDCG@10=%s; saved %s", best_ndcg, best_checkpoint)
            else:
                early_stop_counter += 1
                logging.info("No NDCG@10 improvement; patience=%d", early_stop_counter)
                early_stop_signal = early_stop_counter >= config["early_stop"]
        if is_ddp:
            signal = torch.tensor(
                [1 if early_stop_signal else 0], dtype=torch.uint8, device=device
            )
            dist.broadcast(signal, src=0)
            early_stop_signal = bool(signal.item())
    ...
    if rank == 0:
        ...
        test_loader = GenRecDataLoader(
            ...
            batch_size=config["infer_size"],
            shuffle=False,
            sample_collator=make_sample_collator(test_dataset),
