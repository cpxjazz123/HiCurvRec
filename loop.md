...
| **(K-sweep 扩展)** | **Task #326 (K=384 sweet spot probe)** | **🔄 READY (description + Stage 1 launcher 完成. K-sweep K0=384 = K=256→512 插值点, 未探索. GPU 1 等 task320 释放后启动. 决策阈值 R@10 > 0.1053 vs task194 K=256 anchor. 总耗时 ~1.5-2h)** |
| **(Issue #39)** | **Task #324 (Issue #39 Stage 4 召回改造 5-arm)** | **🟡 Part 1 (ANN dense 3-arm CPU) ❌ NO-GO (R@10=0.011 协议数量级低于 T5.generate SID 0.10; HNSW ≡ dense 0.0109; IVF-PQ -54% 退化 0.0050). Part 2 (T5.generate SID 4-arm) 🔄 READY (Arm C rerank + D beam=100/200 + E control beam=50, 需 GPU 1, 等 task320 释放 ~70 min). Issue #39 protocol split 修订: ANN dense NO-GO + T5.generate SID 待 GPU 验证. GitHub issue #39 已 post R11.5 protocol split 评论** |
| **(Synthesis)** | **Task #325 (Cross-direction synthesis)** | **📋 DONE (verdict + description 完成, R@10 ceiling = task194_k0256 0.1053 +3.3%. 24+ 方向 NO-GO 收口, 剩余未探索: Issue #39 Part 2 + D6 ablation + 跨方向协同. Issue #38 已 post AI 实施进度 update 评论)** |
| **(Issue #39 Stage 4 召回)** | **Task #323 (Issue #39 Stage 4 召回方式改造 5-arm: HNSW/IVF-PQ/cross-encoder/beam/control)** | **🔄 READY (description 登记 @ 09:01. 4-active eval on 4 L40S × ~30 min after task320 全部完成释放 GPU. Stage 1 嵌入 + FAISS-gpu 索引 + cross-encoder rerank 复用基线. 决策阈值 ≥ 1 Arm R@10 > 0.1053 task194_k0256 anchor = 实质突破 baseline. Owner task292 §横向联立 暗示 Stage 3/4 协议改造, Issue #38 + #39 互补)** |
