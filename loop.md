...
| **(Issue #33 双验证)** | **Task #306 (per-item soft-assign on #30 GO 配置 D8 候选, 独立第二次验证)** | **❌ Gate 1 NO-GO (USAGE-KILL epoch 30: L0=54.7%/L1=31.2%/L2=18.0%, collision_rate=0.9602, r_std=0.0000). 跟 task305 独立第二次复现 D8 per-item soft 坍缩模式. K13 新核心发现: per-item soft VQ = Phase 0 mode collapse 第 4 变体 (跟 #28 Gumbel-Softmax / #29 K_l / #32 三轴 共享 root cause = 软/异构分配机制破坏 encoder/codebook co-adaptation). 22 方向 NO-GO 收口 + #30 marginal GO. D7 禁试, D8 双验证 NO-GO. 后续 D9 多样 hash OR Stage 3/4 训练协议 (task290/291/292 verdict 推荐)** |
...
| **(Stage 4 协议 K15)** | **Task #308 (Stage 4 length_penalty ablation on #30 GO ckpt + beam=50: {0.5, 1.0, 2.0})** | **❌ NO-GO (plateau max diff=0). lp=0.5/1.0/2.0 全部 R@10=0.1045, R@20=0.1312, NDCG@10=0.0771. K15 新核心: length_penalty 不是 R@10 杠杆. Stage 4 inference 协议层不是 generate() 参数空间普遍有效, **specifically beam_size 是 unique 杠杆** (K14 验证 beam_size 影响 search space 大小, length_penalty 仅影响 scoring 偏置, 对硬 SID retrieval 不构成 search 质量提升). 24 方向 × 24 verdict 收口. 后续 task312 T5-mini → T5-small (高 ROI, paper Table 1 HG-Rec 用 T5-small) OR task310 num_return_sequences (中 ROI)** |
| **(Stage 4 协议 K16)** | **Task #310 (Stage 4 repetition_penalty ablation on #30 GO ckpt + beam=50: {1.0, 1.2, 1.5})** | **❌ NO-GO (plateau max diff=0). rp=1.0/1.2/1.5 全部 R@10=0.1045, R@20=0.1312, NDCG@10=0.0771. K16 联立 K15: Stage 4 inference 协议层**只 beam_size 是唯一杠杆**, length_penalty + repetition_penalty 都是 scoring 偏置类参数 (对硬 SID retrieval 无效). 25 方向 × 25 verdict 收口. Stage 4 inference 协议层 = 单点 beam_size 杠杆, 收口完成. 后续 task311 housekeeping (零 GPU), task309 Stage 3 T5-small 训练中 (GPU 1 ETA ~4.4h), task312/313 Stage 3 训练协议层等 task309 完成** |
| **(Issue #34 D9)** | **Task #315 (Issue #34 D9 多样 hash simplified implementation 2nd attempt)** | **❌ NO-GO (上游 HG-Rec/model/hrqvae.py 2026-07-30 00:03 修改, 移除 curvature_list / log_r / hyp_mean / hyp_scale / model.hrq.encoder 属性. Stage 1 retraining 失败 (curvature_list 不兼容) + Stage 2 ckpt 加载失败 (即使 strict=False 也失败因 encoder 属性消失). 4 owner 决策选项: A 修订 HRQVAE / B 修订 task301 脚本 / C 重启 Stage 1 / D 关闭 Issue #34. Issue #34 GitHub 仍 OPEN 等 owner decision). 23 方向 × 25 verdict NO-GO 收口 + Issue #30 唯一 GO marginal** |
| **(Issue #38 δ1)** | **Task #316 (Issue #38 Arm δ1 frequency prior rerank)** | **❌ NO-GO (alpha=0.10 R@10=0.1042 in-sigma, alpha≥0.30 NDCG 暴跌, alpha=1.00 R@10=0.0388 -62.7%. 跟 K14-K16 一致: Stage 4 post-process 不是 R@10 杠杆, 唯一 lever = beam_size)** |
...
| **(Issue #38 Arm 1)** | **Task #318 (Issue #38 Arm 1 optimizer 4-arm ablation)** | **❌ FULL NO-GO (Stage 3 4-arm 50 epoch + Stage 4 test @ K=100 跑完. val_NDCG@20 Adam=AdamW 数学等价 (跨 50 epoch Δ<0.001); Stage 4 R@10: AdamW 0.0996 / Adam 0.0971 / SGD 0.0905 / Adafactor 0.0780 — 全 -5% ~ -26% vs anchor 0.1053. K=100 amplifier 0.1045 不是 universal, 是 task301 特定训练产物. optimizer 不是 R@10 杠杆)** |
...
| **(Issue #38 5-arm)** | **Task #320 (Issue #38 5-arm Stage 3 retraining: Optimizer/LR schedule/R-Drop/BF16/control)** | **🔄 RUNNING (4/5 arms alive @ 08:44: GPU 0 Arm A ep25 val_R@10=0.1094 + GPU 1 Arm B v3 ep3 (inv_sqrt fix v2 alive, LR 5e-8→1e-4 ramp, val_R@10=0.0768) + GPU 2 Arm D ep24 val_R@10=0.1105 + GPU 3 Arm E ep30 val_R@10=0.1143 ⭐⭐ control. Arm C R-Drop 待 GPU 释放. task318 50ep proxy test_R@10: AdamW 0.0996 / Adam 0.0971 (val/test -0.021 gap, NO-GO baseline 0.1020). task320 200ep 决策阈值 = Stage 4 test_R@10 > 0.1020 baseline)** |
| **(D6 Issue #30 ablation)** | **Task #321 + #322 (Issue #30 r_l + s_l ablation 3-arm)** | **🔄 READY (description + Arm A launcher 完成, GPU 1 仍被 task320 Arm B 占用. 3-arm 串行: Arm A r_l only + Arm B s_l only + Arm C r_l+s_l control. R11.5 决策 Issue #30 marginal +0.2pp 是 r_l 还是 s_l 还是协同. 待 task320 全部完成释放 GPU 后启动, 估总耗时 4-5 hr)** |
