...
候选方向 (按 Task #278 / #282 / #283 / #284 / #288 联立推论):
- **方向 D3 (Issue #20 §反证 触发换轨)**: task272 m-arm κ-Stereographic v9+ (用户 2026-07-24 提议 + Issue #20 §反证 明示换轨到此). baseline recipe 内部无 L0 ≥ 90% 杠杆, 唯一新方向是结构改动 (κ-Stereographic / Gromov-Softmax / EMA / per-item soft-assign). Stage 1 重训 (200 epoch FreeCurvHRQVAE with κ-Stereographic distance formula) + Stage 2 Sinkhorn + Stage 3 T5-mini 训练 + Stage 4 eval, 估约 4-5 小时 GPU (单 Stage 1 训练 200 epoch ≈ 1.5-2 hr on cuda:0).
...
- **方向 D5 ~~(Task #282 后)~~ 已闭环**: dead_revive frequency NO-GO. baseline recipe 已证不是 L0 ≥ 90% 杠杆.
