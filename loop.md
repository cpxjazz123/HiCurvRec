...
|------|------|-----|------|---------|------|
| **#131** Empty orphan dir cleanup + gitignore hygiene | writeup | 无 GPU | 🟡 待启动 | 2026-07-24 | (a) 删除空 orphan `https:/github.com/zar123123` (0 文件, 用户 clone 失败残留); (b) .gitignore 加显式 rules: `https:/` + `log/` (singular, baseline transient logs) + `logs_curv_*/` (per-curvature transient, Task #88). 当前 `*.log` 已覆盖大多数 transient 内容, 加显式 rule 让 reviewer 一眼看懂. 0 GPU ~3 min. |
