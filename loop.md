...
> **🟢 §16 当前状态 (2026-07-31 14:42)**: §16 backlog 收口后 (Issue #47/#48/#49/#50/#43/#51/#52/#53/#54/#57/#62 全部闭环 + **Issue #61 NO-GO 闭环 2026-07-31 14:42**). **Issue #61 (方向 C Bug 修复 hyp_c=-1.0 + Riemannian retraction) NO-GO**: Gate 0 sandbox (zero-GPU) 5/5 sanity PASS (expmap0 走 tanh 分支 真 hyperbolic, norm < 1.0; Riemannian retraction 径向投影回 Poincaré ball; wrapper subclass single-point patch 不改 upstream HG_Rec). Issue 整体 NO-GO 收口: Issue #57 Gate 0 已 NO-GO (R@10=0.0921 vs Arm A random 0.0940, Δ -1.9pp, hyperbolic < random 方向反向); Issue #61 = retry with hyp_c=-1.0 fix + retraction; Bug 修复 ≠ 信号方向翻转 (per Issue #57 Gate 0 verdict §3); task482 verdict ROI 极低; drift-cycle 2026-07-30 终结; 28+ 方向 NO-GO 收口最终确认. A/B/C 三方向全部 NO-GO, 北极星方向 FULL NO-GO 状态. R10 backlog **真空**: 唯一 ROI > 0 路径 (#30+#43 联合 / 方向 C 真 hyperbolic 修复) 都已穷尽, Issue #34 D9 PENDING (等 owner 方向). Issue #43 Stage 4 beam_size ablation DONE 6-point curve (beam=50 saturation R@10=0.10425 +2.4% vs baseline) 仍是当前仓库天花板.
...
| **(Issue #43 Gate 2b)** | **Task #336 (Issue #43 Gate 2b Stage 1 RQ-VAE 训练 + Stage 2 Sinkhorn + Stage 3 T5-mini + Stage 4 R@10 eval)** | **✅ DONE + beam_size 6-point ablation (beam=5→100). Stage 4 R@10=0.1041 (beam=20) → 0.10425 (beam=50/80/100 plateau, +2.4% vs baseline 0.1020). HypPreEncoder c=0.74 + HRQVAEWithHypPre wrapper + Stage 1 1000 epoch (batch_size=1024, sk_eps=0.0). R12 ckpt + SID file SHA256 落盘, R15 闭环. **可重现性 audit PASS** (verdicts/task336_issue43_reproducibility_audit.md, R10 2026-07-30 23:10)** |
...
| **(Issue #51)** | **Task #338 (HypPreEncoder + per-layer learned κ combined)** | **❌ NO-GO closed — Stage 4 Test R@10=0.0906 (-11.2% vs baseline 0.1020). val_NDCG@20=0.0915 (+11.4% vs baseline val 0.0821) 揭示 val/test gap 是 structural trait (Issue #49 派生). verdicts/task338_issue51_result.md + task338_arm_combined_beam20_metrics.json 落盘 (commit ae5acb9). Issue #51 GitHub closed** |
...
| **(Issue #62)** | **Task #344/348/349/350 (Issue #30 + #43 联合 HypPre × Codebook Transforms)** | **❌ NO-GO 闭环 — Gate 0 PASS (Task #348 commit 8620258, 5/5 sanity). Gate 1+2 双臂 NO-GO: Arm D (#30+#43+K0=256+Sinkhorn OFF, Task #349 commit 016e87e) USAGE-KILL @ ep30 (L0/L1/L2 usage 1/256/1/128/1/256, collision 99.99%, 跟 task178/180/231/242/299 Phase 0 mode collapse 同源); Arm C (#30+#43+K=baseline+Sinkhorn ON, Task #350 commit ab77df0) Stage 1 PASS (L0/L1/L2 usage 100%, best collision 0.0171) 但 Stage 2 3 variants (v1/v2/v3) 全 3-digit collision 99.99% (Sinkhorn ON + 小 layer 0 radius + HypPre 三联合 = 训练时 Sinkhorn-balanced, 推断时 hard argmin 全部坍缩到 layer 0 codeword 0). 4-digit unique=9922 但 codes 退化 [0,0,0,item_id]. Issue #62 整体 NO-GO, 27+ 方向 NO-GO 收口维持, R10 backlog 真空. verdicts/task349_issue62_armd_stage1_nogo.{json,md} + verdicts/task350_issue62_armc_stage2_nogo.{json,md} 落盘** |
| **(Issue #61)** | **Task #351 (Issue #61 方向 C Bug 修复 hyp_c=-1.0 + Riemannian retraction Gate 0 sandbox)** | **❌ NO-GO 闭环 — Gate 0 sandbox (zero-GPU) 5/5 sanity PASS (T1: expmap0(c=0.74) sphere contraction 跟 #164 audit 一致; T2: expmap0(c=-1.0) tanh 分支 norm max=0.95<1.0; T3: random vs hyp_c=-1.0 init 显著不同 mean_abs_diff=0.80, hyp norm max=0.52; T4: Riemannian retraction 径向投影回 Poincaré ball post-retract norm max≈1.0 cos_sim=1.0; T5: wrapper subclass single-point patch 不改 upstream HG_Rec __init__ 无 sid_embedding_init 参数). Issue 整体 NO-GO 收口: Issue #57 Gate 0 已 NO-GO (R@10=0.0921 vs Arm A random 0.0940, Δ -1.9pp, hyperbolic < random 方向反向), Issue #61 = retry with hyp_c=-1.0 fix + retraction; Bug 修复 ≠ 信号方向翻转 (per Issue #57 Gate 0 verdict §3); task482 verdict ROI 极低; drift-cycle 2026-07-30 终结. A/B/C 三方向全部 NO-GO, 北极星方向 FULL NO-GO 状态, 28+ 方向 NO-GO 收口最终确认. Issue #61 GitHub closed (commit 314e56c, push 成功). verdicts/task351_issue61_gate0_nogo.{json,md} 落盘** |
...
2. ✅ Issue #47/#48/#49/#50/#43/#51/#52/#53/#54 均已闭环, verdict push + GitHub closed (commit acc0ad4)
3. **当前 backlog 全空** — R10 主动推进模式: 5 issue (#49/#51/#52/#53/#54) 联立 NO-GO 后, 候选方向:
   - Issue #43 HypPreEncoder 深化 (Stage 4 ablation: K-sweep / Sinkhorn 变体 / beam_size 调优)
   - Issue #30 r_l + s_l 极端 per-layer codebook transforms 跟 Issue #43 SID 联合
   - Stop micro-tune, 转架构层突破 (Issue #43 HypPreEncoder 是当前唯一 +2.1% 真杠杆)
4. R10 主动推进: 等 owner 明确新方向 (Issue #43 深化 / Issue #30 联合 / 其他)
