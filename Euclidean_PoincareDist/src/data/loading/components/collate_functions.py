...
def collate_fn_train(
    ...
    for field_name, field_sequence in batch.items():  # type: ignore
        ...
        # 1. in-batch padding s.t. all sequences have the same length and in the format of pt tensor
        current_sequence = [
            s.reshape(1) if s.dim() == 0 else s for s in current_sequence
        ]
        current_sequence = pad_sequence(
            current_sequence, batch_first=True, padding_value=padding_token
        )
        ...
        if field_name in labels:
            _label_spec = labels[field_name]
            if isinstance(_label_spec, dict) and "transform" in _label_spec:
                label_function = _label_spec["transform"]
            elif hasattr(_label_spec, "transform"):
                label_function = _label_spec.transform
            else:
                label_function = _label_spec
