...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # === 双曲机制参数 (R36/R36-strict-v2) ===
        # "none"           — 纯欧氏 (与 Euclidean_Base 等价)
        # "expmap"         — M1: encoder 输出后接 expmap0 投影到 Poincare 球
        # "poincare_dist"  — M2: Quantize 距离用 Poincare 距离 (frozen κ)
        # "learnable_kappa"— M3: Poincare 距离 + learnable κ (sigmoid-clamp)
        ...
        self.hyperbolic_mechanism = hyperbolic_mechanism
        self.quantize_distance_mode = quantize_distance_mode

        # === 双曲机制模块条件初始化 (避免未启用机制目录加载多余模块) ===
        # expmap: 注入在 encoder 之后; poincare_dist/learnable_kappa: 共享给 Quantize 层
        self.expmap = nn.Identity()
        self.poincare_distance_module = None
        if hyperbolic_mechanism == "expmap":
            from modules.hyperbolic import PoincareExpmap
            self.expmap = PoincareExpmap(curvature=vae_init_curvature)
        elif hyperbolic_mechanism == "poincare_dist":
            from modules.hyperbolic import PoincareDistance
            self.poincare_distance_module = PoincareDistance(curvature=vae_init_curvature)
        elif hyperbolic_mechanism == "learnable_kappa":
            from modules.hyperbolic import LearnableKappaPoincareDistance
            self.poincare_distance_module = LearnableKappaPoincareDistance(init_curvature=vae_init_curvature)
        elif hyperbolic_mechanism != "none":
            raise ValueError(
                f"Unknown hyperbolic_mechanism={hyperbolic_mechanism!r}; "
                f"expected one of: none / expmap / poincare_dist / learnable_kappa"
            )
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    distance_mode=quantize_distance_mode,
                    poincare_distance_module=self.poincare_distance_module,
                )
    ...
    def load_pretrained(self, path: str) -> None:
        ...
        # strict=False: 兼容无 hyperbolic 模块的旧 RQ-VAE ckpt (curvature buffer/param 不在 state_dict)
    ...
    def encode(self, x: Tensor) -> Tensor:
        z = self.encoder(x)
        # M1 Expmap: encoder 输出后接 expmap0; 其他机制 = Identity (no-op)
        z = self.expmap(z)
        return z
