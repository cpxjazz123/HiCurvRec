...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "hyperbolic_fisher_v278"  # ← v278 NOVEL: 双曲几何损失 (R36n f) + v270 cyclic + M2 + M3. 论文支撑: "Hyperbolic Discriminant Analysis" (Chami et al. 2019 NeurIPS) + 双曲空间 Fisher 判别. 机制: (a) C5 margin regularization 启用 MARGIN_REG_WEIGHT=1.0 (top2-top1 距离 margin 正则, v270 baseline 设 0); (b) Hyperbolic Fisher Discriminant Loss = -between/within (推 codebook 元素互相远离 + 拉近 batch sample 到分配的 codeword). 保留 v270 全部 (USE_CYCLIC_CURVATURE=True, USE_M2_INTRINSIC=True, USE_M3_TRANSPORT=True, USE_FIXED_CURVATURE=True, DISTANCE_MODE=L2, hyperbolic_distance=True, sk_eps=0.05). 目标: test_R@10 > 0.1130396262886598 (v270 baseline). 7 R36n (a/b/c/d) variants 全 char-level equal baseline → R36h ceiling 7 次证伪. v278 走 R36n (f) 双曲几何损失 是 orthogonal 方向.
