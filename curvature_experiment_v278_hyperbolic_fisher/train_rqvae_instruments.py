...
MARGIN_REG_WEIGHT = 0.5          # v278 R36o Round 3: 正则权重 (intermediate)
...
# v278 NOVEL: 双曲几何损失 (R36n f) — C5 margin regularization + Hyperbolic Fisher Discriminant Loss
# 论文支撑: "Hyperbolic Discriminant Analysis" (Chami et al. 2019 NeurIPS) + 双曲空间 Fisher 判别
# 机制:
#   (a) C5 margin: 启用 MARGIN_REG_WEIGHT=1.0 (top2-top1 距离 margin 正则, 已在 RqVae 实现)
#   (b) Fisher discriminant loss: -mean(codeword_pairwise_poincare_dist) + mean(sample_to_assigned_codeword_dist)
#       即推 codebook 互相远离 (类间散度大) 同时拉近 batch sample 到分配的 codeword (类内紧度小)
#       ratio = -between / within. 当 between >> within → loss 负 (loss 最小化 → between ↑, within ↓)
USE_MARGIN_REG = True                     # v278 R36o Round 3: margin regularization weight = 0.5 (intermediate)
USE_FISHER_LOSS = False                   # v278 R36p Round 1: disable Fisher loss (Stage 2 L1/L2 collapse 50%/40% < 75% 阈值)
FISHER_LOSS_WEIGHT = 0.0                  # R36p Round 1: 关闭 Fisher (Fisher 触发 positive feedback collapse)
...
def main():
    ...
    model = RqVae(
        ...
        margin_reg_weight=MARGIN_REG_WEIGHT if USE_MARGIN_REG else (0.0 if USE_FIXED_CURVATURE else MARGIN_REG_WEIGHT),  # v278: USE_MARGIN_REG 强制开启
        margin_target=MARGIN_TARGET,
        fisher_loss_weight=FISHER_LOSS_WEIGHT if USE_FISHER_LOSS else 0.0,  # v278: Fisher discriminant loss weight
