...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v278: Hyperbolic Fisher Discriminant Loss (R36n f) — between/within cluster 散度比
        fisher_loss_weight: float = 0.0,
        ...
        self.fisher_loss_weight = fisher_loss_weight
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v278: Hyperbolic Fisher Discriminant Loss (R36n f)
        # between_cluster: 推 codebook 元素互相远离 (Poincaré ball 上 centroid 散开)
        # within_cluster: 拉近 batch sample 到分配的 codeword (类内紧度)
        # loss = -between / within  (Fisher ratio 反向; minimize loss → between↑, within↓)
        fisher_loss_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.fisher_loss_weight > 0:
            from modules.hyperbolic import _poincare_distance_t
            for li in range(self.n_layers):
                layer = self.layers[li]
                c_l = layer.get_c()  # scalar (用全局 c for Fisher loss)
                cb = layer.embedding.weight  # (K, D)
                # between: pairwise Poincaré distance of codebook (取上三角, 屏蔽对角)
                K = cb.shape[0]
                cb_a = cb.unsqueeze(0).expand(K, K, -1)
                cb_b = cb.unsqueeze(1).expand(K, K, -1)
                d_between = _poincare_distance_t(cb_a, cb_b, c_l).squeeze(-1)  # (K, K)
                mask_off = ~torch.eye(K, dtype=torch.bool, device=cb.device)
                d_between_mean = d_between[mask_off].mean()
                # within: 分配 codeword 后 sample (residual) 与 assigned codeword 距离
                # 仅计算 assigned pair (res_l[i] → emb_l[i]), 不算全 (B,B) pairwise (节省显存)
                ids_l = quantized.sem_ids[:, li]  # (B,)
                emb_l = layer.embedding(ids_l)  # (B, D) assigned codeword
                # residuals 形状 (B, D, n_layers), 取第 li 层 → (B, D)
                res_l = quantized.residuals[:, :, li]
                d_within = _poincare_distance_t(res_l, emb_l, c_l).mean()  # (B, 1) -> scalar
                # Fisher ratio 反向: minimize -(between / (within + β))
                # β=0.1 防 within→0 导致 ratio 爆炸 (loss=-1e3 量级, 训练崩溃)
                fisher_loss_l = -d_between_mean / (d_within + 0.1)
                fisher_loss_total = fisher_loss_total + fisher_loss_l
            fisher_loss_total = fisher_loss_total / self.n_layers
        loss = (
            (reconstuction_loss + rqvae_loss).mean()
            + self.margin_reg_weight * margin_loss
            + self.spread_loss_weight * spread_loss_total
            + self.fisher_loss_weight * fisher_loss_total
        )
