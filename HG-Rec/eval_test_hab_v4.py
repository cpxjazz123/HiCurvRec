"""v4 test eval: HG_Rec + PoincareEmbeddingFixed (κ=0.5) + HAB → test.parquet beam=20."""
import os
import sys
import torch
import numpy as np
...
from model.hg_rec_curv_v4 import HG_Rec_Curv_V4
from model.hab import precompute_hab_distances, make_layer_id_lut, HABModule, install_hab

CKPT = sys.argv[1] if len(sys.argv) > 1 else \
    "./ckpt/Instruments/Aug-18-2026_04-35-07/HG_Rec_curv_hab_v4_epoch_59.pth"
...
def main():
    ...
    # 安装 HAB
    D_list, Dbar_list, _ = precompute_hab_distances(STAGE2_CKPT, c_list=(1.0, 1.0, 1.0))
    hab = HABModule(Dbar_list, lambda_max=0.20)
    lut = make_layer_id_lut(vocab_size=1025)
    install_hab(model, hab, lut)
