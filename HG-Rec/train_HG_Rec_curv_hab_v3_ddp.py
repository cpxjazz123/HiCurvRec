"""DDP 4-card training for HG-Rec v3 — PoincareEmbedding + HAB dual curvature.

- HG_Rec_Curv_V3 (Poincaré embedding std=0.1, learnable κ) + HAB at encoder attention
- 双曲几何双重注入 (R36 #2 + #3 组合):
  (a) input embedding: expmap0(v, κ) learnable κ ∈ [0.05, 8.0]
  (b) attention bias: frozen Dbar + learnable λ per layer
...
def main():
    ...
    parser.add_argument('--emb_init_std', type=float, default=0.1)
    ...
    if rank == 0:
        ...
        logging.basicConfig(
            filename=os.path.join(log_path, 'HG_Rec_curv_hab_v3_ddp.log'),
            level=logging.INFO,
    ...
    model = HG_Rec_Curv_V3(config).to(device)
    # 重设 kappa_init + emb_init_std (覆盖 model 内部默认)
    with torch.no_grad():
        model.model.shared.log_kappa.fill_(math.log(args.kappa_init))
        nn.init.normal_(model.model.shared.embedding.weight, std=args.emb_init_std)
        model.model.shared.embedding.weight[args.pad_token_id].zero_()
    if rank == 0:
        print(model.n_parameters, flush=True)
        # Sanity: 报告 embedding norm 与 κ 初始值
        w = model.model.shared.embedding.weight
        norms = w.norm(dim=-1).mean().item()
        print(f"[V3] init: emb_std={args.emb_init_std} ||v||≈{norms:.4f} "
              f"kappa_init={args.kappa_init} √κ||v||/2≈{math.sqrt(args.kappa_init)*norms/2:.4f}",
              flush=True)

    # === HAB 模块: Stage2 ckpt → frozen Dbar + learnable λ ===
    ...
    if rank == 0:
        ...
        n_kappa = sum(p.numel() for p in model.model.shared.parameters() if p.requires_grad)
        print(f"[V3] HAB params: {n_hab}, Poincare params: {n_kappa}", flush=True)
    ...
    for epoch in range(args.num_epochs):
        ...
        if rank == 0:
            ...
            print(f"[V3] E{epoch} κ={cur_kappa:.4f} log_κ={cur_log_kappa:.4f} "
        ...
        if rank == 0:
            print(f"[E{epoch}] loss={avg_loss:.4f} train={t_train:.1f}s eval={t_eval:.1f}s "
                  ...
                  flush=True)
            logging.info(f"E{epoch} loss={avg_loss:.4f} κ={cur_kappa:.4f} λ={[round(x,4) for x in lambda_eff]} "
        ...
        if save_best:
            _ckpt = os.path.join(ckpt_path, f"HG_Rec_curv_hab_v3_epoch_{epoch}.pth")
    ...
    if rank == 0:
        ...
        with open(os.path.join(ckpt_path, "v3_ddp_meta.json"), "w") as f:
