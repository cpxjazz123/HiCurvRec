"""DDP 4-card training for HG-Rec v4 — PoincareEmbeddingFixed (κ=0.5) + HAB + 曲率正则.

- HG_Rec_Curv_V4: PoincareEmbedding 固定 κ=0.5 (不学, 避免 v3 漂移)
- HAB at encoder attention (frozen Dbar + learnable λ)
- 曲率正则项: L_reg = λ_reg * mean((||expmap(v)|| - target_norm)²)
  → 强制 embedding 落在 Poincaré ball 中段, 保持双曲几何
...
from model.hg_rec_curv_v4 import HG_Rec_Curv_V4, PoincareEmbeddingFixed
...
def curvature_regularizer(model_module, target_norm=0.5):
    """曲率正则: 让 expmap(v) 落在 target_norm 附近 (Poincaré ball 中段).

    强制 embedding 保持双曲几何特征, 不会塌缩到 origin 或 Euclidean 边缘.
    """
    pe = model_module.model.shared
    # 重新计算 expmap(embedding.weight) — 与 forward 等价但避免干扰 autograd
    ...
    expmap = v * factor
    # mean over all vocab positions (excluding PAD)
    mask = torch.ones_like(expmap[:, 0])
    mask[pe.padding_idx] = 0
    expmap_norms = expmap.norm(dim=-1)
    masked_norms = expmap_norms * mask
    mean_norm = masked_norms.sum() / mask.sum().clamp(min=1)
    return ((mean_norm - target_norm) ** 2)


def train_one_epoch(model, train_loader, optimizer, device, rank, epoch, lambda_reg=0.01, target_norm=0.5):
    model.train()
    ...
    for batch in pbar:
        ...
        reg = curvature_regularizer(model.module, target_norm=target_norm)
...
def main():
    ...
    parser.add_argument('--kappa_fixed', type=float, default=0.5)
    parser.add_argument('--target_norm', type=float, default=0.5)
    ...
    if rank == 0:
        ...
        logging.basicConfig(
            filename=os.path.join(log_path, 'HG_Rec_curv_hab_v4_ddp.log'),
            level=logging.INFO,
    ...
    model = HG_Rec_Curv_V4(config).to(device)
    # 重设 kappa (覆盖 model 内部默认, 用于 sweep)
    with torch.no_grad():
        model.model.shared.kappa.fill_(args.kappa_fixed)
        model.model.shared.log_kappa.fill_(math.log(args.kappa_fixed))
    if rank == 0:
        print(model.n_parameters, flush=True)
        w = model.model.shared.embedding.weight
        norms = w.norm(dim=-1).mean().item()
        sqrt_k = math.sqrt(args.kappa_fixed)
        print(f"[V4] init: kappa_fixed={args.kappa_fixed} ||v||≈{norms:.4f} "
              f"√κ||v||/2≈{sqrt_k*norms/2:.4f}", flush=True)
    ...
    if rank == 0:
        ...
        n_pe = sum(p.numel() for p in model.model.shared.embedding.parameters())
        print(f"[V4] HAB params: {n_hab}, Embedding params: {n_pe}, κ FIXED={args.kappa_fixed}", flush=True)
    ...
    for epoch in range(args.num_epochs):
        ...
        avg_ce, avg_reg = train_one_epoch(model, train_loader, optimizer, device, rank, epoch,
                                           lambda_reg=args.lambda_reg, target_norm=args.target_norm)
        ...
        if rank == 0:
            ...
            with torch.no_grad():
                ...
                factor = torch.tanh(sqrt_k * v_norm / 2.0) / (sqrt_k * v_norm)
                expmap = v * factor
                expmap_norm_mean = expmap.norm(dim=-1).mean().item()
            print(f"[V4] E{epoch} expmap_norm={expmap_norm_mean:.4f} "
        ...
        if save_best:
            _ckpt = os.path.join(ckpt_path, f"HG_Rec_curv_hab_v4_epoch_{epoch}.pth")
    ...
    if rank == 0:
        ...
        with open(os.path.join(ckpt_path, "v4_ddp_meta.json"), "w") as f:
