...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "per_layer_c_end_stagger_v180"  # ← v180 NOVEL: per-layer c_end curriculum stagger. v161 = L0/L1/L2 共用 c_start=0.05→c_end=0.7 (uniform); v180 = c_start 统一 0.05, 但 c_end per-layer 不同: L0=0.5, L1=0.6, L2=0.7. 三层 c_end 阶梯递增, 形成 per-layer 异质曲率天花板. R36n (b) per-layer异质曲率. 论文支撑: "Hyperbolic Attention" (Gulcehre 2019, 第 4 节: 不同层不同曲率提升表达力) + product manifold learning. 所有层保持 v161 baseline distance_mode=MAHALANOBIS + hyperbolic_distance=True, 避免 per-layer distance 异质触发的 L0 collapse (v176+v179 R36p). v161 baseline test_R@10=0.35337467783505155 LOCKED.
