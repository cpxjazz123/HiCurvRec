...
# 路径 helper: _resolve_code_file 期望两个根: stage0 output 目录
# (含 train/valid/test + Amazon_2023_Instruments 子目录) 和 stage3 旧
# dataset/Amazon_2023_Instruments/ (stage2 直接落地的 item_sids_*.json).
...
# 2026-09-23 调整: 在 150 epoch 预算下, warmup 由 0.05->0.10(15 epoch, 0.3e-3->3e-3 更稳),
#                  LR_MIN_FACTOR 由 0.1->0.20 (末段 LR=6e-4 而非 3e-4, 末 10 epoch loss 仍在动,
#                  给末段精调更多空间).
USE_LR_SCHEDULER = True   # iter12: 启用 cosine annealing
LR_WARMUP_PCT   = 0.10    # 10% warmup (前 15 epoch 从 0.3e-3 升到 3e-3)
LR_MIN_FACTOR   = 0.20    # cosine 末端 LR = LR * 0.20 (= 6e-4, 旧 3e-4)
...
# Stage0 emits train/valid/test.parquet into results/stage0_build_parquet/.
# Stage3 reads from there directly -- the per-stage duplicate under
# stage3_T5Train/dataset/Amazon_2023_Instruments/ is obsolete.
DATASET_PATH  = "/home/wlia0047/ar57/wenyu/GeneRec/results/stage0_build_parquet/"
# Stage1 keeps ``sentence_t5.npy`` (and sidecar ``item_ids.json``) at
# stage1_GeneEmbedding/output/ — declared here only so the contract is
# visible; stage3 itself does not consume them.
STAGE1_OUT_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/stage1_GeneEmbedding/output"
SENT_EMB_NPY   = os.path.join(STAGE1_OUT_DIR, "sentence_t5.npy")
ITEM_IDS_JSON  = os.path.join(STAGE1_OUT_DIR, "item_ids.json")
# Stage2 RQ-VAE writes the per-variant item_sids_<variant>.json (the SID
# codebook) into either the project-level results/ root or the historical
# stage3_T5Train/dataset/Amazon_2023_Instruments/ folder.  ``_resolve_code_file``
# below checks a small set of fallback paths so the SID file is found
# regardless of which stage emits it (see CLAUDE.md §8: only ``main``
# branch touched; the fallback chain mirrors the upstream layout).
RESULTS_DIR    = "/home/wlia0047/ar57/wenyu/GeneRec/results"
...
LOG_PATH      = "/home/wlia0047/ar57/wenyu/GeneRec/results/stage3_T5Train/" + RQVAE_VARIANT + "/logs/"
SAVE_PATH     = "/home/wlia0047/ar57/wenyu/GeneRec/results/stage3_T5Train/" + RQVAE_VARIANT + "/ckpt/"
SEED          = 42

# 评估 / Screen
TOPK_LIST                = [5, 10]   # 必须包含 10 (RecBole 监控 ndcg@10)
BEAM_SIZE                = 20        # >= max(TOPK_LIST)
NO_EVAL                  = True      # 2026-09-22: 150 epoch 短训练关闭评估, 仅 train_loss 监控
...
def _resolve_code_file(dataset_dir: Path, requested: str) -> Path:
    """Resolve the SID codebook file ``CODE_PATH``.

    Post-Refactor (2026-09-23): stage2 RQ-VAE is expected to write
    ``item_sids_<variant>.json`` into ``results/stage2_RQ-VAE/<variant>/``
    (the project-wide ``results/`` root houses every stage's output
    independently).  Historical locations are checked as fallbacks so
    legacy checkpoints keep working.
    """
    ...
    if requested_path.is_absolute():
        ...
        # Stage0 output root may also host the SID JSON for backward compat
        # (curvature_config.py names it "sids_for_hgrec.npy" but JSON is a
        # common alias).
        candidates.extend(
            [
    ...
    # Layered fallback chain across the post-Refactor layout:
    # 1. stage2 RQ-VAE's per-variant output dirs (TIGER + curvature RQ-VAE
    #    each write to a slightly different shape).
    # 2. Historical stage3_T5Train/dataset/Amazon_2023_Instruments/ folder
    #    (where export_sids_for_stage3.py historically wrote the JSON).
    candidates.extend(
        [
            SCRIPT_DIR.parent / "results" / "stage2_RQ-VAE" / "TIGER_RQ-VAE" / requested_path,
            SCRIPT_DIR.parent / "results" / "stage2_RQ-VAE" / "curvature_RQ-VAE" / requested_path,
            SCRIPT_DIR.parent / "results" / "stage2_RQ-VAE" / requested_path,
            SCRIPT_DIR / "dataset" / "Amazon_2023_Instruments" / requested_path,
            SCRIPT_DIR / "dataset" / "Amazon_2023_Instruments"
            / ("Amazon_2023_Instruments" + requested_path.name),
        ]
    )
...
def _build_dataset(config, filename, mode, code_file):
    # Post-stage0: train/valid/test sit directly inside ``DATASET_PATH``;
    # ``DATASET_NAME`` is preserved only as a log/ckpt path segment below.
...
def main():
    ...
    screen_failed = False
    screen_history = []
    # === 2026-09-22: 全程训练数据 JSONL 收集器 ===
    _metrics_path = os.path.join(log_path, "training_metrics.jsonl") if (rank == 0 and log_path is not None) else None
    _metrics_lock = threading.Lock() if (rank == 0 and log_path is not None) else None
    _T0 = time.time()
        ...
        with _metrics_lock:
            with open(_metrics_path, "a", encoding="utf-8") as _fh:
                ...
                try:
                    ...
                    pass
    _record("train_start", num_epochs=config["num_epochs"], no_eval=config["no_eval"], batch_size=config["batch_size"], code_path=config["code_path"], variant=config.get("variant", "unknown_variant"), log_path=log_path, ckpt_path=ckpt_path, rank=rank, world_size=world_size)
    ...
    for epoch in range(1, config["num_epochs"] + 1):
        ...
        _epoch_train_time_s = round(time.time() - _epoch_t0, 3)
    ...
    if rank == 0:
        ...
        _record(
            ...
            n_eval=test_result["n_eval"],
            test_recall_at_5=test_result.get("test_recall@5"),
            test_recall_at_10=test_result.get("test_recall@10"),
            test_ndcg_at_5=test_result.get("test_ndcg@5"),
...
_LAUNCHER = {
    ...
    "master_port": 50201,
    "log":        "logs/_stage3_launcher_curvature_repaired.log",
