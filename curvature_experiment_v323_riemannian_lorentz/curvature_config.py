...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "riemannian_lorentz_v323"  # ← v323 NOVEL (R36n c+d 联合, 跳出 R36h ceiling): v316_fixed backbone (anisotropy_loss + spread_loss 真正反向传播) + Lorentz manifold commit path + **真正的 Riemannian Adam optimizer** (Lorentz exp_map retraction 更新 codebook, 不 detach). 论文支撑: Bécigneul & Ganea "Riemannian Adaptive Optimization Methods" ICLR 2019 + Law et al. NeurIPS 2019 "Lorentzian Distance Learning". v305/v128/v285/v290/v307 之前都用 Poincaré retraction, 5 次证伪; v323 用 Lorentz retraction (acosh 数值稳定, artanh 在边界 saturate). 比较 baseline = v316_fixed (test_R@10=0.11606). R36q Gate: 命中 (c) manifold 几何替换 + (d) Riemannian 优化器, 论文关键词命中. R36h ceiling 风险: Stage 1 端纯曲率变更 60+ 次锁死, v323 是 (c)+(d) 联合新组合, 可能打破 ceiling.
