...
USE_FIXED_CURVATURE = True                # C26 HG-Rec default: c=1 固定 (关 M2/M3/C5) — v323 R36p Round 1 改 c=1 fixed (cyclic 触发 collapse)
...
USE_CYCLIC_CURVATURE = False              # v323 R36p Round 1 诊断: 禁用 cyclic (c 极小时 Lorentz retraction + commit 数值不稳, 触发 L0/L1/L2 collapse)
...
USE_GEODESIC_MIDPOINT_COMMIT = False      # v323: 关 midpoint (避免与 Lorentz commit 互斥, 只保留 v323 核心机制)

# v323 NOVEL: Riemannian Adam optimizer with Lorentz retraction (R36n c+d 联合)
USE_RIEMANNIAN_ADAM = False               # v323 R36p Round 3: 禁用 Riemannian Adam (R1/R2 都 collapse 1/1/1/256)
USE_LORENTZ_COMMIT = True                 # v323 R36p Round 3: 启用 Lorentz commit, 测试 Lorentz commit + v316 backbone 联合 (R36n c+f, 放弃 d)
RIEMANNIAN_ADAM_LR = 1e-3                 # v323: Riemannian Adam learning rate
RIEMANNIAN_ADAM_C = 1.0                   # v323: Lorentz manifold 曲率 (与 use_fixed_curvature 一致)

# v316 backbone: Riemannian Pairwise Distance Std-Matching Loss (RPDVM)
# v323 必须保留 v316_fixed 真正反向传播的 anisotropy (baseline = v316_fixed, 退化为 baseline 即 R37 FAIL)
# target_std=2.0 (Poincaré 单位) 约为球面直径的 40% (球面直径 ≈ 5.0).
USE_ANISOTROPY_REG = True         # v323: 保留 v316 backbone (与 v316_fixed baseline 一致)
ANISOTROPY_LOSS_WEIGHT = 0.001    # v323: 正则权重 (极小, 不主导 recon loss)
ANISOTROPY_TARGET_STD = 2.0       # v323: 目标 std (Poincaré 单位)
ANISOTROPY_TEMP = 1.0             # v323: 温度缩放
...
def main():
    ...
    model = RqVae(
        ...
        hyperbolic_distance=False,  # v323: 关 Poincaré 距离 (用 Lorentz commit 替代)
        use_lorentz_commit=USE_LORENTZ_COMMIT,  # v323: Lorentz manifold commit path (R36n c) — R36p R2 禁用
        ...
        anisotropy_loss_weight=ANISOTROPY_LOSS_WEIGHT if USE_ANISOTROPY_REG else 0.0,  # v323: v316 backbone
        use_anisotropy_reg=USE_ANISOTROPY_REG,  # v323: v316 backbone
        anisotropy_target_std=ANISOTROPY_TARGET_STD,  # v323: v316 backbone
        anisotropy_temp=ANISOTROPY_TEMP,  # v323: v316 backbone
    ).to(device)
    ...
    # v323 NOVEL: Riemannian Adam for codebook parameters, AdamW for others
    other_optimizer = None
    if USE_RIEMANNIAN_ADAM:
        from modules.riemannian_optimizer import RiemannianAdam
        # 分离 codebook 参数 (用 Riemannian Adam) 和其他参数 (用 AdamW)
        codebook_params = []
        other_params = []
        for name, p in model.named_parameters():
            if 'embedding.weight' in name and 'quantize' in name.lower() or 'layers.' in name and 'embedding' in name:
                codebook_params.append(p)
            else:
                other_params.append(p)
        # 如果没有匹配, 用 fallback — 全部用 AdamW (避免 silent no-op)
        if len(codebook_params) == 0:
            print(f"[v323 WARN] no codebook params found, fallback to AdamW", flush=True)
            ...
            other_optimizer = torch.optim.AdamW(other_params, lr=1e-3, weight_decay=1e-4)
            print(f"[v323] RiemannianAdam: {len(codebook_params)} codebook params | AdamW: {len(other_params)} other params", flush=True)
    else:
        optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v323: 第二个 optimizer (其他参数用 AdamW)
            if other_optimizer is not None:
