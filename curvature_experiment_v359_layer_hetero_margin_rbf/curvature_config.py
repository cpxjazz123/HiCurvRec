...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "v359_layer_hetero_margin_rbf"  # ← v359 NEW EXPERIMENT (2026-09-04, R36h ceiling 第 79 次探索). R36n (b)+(f) 三联组合: Per-Layer 异质曲率 c_l + c_l-aware RBF margin 双曲几何损失. 论文支撑: Gu et al. ICLR 2019 "Learning Mixed Curvature Representations" (b) + Chami et al. ICML 2021 "Hyperbolic Knowledge Graph Embeddings" / Ganea et al. NeurIPS 2018 (f RBF kernel). 机制核心: 在 v318 cyclic c(t) 全局基础 + v317 midpoint commit 基础上, 三层独立 c_l (阶梯 [0.5, 0.7, 1.0]), 每层 RBF margin loss exp(-γ_l * d_P²(ze, eq, c_l)²) with γ_l = c_l/2 (RBF bandwidth 与曲率联动). 历史 lineage: v270 cyclic+c_end, v282 midpoint commit, v317 baseline, v318 promoted, v319-v346 各种 R36n 组合 R37 FAIL (R36h ceiling 第 78 次验证), v359 尝试 (b)+(f) 三联新组合. **目标**: 突破 v318 0.11791 ceiling. **风险**: R36h ceiling 已 78 次验证 Stage 1 端纯曲率变更不突破; R36p cascade collapse risk (per-layer 异质 c 在 cascade RQ-VAE 跨层耦合).
