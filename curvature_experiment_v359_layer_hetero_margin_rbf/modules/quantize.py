...
class QuantizeOutput(NamedTuple):
    ...
    rbf_margin_loss: Tensor  # v359 NOVEL: per-layer RBF margin 双曲几何损失 (R36n f), exp(-γ_l * d_P²(ze, eq_top1))
...
class Quantize(nn.Module):
    def __init__(
        ...
        # === v359 NOVEL: Per-Layer 异质曲率 + RBF margin 双曲几何损失 ===
        use_per_layer_hetero_c: bool = False,    # v359: 三层独立 c_l 静态阶梯 (R36n b)
        c_per_layer_list: list = None,            # v359: 静态 c 阶梯, e.g. [0.5, 0.7, 1.0]
        use_rbf_margin_loss: bool = False,        # v359: RBF margin loss (R36n f) 开关
        rbf_margin_weight: float = 0.005,          # v359: RBF margin 正则权重 (低权重, 与 spread_loss 对齐)
        rbf_margin_bandwidth_scale: float = 0.5,  # v359: γ_l = c_l * scale (RBF bandwidth 联动 c_l)
    ) -> None:
        super().__init__()
        ...
        # v359 NOVEL: Per-Layer 异质曲率 c_l (R36n b) — 三层独立 c 阶梯
        # 与 use_cyclic_curvature / use_curriculum_curvature / use_fixed_curvature 互斥 (优先级: hetero > cyclic > fixed)
        self.use_per_layer_hetero_c = bool(use_per_layer_hetero_c)
        if c_per_layer_list is not None and len(c_per_layer_list) > 0:
            self.c_per_layer_list = list(c_per_layer_list)
        else:
            self.c_per_layer_list = [1.0, 1.0, 1.0]  # 默认全 1.0 (与 HG-Rec c=1 对齐)
        # v359 NOVEL: RBF margin loss (R36n f) — 双曲几何 RBF kernel margin 正则
        # L_rbf_l = mean_b [exp(-γ_l * d_P(ze, eq, c_l)²)]  (d_P² ∈ [0, ~10], γ_l = c_l * scale)
        # 数学性质: top1-top2 距离越大 → d_P(ze, eq)² 越大 → exp(-γ*d²) 越小 → margin loss 越小
        # → 反向传播鼓励拉大 top1-top2 距离 (提高量化决策鲁棒性)
        # R36r 防护: 不 detach — 梯度从 exp 流回 dist → 经 d_P 流向 codebook/encoder (STE)
        self.use_rbf_margin_loss = bool(use_rbf_margin_loss)
        self.rbf_margin_weight = float(rbf_margin_weight)
        self.rbf_margin_bandwidth_scale = float(rbf_margin_bandwidth_scale)
        # v359: _last_rbf_margin_loss 监控 (R36r 检测 3 强制逐项打印)
        self.register_buffer("_last_rbf_margin_loss", torch.tensor(0.0), persistent=False)
    ...
    def get_c(self) -> Tensor:
        ...
        v359 NOVEL (R36n b): use_per_layer_hetero_c=True → 返回 c_per_layer_list[layer_idx],
             优先级最高 (hetero > cyclic > curriculum > fixed > learnable theta).
        """
        if self.use_per_layer_hetero_c:
            # v359: per-layer 异质 c — 用 layer_idx 索引 c_per_layer_list
            # layer_idx 通过 _layer_idx 显式设置 (在 RqVae 构造时)
            li = getattr(self, "_layer_idx", 0)
            if li < len(self.c_per_layer_list):
                c_val = self.c_per_layer_list[li]
            else:
                c_val = self.c_per_layer_list[-1]
            return torch.tensor(c_val, device=self.embedding.weight.device, dtype=self.embedding.weight.dtype)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v359 NOVEL (R36n f): RBF margin 双曲几何损失
        # L_rbf = exp(-γ_l * d_P(ze, eq, c_l)²)
        # 计算 top1 codeword 到 latent z_e 的双曲距离平方 d_P²(ze, eq_id), 然后 RBF kernel 衰减
        # 数学性质: top1-top2 距离越大 → d_P²(ze, eq)² 越大 (argmin 选中最远) → exp(-γ*d²) 越小 → loss 越小
        # 梯度反向: 鼓励拉大 top1-top2 双曲距离 → 量化决策更鲁棒
        # R36r 防护: 不 detach — 梯度从 exp 经 d_P 流回 dist → codebook (STE) + encoder
        if self.use_rbf_margin_loss and self.training and not self.hypervq:
            try:
                from modules.hyperbolic import _poincare_distance_t
                # 取 top1 ids (当前 batch argmin, 不 detach ids 因为只用其索引 codebook)
                with torch.no_grad():
                    top1_ids = ids.detach()
                # codebook 中 top1 codeword + 当前 z_e (双曲空间)
                # 注意: x 仍在 Euclidean, 需 _expmap0 到 Poincaré ball
                from modules.hyperbolic import _expmap0_t
                c_l = self.get_c()  # (1,)
                c_exp = c_l.view(-1, 1, 1)
                # expmap 潜变量到 Poincaré ball (B, 1, D)
                z_e_h = _expmap0_t(x.unsqueeze(1), c_exp)
                # 取 top1 codeword (B, D) — 用 codebook embedding.weight
                eq_top1 = self.embedding.weight[top1_ids]  # (B, D)
                # expmap 到 Poincaré ball (B, 1, D)
                eq_top1_h = _expmap0_t(eq_top1.unsqueeze(1), c_exp)
                # 双曲距离平方 d_P² (B, 1)
                d_p_sq = _poincare_distance_t(z_e_h, eq_top1_h, c_exp).squeeze(-1).squeeze(-1)
                d_p_sq = d_p_sq.clamp_min(1e-8)  # 防 exp(-γ*0) = 1 数值恒定
                # RBF bandwidth γ_l = c_l * scale (曲率联动)
                gamma = c_l * self.rbf_margin_bandwidth_scale
                # RBF kernel margin loss: exp(-γ * d²).mean()
                rbf_margin = torch.exp(-gamma * d_p_sq).mean()
                # 缓存供 R36r 检测 3 逐项打印 (无 detach, R36r 防护)
                self._last_rbf_margin_loss = rbf_margin.detach()
            except Exception:
                # 任何数值异常降级为 0 (R36r FAIL 信号: 立即 Stage 1 终止检查)
                rbf_margin = torch.tensor(0.0, device=x.device, dtype=x.dtype)
                self._last_rbf_margin_loss = torch.tensor(0.0, device=x.device, dtype=x.dtype)
        else:
            rbf_margin = torch.tensor(0.0, device=x.device, dtype=x.dtype)
        ...
        return QuantizeOutput(embeddings=emb_out, ids=ids, loss=loss, margin=margin, spread_loss=spread_loss, rbf_margin_loss=rbf_margin)
