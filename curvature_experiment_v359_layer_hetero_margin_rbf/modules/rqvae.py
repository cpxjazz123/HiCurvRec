...
class RqVaeComputedLosses(NamedTuple):
    ...
    rbf_margin_loss: Tensor  # v359 NOVEL: per-layer RBF margin 双曲几何损失 (R36n f) 总和 (mean of per-layer)
...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # === v359 NOVEL: Per-Layer 异质 c_l + RBF margin ===
        use_per_layer_hetero_c: bool = False,    # v359 (R36n b)
        c_per_layer_list: Optional[List[float]] = None,  # v359: 三层独立 c 阶梯
        use_rbf_margin_loss: bool = False,        # v359 (R36n f) RBF margin 开关
        rbf_margin_weight: float = 0.005,          # v359: 权重 (低, 与 spread_loss 对齐)
        rbf_margin_bandwidth_scale: float = 0.5,  # v359: γ_l = c_l * scale
    ) -> None:
        self._config = locals()
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    use_per_layer_hetero_c=use_per_layer_hetero_c,  # v359 (R36n b)
                    c_per_layer_list=c_per_layer_list,              # v359 三层独立 c
                    use_rbf_margin_loss=use_rbf_margin_loss,         # v359 (R36n f)
                    rbf_margin_weight=rbf_margin_weight,
                    rbf_margin_bandwidth_scale=rbf_margin_bandwidth_scale,
        ...
        # v359 NOVEL: 设置每层 _layer_idx 让 get_c() 能找到 c_per_layer_list[li]
        ...
        # v359 NOVEL: 汇总 v359 机制开关到 RqVae 顶层 (forward 路径用)
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v359 NOVEL: RBF margin loss 汇总 + 加权到 total loss
        rbf_margin_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.use_rbf_margin_loss:
            rbf_per_layer = [
                l._last_rbf_margin_loss for l in self.layers if hasattr(l, "_last_rbf_margin_loss")
            ]
            if rbf_per_layer:
                rbf_margin_total = torch.stack(rbf_per_layer).mean()
                loss = loss + self.rbf_margin_weight * rbf_margin_total
        ...
        return RqVaeComputedLosses(
            ...
            rbf_margin_loss=rbf_margin_total,
