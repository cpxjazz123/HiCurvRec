...
USE_CYCLIC_CURVATURE = False              # v359: 关 cyclic (改用 per-layer 异质 c_l 静态, R36n b + f)
C_CYCLIC_MIN = 0.3                        # v318: cyclic c_min — v359 不使用
C_CYCLIC_MAX = 1.0                        # v318: cyclic c_max — v359 不使用
C_CYCLIC_PERIOD = 50_000                  # v318: cyclic 周期 — v359 不使用

# === v359 NOVEL (2026-09-04): Per-Layer 异质曲率 c_l 静态阶梯 + RBF margin 双曲几何损失 ===
# R36n (b) per-item-per-layer-per-codebook 异质曲率 — 在 v317 midpoint + v318 cyclic c(t) 全局 curriculum
# 基础上, 改用"三层独立静态 c_l 阶梯" (L0=0.5, L1=0.7, L2=1.0), 让每层曲率在不同尺度上对齐自身表达.
# R36n (f) 双曲几何损失 (margin 正则) — 每层 RBF margin loss exp(-γ_l * d_P(ze, eq, c_l)²) with γ_l = c_l/2,
# RBF bandwidth 与 c_l 联动: 曲率越大允许 margin 越宽 (数学自洽: 高曲率空间 d_P 距离衰减快).
# 论文支撑: Gu et al. ICLR 2019 (b) + Ganea et al. NeurIPS 2018 (f RBF kernel).
# 风险: R36h ceiling 第 79 次探索 — Stage 1 端纯曲率变更, R36n 6 类 (b)+(f) 三联新组合, 历史无同模式失败.
# R36p 风险: cascade RQ-VAE L0/L1 残差 norm 极小, per-layer 异质 c 在 L0 c=0.5 可能触发 manifold 半径吞没
# → KMeans init 收束到 0 → collapse (R36g cascade risk). 缓解: c_l ∈ [0.5, 1.0] + RBF margin 拉大 top1/top2 距离.
USE_PER_LAYER_HETERO_C = True             # v359: per-layer 异质 c 静态阶梯开关
C_PER_LAYER_LIST = [0.5, 0.7, 1.0]         # v359: L0/L1/L2 静态 c 阶梯 (浅层 c 小几何空间大, 深层 c 大几何空间小)
USE_RBF_MARGIN_LOSS = True                # v359: RBF margin 双曲几何损失 (f) 开关
RBF_MARGIN_WEIGHT = 0.005                 # v359: RBF margin 正则权重 (极小, 与 spread_loss=0.005 对齐, 仅作 hint)
RBF_MARGIN_BANDWIDTH_SCALE = 0.5          # v359: γ_l = c_l * scale (scale=0.5 → γ_0=0.25, γ_1=0.35, γ_2=0.5)
USE_GEODESIC_MIDPOINT_COMMIT = True       # v359: 保留 v317 midpoint commit (R36n e)
# v359: 关 USE_MGC / USE_ANISOTROPY_REG / USE_SPREAD_LOSS — 隔离 R36n (b)+(f) 三联贡献
...
def main():
    ...
    model = RqVae(
        ...
        # === v359 NOVEL: per-layer 异质 c + RBF margin ===
        use_per_layer_hetero_c=USE_PER_LAYER_HETERO_C,  # v359: 三层独立 c_l
        c_per_layer_list=C_PER_LAYER_LIST,              # v359: [0.5, 0.7, 1.0]
        use_rbf_margin_loss=USE_RBF_MARGIN_LOSS,         # v359: RBF margin 开关
        rbf_margin_weight=RBF_MARGIN_WEIGHT,             # v359: 权重
        rbf_margin_bandwidth_scale=RBF_MARGIN_BANDWIDTH_SCALE,  # v359: γ_l = c_l * scale
    ).to(device)
        ...
        for batch in train_loader:
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                print(
                    ...
                    f"| rbfl={float(out.rbf_margin_loss.item() if hasattr(out, 'rbf_margin_loss') and hasattr(out.rbf_margin_loss, 'item') else 0.0):.4f} "
