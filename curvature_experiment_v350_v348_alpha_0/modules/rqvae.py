...
# v350 SWEEP: hardcoded module-level α for L0 geodesic midpoint commit
# v348 baseline α=0.5 (midpoint), v350 α=0.0 (pure codeword passthrough)
# v351 α=0.25, v352 α=0.75, v353 α=1.0 (pure input passthrough)
# R30: 模块级常量硬编码 (禁 os.environ.get), 实验隔离 cp -r 副本
MIDPOINT_ALPHA = 0.0
        ...
        for li, layer in enumerate(self.layers):
            ...
            if self.gate_M2_intrinsic:
                ...
                if self.use_geodesic_midpoint_commit and self.midpoint_layer_mask[li]:
                    ...
                    # v350 SWEEP: α parameterizes geodesic commitment depth
                    # α=0 → log_mid=log_e (pure codeword), α=1 → log_mid=log_r (pure input), α=0.5 → midpoint
                    log_mid = MIDPOINT_ALPHA * log_r + (1.0 - MIDPOINT_ALPHA) * log_e
