...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "riemannian_bridge_v319"  # ← v319 NEW: Cauchy Riemannian Bridge commit path (R36n e 几何变换). 论文 Cauchy-Riemann bridge (Gu et al. ICLR 2019). commit path = exp_(0.5*c1+0.5*c2)( 0.5 * log_(c1_emb)(res) + 0.5 * log_(c2_emb)(res) ), 双曲 midpoint on mixed-curvature geodesic. c1=per-layer baseline c=1, c2=per-layer cyclic c(t) ∈ [0.5, 1.5]. 与 v282 (single curvature midpoint) 区别: v282 用固定 c, v319 用 0.5/0.5 混合 c1+c2, 引入时间维度 + 异质维度, 同时 R36n (a) + (e) 联合. R36n (a) cyclic 是 curriculum 基础, R36n (e) midpoint 是几何变换. v316_fixed backbone (anisotropy + spread_loss) 提供曲率正则.
