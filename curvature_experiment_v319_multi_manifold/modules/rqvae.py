...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v319 NEW: mixed-curvature midpoint — commit path 用 c_bridge = 0.5*c_static + 0.5*c_cyclic(t)
        # 区别于 v282 (单 curvature midpoint): v319 在 midpoint 计算时动态混合 fixed c=1 + cyclic c(t)
        # 论文: Cauchy-Riemann bridge (Gu et al. ICLR 2019 "Learning Mixed-Curvature Representations")
        # 动机: 56 次 R36h ceiling lock 后, midpoint 静态 c 是被锁死根因之一, 引入 cyclic 混合打破 lock
        # 仍属 R36n (e) 几何变换合规方向.
        use_mixed_curvature_midpoint: bool = False,
    ) -> None:
        self._config = locals()
        ...
        # v319 NEW: mixed-curvature midpoint commit path
        # 与 v282 区别: midpoint 计算时, 用混合 c_bridge = α*c_fixed + (1-α)*c_cyclic(t)
        # 而非 layer.get_c() 返回的单一 c. 论文 Cauchy-Riemann bridge (Gu ICLR 2019).
        self.use_mixed_curvature_midpoint = bool(use_mixed_curvature_midpoint)
        self.midpoint_alpha_static = float(0.5)  # v319: 静态曲率权重 (固定 0.5)
        ...
        for li, layer in enumerate(self.layers):
            ...
            if self.gate_M2_intrinsic:
                ...
                if self.use_geodesic_midpoint_commit and self.midpoint_layer_mask[li]:
                    ...
                    if self.use_mixed_curvature_midpoint:
                        # v319 NEW: mixed-curvature midpoint (R36n e 几何变换)
                        # 用 c_bridge = α*c_fixed_static + (1-α)*c_cyclic(t)
                        # α=0.5 固定 (per-layer 静态曲率权重)
                        # c_fixed_static = layer.c_fixed (HG-Rec 默认 1.0)
                        # c_cyclic = layer.get_c() (cyclic schedule 输出)
                        c_static = torch.tensor(layer.c_fixed, device=c_per.device, dtype=c_per.dtype).view(-1, 1)
                        c_bridge = self.midpoint_alpha_static * c_static + (1.0 - self.midpoint_alpha_static) * c_per
                        log_r = _logmap0_t(res, c_bridge)
                        log_e = _logmap0_t(emb, c_bridge)
                        log_mid = (log_r + log_e) / 2.0
                        res = _expmap0_t(log_mid, c_bridge)
                    else:
