...
# v319 NEW: mixed-curvature midpoint commit path (R36n e 几何变换全新方向)
# 公式: c_bridge = 0.5 * c_static (1.0) + 0.5 * c_cyclic(t)
#        res_mid = exp_0((log_0(res) + log_0(emb))/2, c_bridge)
# 区别 v282 (单 c midpoint): v319 用混合 c, 在 fixed + cyclic 之间 0.5/0.5 平滑过渡
# 区别 v270 (cyclic curriculum): v319 cyclic 只用于 midpoint 几何, 不替代 layer.c
# 论文: Cauchy-Riemann bridge (Gu et al. ICLR 2019 "Learning Mixed-Curvature Representations")
# 动机: 56 次 R36h ceiling lock 后, midpoint 用静态 c 是 ceiling 锁死根因之一
# R36n (e) 几何变换合规方向.  比较 baseline = v316_fixed (test_R@10=0.11606).
USE_MIXED_CURVATURE_MIDPOINT = True  # v319 启用 mixed-curvature midpoint (L0 only, midpoint_layer_mask=[True, False, False])
...
def main():
    ...
    model = RqVae(
        ...
        use_mixed_curvature_midpoint=USE_MIXED_CURVATURE_MIDPOINT,  # v319 NEW: mixed-curvature midpoint (R36n e 联合)
    ).to(device)
