...
USE_GEODESIC_MIDPOINT_COMMIT = False      # v286 stochastic 覆盖 (per-batch 随机选 midpoint/slerp/sub)
# v286 NOVEL: Stochastic commit path (R36n k 几何变换变种 #5)
# 每 batch 从 {midpoint, slerp, sub} 按权重随机选一条, 三层共享同一 commit path.
# 引入 commit path 多样性, 避免 Stage 3 T5 表征锁定到单一 commit 路径.
# 与 v282 midpoint (固定 midpoint) / v285 SLERP (固定 slerp) / v284 per-layer hybrid (固定 mask) 的本质差异:
# 每 step 都是新的 commit path, 梯度信号在三种 commit 路径间随机混合.
# 论文支撑: 同 v262/v282/v285 (Ungar 2008 Gyrogroup / 4955 Álvarez 2022 SLERP) — 几何变换家族.
USE_STOCHASTIC_COMMIT = True               # v286 启用 stochastic commit path (覆盖 midpoint/slerp/mask)
# R36p Round 3 修复: 完全去掉 sub (collapses 严重), 只保留 midpoint + slerp, 权重 50/50
COMMIT_PATH_WEIGHTS = [0.5, 0.5]          # R36p R3: midpoint=0.5 / slerp=0.5 (sub 完全去掉)
COMMIT_PATH_NAMES = ["midpoint", "slerp"]
STOCHASTIC_WARMUP_STEPS = 30000           # R36p R3: 前 30k step 用 midpoint 锁定 codebook, 再开 stochastic (更长 warmup)
...
def main():
    ...
    model = RqVae(
        ...
        use_geodesic_midpoint_commit=USE_GEODESIC_MIDPOINT_COMMIT,  # v286: 关闭 (stochastic 接管)
        midpoint_layer_mask=[False, False, False],  # v286: stochastic 覆盖
        use_slerp_commit=False,  # v286: 关闭 (stochastic 接管)
        slerp_layer_mask=[False, False, False],  # v286: stochastic 覆盖
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            t_step = time.time()
            # v286 NOVEL: per-batch stochastic commit path (with R36p Round 2 warmup)
            if USE_STOCHASTIC_COMMIT and global_step >= STOCHASTIC_WARMUP_STEPS:
                import random as _random_local
                path_idx = _random_local.choices(range(len(COMMIT_PATH_NAMES)), weights=COMMIT_PATH_WEIGHTS, k=1)[0]
                stochastic_path = COMMIT_PATH_NAMES[path_idx]
            else:
                stochastic_path = "midpoint"  # R36p R2: warmup 阶段强制 midpoint (避免初始 collapse)
            out = model(seq_batch, gumbel_t=0.2, stochastic_commit_path=stochastic_path)
            loss = out.loss
            loss.backward()
