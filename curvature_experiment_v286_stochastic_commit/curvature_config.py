...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "stochastic_commit_v286"  # ← v286 NOVEL: R36n (k) Stochastic Commit Path (变种 #5 — per-batch 随机选 midpoint/slerp/sub, 三层共享). 公式: 每 batch 从 {midpoint, slerp, sub} 按权重 [0.4, 0.4, 0.2] 随机选一条 commit 路径, 三层 cascade 共享同一条路径. 论文支撑: 同 v262/v282/v285 (Ungar 2008 Gyrogroup + Álvarez 2022 SLERP + 几何变换家族). 与 v282 固定 midpoint / v285 固定 SLERP / v284 per-layer mask 的本质差异: per-step commit path 多样性, 梯度信号在三种 commit 路径间随机混合, 避免 Stage 3 T5 表征锁定到单一 commit 路径. R36h ceiling 第 22 次验证. 目标: test_R@10 > 0.1135631443298969 (v282 新 baseline).
