...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v286 NOVEL: SLERP commit (R36n e variant #4) — 即使 v286 stochastic 用, 保留 SLERP 分支以备 fallback
        ...
        # v286: SLERP commit (R36n e variant #4) — 即使 stochastic 覆盖, 保留参数以兼容接口
        self.use_slerp_commit = bool(use_slerp_commit)
        if slerp_layer_mask is None:
            slerp_layer_mask = [False] * n_layers
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001, stochastic_commit_path: Optional[str] = None) -> RqVaeOutput:
        ...
        for li, layer in enumerate(self.layers):
            ...
            if self.gate_M2_intrinsic:
                ...
                # v286 NOVEL: stochastic commit path (per-batch, R36n k 几何变换变种 #5)
                # 每 batch 随机从 {midpoint, slerp, sub} 选一条, 三层共享.
                # 引入 per-step commit path 多样性, 避免 Stage 3 T5 表征锁定到单一 commit 路径.
                if stochastic_commit_path is not None:
                    path = stochastic_commit_path
                elif self.use_slerp_commit and self.slerp_layer_mask[li]:
                    path = "slerp"
                elif self.use_geodesic_midpoint_commit and self.midpoint_layer_mask[li]:
                    path = "midpoint"
                else:
                    path = "sub"
                if path == "midpoint":
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float, stochastic_commit_path: Optional[str] = None) -> RqVaeComputedLosses:
        x = batch.x
        quantized = self.get_semantic_ids(x, gumbel_t, stochastic_commit_path=stochastic_commit_path)
