...
MOBIUS_GYROVECTOR_ALPHA = 0.5            # v337: Möbius scalar mul step α=0.5 (单值, 向后兼容)
# v338 NOVEL: per-layer Möbius scalar mul α heterogeneity (R36n b 异质 + e 几何变换联合)
# 理论动机: 早期层保留更多 residual (小 α), 后期层 commit 更充分 (大 α)
# 长度必须等于 N_LAYERS=3: L0=0.3 (commit 30% 保留 70%), L1=0.5 (中点), L2=0.7 (commit 70%)
# 论文支撑: Ungar 2008 gyrovector 中 α ⊗ x 是 Möbius 数乘 (含可调 step), per-layer 异质
# 是 R36n (b) per-layer 异质参数 + (e) 几何变换联合, R36q Gate PASS.
# === v338 Round 1 修复 (R36p v3.13 诊断): α=[0.3, 0.5, 0.7] → L1/L2 catastrophic collapse
# (L1 utility 1.2%, L2 utility 2.0%). Round 1: 全部 uniform 0.5 (等价 v337 baseline),
# 验证 per-layer 异质化是 collapse 根因, 还是 list 格式本身有 bug.
MOBIUS_GYROVECTOR_ALPHAS = [0.5, 0.5, 0.5]  # v338 Round 1: uniform (验证 list 格式无误)
...
def main():
    ...
    model = RqVae(
        ...
        mobius_gyrovector_alpha=MOBIUS_GYROVECTOR_ALPHA,  # v337: α=0.5 (向后兼容)
        mobius_gyrovector_alphas=MOBIUS_GYROVECTOR_ALPHAS,  # v338: per-layer α=[0.3, 0.5, 0.7]
    ).to(device)
        ...
        for batch in train_loader:
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                if USE_MCDQ:
                    ...
                    alpha_str = "off"
                # v338: per-layer Möbius scalar mul α (MGC commit step)
                if USE_MGC and model.module.mobius_gyrovector_alphas is not None:
                    mgc_alphas = [float(l.mobius_gyrovector_alpha) for l in model.module.layers]
                    mgc_alpha_str = "/".join(f"{a:.3f}" for a in mgc_alphas)
                else:
                    mgc_alpha_str = "off"
