...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # 默认 None = 单值 (向后兼容 v337). 若提供 List[float] length=n_layers, 每层用对应 α.
        mobius_gyrovector_alphas: Optional[List[float]] = None,
    ) -> None:
        self._config = locals()
        ...
        # v338: per-layer α list (长度 n_layers) 或 None (用单值向后兼容)
        if mobius_gyrovector_alphas is not None:
            assert len(mobius_gyrovector_alphas) == n_layers, (
                f"mobius_gyrovector_alphas len={len(mobius_gyrovector_alphas)} "
                f"必须等于 n_layers={n_layers}"
            )
            self.mobius_gyrovector_alphas = [float(a) for a in mobius_gyrovector_alphas]
        else:
            self.mobius_gyrovector_alphas = None
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    mobius_gyrovector_alpha=(
                        self.mobius_gyrovector_alphas[i]
                        if self.mobius_gyrovector_alphas is not None
                        else mobius_gyrovector_alpha
                    ),  # v338: per-layer α (List[float] length=n_layers) 或单值
