...
class HRQVAE(nn.Module):
    ...
    def forward(self, x, use_sk=True, rho_target_batch=None):
        # 2026-07-30: trainer expects 5-tuple return (out, rq_loss, indices, path_loss, (div,ent,anchor,angular)).
        # baseline HRQVAE has no path_loss / div_ent — return None placeholders for trainer compat.
        x = self.encoder(x)
        x_q, rq_loss, indices = self.hrq(x, use_sk=use_sk)
        out = self.decoder(x_q)

        path_loss = None
        _div_ent = (None, None, None, None)  # (div_loss, ent_loss, anchor_loss, angular_loss)
    ...
    def compute_loss(self, out, quent_loss, xs=None, path_loss=None, anchor_loss=None, ent_loss=None, div_loss=None, angular_loss=None):
        # 2026-07-30: trainer passes kwargs (path_loss, anchor_loss, ent_loss, div_loss, angular_loss).
        # baseline HRQVAE doesn't use them — accept and ignore for trainer compat.
