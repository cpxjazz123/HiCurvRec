...
MAX_GLOBAL_STEPS = 50_000        # v92 smoke: 50k steps (curriculum c 50k 达到 0.7, 节省 50% 时间) ≈ 4 分钟@4卡
...
# === v92 Radial Regularization (R92-Diagnostic 驱动): 推 codewords 离开球中心, 让 curvature 真正生效 ===
# R92 Task 2 发现 baseline codewords ρ=√c·||e|| ≈ 0.07-0.10 (距 Poincaré ball 中心 6-8% 球半径).
# 双曲几何在 ρ≈0 区域事实上退化为 Euclidean, 所以改 c 不改 SID (Task 3 change rate <3%).
# 区间约束 (而非固定 ρ target): ρ_min=0.20 (太靠中心 → 往外推), ρ_max=0.70 (太靠边界 → 往内拉).
# 中间不管, 避免 v91 collapse 式全锁死.
USE_RADIAL_REG = True            # v92: enable radial regularization (A)
RHO_MIN = 0.20                   # v92: codeword 距中心下限 (Poincaré 单位)
RHO_MAX = 0.70                   # v92: codeword 距中心上限 (避免接近 ball 边界)
RADIAL_LAMBDA = 1e-3             # v92: 损失权重 (中等, 不主导 recon loss; smoke 第一轮 λ=1e-3)

# === v93 Relational Distortion Supervision (D): 直接优化量化前/后 pairwise dist 关系保留 ===
# R92 Task 5: 量化本身破坏 residual relationship (L0 corr=0.16, L1=0.30, L2=0.21). v92 Gate 4 fail.
# 加 L_dist: Σ_l (1 - spearman_corr(D_l^ref, D_l^(c_l))), D_l^ref = 量化前 residual pair Euclidean dist,
# D_l^(c_l) = 量化后 codeword pair Poincaré dist with c_l. 直接监督 curvature 保留 relationship.
USE_REL_DIST_SUPERVISION = True   # v93: enable relational distortion loss (D)
REL_DIST_LAMBDA = 1e-2            # v93: 损失权重 (vs RADIAL_LAMBDA=1e-3, 略强主导 relationship 保留)
REL_DIST_N_PAIRS = 2048           # v93: 每 layer 随机 sample pair 数 (2560 items × 2048 pairs ≈ 0.8 pair/item)
...
def main():
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            loss = out.loss
            # === v92 Radial Regularization (A): L_rad = mean_{l,k} [max(0,ρ_min-ρ)² + max(0,ρ-ρ_max)²] ===
            if USE_RADIAL_REG:
                rad_terms = []
                for _layer in model.module.layers:
                    _cb = _layer.embedding.weight  # (K, D)
                    _c = float(_layer.get_c().item())
                    _norm = _cb.norm(dim=-1)  # (K,)
                    _rho = (_c ** 0.5) * _norm
                    _viol_low = torch.clamp(RHO_MIN - _rho, min=0.0) ** 2  # (K,)
                    _viol_high = torch.clamp(_rho - RHO_MAX, min=0.0) ** 2  # (K,)
                    rad_terms.append((_viol_low + _viol_high).mean())
                rad_loss = torch.stack(rad_terms).mean()
                loss = loss + RADIAL_LAMBDA * rad_loss
            # === v93 Relational Distortion Supervision (D): L_dist = Σ_l MSE(α·d_c(q_i,q_j), ||r_i-r_j||) ===
            if USE_REL_DIST_SUPERVISION:
                # Re-compute encoder output (no grad, detach); current = z.detach() 让 q 梯度只走 codebook
                with torch.no_grad():
                    z_distill = model.module.encoder(x).detach()
                current_distill = z_distill
                dist_terms = []
                for _layer in model.module.layers:
                    _cb = _layer.embedding.weight  # (K, D), 梯度流通
                    _c = float(_layer.get_c().item())
                    # Soft assignment (梯度流通到 cb)
                    _d = ((current_distill.unsqueeze(1) - _cb.unsqueeze(0)) ** 2).sum(-1)  # (N, K)
                    _soft_w = torch.softmax(-_d / 0.1, dim=-1)  # (N, K) 温度 0.1 接近 hard
                    _q = _soft_w @ _cb  # (N, D), 梯度到 cb
                    _r = current_distill - _q  # (N, D) residual
                    _N = _r.shape[0]
                    _idx = torch.randint(0, _N, (REL_DIST_N_PAIRS * 2,), device=_r.device)
                    _i_idx = _idx[:REL_DIST_N_PAIRS]
                    _j_idx = _idx[REL_DIST_N_PAIRS:]
                    # Reference: 量化前 residual pairwise Euclidean dist
                    _d_ref = ((_r[_i_idx] - _r[_j_idx]) ** 2).sum(-1).clamp(min=1e-8).sqrt()
                    # Quantized: 量化后 pairwise Poincaré dist with c_l
                    _sqrt_c = _c ** 0.5
                    _u_sq = (_q[_i_idx] * _q[_i_idx]).sum(-1)
                    _v_sq = (_q[_j_idx] * _q[_j_idx]).sum(-1)
                    _diff_sq = ((_q[_i_idx] - _q[_j_idx]) ** 2).sum(-1)
                    _denom = (1 - _c * _u_sq) * (1 - _c * _v_sq).clamp(min=1e-12)
                    _arg = (1 + 2 * _c * _diff_sq / _denom).clamp(min=1.0 + 1e-7)
                    _d_c = (1 / _sqrt_c) * torch.arccosh(_arg)
                    # Optimal linear scaling a* + MSE
                    _a = (_d_ref * _d_c).sum() / (_d_c * _d_c).sum().clamp(min=1e-8)
                    _d_scaled = _a * _d_c
                    dist_terms.append(((_d_scaled - _d_ref) ** 2).mean())
                dist_loss = torch.stack(dist_terms).mean()
                loss = loss + REL_DIST_LAMBDA * dist_loss
            loss.backward()
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                if USE_SCS:
                    ...
                    scs_str = "off"
                # v92: 加 rad_loss + ρ mean 监控
                if USE_RADIAL_REG:
                    rad_str = "/".join(f"{float(rt):.4f}" for rt in rad_terms)
                    rho_means = []
                    for _layer in model.module.layers:
                        _cb = _layer.embedding.weight
                        _c = float(_layer.get_c().item())
                        rho_means.append((_c ** 0.5) * _cb.norm(dim=-1).mean().item())
