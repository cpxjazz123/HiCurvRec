...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v104 HRQ (Hyperbolic Residual Quantization, arxiv 2505.12404):
        # reconstruction 端用 Möbius chain addition 替代 Euclidean sum (保留 Euclidean codebook update 避免 R36g cascade 风险)
        # 论文支撑: Piękos et al. 2025 KAUST/Amazon, WordNet +20% Recall@10
        # 合规: R36+R36n manifold 几何替换 (Euclidean sum → Möbius chain), R36g 风险已通过保留 Euclidean codebook update 缓解
        use_hrq_recon: bool = False,       # v104: Möbius chain addition in decoder path
    ) -> None:
        self._config = locals()
        ...
        self.use_hrq_recon = bool(use_hrq_recon)  # v104: HRQ Möbius chain recon
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        if self.use_hrq_recon:
            # v104 HRQ: Möbius chain addition reconstruction (arxiv 2505.12404)
            # embs actual shape (B, D, n_layers); chain add at n_layers dim
            from modules.hyperbolic import _expmap0_t, _mobius_add_t, _logmap0_t
            c_last = self.layers[-1].get_c().view(1, 1)  # (1,1) broadcast
            h = _expmap0_t(embs[..., 0:1], c_last)  # (B, D, 1)
            for li in range(1, self.n_layers):
                h = _mobius_add_t(
                    h,
                    _expmap0_t(embs[..., li:li+1], c_last),
                    c_last,
                )  # (B, D, 1)
            h_tan = _logmap0_t(h, c_last).squeeze(-1)  # (B, D)
            x_hat = self.decode(h_tan)  # (B, D) → decode → (B, output_dim)
        else:
