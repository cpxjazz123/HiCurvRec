...
# Möbius chain addition 替代 Euclidean sum 在 decoder 输入端
# 保留 Euclidean codebook update + hyperbolic_distance=True (Poincaré argmin)
# 简化版 (避免 R36g cascade manifold 复合风险, 仅改 recon 端)
USE_HRQ_RECON = True                      # v104: enable HRQ Möbius chain recon
...
def main():
    ...
    model = RqVae(
        ...
        # v104 HRQ: Möbius chain addition in decoder reconstruction (arxiv 2505.12404)
        use_hrq_recon=USE_HRQ_RECON,
    ).to(device)
