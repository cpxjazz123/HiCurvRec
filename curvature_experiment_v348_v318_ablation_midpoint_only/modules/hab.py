"""HAB (Hyperbolic Attention Bias) 迁移 (Issue #64/#71/#125/#132/#162 链).

来源: tasks/Issue162_layer_curvature_mixer/treatment/_lib/hyperbolic_attention_bias.py
适配 RQ-VAE 系 (Euclidean_Base_M2M3):
- vocab 768 (3x256, token = h*256 + c) → HAB_CODEWORD_OFFSETS = [0, 256, 512]
- encoder 走 inputs_embeds 路径 (item_sid_embedding_table 预计算), input_ids 仅用于 B_geo
"""
import math
import types
...
HAB_CODEWORD_OFFSETS = [0, 256, 512]  # 适配: token = h*256 + c
HAB_LAMBDA_MAX = 0.20
HAB_BIAS_RANK = 16
def load_hab_assets_from_stage2_ckpt(stage2_ckpt_path):
    ...
    if "model_state_dict" in ckpt:
        ...
        sd = ckpt["model"]  # RQ-VAE-Recommender 系 ckpt 结构
    codebook_list = []
    for l in range(3):
        cb = sd[f"layers.{l}.embedding.weight"].numpy().astype(np.float32)
        codebook_list.append(cb)
    # final_cs: 优先 ckpt['final_cs'], 否则从 theta 反推 (RQ-VAE 系: c = C_MIN + (C_MAX-C_MIN)*sigmoid(theta))
    if "final_cs" in ckpt:
        final_cs = [float(x) for x in ckpt["final_cs"]]
    else:
        _C_MIN, _C_MAX = 0.5, 2.0
        final_cs = []
        for l in range(3):
            th = sd[f"layers.{l}.theta"]
            import torch as _t
            final_cs.append(float(_C_MIN + (_C_MAX - _C_MIN) * _t.sigmoid(_t.as_tensor(th)).item()))
    if len(final_cs) != 3:
...
def install_hab(hg_rec, hab_module, layer_id_lut_array):
    ...
    hg_rec.encoder._hab_t5stack = hg_rec.encoder.encoder  # T5EncoderModel → 内部 T5Stack
    hg_rec.encoder._hab_t5stack._original_forward = hg_rec.encoder._hab_t5stack.forward
    ...
    def hab_encoder_forward(self, input_ids=None, attention_mask=None, inputs_embeds=None,
        ...
        # ===== Encoder 路径 =====
        # 适配 (RQ-VAE 系): 允许同时传 input_ids (B_geo 用) + inputs_embeds (外部预计算嵌入)
        ...
        # 适配 (RQ-VAE 系): 外部预计算 B_geo (含 sep/user 扩展) 优先, 无需 input_ids
        ext_B_geo = getattr(hg_rec, "_hab_external_B_geo", None)
        if input_ids_flat is not None or ext_B_geo is not None:
            ...
            if lambda_is_zero:
                # λ=0 → 严格等价 #61, 直接调原 forward (保持外部传入的 inputs_embeds)
                return self._original_forward(input_ids=input_ids, inputs_embeds=inputs_embeds,
                                              attention_mask=attention_mask, *args, **kwargs)
            # λ>0: 计算 B_geo 并附加到 attention_mask_4d
            if ext_B_geo is not None:
                B_geo = ext_B_geo.to(inputs_embeds.device)
            else:
        ...
        # transformers 4.46: T5Attention 需要 cache_position (line 525 cache_position[-1])
    ...
    hg_rec.encoder._hab_t5stack.forward = types.MethodType(hab_encoder_forward, hg_rec.encoder._hab_t5stack)
    return hg_rec


def make_hab_layer_id_lut(vocab_size=768):
    """构造 layer_id LUT (适配 RQ-VAE 系 vocab=768): token = h*256 + c → 层 0/1/2, 其它=-1.
    """
