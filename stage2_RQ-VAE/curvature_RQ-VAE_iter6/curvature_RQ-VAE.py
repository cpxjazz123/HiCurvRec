...
# === iter6 Riemannian Adam (R36n d 纯版) ===
# 在 loss.backward() 与 optimizer.step() 之间, 对每层 RQ codebook.embedding.weight.grad
# 乘 1/√c 反 rescale, 切断 c 与更新幅度的耦合. c 大时 codebook 移动变慢 (高曲率更需要细更新),
# c 小时 codebook 移动变快 (低曲率允许大步). 不动 encoder/decoder 梯度, 仅动 RQ codebook.
RADAM_ENABLED = True
RADAM_LAYER_MASK = [True, True, True]  # 3 层全部应用 (baseline 是全 Poincaré)
RADAM_EPS_FLOOR = 0.1  # c.sqrt() 下限, 防止 c 极小时放大梯度爆炸
NUM_WORKERS = 4
PIN_MEMORY = True
...
def _apply_riemannian_adam_rescale(model: torch.nn.Module) -> None:
    """iter6 R36n d 纯版: 对每个 RQ 层的 codebook.embedding.weight.grad 乘 1/√c.

    切断 c 与更新幅度的耦合: 高曲率 (c 大) → codebook 移动变慢 (需更精细更新);
    低曲率 (c 小) → codebook 移动变快 (允许大步). RADAM_LAYER_MASK=False 层跳过.
    encoder / decoder / 其他参数 梯度不动 (仅改 RQ codebook).
    """
    if not RADAM_ENABLED:
        return
    layers = model.module.layers if hasattr(model, "module") else model.layers
    for layer_idx, layer in enumerate(layers):
        if layer_idx >= len(RADAM_LAYER_MASK) or not RADAM_LAYER_MASK[layer_idx]:
            continue
        emb = layer.embedding.weight
        if emb.grad is None:
            continue
        c_tensor = layer.get_c().detach()  # 当前 cyclic c
        # 1/√c 反 rescale, RADAM_EPS_FLOOR 防止 c 极小时梯度爆炸
        scale = 1.0 / c_tensor.sqrt().clamp_min(RADAM_EPS_FLOOR)
        emb.grad.mul_(scale)
...
def main():
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # iter6 R36n d 纯版: Riemannian Adam gradient rescale (切断 c 与 codebook 更新幅度耦合)
            _apply_riemannian_adam_rescale(model)
            if rank == 0 and not step_first_pass_logged[9]:
                print(f"[Step9] Riemannian Adam rescale applied (RADAM_ENABLED={RADAM_ENABLED})", flush=True)
