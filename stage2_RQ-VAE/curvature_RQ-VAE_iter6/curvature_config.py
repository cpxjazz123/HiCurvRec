...
# === 路径推导锚点: 本文件所在目录 ===
_CONFIG_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/results/stage2_RQ-VAE/curvature_RQ-VAE_iter6"

# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "iter6_riemannian_adam"  # iter6: Riemannian Adam 单独 (R36n d 纯版, 跳出几何赛道改 optimization path). 对 Poincaré 层 codebook.embedding.weight.grad 乘 1/√c 反 rescale, 切断 c 与更新幅度耦合. v320 已试 a+b+d (per-layer hetero + per-layer phase + RAdam) 失败, 但纯 RAdam 在 iterN 中从未尝试. 其余沿用 v318 baseline (cyclic c[0.3..1.0] period 50k, midpoint, Sinkhorn sk_eps=0.05). 第 90 次 R37 尝试.
...
RQVAE_OUT_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/results/stage2_RQ-VAE/curvature_RQ-VAE_iter6/out/rqvae/instruments"  # iter6 独立 ckpt 目录
RQVAE_CKPT_PATH = os.path.join(RQVAE_OUT_DIR, "rqvae_final.pt")  # Stage 1 ckpt (iter6 训练产出)
