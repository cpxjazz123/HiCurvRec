...
def main():
    ...
    # iter8: warm-start from iter7's checkpoint (the most recent Stage-2 run). iter8
    # only changes the Sinkhorn assignment step (curvature-dependent eps), so keeping
    # iter7's encoder / decoder / codebooks + c_layer_scales lets us measure the
    # pure contribution of the assignment sharpening.
    iter7_ckpt_path = (
        "/home/wlia0047/ar57/wenyu/GeneRec/stage2_RQ-VAE/curvature_RQ-VAE_iter7/"
        "results/out/rqvae/instruments/rqvae_best.pth"
    )
    if os.path.isfile(iter7_ckpt_path):
        if rank == 0:
            warm_start_state = torch.load(
                iter7_ckpt_path, map_location=device, weights_only=False
            )
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter8 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter8 warm-start loaded {len(transferred_weights)} tensors "
                f"from {iter7_ckpt_path}; "
        ...
        print(
            f"[Step4] WARN: iter7 warm-start checkpoint missing at {iter7_ckpt_path}; "
            f"iter8 will train from random initialization",
            flush=True,
        )
