...
# iter8: warm-start from iter7's checkpoint (the most recent Stage-2 run).
REFERENCE_CKPT = (
    SCRIPT_DIR.parents[1]
    / "curvature_RQ-VAE_iter7"
    / "results"
    / "out"
    / "rqvae"
    / "instruments"
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter8", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter8_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    print("MVG PASS")
    print("warm_start=PASS (iter7 weights loaded)")
