...
# balanced per_layer utilization that gave +0.0030 over iter7). iter11 keeps iter8's
# assignment step and only adds c-modulated commitment weight (mild scale, α=0.25),
# so per-layer utilization should remain balanced.
REFERENCE_CKPT = (
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter11", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter11_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
