...
class Quantize(nn.Module):
    ...
    def __init__(
        ...
        if phase_offset_steps < 0:
            raise ValueError("phase_offset_steps 必须非负")
        ...
        # iter27 (R36n b): per-layer phase offset (注册为 buffer 同步跨 rank, 但不进 optimizer).
        self._phase_offset_steps = int(phase_offset_steps) % c_cyclic_period
    ...
    def get_c(self) -> Tensor:
        """返回当前训练步的 cyclic curvature (iter27 per-layer phase offset 异质).

        iter27 (R36n b): 加 self._phase_offset_steps 让每层 c(t) 相位错开.
        baseline (USE_PER_LAYER_HETERO_C=False): _phase_offset_steps=0, 与 v318 baseline 行为完全一致.
        """
        effective_step = (self._curriculum_step + self._phase_offset_steps) % self.c_cyclic_period
        phase = torch.tensor(
            torch.pi * effective_step / self.c_cyclic_period,
            device=self.device,
