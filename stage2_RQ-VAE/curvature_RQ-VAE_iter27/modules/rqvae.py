...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def snapshot_codebooks(self) -> List[torch.Tensor]:
        """iter27: detach-clone every codebook before optimizer.step()."""
        return [
            layer.embedding.weight.detach().clone()
            for layer in self.layers
        ]
    ...
    def apply_hyperbolic_trust_region(
        ...
        """iter27: enforce hyperbolic codebook trust-region per layer."""
        if len(prev_codebooks) != len(self.layers):
            raise ValueError(
                f"snapshot length mismatch: {len(prev_codebooks)} vs {len(self.layers)}"
            )
        summaries: List[dict] = []
        for layer_index, (layer, prev_codebook) in enumerate(
            zip(self.layers, prev_codebooks)
        ):
