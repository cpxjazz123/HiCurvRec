...
# iter27: hyperbolic codebook trust-region (CAO-1 contract).
ITER27_TRUST_RADIUS_FRAC = 0.5   # τ_l = frac · median nearest-neighbor geodesic spacing
ITER27_TRUST_RADIUS_MIN = 1e-3    # floor to avoid degenerate spacing collapse
ITER27_TRUST_LOG_EVERY = 1000     # global steps between trust-region summaries
...
def build_curvature_conditioned_adamw(model, use_conditioned_beta2=True):
    ...
    layer_parameters = [[] for _ in base_model.layers]
    other_parameters = []
    ...
    if any(not parameters for parameters in layer_parameters):
        raise RuntimeError("iter18: every quantizer layer needs optimizer parameters")
    ...
    optimizer_groups = [
        ...
        for layer_index, (parameters, beta2) in enumerate(
            zip(layer_parameters, conditioned_beta2)
        )
    ]
    if other_parameters:
    ...
    check_step5_optimizer(optimizer, base_model)
...
def main():
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # iter27: snapshot codebooks before optimizer.step() so the trust
            # region can compare the post-step displacement on the Poincaré ball.
            ...
            # iter27: rank-0 applies hyperbolic codebook trust-region post-step.
