...
from curvature_config import (
    USE_PER_LAYER_HETERO_C,
    PER_LAYER_PHASE_OFFSET_STEPS,
)
...
def main():
    ...
    # ---------- Step4 模型 ----------
    # iter27 (R36n b): USE_PER_LAYER_HETERO_C=True 时 per_layer_phase_offset_steps 异质
    # (默认 [0, T/3, 2T/3] 让 3 层 c(t) 相位错开 1/3 周期, 形成 per-layer hetero c_l).
    # False 时传 [0, 0, 0] 等价 v318 baseline 行为, 单变量 isolation 干净.
    if USE_PER_LAYER_HETERO_C:
        _phase_offsets = PER_LAYER_PHASE_OFFSET_STEPS
    else:
        _phase_offsets = [0] * N_LAYERS
    model = RqVae(
        input_dim=INPUT_DIM,
        embed_dim=EMBED_DIM,
        hidden_dims=HIDDEN_DIMS,
        ...
        per_layer_phase_offset_steps=_phase_offsets,
    ).to(device)
