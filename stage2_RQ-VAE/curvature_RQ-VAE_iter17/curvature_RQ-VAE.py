...
def main():
    ...
    # ---------- Step5 curvature-metric AdamW (iter17: per-layer 1/c update) ----------
    ...
    if any(not parameters for parameters in layer_parameters):
        raise RuntimeError("iter17: every quantizer layer needs optimizer parameters")
    ...
    def _inverse_curvatures():
        curvatures = [float(layer.get_c().item()) for layer in model.module.layers]
        inverse = [1.0 / (curvature + riemannian_eps) for curvature in curvatures]
        return curvatures, inverse

    model.module.set_curriculum_step(0)
    initial_curvatures, initial_inverse = _inverse_curvatures()
    riemannian_reference = sum(initial_inverse) / len(initial_inverse)
    if not np.isfinite(riemannian_reference) or riemannian_reference <= 0:
        raise RuntimeError("iter17: invalid initial inverse-curvature reference")
    last_riemannian_multipliers = [
        value / riemannian_reference for value in initial_inverse
    ]

    def _set_riemannian_lrs():
        curvatures, inverse = _inverse_curvatures()
        multipliers = [value / riemannian_reference for value in inverse]
        for group in optimizer.param_groups:
            layer_index = group["riemannian_layer_index"]
            multiplier = 1.0 if layer_index is None else multipliers[layer_index]
            group["lr"] = base_learning_rate * multiplier
        last_riemannian_multipliers[:] = multipliers
        return curvatures, multipliers

    if rank == 0:
        print(
            f"[Step5] iter17 curvature-metric AdamW: per-layer adaptive updates "
            f"scale by 1/(c_l+{riemannian_eps}), normalized to the step-0 mean; "
            f"initial c={initial_curvatures} groups={len(optimizer.param_groups)}",
            flush=True,
        )
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # ---------- iter17: set curvature-dependent AdamW update scales ----------
            _set_riemannian_lrs()
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                metric_str = "/".join(
                    f"{scale:.3f}" for scale in last_riemannian_multipliers
                )
                print(
                    ...
                    f"| c={curv_str} | metric={metric_str} "
