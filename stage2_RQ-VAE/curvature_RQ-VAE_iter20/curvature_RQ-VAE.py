...
# Standard AdamW and iter19 reciprocal Sinkhorn epsilon remain unchanged; iter20 modifies only behavior temperature.
ADAMW_BASE_LR = 1e-3
ADAMW_BETA1 = 0.9
...
def build_standard_adamw(model):
    """Build standard AdamW groups while preserving per-layer parameter coverage."""
    ...
    if any(not parameters for parameters in layer_parameters):
        raise RuntimeError("iter19: every quantizer layer needs optimizer parameters")

    def make_group(parameters, layer_index):
        return {
            "params": parameters,
            "lr": ADAMW_BASE_LR,
    ...
    optimizer_groups = [
        make_group(parameters, layer_index)
        for layer_index, parameters in enumerate(layer_parameters)
    ]
    if other_parameters:
        optimizer_groups.append(make_group(other_parameters, None))
...
def main():
    ...
    # ---------- Step5 standard AdamW ----------
    optimizer = build_standard_adamw(model)
    if rank == 0:
        print(
            f"[Step5] standard AdamW: lr={ADAMW_BASE_LR} "
            f"betas=({ADAMW_BETA1}, {ADAMW_BASE_BETA2}) "
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                sinkhorn_eps = [
                    float(layer.effective_sinkhorn_eps(layer.get_c()).item())
                    for layer in model.module.layers
                ]
                sinkhorn_eps_str = "/".join(
                    f"{value:.5f}" for value in sinkhorn_eps
                )
                behavior_temperatures = [
                    float(model.module._effective_behavior_temperature(layer.get_c()).item())
                    for layer in model.module.layers
                ]
                behavior_temperature_str = "/".join(
                    f"{value:.5f}" for value in behavior_temperatures
                )
                print(
                    ...
                    f"| c={curv_str} | sinkhorn_eps={sinkhorn_eps_str} "
                    f"| behavior_temp={behavior_temperature_str} "
