"""Mechanism verification for iter20 curvature-scaled behavior temperature."""
...
def main() -> None:
    ...
    epsilon_endpoints = []
    behavior_temperature_endpoints = []
    for layer in model.layers:
        ...
        low_behavior_temperature = float(
            model._effective_behavior_temperature(low_curvature).item()
        )
        high_behavior_temperature = float(
            model._effective_behavior_temperature(high_curvature).item()
        )
        baseline_behavior_temperature = float(
            model._effective_behavior_temperature(
                low_curvature.new_tensor(rqtrain.C_CYCLIC_MIN)
            ).item()
        )
        expected_high_behavior_temperature = baseline_behavior_temperature * np.sqrt(
            rqtrain.C_CYCLIC_MIN / rqtrain.C_CYCLIC_MAX
        )
        ...
        if (
            not np.isfinite(low_behavior_temperature)
            or abs(low_behavior_temperature - baseline_behavior_temperature) > 1e-7
            or not np.isfinite(high_behavior_temperature)
            or abs(high_behavior_temperature - expected_high_behavior_temperature) > 1e-7
            or high_behavior_temperature >= low_behavior_temperature
        ):
            raise RuntimeError("MVG FAIL: curvature-scaled behavior temperature endpoints are incorrect")
        ...
        behavior_temperature_endpoints.append(
            (low_behavior_temperature, high_behavior_temperature)
        )
    ...
    initial_behavior_temperatures = [
        float(model._effective_behavior_temperature(layer.get_c()).item())
        for layer in model.layers
    ]
    minimum_behavior_temperature = baseline_behavior_temperature * np.sqrt(
        rqtrain.C_CYCLIC_MIN / rqtrain.C_CYCLIC_MAX
    )
    if not all(
        np.isfinite(value)
        and minimum_behavior_temperature - 1e-7 <= value <= baseline_behavior_temperature + 1e-7
        for value in initial_behavior_temperatures
    ):
        raise RuntimeError("MVG FAIL: initial behavior temperature is outside its endpoints")
    ...
    def run_counterfactual(use_curvature_behavior_temperature):
        model.load_state_dict(initial_state, strict=True)
        model.train()
        model.use_curvature_behavior_temperature = use_curvature_behavior_temperature
        for layer in model.layers:
            layer.use_reciprocal_sinkhorn_eps = True
        optimizer = rqtrain.build_standard_adamw(model)
        _assert_optimizer_contract(optimizer, model, rqtrain)
    ...
    loss_on, behavior_on = run_counterfactual(
        use_curvature_behavior_temperature=True
    )
    loss_off, behavior_off = run_counterfactual(
        use_curvature_behavior_temperature=False
    )
    ...
    behavior_delta = abs(behavior_on - behavior_off)
    if behavior_delta <= 1e-6:
        raise RuntimeError(
            f"MVG FAIL: 200-step ON/OFF behavior-loss delta {behavior_delta:.3e} "
            "does not exceed 1e-6"
        )

    # Confirm curvature, epsilon, and behavior-temperature schedules are cyclic and finite.
    model.load_state_dict(initial_state, strict=True)
    for layer in model.layers:
        layer.use_reciprocal_sinkhorn_eps = True
    curvature_snapshots = {}
    model.use_curvature_behavior_temperature = True
    epsilon_snapshots = {}
    behavior_temperature_snapshots = {}
    for step in (0, rqtrain.C_CYCLIC_PERIOD // 2, rqtrain.C_CYCLIC_PERIOD):
        ...
        if not all(
            np.isfinite(value)
            and minimum_behavior_temperature - 1e-7 <= value <= baseline_behavior_temperature + 1e-7
            for value in behavior_temperatures
        ):
            raise RuntimeError(f"MVG FAIL: invalid behavior temperature at step {step}")
        curvature_snapshots[step] = curvatures
        epsilon_snapshots[step] = epsilons
        behavior_temperature_snapshots[step] = behavior_temperatures
    ...
    if not all(
        initial > middle
        for initial, middle in zip(
            behavior_temperature_snapshots[0], behavior_temperature_snapshots[midpoint]
        )
    ):
        raise RuntimeError("MVG FAIL: behavior temperature does not decrease at high curvature")
    if not all(
        abs(initial - end) <= 1e-6
        for initial, end in zip(
            behavior_temperature_snapshots[0],
            behavior_temperature_snapshots[rqtrain.C_CYCLIC_PERIOD],
        )
    ):
        raise RuntimeError("MVG FAIL: behavior temperature does not return at cycle boundary")
    ...
    print(f"DE-1 behavior temperature endpoints (c_min,c_max)={behavior_temperature_endpoints}")
    print(f"DE-1 initial behavior temperature_l={initial_behavior_temperatures}")
    ...
    print(f"DE-1 behavior temperature cycle snapshots={behavior_temperature_snapshots}")
    ...
    print(
        ...
        f"behavior_delta={behavior_delta:.3e}"
    )
