...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.use_curvature_behavior_temperature = True
    ...
    def _effective_behavior_temperature(self, curvature: Tensor) -> Tensor:
        """Sharpen high-curvature behavior logits without adding a curvature gradient."""
        c_min = float(self._config["c_cyclic_min"])
        c_max = float(self._config["c_cyclic_max"])
        c_value = curvature.detach().reshape(()).clamp(min=c_min, max=c_max)
        if not self.use_curvature_behavior_temperature:
            return c_value.new_tensor(BEHAVIOR_TEMPERATURE)
        return BEHAVIOR_TEMPERATURE * torch.sqrt(c_min / c_value)

    def _behavior_contrastive_loss(
        self, quantized: RqVaeOutput, batch: SeqBatch, batch_size: int
    ) -> Tensor:
        """Align per-layer hyperbolic distances with train-only next-item pairs."""
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            temperature = self._effective_behavior_temperature(curvature)
            logits = -distances / temperature
            logits = logits.masked_fill(duplicate_targets, float("-inf"))
            if bool(valid_rows.any().item()):
                labels = torch.arange(batch_size, device=batch.x.device)
                losses.append(F.cross_entropy(logits[valid_rows], labels[valid_rows]))
            else:
