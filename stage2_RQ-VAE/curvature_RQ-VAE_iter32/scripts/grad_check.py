...
model = rqtrain.RqVae(
    ...
    c_cyclic_min_per_layer=rqtrain.C_CYCLIC_MIN_PER_LAYER,  # R36n b iter31 per-layer hetero
    c_cyclic_max_per_layer=rqtrain.C_CYCLIC_MAX_PER_LAYER,  # R36n b iter31 per-layer hetero
    sk_eps_per_layer=rqtrain.SK_EPS_PER_LAYER,  # R36n f iter32 per-layer hetero sk_eps
...
# R36n b: 验证 per-layer 异质 c 真的落到每层 (不同 layer 在同 step 应该有不同 c_min/c_max)
if hasattr(rqtrain, "C_CYCLIC_MIN_PER_LAYER") and hasattr(rqtrain, "C_CYCLIC_MAX_PER_LAYER"):
    model.set_curriculum_step(0)
    layer_c_at_t0 = [float(layer.get_c().item()) for layer in model.layers]
    layer_c_at_t_half = []
    model.set_curriculum_step(rqtrain.C_CYCLIC_PERIOD // 4)  # quarter period
    for layer in model.layers:
        layer_c_at_t_half.append(float(layer.get_c().item()))
    # 每层 c_min/c_max 应当不同 (per-layer hetero)
    layer_min_actual = [layer.c_cyclic_min for layer in model.layers]
    layer_max_actual = [layer.c_cyclic_max for layer in model.layers]
    if len(set(layer_min_actual)) < 2 or len(set(layer_max_actual)) < 2:
        raise RuntimeError(
            f"R36n b: per-layer c_min/c_max 未异质化: min={layer_min_actual} max={layer_max_actual}"
        )
    # 在 t=0 时 sin(0)=0 → c = c_min (应等于 layer 自己的 c_min, 不是全局)
    for layer_idx, c_val in enumerate(layer_c_at_t0):
        expected_min = layer_min_actual[layer_idx]
        if abs(c_val - expected_min) > 1e-4:
            raise RuntimeError(
                f"R36n b: layer {layer_idx} c at t=0 = {c_val}, 期望 = per-layer c_min {expected_min}"
            )
    print("R36N_B_PER_LAYER_HETERO_C: PASS")
    print(f"per_layer_c_min={layer_min_actual}")
    print(f"per_layer_c_max={layer_max_actual}")
    print(f"c_at_step_0={layer_c_at_t0}")
    print(f"c_at_step_quarter={layer_c_at_t_half}")

# R36n f: 验证 per-layer 异质 sk_eps 真的落到每层 (iter32)
if hasattr(rqtrain, "SK_EPS_PER_LAYER"):
    layer_sk_actual = [layer.sk_eps for layer in model.layers]
    if len(set(layer_sk_actual)) < 2:
        raise RuntimeError(
            f"R36n f: per-layer sk_eps 未异质化: sk_eps={layer_sk_actual}"
        )
    if layer_sk_actual != list(rqtrain.SK_EPS_PER_LAYER):
        raise RuntimeError(
            f"R36n f: sk_eps_per_layer mismatch: actual={layer_sk_actual} expected={list(rqtrain.SK_EPS_PER_LAYER)}"
        )
    print("R36N_F_PER_LAYER_HETERO_SK_EPS: PASS")
    print(f"per_layer_sk_eps={layer_sk_actual}")
