...
BRANCHING_FACTORS = [19.32, 1.46, 1.015]
RAW_RESIDUAL_MEDIANS = [1.0, 0.10941, 0.09331]
LAYER_SCALE_PRIOR_MIN = 0.01
LAYER_SCALE_PRIOR_MAX = 0.99


def _build_layer_scale_prior() -> list[float]:
    """Combine normalized log-branching and raw-residual scales."""
    if len(BRANCHING_FACTORS) != N_LAYERS or len(RAW_RESIDUAL_MEDIANS) != N_LAYERS:
        raise ValueError("Iter25 prior inputs must match N_LAYERS")
    branching = np.asarray(BRANCHING_FACTORS, dtype=np.float64)
    residual = np.asarray(RAW_RESIDUAL_MEDIANS, dtype=np.float64)
    if (
        not np.isfinite(branching).all()
        or not np.isfinite(residual).all()
        or np.any(branching <= 1.0)
        or np.any(residual <= 0.0)
    ):
        raise ValueError("Iter25 prior inputs must be finite and positive (B > 1)")
    h = np.log(branching)
    h /= h.max()
    r = residual / residual.max()
    prior = LAYER_SCALE_PRIOR_MIN + (
        LAYER_SCALE_PRIOR_MAX - LAYER_SCALE_PRIOR_MIN
    ) * (h + r) / 2.0
    if not np.isfinite(prior).all() or np.any(prior <= 0.0) or np.any(prior > 1.0):
        raise ValueError("Iter25 layer-scale prior is outside (0, 1]")
    return [float(value) for value in prior]
...
def main():
    ...
    # ---------- Step3.5: build the fixed branching/residual prior ----------
    layer_scale_prior = _build_layer_scale_prior()
    if rank == 0:
        prior_str = "/".join(f"{value:.6f}" for value in layer_scale_prior)
        print(
            f"[Step3.5] layer_scale_prior (per-layer u_l) = [{prior_str}] | "
    ...
    model = RqVae(
        ...
        residual_layer_norms=layer_scale_prior,
    ).to(device)

    # Iter25 inherits Iter11's warm start from iter8, excluding only the
    # learnable layer scales so the new prior initializes and anchors them.
    iter8_ckpt_path = (
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter25 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] Iter25 warm-start loaded {len(transferred_weights)} tensors "
                f"from {iter8_ckpt_path}; c_layer_scale prior=[{prior_str}]",
                flush=True,
            )
        ...
        print(
            ...
            f"iter25 will train from random initialization",
            flush=True,
        )
