...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        c_cyclic_min_per_layer: List[float] | None = None,
        c_cyclic_max_per_layer: List[float] | None = None,
        sk_eps_per_layer: List[float] | None = None,
    ) -> None:
        super().__init__()
        if midpoint_layer_mask is None:
            midpoint_layer_mask = [True] + [False] * (n_layers - 1)
        # R36n b: per-layer hetero c (iter31). 解析为每层的 (c_min, c_max), 兼容旧版 scalar 形式
        if c_cyclic_min_per_layer is None:
            c_cyclic_min_per_layer = [float(c_cyclic_min)] * n_layers
        if c_cyclic_max_per_layer is None:
            c_cyclic_max_per_layer = [float(c_cyclic_max)] * n_layers
        if len(c_cyclic_min_per_layer) != n_layers:
            raise ValueError(
                f"Step1: c_cyclic_min_per_layer 长度 {len(c_cyclic_min_per_layer)} 必须等于 n_layers {n_layers}"
            )
        if len(c_cyclic_max_per_layer) != n_layers:
            raise ValueError(
                f"Step1: c_cyclic_max_per_layer 长度 {len(c_cyclic_max_per_layer)} 必须等于 n_layers {n_layers}"
            )
        for layer_idx in range(n_layers):
            cm = float(c_cyclic_min_per_layer[layer_idx])
            cM = float(c_cyclic_max_per_layer[layer_idx])
            if not (0 < cm <= cM):
                raise ValueError(
                    f"Step1: cyclic curvature per-layer 范围无效 @layer={layer_idx}: c_min={cm}, c_max={cM}"
                )
        # R36n f: per-layer hetero sk_eps (iter32). 兼容旧版 scalar 形式
        if sk_eps_per_layer is None:
            sk_eps_per_layer = [float(sk_eps)] * n_layers
        if len(sk_eps_per_layer) != n_layers:
            raise ValueError(
                f"Step1: sk_eps_per_layer 长度 {len(sk_eps_per_layer)} 必须等于 n_layers {n_layers}"
            )
        for layer_idx in range(n_layers):
            if float(sk_eps_per_layer[layer_idx]) <= 0:
                raise ValueError(
                    f"Step1: sk_eps per-layer 必须正数 @layer={layer_idx}: sk_eps={sk_eps_per_layer[layer_idx]}"
                )
        check_step1_config(
            input_dim=input_dim,
            embed_dim=embed_dim,
        ...
        self._config = {
            ...
            "sk_eps": sk_eps,
            "sk_eps_per_layer": list(sk_eps_per_layer),
            "sk_iters": sk_iters,
            "c_cyclic_min": c_cyclic_min,
            "c_cyclic_max": c_cyclic_max,
            "c_cyclic_period": c_cyclic_period,
            "c_cyclic_min_per_layer": list(c_cyclic_min_per_layer),
            "c_cyclic_max_per_layer": list(c_cyclic_max_per_layer),
        ...
        self.layers = nn.ModuleList(
            [
                Quantize(
                    ...
                    commitment_weight=commitment_weight,
                    sk_eps=float(sk_eps_per_layer[layer_idx]),  # R36n f: per-layer hetero sk_eps
                    sk_iters=sk_iters,
                    c_cyclic_min=float(c_cyclic_min_per_layer[layer_idx]),
                    c_cyclic_max=float(c_cyclic_max_per_layer[layer_idx]),
                    c_cyclic_period=c_cyclic_period,
                )
                for layer_idx in range(n_layers)
            ]
