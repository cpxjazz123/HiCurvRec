...
# === R36n b: per-layer 异质 c (iter31, 保留) ===
# 3 层各自独立 c_min/c_max, 形成"由粗到细"的几何梯度:
# L0 (coarsest, 大 radius): c ∈ [0.2, 0.8] — 低曲率, 较松
# L1 (middle, 中 radius):  c ∈ [0.3, 1.0] — 与 iter11 baseline 等价
# L2 (finest, 小 radius):   c ∈ [0.4, 1.3] — 较高曲率, 较紧
C_CYCLIC_MIN_PER_LAYER = [0.2, 0.3, 0.4]
C_CYCLIC_MAX_PER_LAYER = [0.8, 1.0, 1.3]
# === R36n f: per-layer 异质 sk_eps (iter32 新增) ===
# sk_eps 控制 Sinkhorn-Knopp codebook balance 的严格度:
# L0: 0.05 (标准, baseline 等价)
# L1: 0.10 (looser, 鼓励更多 code 利用)
# L2: 0.03 (stricter, 强制 uniform 平衡)
SK_EPS_PER_LAYER = [0.05, 0.10, 0.03]
C_CYCLIC_MIN = 0.3
C_CYCLIC_MAX = 1.0
...
def main():
    ...
    model = RqVae(
        ...
        c_cyclic_min_per_layer=C_CYCLIC_MIN_PER_LAYER,  # R36n b: per-layer hetero c
        c_cyclic_max_per_layer=C_CYCLIC_MAX_PER_LAYER,  # R36n b: per-layer hetero c
        sk_eps_per_layer=SK_EPS_PER_LAYER,  # R36n f: per-layer hetero sk_eps (iter32)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            check_step11_curvature(
                ...
                C_CYCLIC_MIN,
                C_CYCLIC_MAX,
                c_min_per_layer=C_CYCLIC_MIN_PER_LAYER,  # R36n b: per-layer hetero c
                c_max_per_layer=C_CYCLIC_MAX_PER_LAYER,  # R36n b: per-layer hetero c
            )
