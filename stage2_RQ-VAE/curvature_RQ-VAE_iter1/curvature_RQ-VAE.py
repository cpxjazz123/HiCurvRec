...
def main():
    ...
    # === iter1 v371: cosine LR + 5% warmup (T5/RQ-VAE lit standard) ===
    _WARMUP_RATIO = 0.05
    _MIN_LR_RATIO = 0.1  # decay 到 LR/10
    _warmup_steps = max(1, int(_WARMUP_RATIO * MAX_GLOBAL_STEPS))
    _total_steps = MAX_GLOBAL_STEPS
    def _lr_lambda(step: int) -> float:
        # step 含义 = scheduler.step() 调用次数, 从 0 开始;
        # warmup 期间返回 (step+1)/warmup_steps (避免 step=0 时 LR=0 导致 optimizer 不动)
        if step < _warmup_steps:
            return (step + 1) / float(_warmup_steps)
        progress = (step - _warmup_steps) / max(1, _total_steps - _warmup_steps)
        # cosine decay from 1.0 to _MIN_LR_RATIO
        cos_factor = 0.5 * (1.0 + math.cos(math.pi * progress))
        return _MIN_LR_RATIO + (1.0 - _MIN_LR_RATIO) * cos_factor
    import math
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, _lr_lambda)
    if rank == 0:
        print(
            f"[Step5-scheduler] cosine LR + warmup: warmup_steps={_warmup_steps}, "
            f"total_steps={_total_steps}, min_lr_ratio={_MIN_LR_RATIO}, "
            f"initial LR (before any step): {optimizer.param_groups[0]['lr']:.2e}",
            flush=True,
        )
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # === iter1 v371: cosine LR + warmup (用 local step 触发, 每张卡都调一次) ===
            scheduler.step()
            global_step += 1
