...
class Quantize(nn.Module):
    ...
    def __init__(
        ...
        # The offline MVG toggles this only for the legacy counterfactual.
        self.use_reciprocal_sinkhorn_eps = True
    ...
    def effective_sinkhorn_eps(self, curvature: Tensor) -> Tensor:
        """Map cyclic curvature to the Sinkhorn temperature."""
        if self.use_reciprocal_sinkhorn_eps:
            scale = self.c_cyclic_min / curvature.clamp_min(1e-12)
        else:
            scale = curvature / self.c_cyclic_max
        return (self.sk_eps * scale).clamp_min(1e-4)
    ...
    def forward(self, x: Tensor) -> QuantizeOutput:
        ...
        # The Sinkhorn kernel is exp(-cost / epsilon): smaller epsilon sharpens.
        # Reciprocal curvature restores the documented low-c soft/high-c sharp
        # schedule; Cuturi (2013) gives the equivalent exp(-lambda * cost) kernel.
        effective_eps = self.effective_sinkhorn_eps(curvature).item()
