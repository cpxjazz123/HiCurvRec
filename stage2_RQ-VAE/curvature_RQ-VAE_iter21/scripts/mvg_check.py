"""Mechanism verification for iter21 multi-positive behavior supervision."""
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter21_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    def run_counterfactual(use_multi_positive_behavior):
        model.load_state_dict(initial_state, strict=True)
        model.train()
        model.use_multi_positive_behavior = use_multi_positive_behavior
        for layer in model.layers:
            layer.use_reciprocal_sinkhorn_eps = True
        optimizer = rqtrain.build_standard_adamw(model)
        _assert_optimizer_contract(optimizer, model, rqtrain)
    ...
    loss_on, behavior_on = run_counterfactual(use_multi_positive_behavior=True)
    loss_off, behavior_off = run_counterfactual(use_multi_positive_behavior=False)
