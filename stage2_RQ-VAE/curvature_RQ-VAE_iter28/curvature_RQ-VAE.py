...
# iter18: fixed initial-curvature-conditioned AdamW second-moment decay.
ADAMW_BASE_LR = 1e-3
ADAMW_BETA1 = 0.9
...
# iter28: Sinkhorn-weighted Riemannian EMA codebook (SREMA, CAO-1 contract).
# Codebook `embedding.weight` is excluded from the AdamW optimizer entirely;
# it is updated by `model.srema_update(beta)` after each AdamW step.
ITER28_SREMA_BETA = 0.99          # EMA momentum; per-step contribution = (1 - β)
ITER28_SREMA_LOG_EVERY = 1000     # global steps between SREMA summaries
...
def build_curvature_conditioned_adamw(model, use_conditioned_beta2=True):
    """Build fixed per-layer AdamW beta2 values from the initial curvature.

    iter28 modification: codebook layers.X.embedding.weight is excluded from
    the optimizer.  The codebook is updated by model.srema_update(beta) after
    each optimizer step.  All other parameters (encoder, decoder, per-layer
    c_layer_scale) remain in the optimizer with the iter18 per-layer beta2
    from initial curvature.
    """
    base_model = model.module if hasattr(model, "module") else model
    base_model.set_curriculum_step(0)
    initial_curvatures = [
        float(layer.get_c().detach().item()) for layer in base_model.layers
    ]
    log_curvature_range = np.log(C_CYCLIC_MAX / C_CYCLIC_MIN)
    if not np.isfinite(log_curvature_range) or log_curvature_range <= 0:
        raise RuntimeError("iter18: invalid cyclic-curvature range")
    curvature_u = [
        float(
            np.clip(
                2.0 * np.log(curvature / C_CYCLIC_MIN) / log_curvature_range,
                0.0,
                1.0,
            )
        )
    ...
    if (
        not all(np.isfinite(value) for value in initial_curvatures)
        or not all(np.isfinite(value) and 0.0 <= value <= 1.0 for value in curvature_u)
        or not all(
            np.isfinite(value) and 0.990 <= value <= ADAMW_BASE_BETA2
            for value in conditioned_beta2
        )
    ):
        raise RuntimeError("iter18: invalid curvature-conditioned beta2 values")
    for i, curvature_i in enumerate(initial_curvatures):
        for j, curvature_j in enumerate(initial_curvatures):
            if curvature_i > curvature_j and conditioned_beta2[i] > conditioned_beta2[j] + 1e-12:
                raise RuntimeError("iter18: beta2 is not monotone with initial curvature")

    # iter28: separate c_layer_scale (per-layer, gets curvature-conditioned beta2)
    # from codebook embedding.weight (excluded; updated by SREMA).
    c_layer_scale_params = [[] for _ in base_model.layers]
    other_parameters = []
    excluded_codebook_params = []
    for name, parameter in base_model.named_parameters():
        if not name.startswith("layers."):
            other_parameters.append(parameter)
            continue
        layer_index = int(name.split(".")[1])
        if name.endswith(".c_layer_scale"):
            c_layer_scale_params[layer_index].append(parameter)
        elif name.endswith(".embedding.weight"):
            # iter28: codebook is updated by SREMA, NOT by AdamW.
            excluded_codebook_params.append((name, parameter))
        else:
            raise RuntimeError(
                f"iter28: unexpected quantizer-layer parameter name {name!r}"
            )
    if any(not params for params in c_layer_scale_params):
        raise RuntimeError(
            "iter28: every quantizer layer must contribute its c_layer_scale"
        )
    if len(excluded_codebook_params) != len(base_model.layers):
        raise RuntimeError(
            f"iter28: expected {len(base_model.layers)} codebook "
            f"embedding.weight params to exclude; found "
            f"{len(excluded_codebook_params)}"
        )
    ...
    optimizer_groups = [
        {
            ...
            "curvature_layer_index": layer_index,
        }
        for layer_index, (params, beta2) in enumerate(
            zip(c_layer_scale_params, conditioned_beta2)
        )
    ]
    if other_parameters:
    ...
    if not use_conditioned_beta2:
        for group in optimizer_groups:
            group["betas"] = (ADAMW_BETA1, ADAMW_BASE_BETA2)
    optimizer = torch.optim.AdamW(optimizer_groups)
    # iter28: skip iter18 strict check_step5_optimizer (it would fail because
    # codebook is intentionally excluded).  Custom check: every non-codebook
    # trainable parameter is in some optimizer group; every codebook
    # embedding.weight is excluded (handled by SREMA).
    model_trainable_ids = {
        id(parameter)
    ...
    excluded_codebook_ids = {id(parameter) for _, parameter in excluded_codebook_params}
    missing = model_trainable_ids - optimizer_ids - excluded_codebook_ids
    if missing:
        raise RuntimeError(
            "iter28: optimizer does not cover and SREMA does not exclude: "
            f"{sorted(id(parameter) for parameter in base_model.parameters() if id(parameter) in missing)}"
        )
    overlap = optimizer_ids & excluded_codebook_ids
    if overlap:
        raise RuntimeError(
            "iter28: codebook embedding.weight must NOT be in the AdamW "
            "optimizer; detected overlap."
        )
    return optimizer, initial_curvatures, curvature_u, conditioned_beta2
...
def _extend_collisions(tokens: np.ndarray, codebook_sizes: list[int]) -> np.ndarray:
    """Append a fixed extension level to disambiguate collisions (TIGER convention)."""
...
def _export_4token_sids_for_stage3() -> None:
    """End-of-run: extend raw 3-token SIDs into TIGER-compatible 4-token form.

    Writes both `SIDS_NPY` (npy, (N, 4) int64) and `ITEM_SIDS_JSON`
    (TIGER-compatible stage2 SID mapping).
    """
    raw_path = Path(RAW_SIDS_NPY)
    sids_npy_path = Path(SIDS_NPY)
    json_path = Path(ITEM_SIDS_JSON)
    if not raw_path.is_file():
        raise FileNotFoundError(f"源 3-token SID 不存在: {raw_path}")
    raw = np.load(raw_path, allow_pickle=False)
    codebook_sizes = [CODEBOOK_SIZE] * N_LAYERS
    if raw.shape != (len(raw), len(codebook_sizes)):
        raise ValueError(
            f"源 SID 形状 {raw.shape} 与预期 "
            f"({len(raw)}, {len(codebook_sizes)}) 不一致"
        )
...
def main():
    ...
    # iter11: warm-start from iter8's checkpoint (the most recent Stage-2 run with
    # balanced per_layer utilization that gave +0.0030 over iter7). iter11 keeps
    # iter8's assignment step and only adds c-modulated commitment weight (mild scale,
    # α=0.25), so per-layer utilization should remain balanced.
    iter8_ckpt_path = (
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            if (
                set(missing) != trainable_curvature_keys
                or unexpected
            ):
                raise RuntimeError(
                    f"iter11 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter11 warm-start loaded {len(transferred_weights)} tensors "
        ...
        print(
            ...
            f"iter11 will train from random initialization",
            flush=True,
        )
    ...
    # ---------- Step5 iter18 curvature-conditioned AdamW beta2 ----------
    optimizer, initial_curvatures, initial_curvature_u, layer_beta2 = (
        build_curvature_conditioned_adamw(model)
    )
    if rank == 0:
        print(
            f"[Step5] iter18 curvature-conditioned AdamW beta2: "
            f"c0={initial_curvatures} u={initial_curvature_u} "
            f"beta2={layer_beta2} lr={ADAMW_BASE_LR} "
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # iter28 SREMA: rank 0 updates every codebook with the
            # Sinkhorn-weighted Riemannian EMA, using the Sinkhorn assignments
            # and query residuals that forward() cached on each layer.
            srema_summaries = (
            ...
            if rank == 0 and (now - last_log_t >= 5.0):
                ...
                beta2_str = "/".join(f"{value:.4f}" for value in layer_beta2)
                print(
            ...
            if global_step_sync >= MAX_GLOBAL_STEPS or (
                ...
                # Final step: build 4-token SID (collision extension) for Stage3.
                if global_step_sync >= MAX_GLOBAL_STEPS and rank == 0:
                    try:
                ...
                if (
                    ...
                    raise RuntimeError(
                        "Final 3-token SID generation or 4-token Stage3 export failed"
                    )
