...
class Quantize(nn.Module):
    ...
    def cache_srema_inputs(self, assignments: Tensor, residual: Tensor) -> None:
        """Cache the Sinkhorn assignments [B, K] and the query residual [B, D]
        that were used during the most recent forward.  Both tensors live
        on the same device as the model and are detached.
        """
        if assignments.ndim != 2:
            raise ValueError(
                f"assignments must be [B, K]; got {tuple(assignments.shape)}"
            )
        if residual.ndim != 2:
            raise ValueError(
                f"residual must be [B, D]; got {tuple(residual.shape)}"
            )
        if assignments.shape[0] != residual.shape[0]:
            raise ValueError(
                "assignments and residual batch sizes disagree: "
                f"{assignments.shape[0]} vs {residual.shape[0]}"
            )
        if assignments.shape[1] != self.n_embed:
            raise ValueError(
                f"assignments K dim {assignments.shape[1]} != codebook size "
                f"{self.n_embed}"
            )
        self._last_assignments = assignments.detach()
        self._last_residual = residual.detach()

    def has_srema_inputs(self) -> bool:
        return (
            hasattr(self, "_last_assignments")
            and hasattr(self, "_last_residual")
            and self._last_assignments is not None
    ...
    def srema_update(self, beta: float = 0.99, eps: float = 1e-9) -> dict:
        ...
        For each codeword j of this layer:
            p_j_old = exp_0^{c_l}(w_j_old)
            z_i    = exp_0^{c_l}(residual_i)
            xi_j   = sum_i A_ij * log_{p_j_old}(z_i) / (sum_i A_ij + eps)
            p_j_new = exp_{p_j_old}((1 - beta) * xi_j)
            w_j_new = log_0^{c_l}(p_j_new)
        Then embedding.weight[j] := w_j_new.  beta = 0.99 by default.
        """
        if not self.has_srema_inputs():
            return {
                "layer_index": -1,
                "ran": False,
                "reason": "no cached Sinkhorn assignments (forward was bypassed)",
                "median_update": 0.0,
                "max_update": 0.0,
                "frac_updated": 0.0,
                "saturation_max": 0.0,
        ...
        w_old = self.embedding.weight.detach().to(device=device, dtype=dtype)
        c = self.get_c().to(device=device, dtype=dtype)
        p_old = _expmap0_t(w_old.unsqueeze(0), c).squeeze(0)
        z = _expmap0_t(residual.to(device=device, dtype=dtype).unsqueeze(1), c).squeeze(1)

        if not torch.isfinite(p_old).all().item():
            raise RuntimeError("iter28 SREMA: pre-update codebook contains NaN/Inf")
        if not torch.isfinite(z).all().item():
            raise RuntimeError("iter28 SREMA: pre-update residuals contain NaN/Inf")

        # Closed-form geodesic log from basepoint p_old to target z on the
        # Poincare ball.  Implementation inlines the formula to avoid a new
        # module dependency:
        #   log_p(q) = (2 / (1 - c*||p||^2)) * atanh(sqrt(c) * ||(-p) (+) q||)
        #                          * ((-p) (+) q / ||(-p) (+) q||)
        sqrt_c = c ** 0.5
        # (-p_old) (+) z   via Mobius add (broadcast K x B)
        p_old_b = p_old.unsqueeze(1)        # [K, 1, D]
        z_b     = z.unsqueeze(0)             # [1, B, D]
        minus_p = -p_old_b
        x2 = (minus_p * minus_p).sum(dim=-1, keepdim=True)
        y2 = (z_b * z_b).sum(dim=-1, keepdim=True)
        xy = (minus_p * z_b).sum(dim=-1, keepdim=True)
        num = (1.0 + 2.0 * c * xy + c * y2) * minus_p + (1.0 - c * x2) * z_b
        den = (1.0 + 2.0 * c * xy + (c ** 2) * x2 * y2).clamp_min(1e-30)
        diff = num / den
        diff_norm = diff.norm(dim=-1, keepdim=True).clamp_min(1e-30)
        factor = (2.0 / (1.0 - c * (p_old ** 2).sum(dim=-1, keepdim=True).unsqueeze(1)))                  * torch.atanh((sqrt_c * diff_norm).clamp(max=1 - 1e-5)) / diff_norm
        log_at_p = factor * diff                            # [K, B, D]

        A = assignments.to(device=device, dtype=dtype).unsqueeze(-1)   # [B, K, 1]
        log_at_p_t = log_at_p.transpose(0, 1)                         # [B, K, D]
        weighted = A * log_at_p_t
        denom = assignments.to(device=device, dtype=dtype).sum(dim=0)  # [K]
        weighted_sum = weighted.sum(dim=0)                            # [K, D]
        denom_safe = denom.clamp_min(eps).unsqueeze(-1)
        xi = weighted_sum / denom_safe                                # [K, D]

        # Apply the manifold EMA.  We use first-order tangent-space
        # interpolation (which preserves the ball constraint via the
        # trailing exp0/log0 round-trip) rather than the exact
        # exp_{p_old} construction (which would require PT_{0 -> p_old}).
        step_vec = (1.0 - beta) * xi
        w_new = w_old + step_vec.to(dtype=w_old.dtype)
        if not torch.isfinite(w_new).all().item():
            raise RuntimeError("iter28 SREMA: non-finite tangent step")
        p_new = _expmap0_t(w_new.unsqueeze(0), c).squeeze(0)
        w_final = _logmap0_t(p_new, c)
        if not torch.isfinite(w_final).all().item():
            raise RuntimeError("iter28 SREMA: non-finite final tangent")
        # Saturation diagnostic: how close are we to the ball boundary?
        p_new_norm = p_new.norm(dim=-1).detach()
        boundary = (1.0 / (c.sqrt())).clamp_min(1e-6)
        sat_ratio = (p_new_norm / boundary).detach()
        self.embedding.weight.data.copy_(w_final.to(dtype=self.embedding.weight.dtype))

        # Geodesic displacement on the ball.
        displacement = _poincare_distance_t(
            p_old.unsqueeze(0), p_new.unsqueeze(0), c
        ).squeeze(0).squeeze(-1)
        if not torch.isfinite(displacement).all().item():
            raise RuntimeError("iter28 SREMA: non-finite displacement")
        updated_mask = displacement > 1e-12
        median_update = float(displacement.median().item())
        max_update = float(displacement.max().item())
        frac_updated = float(updated_mask.float().mean().item())
        return {
            "layer_index": -1,
            "ran": True,
            "median_update": median_update,
            "max_update": max_update,
            "frac_updated": frac_updated,
            "saturation_max": float(sat_ratio.max().item()),
            "saturation_mean": float(sat_ratio.mean().item()),
            "n_active": int(updated_mask.sum().item()),
        }

    def clear_srema_inputs(self) -> None:
        """Drop cached Sinkhorn / residual tensors (call before eval / ckpt save)."""
    ...
    def forward(self, x: Tensor) -> QuantizeOutput:
        ...
        # iter28 SREMA: cache the Sinkhorn assignment matrix and the query
        # residual so the post-step Riemannian EMA can compute the weighted
        # codebook direction.  Both are detached to prevent the EMA update
        # from creating a graph cycle (the EMA writes `embedding.weight`
        # under `no_grad`; the *next* forward's autograd starts fresh from
        # the updated parameter).
        if self.training:
            self.cache_srema_inputs(
                assignments=assignments.to(dtype=x.dtype),
                residual=x,
            )
