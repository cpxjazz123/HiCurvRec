...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def srema_update(self, beta: float = 0.99) -> List[dict]:
        """iter28 SREMA: per-layer Sinkhorn-weighted Riemannian EMA codebook
        update.  Runs **after** the (codebook-free) AdamW step.

        Each `Quantize` layer uses the Sinkhorn assignment and query residual
        it cached during its most recent `forward()` to compute a weighted
        geodesic direction, then performs one manifold-EMA step on its
        codebook.  No other parameter is touched.  Returns a per-layer
        summary list.
        """
        summaries: List[dict] = []
        for layer_index, layer in enumerate(self.layers):
            summary = layer.srema_update(beta=beta)
            summary["layer_index"] = layer_index
            summaries.append(summary)
        return summaries

    def clear_srema_inputs(self) -> None:
        """Drop cached Sinkhorn / residual tensors on every layer."""
        for layer in self.layers:
            layer.clear_srema_inputs()
