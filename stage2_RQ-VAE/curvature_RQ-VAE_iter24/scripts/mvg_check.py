"""Mechanism verification for iter24 inverse-sqrt target-frequency-balanced behavior."""
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter24", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    positive_target_mask = batch.ids_fut[:, None].eq(batch.ids_fut[None, :])
    positive_counts = positive_target_mask.sum(dim=1)
    valid_rows = batch.ids.ne(batch.ids_fut)
    multi_positive_rows = int(((positive_counts > 1) & valid_rows).sum().item())
    multi_positive_pairs = int(
        (positive_counts[valid_rows] - 1).clamp_min(0).sum().item()
    )
    if multi_positive_rows == 0:
        raise RuntimeError("MVG FAIL: batch has no duplicate next-item targets")
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter24_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    model.use_bidirectional_behavior = True
    model.use_target_frequency_balance = True
    torch.manual_seed(rqtrain.SEED)
    balanced_behavior = model(batch).behavior_loss
    model.use_target_frequency_balance = False
    torch.manual_seed(rqtrain.SEED)
    unbalanced_behavior = model(batch).behavior_loss
    model.use_target_frequency_balance = True
    frequency_balance_delta = balanced_behavior - unbalanced_behavior
    if (
        not frequency_balance_delta.requires_grad
        or frequency_balance_delta.grad_fn is None
        or not torch.isfinite(frequency_balance_delta).item()
    ):
        raise RuntimeError(
            "MVG FAIL: target-frequency behavior delta is detached or non-finite"
        )
    frequency_balance_norm = _loss_gradient_norm(
        frequency_balance_delta, parameters
    )
    if (
        not np.isfinite(frequency_balance_norm)
        or frequency_balance_norm <= GRAD_EPSILON
    ):
        raise RuntimeError(
            "MVG FAIL: target-frequency behavior delta gradient norm "
            f"{frequency_balance_norm:.3e}"
        )
    component_norms["target_frequency_balance_delta"] = frequency_balance_norm
    ...
    def run_counterfactual(use_target_frequency_balance):
        model.load_state_dict(initial_state, strict=True)
        model.train()
        model.use_multi_positive_behavior = True
        model.use_bidirectional_behavior = True
        model.use_target_frequency_balance = use_target_frequency_balance
        for layer in model.layers:
            layer.use_reciprocal_sinkhorn_eps = True
        optimizer = rqtrain.build_standard_adamw(model)
        _assert_optimizer_contract(optimizer, model, rqtrain)
    ...
    loss_on, behavior_on = run_counterfactual(use_target_frequency_balance=True)
    loss_off, behavior_off = run_counterfactual(use_target_frequency_balance=False)
    ...
    print(
        f"DE-1 duplicate-target coverage: rows={multi_positive_rows}, "
        f"extra-positive-pairs={multi_positive_pairs}"
    )
    ...
    print(
        f"DE-2 inverse-sqrt target-frequency balance delta gradient norm="
        f"{frequency_balance_norm:.8f}"
    )
    print(f"DE-2 quantizer parameter gradient L2 norms={gradient_norms}")
    print(f"DE-3 five-step relative updates={update_ratios}")
    print(
        f"DE-4 200-step balanced/unbalanced counterfactual: L_on={loss_on:.8f} "
