...
# iter7 widens the cyclic range; tighter temperature sharpens contrastive behavior
...
class RqVaeComputedLosses(NamedTuple):
    ...
    rqvae_loss: Tensor
    behavior_loss: Tensor
    curvature_regularization: Tensor
    embs_norm: Tensor
    p_unique_ids: Tensor
...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # iter1: 每层独立的归一化尺度 u_l；缺省时全部为 1.0（等价 v318 共享 c）。
        if residual_layer_norms is None:
            layer_norms = [1.0] * n_layers
        else:
            if len(residual_layer_norms) != n_layers:
                raise ValueError("residual_layer_norms 长度必须等于 n_layers")
        ...
        self.use_multi_positive_behavior = True
        self.use_bidirectional_behavior = True
        self.use_target_frequency_balance = True
    ...
    def _behavior_contrastive_loss(
        self, quantized: RqVaeOutput, batch: SeqBatch, batch_size: int
    ) -> Tensor:
        """Align train-only transitions with target-frequency-balanced symmetric loss on residuals."""
        positive_targets = batch.ids_fut[:, None].eq(batch.ids_fut[None, :])
        valid_rows = batch.ids.ne(batch.ids_fut)
        losses = []
        representations = quantized.residuals
        target_counts = positive_targets.sum(dim=1).to(dtype=representations.dtype)
        target_weights = (
            target_counts.clamp_min(1.0).rsqrt()
            if self.use_target_frequency_balance
            else torch.ones_like(target_counts)
        )
        for layer_index, layer in enumerate(self.layers):
            source = representations[layer_index, :, :batch_size].transpose(0, 1)
            candidates = representations[layer_index, :, batch_size:].transpose(0, 1)
            ...
            if bool(valid_rows.any().item()):
                directions = [(logits, positive_targets)]
                if self.use_bidirectional_behavior:
                    directions.append(
                        (logits.transpose(0, 1), positive_targets.transpose(0, 1))
                    )
                for direction_logits, direction_positive_mask in directions:
                    if self.use_multi_positive_behavior:
                        row_logits = direction_logits[valid_rows]
                        positive_mask = direction_positive_mask[valid_rows]
                        log_probabilities = row_logits - torch.logsumexp(
                            row_logits, dim=1, keepdim=True
                        )
                        ...
                        row_losses = -(
                            positive_log_probabilities.sum(dim=1)
                            / positive_counts
                        )
                        row_weights = target_weights[valid_rows]
                        losses.append(
                            (row_losses * row_weights).sum() / row_weights.sum()
                        )
                    else:
                        duplicate_targets = direction_positive_mask.clone()
                        duplicate_targets.fill_diagonal_(False)
                        single_positive_logits = direction_logits.masked_fill(
                            duplicate_targets, float("-inf")
                        )
                        labels = torch.arange(batch_size, device=batch.x.device)
                        row_losses = F.cross_entropy(
                            single_positive_logits[valid_rows],
                            labels[valid_rows],
                            reduction="none",
                        )
                        row_weights = target_weights[valid_rows]
                ...
                losses.append(distances.sum() * 0.0)
        return torch.stack(losses).mean()

    def forward(self, batch: SeqBatch) -> RqVaeComputedLosses:
        """Reconstruct items and learn curvature from train-only transitions."""
        check_step6_batch(batch.x, self.input_dim, self.device)
        check_step6_batch(batch.x_fut, self.input_dim, self.device)
        batch_size = batch.x.shape[0]
        paired_x = torch.cat((batch.x, batch.x_fut), dim=0)
        quantized = self.get_semantic_ids(paired_x)
        ...
        reconstruction_loss = self.reconstruction_loss(
            x_hat, paired_x, c=reconstruction_curvature
        )
        rqvae_loss = quantized.quantize_loss
        behavior_loss = self._behavior_contrastive_loss(
            quantized, batch, batch_size
        )
        curvature_regularization = torch.stack(
            [layer.curvature_regularization() for layer in self.layers]
        ).mean()
        loss = (
            (reconstruction_loss + rqvae_loss).mean()
            + BEHAVIOR_LOSS_WEIGHT * behavior_loss
            + CURVATURE_REG_WEIGHT * curvature_regularization
        )
