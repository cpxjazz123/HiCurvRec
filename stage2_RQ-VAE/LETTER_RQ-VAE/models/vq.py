...
class VectorQuantizer(nn.Module):
    ...
    def constrained_km(self, data, n_clusters=10):
        ...
        x = data.cpu().detach().numpy().copy()
        clf = KMeans(n_clusters=n_clusters, n_init=3, max_iter=10, random_state=42, verbose=False)
        clf.fit(x)
    ...
    def diversity_loss(self, x_q, indices, indices_cluster, indices_list):
        ...
        for idx, pos in enumerate(pos_list):
            alternatives = [candidate for candidate in pos if candidate != indices[idx]]
            random_element = random.choice(alternatives) if alternatives else indices[idx]
    ...
    def diversity_loss_main_entry(self, x, x_q, indices, labels):
        ...
        target_numbers = sorted(set(int(label) for label in labels))
        indices_list = {}
    ...
    def forward(self,  x, label, idx, use_sk=True):
        ...
        # Diversity (skipped if label is None — inference mode)
        if label is None:
