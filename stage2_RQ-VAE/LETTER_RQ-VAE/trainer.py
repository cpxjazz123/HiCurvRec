...
class Trainer(object):
    ...
    def __init__(self, args, model):
        ...
        # Codebook labels change slowly; refreshing them every few epochs
        # avoids repeating the same small KMeans job on every epoch.
        self.label_update_interval = 5
    ...
    def constrained_km(self, data, n_clusters=None):
        """Cluster codebook vectors for the diversity labels.

        The old implementation silently fixed this to 10 clusters, which
        made the ``--n_clusters`` configuration ineffective.  Use the
        configured value and a deterministic sklearn implementation so the
        training and inference configurations agree.
        """
        from sklearn.cluster import KMeans

        if n_clusters is None:
            n_clusters = int(getattr(self.args, "n_clusters", 10))
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        if epoch_idx % self.label_update_interval == 0 or not self.labels["0"]:
