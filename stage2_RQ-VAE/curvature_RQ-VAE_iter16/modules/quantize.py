...
class Quantize(nn.Module):
    ...
    def __init__(
        ...
        if fixed_curvature is None and not 0 < c_layer_norm <= 1.0:
            raise ValueError("c_layer_norm 必须在 (0, 1] 内（对 c_cyclic_max 的归一化尺度）")
        if fixed_curvature is not None and not (0.05 <= float(fixed_curvature) <= 1.5):
            raise ValueError("fixed_curvature 必须在 [0.05, 1.5] 内")
        ...
        self.fixed_curvature = (
            float(fixed_curvature) if fixed_curvature is not None else None
        )
        self.c_layer_norm = float(c_layer_norm)
        self.freeze_layer_scale = bool(freeze_layer_scale)
        if self.fixed_curvature is not None:
            self.register_buffer(
                "_fixed_c",
                torch.tensor(self.fixed_curvature, dtype=torch.float32),
                persistent=True,
            )
        elif self.freeze_layer_scale:
            frozen_u = float(min(max(self.c_layer_norm, 1e-4), 1.0))
            self.register_buffer(
                "c_layer_scale",
                torch.tensor(frozen_u, dtype=torch.float32),
                persistent=False,
            )
        else:
    ...
    def get_u_layer(self) -> float:
        ...
        if self.fixed_curvature is not None:
            return float(self.fixed_curvature)
        if self.freeze_layer_scale:
            return float(self.c_layer_scale.item())
        return float(self.c_layer_scale.clamp(1e-4, 1.0 - 1e-4).item())

    def curvature_regularization(self) -> Tensor:
        """Penalize drift of learnable u_l from calibration; zero when frozen."""
        if self.fixed_curvature is not None or self.freeze_layer_scale:
            return torch.tensor(0.0, device=self.device, dtype=self.embedding.weight.dtype)
