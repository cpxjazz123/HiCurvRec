...
def evaluate_sid_quality_full(sids: np.ndarray, item_emb_path: str) -> SidMetrics:
    """调 scripts/sid_metrics_any.py (subprocess) 拿 HitRate@K=50 + Gini 全套.

    走 subprocess 而非 import 是为了复用既有 baseline 口径, 避免重写 HitRate.
    路径相对本文件位于 stage2_RQ-VAE/curvature_RQ-VAE/scripts/.
    """
    metrics = evaluate_sid_quality(sids)
    label = f"step{sids.shape[0]}"
    sid_metrics_script = Path(__file__).resolve().parent.parent / "scripts" / "sid_metrics_any.py"
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"evaluate_sid_quality_full: sid_metrics_any.py 退出码 "
            f"{proc.returncode}\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
...
def _parse_sid_metrics_stdout(stdout: str) -> Dict[str, float]:
    """从 scripts/sid_metrics_any.py 的 print 中解析 HitRate@K=50 和 Gini."""
    hitrate = None
    full_gini = None
    layer_gini = []
