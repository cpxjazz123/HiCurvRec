...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        freeze_layer_scale: bool = False,
        fixed_layer_curvatures: List[float] | None = None,
    ) -> None:
        super().__init__()
        ...
        self._config = {
            ...
            "freeze_layer_scale": bool(freeze_layer_scale),
            "fixed_layer_curvatures": (
                list(fixed_layer_curvatures)
                if fixed_layer_curvatures is not None
                else None
            ),
        }
        ...
        if fixed_layer_curvatures is not None:
            if len(fixed_layer_curvatures) != n_layers:
                raise ValueError("fixed_layer_curvatures 长度必须等于 n_layers")
        ...
        self.layers = nn.ModuleList(
            [
                Quantize(
                    ...
                    freeze_layer_scale=freeze_layer_scale,
                    fixed_curvature=(
                        float(fixed_layer_curvatures[index])
                        if fixed_layer_curvatures is not None
                        else None
                    ),
                )
