...
# iter26: contract — fixed closed-form c_l must remain identical (1e-6) at these
...
# iter16: closed-form fixed c_l (no cyclic, no learnable u_l).
...
def _load_closed_form_curvatures() -> list[float]:
    """iter16: s_l = log(1+B_l)/log(1+m_l/m_min); c_l = c_base*exp(alpha*z_l)."""
    branch_path = Path(__file__).resolve().parent / BEHAVIOR_BRANCH_FILE
    if not branch_path.is_file():
        raise FileNotFoundError(f"iter16 requires {branch_path}")
    payload = json.loads(branch_path.read_text(encoding="utf-8"))
    closed = payload.get("closed_form_c_l")
    if closed is None or len(closed) != N_LAYERS:
        raise ValueError(f"{branch_path} missing closed_form_c_l len={N_LAYERS}")
    values = [float(v) for v in closed]
    if not all(0.05 <= v <= 1.5 for v in values):
        raise ValueError(f"closed_form_c_l out of range: {values}")
    return values


def _log_curvature_snapshot(model, global_step: int, fixed_c: list[float]) -> None:
    """iter16: print fixed closed-form c_l at mandated steps."""
    layers = model.layers
    cs = [float(layer.get_c().item()) for layer in layers]
    print(
        f"[iter16_curvature] step={global_step} "
...
def _check_fixed_curvature_invariant(model, global_step: int, fixed_c: list[float]) -> None:
    """iter26: hard contract — c_l must remain identical within tolerance.
    ...
    print(
        f"[iter26_invariant] step={global_step} c_live="
...
def _export_4token_sids_for_stage3() -> None:
    ...
    (TIGER-compatible stage2 SID mapping). All paths come from
    curvature_config so iter-side is single-source-of-truth.
    """
...
def main():
    ...
    # ---------- Step3.5 iter16: closed-form per-layer curvature ----------
    fixed_layer_curvatures = _load_closed_form_curvatures()
    ...
    if rank == 0:
        ...
        print(
            f"[Step3.5] closed_form_c_l = [{c_str}] | no cyclic | "
    ...
    # iter16: warm-start from iter8; skip legacy c_layer_scale buffers.
    iter8_ckpt_path = (
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            skip_keys = {
                f"layers.{index}.c_layer_scale" for index in range(N_LAYERS)
            } | {f"layers.{index}._fixed_c" for index in range(N_LAYERS)}
            transferred_weights = {
                name: value
            ...
            if unexpected:
                raise RuntimeError(
                    f"iter16 warm-start unexpected keys: {sorted(unexpected)}"
                )
            print(
                f"[Step4] iter16 warm-start loaded {len(transferred_weights)} tensors "
                f"from {iter8_ckpt_path}; closed-form c_l preserved "
                f"(skipped {sorted(skip_keys)})",
                flush=True,
            )
        ...
        print(
            ...
            f"iter16 will train from random initialization",
            flush=True,
        )
