...
from curvature_config import (
    USE_RIEMANNIAN_ADAM,
    RIEMANNIAN_ADAM_LR,
    RIEMANNIAN_ADAM_BETAS,
    RIEMANNIAN_ADAM_EPS,
    RIEMANNIAN_ADAM_WEIGHT_DECAY,
    RIEMANNIAN_ADAM_NORM_CLAMP,
)
...
# === iter26 Riemannian AdamW optimizer (R36n d) ===
# 论文支撑: Nickel & Kiela 2017 "Poincaré Embeddings" + Becigneul & Ganea 2019
# "Riemannian Adaptive Optimization Methods for Neural Networks".
# 在 cyclic c(t) curriculum (v318 baseline) 基础上, 把 AdamW 换成 Riemannian Adam:
#   1) 每次 optimizer.step() 之前, 对每个 hyperbolic 参数的 .grad 做
#      Poincaré metric rescale: grad_r = grad_euclid * ((1 - c·||x||²)² / (4c)).
#   2) 然后走标准 AdamW 更新 (m/v 缓存 + bias correction + weight decay + step).
#   3) 不做 retraction (exponential map), 让 Euclidean step 自然落在 ball 内 (||
#      x||² ≤ 1/c - RIEMANNIAN_ADAM_NORM_CLAMP 防 boundary 数值发散).
# 历史 v320 R37 FAIL (-6.49%) + v361 R37 FAIL (utility 71.5%): v320 是无 cyclic
# 直接 Riemannian Adam (c=1.0 固定, 但 gradient rescale 在 lr=1e-3 下过于激进),
# v361 是叠 Lorentz. iter26 是 cyclic c(t) + Riemannian Adam 单变量, c(t)∈[0.3, 1.0]
# 周期性变化, 与 v320/v361 失败条件不同.
class RiemannianAdamW(torch.optim.AdamW):
    """Riemannian AdamW: 在 AdamW.step() 之前对每个 param.grad 做 Poincaré metric rescale.

    与 baseline AdamW 行为一致 (m/v/bias correction/weight decay), 唯一区别是
    step() 调用前先 set_curvature(c_scalar), 然后 step() 自动 rescale.
    """
    ...
    def __init__(
        ...
        if not 0.0 < norm_clamp < 1.0:
            raise ValueError(f"RiemannianAdamW: norm_clamp={norm_clamp} 必须在 (0,1)")
        self._c_scalar: float = 1.0  # 默认 c=1 (Poincaré), 调用方 set_curriculum_step 后才覆盖
        self._norm_clamp: float = float(norm_clamp)
        self._rescale_count: int = 0  # 调试: 累计 rescale 次数

    def set_curvature(self, c_scalar: float) -> None:
        """在 optimizer.step() 之前调一次, 设置当前 cyclic c(t)."""
        if not (c_scalar > 0 and np.isfinite(c_scalar)):
            raise ValueError(f"RiemannianAdamW.set_curvature: c={c_scalar} 必须 > 0 且有限")
        self._c_scalar = float(c_scalar)

    def _poincare_rescale_(self) -> None:
        """Poincaré metric rescale on .grad:
            grad_r = grad_euclid * ((1 - c·||x||²)² / (4c))
        ||x||² 沿 D 维求和, clamp 到 (1/c) * norm_clamp 防 ball boundary.
        """
        c = self._c_scalar
        if c <= 0:
            raise RuntimeError(f"RiemannianAdamW._poincare_rescale_: c={c} 必须 > 0")
        ball_safe_sq = (1.0 / c) * self._norm_clamp  # ||x||² 上限
        scale = 1.0 / (4.0 * c)
        n_rescaled = 0
        for group in self.param_groups:
            for p in group["params"]:
                ...
                flat = p.data
                if flat.ndim == 0:
                    # scalar 参数 (例如 bias 的最后一维退化): ||x||² = x²
                    norm_sq = (flat ** 2).clamp(max=ball_safe_sq)
                else:
                    # sum over all dims except 0 (batch dim 不存在, 0 是首个参数维)
                    dims = tuple(range(1, flat.ndim))
                    norm_sq = (flat ** 2).sum(dim=dims, keepdim=True).clamp(max=ball_safe_sq)
                factor = (1.0 - c * norm_sq) ** 2
                p.grad.mul_(factor * scale)
                n_rescaled += 1
        self._rescale_count += 1
        # rank 0 调试日志: 每 1000 次打印一次, 不影响主训练循环
        # if self._rescale_count % 1000 == 0:
        #     print(f"[RiemannianAdamW] rescale #{self._rescale_count} c={c} n_params={n_rescaled}", flush=True)

    @torch.no_grad()
    def step(self, closure=None):
        self._poincare_rescale_()
...
def main():
    ...
    # ---------- Step5 优化器 ----------
    # iter26 (R36n d): USE_RIEMANNIAN_ADAM=True 时用 RiemannianAdamW (Poincaré metric
    # rescale + 标准 AdamW 更新), False 时退化为 baseline AdamW.
    if USE_RIEMANNIAN_ADAM:
        optimizer = RiemannianAdamW(
            model.parameters(),
            lr=RIEMANNIAN_ADAM_LR,
            betas=RIEMANNIAN_ADAM_BETAS,
            eps=RIEMANNIAN_ADAM_EPS,
            weight_decay=RIEMANNIAN_ADAM_WEIGHT_DECAY,
            norm_clamp=RIEMANNIAN_ADAM_NORM_CLAMP,
        )
        optimizer_kind = "RiemannianAdamW(R36n d, c-rescale + AdamW)"
    else:
        optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
        optimizer_kind = "AdamW(baseline)"
    check_step5_optimizer(optimizer, model.module)
    if rank == 0:
        print(
            f"[Step5] optimizer={optimizer_kind} | "
            f"{len(optimizer.param_groups)} group(s) | "
            f"USE_RIEMANNIAN_ADAM={USE_RIEMANNIAN_ADAM}",
            flush=True,
        )
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # iter26 (R36n d): 在 optimizer.step() 之前把当前 c(t) 传给 Riemannian Adam.
            # 用 local global_step (未 all_reduce), 与 cyclic period=50k 相比误差可忽略
            # (单卡 ±world_size 步的偏移, 远小于 50k 周期). Step11 仍做 all_reduce 同步
            # 保证 _curriculum_step 跨 rank 一致 (sanity check).
            if USE_RIEMANNIAN_ADAM and isinstance(optimizer, RiemannianAdamW):
                _cur_module = model.module if hasattr(model, "module") else model
                _cur_module.set_curriculum_step(global_step)
                _c_t = float(_cur_module.layers[0].get_c().item())
                optimizer.set_curvature(_c_t)
            optimizer.step()
