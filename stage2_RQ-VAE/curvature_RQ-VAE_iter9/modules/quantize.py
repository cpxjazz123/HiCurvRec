...
class Quantize(nn.Module):
    ...
    def forward(self, x: Tensor) -> QuantizeOutput:
        ...
        # iter9: power-law ε schedule. iter8 used linear `(c / c_max)` which only
        # mildly softens at low c (ratio c_min/c_max = 0.05/1.5 ≈ 0.033). To amplify
        # the cycle contrast, raise the exponent to α = 0.5 (square-root), so:
        #   effective_eps = sk_eps * (c / c_max) ** 0.5
        # At c = c_max -> eps stays at sk_eps (sharp).
        # At c = c_min -> eps shrinks by sqrt(c_min/c_max) ≈ 0.183 (still soft).
        # The square-root keeps numerical stability and avoids the steep drop of a
        # higher exponent, while making the high-c sharpening twice as contrastive
        # vs the linear schedule (since (c/c_max)^0.5 > (c/c_max) for c in [0, c_max]).
        ...
        c_normalized = (current_c / c_max_reference.clamp_min(1e-12)).clamp(1e-6, 1.0 + 1e-6)
        # pow with float exponent -> tensor on centered.device / dtype
        c_scale = torch.pow(c_normalized, 0.5).view(1, 1)
        effective_eps = (
            self.sk_eps * c_scale
        ).clamp_min(1e-4).item()
