...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, batch: SeqBatch) -> RqVaeComputedLosses:
        ...
        # iter9 P2: SID dropout augmentation (training-only).
        # 训练时对量化 embedding 的 L1/L2 层做随机 mask (p_keep=0.85, L0 always kept),
        # 让 RQ-VAE 学会"残差路径"而不完全依赖每层精确量化.
        # Mask 用乘法而非置零, 保留 gradient flow;
        # inference 时 self.training=False, 跳过 mask.
        if (
            self.training
            and getattr(self, "_use_sid_dropout_aug", False)
        ):
            mask_rate_l1 = float(getattr(self, "_sid_mask_rate_l1", 0.15))
            mask_rate_l2 = float(getattr(self, "_sid_mask_rate_l2", 0.15))
            with torch.no_grad():
                # quantized.embeddings shape = (n_layers, embed_dim, batch)
                # 每个样本独立 mask 整个 L1/L2 codeword: keep shape = (1, batch)
                batch_size = quantized.embeddings.shape[2]
                keep_l1 = torch.bernoulli(
                    torch.full(
                        (1, batch_size),
                        1.0 - mask_rate_l1,
                        device=quantized.embeddings.device,
                    )
                )  # (1, batch)
                keep_l2 = torch.bernoulli(
                    torch.full(
                        (1, batch_size),
                        1.0 - mask_rate_l2,
                        device=quantized.embeddings.device,
            ...
            masked_embeddings = quantized.embeddings.clone()
            # quantized.embeddings: (n_layers, embed_dim, batch) — layer dim = 0
            masked_embeddings[1] = masked_embeddings[1] * keep_l1
            masked_embeddings[2] = masked_embeddings[2] * keep_l2
            summed_embeddings = self._step6_sum_embeddings(
                RqVaeOutput(
