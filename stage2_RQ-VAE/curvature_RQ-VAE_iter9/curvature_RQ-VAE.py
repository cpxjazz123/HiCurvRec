...
def main():
    ...
    # iter9: warm-start from iter8's checkpoint (the most recent Stage-2 run). iter9
    # only changes the Sinkhorn ε schedule (linear -> power-law 0.5), so keeping
    # iter8's encoder / decoder / codebooks + c_layer_scales lets us measure the
    # pure contribution of the power-law sharpening.
    iter8_ckpt_path = (
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter9 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter9 warm-start loaded {len(transferred_weights)} tensors "
        ...
        print(
            ...
            f"iter9 will train from random initialization",
            flush=True,
        )
