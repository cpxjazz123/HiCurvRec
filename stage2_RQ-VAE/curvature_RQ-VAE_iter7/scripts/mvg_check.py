...
# iter7: warm-start from iter4's checkpoint while keeping iter7's wider curvature range.
REFERENCE_CKPT = (
    SCRIPT_DIR.parents[1]
    / "curvature_RQ-VAE_iter4"
    / "results"
    / "out"
    / "rqvae"
    / "instruments"
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter7", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter7_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    print("MVG PASS")
    print("warm_start=PASS (iter4 weights loaded)")
