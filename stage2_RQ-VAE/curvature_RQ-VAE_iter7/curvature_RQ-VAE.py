...
def main():
    ...
    # iter7: warm-start the encoder / decoder / codebooks from iter4's checkpoint while
    # letting iter7's wider curvature range explore fresh c_layer_scales. The reference
    # ckpt must exist; if it does not we fail fast rather than silently re-init.
    iter4_ckpt_path = (
        "/home/wlia0047/ar57/wenyu/GeneRec/stage2_RQ-VAE/curvature_RQ-VAE_iter4/"
        "results/out/rqvae/instruments/rqvae_best.pth"
    )
    if os.path.isfile(iter4_ckpt_path):
        if rank == 0:
            warm_start_state = torch.load(
                iter4_ckpt_path, map_location=device, weights_only=False
            )
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter7 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter7 warm-start loaded {len(transferred_weights)} tensors "
                f"from {iter4_ckpt_path}; "
        ...
        print(
            f"[Step4] WARN: iter4 warm-start checkpoint missing at {iter4_ckpt_path}; "
            f"iter7 will train from random initialization",
            flush=True,
        )
