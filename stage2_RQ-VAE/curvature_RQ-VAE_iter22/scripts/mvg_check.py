"""Mechanism verification for iter22 bidirectional behavior supervision."""
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter22", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter22_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    torch.manual_seed(rqtrain.SEED)
    symmetric_behavior = model(batch).behavior_loss
    model.use_bidirectional_behavior = False
    torch.manual_seed(rqtrain.SEED)
    forward_behavior = model(batch).behavior_loss
    model.use_bidirectional_behavior = True
    bidirectional_delta = symmetric_behavior - forward_behavior
    if (
        not bidirectional_delta.requires_grad
        or bidirectional_delta.grad_fn is None
        or not torch.isfinite(bidirectional_delta).item()
    ):
        raise RuntimeError("MVG FAIL: bidirectional behavior delta is detached or non-finite")
    bidirectional_norm = _loss_gradient_norm(bidirectional_delta, parameters)
    if not np.isfinite(bidirectional_norm) or bidirectional_norm <= GRAD_EPSILON:
        raise RuntimeError(
            f"MVG FAIL: bidirectional delta gradient norm {bidirectional_norm:.3e}"
        )
    component_norms["bidirectional_behavior_delta"] = bidirectional_norm
    ...
    def run_counterfactual(use_bidirectional_behavior):
        model.load_state_dict(initial_state, strict=True)
        model.train()
        model.use_multi_positive_behavior = True
        model.use_bidirectional_behavior = use_bidirectional_behavior
        for layer in model.layers:
            layer.use_reciprocal_sinkhorn_eps = True
        optimizer = rqtrain.build_standard_adamw(model)
        _assert_optimizer_contract(optimizer, model, rqtrain)
    ...
    loss_on, behavior_on = run_counterfactual(use_bidirectional_behavior=True)
    loss_off, behavior_off = run_counterfactual(use_bidirectional_behavior=False)
    ...
    print(
        f"DE-2 bidirectional incremental-loss gradient norm={bidirectional_norm:.8f}"
    )
