...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def _behavior_contrastive_loss(
        ...
        """Align train-only next-item pairs with symmetric multi-positive contrastive loss."""
        positive_targets = batch.ids_fut[:, None].eq(batch.ids_fut[None, :])
        valid_rows = batch.ids.ne(batch.ids_fut)
        losses = []
        for layer_index, layer in enumerate(self.layers):
            source = quantized.residuals[layer_index, :, :batch_size].transpose(0, 1)
            candidates = quantized.residuals[layer_index, :, batch_size:].transpose(0, 1)
