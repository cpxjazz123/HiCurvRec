...
def check_step11_curvature(model: nn.Module, step: int, c_min: float, c_max: float) -> None:
    ...
    for layer_index, layer in enumerate(model.layers):
        ...
        _require(c_min <= value <= c_max, f"Step11 layer {layer_index} curvature={value} 超出范围")
