...
# linear ε schedule that gave +0.0030 over iter7). iter10 keeps iter8's mechanism
# and only tightens Sinkhorn iterations (3 -> 5) for slightly sharper assignment
# convergence.
REFERENCE_CKPT = (
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter10", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter10_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
