...
def main():
    ...
    # iter10: warm-start from iter8's checkpoint (the most recent Stage-2 run with
    # the linear ε schedule that gave +0.0030 over iter7). iter10 keeps iter8's
    # mechanism and only tightens Sinkhorn iterations (3 -> 5) for slightly sharper
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter10 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter10 warm-start loaded {len(transferred_weights)} tensors "
        ...
        print(
            ...
            f"iter10 will train from random initialization",
            flush=True,
        )
