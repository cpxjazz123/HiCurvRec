...
class Quantize(nn.Module):
    ...
    def __init__(
        ...
        if not 0 < c_branch_mid:
            raise ValueError("c_branch_mid 必须为正数（behavior branching 因子）")
        ...
        # iter12: per-layer behavior-branching curvature multiplier. Non-trainable;
        # computed once by scripts/compute_behavior_branching.py from the iter8
        # warm-start checkpoint on the train set. Multiplying the cyclic schedule
        # by branch_mid shifts each layer's c_l trajectory toward the geometric
        # capacity that its behavior-branching actually requires (instead of the
        # shared cyclic schedule used by iter4-iter11).
    ...
    def get_c(self) -> Tensor:
        """Return cyclic curvature warped by per-layer behavior-branching factor.

        Base schedule (shared across layers):
            c_schedule(t) = c_min * exp(((u_l + g_t) / 2) * log(c_max / c_min))

        iter12: per-layer behavior branching warps the schedule by `c_branch_mid`:
            exponent_l(t) = (u_l + g_t) / 2
                         + log(c_branch_mid) / log(c_max / c_min)
            c_l(t) = c_min * exp(exponent_l(t) * log(c_max / c_min))

        The shifted exponent is clamped to the configured interval. This is
        equivalent to multiplying the schedule when it stays in range, while
        preventing a behavior multiplier from producing invalid curvature.
        """
        phase = torch.tensor(
        ...
        exponent = (u_layer + g_t) / 2.0 + branch_shift / log_ratio
        exponent = exponent.clamp(1e-4, 1.0 - 1e-4)
