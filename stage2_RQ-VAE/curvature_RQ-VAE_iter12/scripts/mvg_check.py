...
# iter12: warm-start from iter8's checkpoint (now lives under unified results/ dir).
REFERENCE_CKPT = Path(
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter12", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    model = rqtrain.RqVae(
        ...
        c_branch_mid=rqtrain._load_branch_mid(),
    ).to(device)
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter12_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    print("MVG PASS")
    print("warm_start=PASS (iter8 weights loaded, period 50k)")
