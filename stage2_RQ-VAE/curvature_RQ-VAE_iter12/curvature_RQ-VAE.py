...
def main():
    ...
    # iter12: 加载 per-layer behavior branching multiplier (从 iter8 warm-start ckpt
    # 训练前一次性统计, 写进 scripts/computed_behavior_branching.json).
    ...
    if rank == 0:
        ...
        print(
            ...
            f"behavior_branch_mid (per-layer B_l warp) = [{branch_str}] | "
    ...
    # iter12: warm-start from iter8's checkpoint. iter12 = iter11 + cyclic period
    # 100k -> 50k, doubling the curvature cycle frequency within the 100k step budget.
    iter8_ckpt_path = (
    ...
    if os.path.isfile(iter8_ckpt_path):
        if rank == 0:
            ...
            if (
                ...
                raise RuntimeError(
                    f"iter12 warm-start mismatch "
                    f"(missing={sorted(missing)}, unexpected={sorted(unexpected)})"
                )
            print(
                f"[Step4] iter12 warm-start loaded {len(transferred_weights)} tensors "
        ...
        print(
            ...
            f"iter12 will train from random initialization",
            flush=True,
        )
