...
# === 早停门控 (硬阈值, 无 fallback) ===
# 2026-09-18: Gini (full_gini, per_layer_gini) 不再作为 gate, 改为描述性指标;
# 经验上 L0 极端塌陷的合法变体 (iter11 sk_eps=0.5 当前最佳, full_gini=0.1143) 会被 Gini hard gate 误杀.
# Gate 改为 3 项: HitRate (safety floor) + l01_unique_pairs (联合结构多样性) + H(L1|L0) (联合信息量).
...
def should_early_stop(metrics: SidMetrics) -> tuple[bool, str]:
    ...
    if not np.isfinite(metrics.hitrate_k50):
        # HitRate 不可用时不再用 Gini 作 gate (2026-09-18 用户决定去掉 Gini gate);
        # 此时仅靠 l01_pairs / H(L1|L0) 联合结构指标覆盖 safety floor.
        pass
    ...
    # Gini 类指标 (full_gini, per_layer_gini) 不再作为 early-stop gate;
    # 仅作为描述性指标记录到 quality_*.json, 供事后分析使用.
