...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.use_quantized_behavior = True
    ...
    def _behavior_contrastive_loss(
        ...
        """Align train-only transitions using symmetric contrastive loss on selected code embeddings."""
        positive_targets = batch.ids_fut[:, None].eq(batch.ids_fut[None, :])
        valid_rows = batch.ids.ne(batch.ids_fut)
        losses = []
        representations = (
            quantized.embeddings
            if self.use_quantized_behavior
            else quantized.residuals
        )
            ...
            if bool(valid_rows.any().item()):
                ...
                for direction_logits, direction_positive_mask in directions:
                    if self.use_multi_positive_behavior:
                        ...
                        losses.append(
                            F.cross_entropy(
                                single_positive_logits[valid_rows], labels[valid_rows]
                            )
                        )
            else:
