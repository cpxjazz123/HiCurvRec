"""Mechanism verification for iter23 quantized-code behavior alignment."""
...
def _load_training_module():
    spec = importlib.util.spec_from_file_location("rqtrain_iter23", TRAIN_SCRIPT)
...
def main() -> None:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="rqvae_iter23_mvg_", suffix=".pt", dir=SCRIPT_DIR, delete=False
    ) as handle:
    ...
    model.use_quantized_behavior = True
    torch.manual_seed(rqtrain.SEED)
    quantized_behavior = model(batch).behavior_loss
    model.use_quantized_behavior = False
    torch.manual_seed(rqtrain.SEED)
    residual_behavior = model(batch).behavior_loss
    model.use_quantized_behavior = True
    quantized_delta = quantized_behavior - residual_behavior
    if (
        not quantized_delta.requires_grad
        or quantized_delta.grad_fn is None
        or not torch.isfinite(quantized_delta).item()
    ):
        raise RuntimeError("MVG FAIL: quantized behavior delta is detached or non-finite")
    quantized_norm = _loss_gradient_norm(quantized_delta, parameters)
    if not np.isfinite(quantized_norm) or quantized_norm <= GRAD_EPSILON:
        raise RuntimeError(
            f"MVG FAIL: quantized behavior delta gradient norm {quantized_norm:.3e}"
        )
    component_norms["quantized_behavior_delta"] = quantized_norm
    ...
    def run_counterfactual(use_quantized_behavior):
        model.load_state_dict(initial_state, strict=True)
        model.train()
        model.use_multi_positive_behavior = True
        model.use_bidirectional_behavior = True
        model.use_quantized_behavior = use_quantized_behavior
        for layer in model.layers:
            layer.use_reciprocal_sinkhorn_eps = True
        optimizer = rqtrain.build_standard_adamw(model)
        _assert_optimizer_contract(optimizer, model, rqtrain)
    ...
    loss_on, behavior_on = run_counterfactual(use_quantized_behavior=True)
    loss_off, behavior_off = run_counterfactual(use_quantized_behavior=False)
    ...
    print(
        f"DE-2 quantized-minus-residual behavior delta gradient norm={quantized_norm:.8f}"
    )
