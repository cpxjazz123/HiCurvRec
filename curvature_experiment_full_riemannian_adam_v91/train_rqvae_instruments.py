...
# v91: Full Riemannian Adam (Poincaré ball)
from modules.riemannian_optim import RiemannianAdam
...
def main():
    ...
    # v91: Full Riemannian Adam — codebook params 用 Riemannian gradient + Möbius expmap retraction
    # 其他参数 (encoder/decoder/curvature) 仍用标准 Euclidean Adam
    # param.riemannian_ball=True 标记需要在 optimizer.step() 前更新 riemannian_c (curriculum 动态)
    codebook_params = []
    other_params = []
    for name, p in model.named_parameters():
        if 'layers.' in name and name.endswith('.embedding.weight'):
            p.riemannian_ball = True   # marker: 用 Riemannian Adam 更新
            p.riemannian_c = 1.0        # placeholder, 每 step 更新
            codebook_params.append(p)
        else:
            other_params.append(p)
    if rank == 0:
        print(f"[v91] Riemannian codebook params: {len(codebook_params)}, Euclidean params: {len(other_params)}", flush=True)
    optimizer = RiemannianAdam([
        {'params': codebook_params, 'lr': 1e-3, 'weight_decay': 0.0},  # v91: codebook 无 weight decay (避免拉离 manifold)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            # v91: 同步每层 codebook param 的 riemannian_c (curriculum c)
            for _layer in model.module.layers:
                _c_l = float(_layer.get_c().item())
                if hasattr(_layer.embedding.weight, 'riemannian_ball'):
                    _layer.embedding.weight.riemannian_c = _c_l
            optimizer.step()
            global_step += 1
