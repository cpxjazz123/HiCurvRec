...
def main():
    ...
    # R99 v3: 注入 RadiallyDiverseCodebook + R99 scale-decoupled forward, 与训练一致
    from train_rqvae_instruments import (
        inject_radially_diverse, set_radial_schedule_all, compute_radial_mu_sigma,
    )
    inject_radially_diverse(model)
    # 设置 radial schedule 到 final step (=10000)
    mu_final, sigma_final = compute_radial_mu_sigma(10000)
    set_radial_schedule_all(model, mu_final, sigma_final)
    # 恢复 EMA state (R99 scale-decoupled 需要)
    ema_state = state.get("ema_state", {})
    for li_str, e in ema_state.items():
        li = int(li_str)
        if li < len(model.layers):
            model.layers[li]._sd_ema_norm = e.get("ema_norm")
            model.layers[li]._sd_gamma = e.get("gamma")
    print(f"[inject] R99 scale-decoupled injected, EMA restored for {len(ema_state)} layers")
