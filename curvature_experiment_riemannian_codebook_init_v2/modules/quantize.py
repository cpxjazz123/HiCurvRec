...
class Quantize(nn.Module):
    ...
    def _kmeans_init(self, x) -> None:
        ...
        # issue74 v2: 硬编码投影到 Poincaré 球 (c=1.0, max_norm=0.999)
        # 无 import / 无新文件 / 与 baseline 唯一差异 = 1/256 codeword 缩放
        # v1 用 try/except import 路径被怀疑引入 cascade, v2 直接硬编码
        norm = self.embedding.weight.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        max_norm = 1.0 - 1e-3
