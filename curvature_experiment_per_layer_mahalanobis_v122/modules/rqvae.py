...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v122 NOVEL: Per-layer Mahalanobis init_var (异方差 cascade 自适应)
        # L0 raw encoder output norm 较大 → init_var 较小 (L0=0.5)
        # L1/L2 residual norm 较小 → init_var 较大 (L1/L2=2.0)
        # 捕捉 cascade RQ-VAE 3 层输入异方差性
        mahalanobis_init_var_per_layer: Optional[List[float]] = None,  # v122: per-layer init_var 列表 (length=n_layers)
        ...
        # v122 NOVEL: Per-layer Mahalanobis init_var (异方差 cascade 自适应)
        if mahalanobis_init_var_per_layer is None:
            mahalanobis_init_var_per_layer = [mahalanobis_init_var] * n_layers
        assert len(mahalanobis_init_var_per_layer) == n_layers, \
            f"v122 per_layer mismatch: got {len(mahalanobis_init_var_per_layer)} for n_layers={n_layers}"
        self.mahalanobis_init_var_per_layer = list(mahalanobis_init_var_per_layer)
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    mahalanobis_init_var=mahalanobis_init_var_per_layer[i],  # v122 NOVEL: per-layer 异方差 init_var
