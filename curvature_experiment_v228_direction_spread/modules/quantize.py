...
class Quantize(nn.Module):
    def __init__(
        ...
        use_sphere_anchor: bool = False,   # v226: 球壳投影 (anchor codeword norm 到 [min, max] 区间)
        anchor_min_norm: float = 0.0,      # v226: codeword 最小 norm (0=禁用, >0=强制 push up 到该值)
        anchor_max_norm: float = -1.0,     # v226: codeword 最大 norm (-1=禁用, >0=强制 scale down 到该值)
        use_direction_spread: bool = False,    # v228: KMeans init 后给 direction 加 random noise
        direction_noise_alpha: float = 0.5,    # v228: noise 强度 (0=纯 KMeans, 1=完全 random direction)
    ) -> None:
        super().__init__()
        ...
        # v226: Sphere Anchor — 强制 codeword 落在 [min_norm, max_norm] 球壳内
        # 解决 retraction (v222-v225) 失效: L0 输入 norm≈0.04 << max_norm, max_norm clip 不触发
        # v226: 改用 min_norm 下限 push codeword 到球面 (几何变换 e + manifold 约束 c)
        # R36n 合规: (c) manifold 几何替换 (codebook 在球壳 1D manifold), (e) 几何变换 (球面投影)
        # 用法: anchor_min_norm=0.3 → norm<0.3 时 scale up 到 0.3; anchor_max_norm=0.5 → norm>0.5 时 scale down
        self.use_sphere_anchor = bool(use_sphere_anchor)
        self.anchor_min_norm = float(anchor_min_norm)
        self.anchor_max_norm = float(anchor_max_norm)

        # v228: Direction Spread — KMeans init 后给每个 codeword direction 加 random noise
        # 解决 v226-v227 L0 plateau @ ~50 codes: KMeans init direction cluster → input 仅选 ~50 distinct
        # v228: noise = α * random_unit_vec, normalize, re-project to anchor norm sphere
        # R36n 合规: (e) 几何变换 (random rotation in tangent space) + (c) manifold (球面投影)
        self.use_direction_spread = bool(use_direction_spread)
        self.direction_noise_alpha = float(direction_noise_alpha)
    ...
    def _kmeans_init(self, x) -> None:
        ...
        # v228: Direction Spread — 给每个 codeword direction 加 random unit noise
        # 目的: 打破 KMeans init direction cluster (256 codeword 聚集在 input 数据方向附近)
        # 实现: new_direction = normalize(d_i + α * r_i), d_i=原 direction, r_i=random unit vector
        #      new_codeword = new_direction * target_norm (target = anchor_max_norm if > 0 else 原 norm)
        if self.use_direction_spread and self.direction_noise_alpha > 0:
            w = self.embedding.weight.data  # (K, D)
            # 1. 随机 noise: Gaussian 然后单位化
            noise = torch.randn_like(w)
            noise = noise / noise.norm(dim=-1, keepdim=True).clamp_min(1e-6)
            # 2. 原 codeword 方向 + noise, 单位化
            d_orig = w / w.norm(dim=-1, keepdim=True).clamp_min(1e-6)
            d_new = d_orig + self.direction_noise_alpha * noise
            d_new = d_new / d_new.norm(dim=-1, keepdim=True).clamp_min(1e-6)
            # 3. 投到 anchor 球面 (若 anchor_max_norm>0) 或原 norm
            if self.anchor_max_norm > 0:
                target_norm = self.anchor_max_norm
            elif self.anchor_min_norm > 0:
                target_norm = self.anchor_min_norm
            else:
                target_norm = w.norm(dim=-1).mean().item()  # fallback: 用原 norm 均值
            self.embedding.weight.data.copy_(d_new * target_norm)

    @torch.no_grad()
    def project_to_sphere(self) -> None:
        """v226: Sphere Anchor — 强制 codeword norm 落在 [anchor_min_norm, anchor_max_norm] 区间.

        解决 v222-v225 4 轮 R36p FAIL 根因: L0 输入 norm≈0.04 << max_norm=0.5, 纯 max_norm clip
        不触发, codeword 永久卡在 norm 0.04 → KMeans init 收束到 0 → collapse.

        v226: 加入 min_norm 下限 (anchor_min_norm > 0 时强制 push codeword 到球面), 形成球壳约束:
            norm → max(min_norm, min(norm, max_norm))
        用例: L0 min=0.3 max=0.5 → L0 codeword norm ∈ [0.3, 0.5] (球壳宽度 0.2)

        R36n 合规: (c) manifold 几何 (codebook 在 1D 球壳 manifold), (e) 几何变换 (球面投影).
        """
        if not self.use_sphere_anchor:
            return
        norms = self.embedding.weight.data.norm(dim=-1, keepdim=True)
        # 1) Min norm push-up: norm < min → scale up to min
        if self.anchor_min_norm > 0:
            min_scale = torch.where(
                norms < self.anchor_min_norm,
                self.anchor_min_norm / norms.clamp_min(1e-6),
                torch.ones_like(norms),
            )
            self.embedding.weight.data.mul_(min_scale)
            norms = self.embedding.weight.data.norm(dim=-1, keepdim=True)
        # 2) Max norm clip: norm > max → scale down to max
        if self.anchor_max_norm > 0:
            max_scale = torch.where(
                norms > self.anchor_max_norm,
                self.anchor_max_norm / norms.clamp_min(1e-6),
                torch.ones_like(norms),
            )
            self.embedding.weight.data.mul_(max_scale)
