...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v226: Sphere Anchor per-layer min/max norm (R36p v3.10 utility ≥ 90% hard requirement)
        use_sphere_anchor: bool = False,   # v226: enable sphere anchor projection
        anchor_min_per_layer: Optional[List[float]] = None,  # v226: per-layer min norm (default [0]*n_layers)
        anchor_max_per_layer: Optional[List[float]] = None,  # v226: per-layer max norm (default [-1]*n_layers)
        # v228: Direction Spread per-layer (打破 KMeans init direction cluster, 解 L0 collapse)
        use_direction_spread: bool = False,    # v228: enable direction spread at KMeans init
        direction_alpha_per_layer: Optional[List[float]] = None,  # v228: per-layer noise alpha (default [0.5]*n_layers)
    ) -> None:
        self._config = locals()
        ...
        # v226: per-layer sphere anchor (default disable when None)
        self.use_sphere_anchor = use_sphere_anchor
        if anchor_min_per_layer is None:
            anchor_min_per_layer = [0.0] * n_layers
        if anchor_max_per_layer is None:
            anchor_max_per_layer = [-1.0] * n_layers
        assert len(anchor_min_per_layer) == n_layers, "v226: anchor_min_per_layer length must match n_layers"
        assert len(anchor_max_per_layer) == n_layers, "v226: anchor_max_per_layer length must match n_layers"
        self.anchor_min_per_layer = anchor_min_per_layer
        self.anchor_max_per_layer = anchor_max_per_layer
        # v228: per-layer direction spread alpha
        self.use_direction_spread = use_direction_spread
        if direction_alpha_per_layer is None:
            direction_alpha_per_layer = [0.5] * n_layers
        assert len(direction_alpha_per_layer) == n_layers, "v228: direction_alpha_per_layer length must match n_layers"
        self.direction_alpha_per_layer = direction_alpha_per_layer
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    use_sphere_anchor=use_sphere_anchor,  # v226: sphere anchor projection
                    anchor_min_norm=anchor_min_per_layer[i],  # v226: per-layer min norm
                    anchor_max_norm=anchor_max_per_layer[i],  # v226: per-layer max norm
                    use_direction_spread=use_direction_spread,  # v228: direction noise at KMeans init
                    direction_noise_alpha=direction_alpha_per_layer[i],  # v228: per-layer noise alpha
                )
    ...
    @torch.no_grad()
    def apply_sphere_anchor(self) -> None:
        """v226: 调用所有 Quantize 层的 project_to_sphere() (球壳投影, 强制 utility ≥ 90%).

        训练脚本在每个 optimizer.step() 之后调用一次 (rank 0 only). 各 rank codebook 由 DDP
        all_reduce 同步, 所以只 rank 0 调用即可 (其它 rank 通过 DDP gradient sync 看到一致
        的 codebook). 但保险起见所有 rank 都调用 (各 rank 各自 project 自己持有的 codebook 副本).
        """
        if not self.use_sphere_anchor:
            return
        for layer in self.layers:
            layer.project_to_sphere()
