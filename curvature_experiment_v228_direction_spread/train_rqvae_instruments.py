...
# v227: Hybrid Manifold — per-layer 异质 manifold 约束
# v227 L0 sphere anchor 推 utility + 但 L0 卡在 ~50 codes (direction cluster 问题).
# v227 保留 sphere anchor, v228 加 direction spread.
USE_SPHERE_ANCHOR = True        # v227: 启用混合 manifold (L0 sphere anchor + L1/L2 retraction)
L0_ANCHOR_MIN, L0_ANCHOR_MAX = 0.3, 0.5   # L0: 强 sphere anchor
L1_ANCHOR_MIN, L1_ANCHOR_MAX = 0.0, 0.3   # L1: 仅 max_norm clip
L2_ANCHOR_MIN, L2_ANCHOR_MAX = 0.0, 0.3   # L2: 仅 max_norm clip

# v228: Direction Spread — KMeans init 后给每个 codeword direction 加 random noise
# 解决 v227 L0 卡在 ~50 codes 根因: KMeans init direction cluster → input 仅选 ~50 distinct
# v228: new_d = normalize(d_orig + α * random_unit_vec), 再投到 anchor norm.
# Per-layer α: L0 强 (input norm 极小, 需要 strong spread), L1/L2 弱 (input 已有 spread).
USE_DIRECTION_SPREAD = True     # v228: 启用 direction spread
L0_DIRECTION_ALPHA = 1.0        # L0: 强 noise (α=1.0 = 完全 random direction, 打破 cluster)
L1_DIRECTION_ALPHA = 0.3        # L1: 弱 noise (保持 KMeans init 提供的数据接近性)
L2_DIRECTION_ALPHA = 0.3        # L2: 弱 noise (同 L1)
...
def main():
    ...
    model = RqVae(
        ...
        use_sphere_anchor=USE_SPHERE_ANCHOR,  # v227: hybrid manifold projection
        anchor_min_per_layer=[L0_ANCHOR_MIN, L1_ANCHOR_MIN, L2_ANCHOR_MIN],  # v227: per-layer min norm
        anchor_max_per_layer=[L0_ANCHOR_MAX, L1_ANCHOR_MAX, L2_ANCHOR_MAX],  # v227: per-layer max norm
        use_direction_spread=USE_DIRECTION_SPREAD,  # v228: KMeans init 后 direction noise
        direction_alpha_per_layer=[L0_DIRECTION_ALPHA, L1_DIRECTION_ALPHA, L2_DIRECTION_ALPHA],  # v228: per-layer alpha
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v227/v228: sphere anchor + direction spread (L0 sphere anchor + L1/L2 retraction)
            model.module.apply_sphere_anchor()
            global_step += 1
