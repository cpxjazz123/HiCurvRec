...
# 取代 v282 midpoint (单码 α=0.5) + v327 Möbius EMA (alpha=0.05 silent no-op)
# 公式: top-K 最近码 → inverse-distance softmax 加权 → 切空间 Karcher 均值 → expmap 回 ball
USE_HDC = True                   # v328: 启用 HDC commit
HDC_TOPK = 3                     # top-K codes used for Karcher commit (β=2.0 让 top-1 主导但 K-2/K-3 贡献)
HDC_BETA = 2.0                   # inverse-distance softmax 温度 (β → ∞: top-1 dominates)
...
def main():
    ...
    model = RqVae(
        ...
        # v328 NOVEL: HDC commit (R36n e 几何变换全新子方向 — 跳出 v282 midpoint + v327 Möbius EMA)
        use_hdc=USE_HDC,
        hdc_topk=HDC_TOPK,
        hdc_beta=HDC_BETA,
        hdc_layer_mask=[True, False, False],  # v328 R1: 仅 L0 用 HDC, L1/L2 baseline (避免 collapse)
    ).to(device)
