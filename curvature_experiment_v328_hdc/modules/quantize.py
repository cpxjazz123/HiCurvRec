...
class Quantize(nn.Module):
    def __init__(
        ...
        # v328 NOVEL: Hyperbolic Distance-Weighted Commit (HDC)
        use_hdc: bool = False,             # v328: enable HDC commit (top-K Karcher weighted)
        hdc_topk: int = 3,                 # v328: top-K codes used for Karcher commit
        hdc_beta: float = 2.0,             # v328: inverse-distance softmax 温度
    ) -> None:
        super().__init__()
        ...
        # v328 NOVEL: HDC commit (R36n e 几何变换新子方向)
        self.use_hdc = bool(use_hdc)
        self.hdc_topk = int(hdc_topk)
        self.hdc_beta = float(hdc_beta)

        # v328 NOVEL: Hyperbolic Distance-Weighted Commit (HDC, R36n e 几何变换全新子方向)
        # 取代 v282 midpoint commit (用单一最近码 α=0.5) + v327 Möbius EMA (silent no-op 因alpha太小)
        # 公式: 对每 item 取 top-K 最近码, 用 inverse-distance 加权 (softmax(-β·d))
        #       在 Poincaré 切空间做加权 Karcher 均值, expmap 回 ball 作为 commit target
        # 论文支撑: Fréchet Mean on Riemannian Manifolds (Fréchet 1948), Hyperbolic K-Means (Chami ICML 2019)
        # 数学 signature:
        #   - 多码贡献 (top-K=3) ≠ v282 单码 midpoint (α=0.5)
        #   - 自适应权重 (β-driven softmax) ≠ v262 固定 subtraction
        #   - Karcher 均值在切空间而非 Euclidean 加权 → 真正 manifold-aware
        # Stage 1 端纯几何变更, R36n (e) 合规
        self.use_hdc = False  # 默认关闭, v328 启用
        self.hdc_topk = 3     # top-K codes used for Karcher weighted commit
        self.hdc_beta = 2.0   # inverse-distance softmax 温度 (β → ∞: top-1 dominates, β → 0: uniform)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        if self.training:
            if self.forward_mode == QuantizeForwardMode.GUMBEL_SOFTMAX:
                ...
                # v328: HDC commit — top-K Karcher weighted mean 替代单一最近码 STE
                # 数学形式: 每 item 取 top-K 最近码 → inverse-distance softmax 权重 →
                #          在 Poincaré 切空间做加权 Karcher 均值 → expmap 回 ball
                # 与 v282 midpoint 区别: 多码贡献 (top-K=3) + 自适应权重 (β-driven)
                # 与 v262 subtraction 区别: 加性 (multi-code weighted avg) 而非 subtract (single code)
                # 与 v327 Möbius EMA 区别: 直接 commit path 变更, 非 codebook update 路径
                if self.use_hdc and self.hyperbolic_distance:
                    emb = self._hdc_karcher_commit(x, codebook, dist)
                else:
    ...
    def _hdc_karcher_commit(self, x: Tensor, codebook: Tensor, dist: Tensor) -> Tensor:
        """v328 NOVEL: Hyperbolic Distance-Weighted Commit (HDC).

        对每 item 取 top-K 最近码, 用 inverse-distance softmax 加权 (温度 β),
        在 Poincaré 切空间做加权 Karcher 均值, expmap 回 ball 作为 commit target.

        数学形式:
          1) top-K indices per item: ids_topk (B, K)
          2) top-K distances: d_topk (B, K), weights w_k = softmax(-β·d_topk) (B, K)
          3) cb_topk_h (B, K, D): top-K codes 推到 Poincaré 球
          4) cb_topk_v (B, K, D): logmap 到切空间 (at origin)
          5) μ_v (B, D): Σ_k w_k · cb_topk_v
          6) μ_h (B, D): expmap(μ_v, c) back to ball
          7) commit target μ (B, D): logmap(μ_h, c)

        Args:
            x: 输入 latent (B, D)
            codebook: 当前 codebook (K, D)
            dist: 距离矩阵 (B, K), 已 detach 用于 nearest 选取

        Returns:
            emb: commit target (B, D), differentiable via top-K + weighted sum
        """
        from modules.hyperbolic import _expmap0_t, _logmap0_t

        B, K = dist.shape
        K_eff = min(self.hdc_topk, K)
        # 1) top-K 距离 + indices
        d_topk, idx_topk = dist.topk(K_eff, dim=-1, largest=False)  # (B, K_eff)
        # 2) inverse-distance softmax weights (sharpening via β)
        # d_topk 是 hyperbolic distance ≥ 0, β=2.0 → 较小 d 占主导但 top-K 都贡献
        logits = -self.hdc_beta * d_topk  # (B, K_eff)
        w = torch.softmax(logits, dim=-1)  # (B, K_eff), 可微 (走 d_topk, 但 d_topk 来自 nearest dist 已 detach)
        # 注: d 来自 detach dist, 所以 w 不直接梯度到 codebook (但 loss 反向通过 expmap/logmap 流到 codebook)
        # 3) gather top-K codes
        cb_topk = codebook[idx_topk]  # (B, K_eff, D)
        # 4) 该层曲率
        c = self.get_c()
        c_scalar = float(c.item()) if c.dim() == 0 else float(c.mean().item())
        c_t = torch.tensor(c_scalar, device=x.device, dtype=x.dtype)
        # 5) 推到 Poincaré ball + logmap 到切空间
        cb_topk_h = _expmap0_t(cb_topk, c_t)              # (B, K_eff, D)
        cb_topk_v = _logmap0_t(cb_topk_h, c_t)            # (B, K_eff, D)
        # 6) 加权 Karcher mean in tangent space
        # μ_v = Σ_k w_k · v_k  (per-item weighted sum, manifold-aware)
        w_expanded = w.unsqueeze(-1)                      # (B, K_eff, 1)
        mu_v = (w_expanded * cb_topk_v).sum(dim=1)        # (B, D)
        # 7) expmap 回 ball
        mu_h = _expmap0_t(mu_v, c_t)                      # (B, D)
        # 8) logmap 回 Euclidean coord (与 baseline codebook embedding 同空间)
        mu = _logmap0_t(mu_h, c_t)                        # (B, D)
        return mu
