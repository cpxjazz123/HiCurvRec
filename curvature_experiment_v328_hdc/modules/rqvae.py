...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v328 NOVEL: HDC (Hyperbolic Distance-Weighted Commit) — R36n (e) 几何变换全新子方向
        # 公式: top-K 最近码 → inverse-distance softmax 加权 → 切空间 Karcher 均值 → expmap 回 ball
        # 替代 v282 midpoint (α=0.5 单码) + v327 Möbius EMA (alpha=0.05 silent no-op)
        use_hdc: bool = False,
        hdc_topk: int = 3,
        hdc_beta: float = 2.0,
        # v328 R1: 层-wise HDC (避免 L1/L2 collapse), 默认 [True, False, False]
        hdc_layer_mask: Optional[List[bool]] = None,
    ) -> None:
        self._config = locals()
        ...
        # v328 NOVEL: HDC (Hyperbolic Distance-Weighted Commit)
        self.use_hdc = bool(use_hdc)
        self.hdc_topk = int(hdc_topk)
        self.hdc_beta = float(hdc_beta)
        if hdc_layer_mask is None:
            hdc_layer_mask = [True] + [False] * (n_layers - 1)
        assert len(hdc_layer_mask) == n_layers
        self.hdc_layer_mask = [bool(x) for x in hdc_layer_mask]
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    # v328: HDC commit (R36n e 几何变换新子方向)
                    use_hdc=use_hdc and hdc_layer_mask[i],
                    hdc_topk=hdc_topk,
                    hdc_beta=hdc_beta,
                )
