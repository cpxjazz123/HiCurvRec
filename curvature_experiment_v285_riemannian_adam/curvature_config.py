...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "riemannian_slerp_v285"  # ← v285 NOVEL: R36n (e) Riemannian SLERP Commit (variant #4 — 切空间球面线性插值). 公式: omega = angle(log_r, log_e); log_mid = (sin(0.5*omega) * log_r + sin(0.5*omega) * log_e) / sin(omega); res_mid = exp_0(log_mid, c). 论文: Álvarez et al. 2022 "Spherical Poincaré Embeddings" / Ungar 2008 Gyrogroup SLERP. 与 v282 midpoint (Euclidean mean in tangent space) 的本质差异: SLERP 在切空间沿 great-circle arc 插值, 保留方向角信息; midpoint 仅做算术平均. 当 log_r ∥ log_e 时两者等价; 当 log_r ⊥ log_e 时, SLERP 给出不同 commit 点 (居中于 great-circle arc 上, 不是 Euclidean mean). R36n (e) 几何变换变种 #4. 保留 v282 baseline 全部配置: cyclic + M2 commit loss + M3 transport + midpoint commit (但 L0 用 SLERP 接管). R36h ceiling 第 21 次验证. 目标: test_R@10 > 0.1135631443298969 (v282 新 baseline).
