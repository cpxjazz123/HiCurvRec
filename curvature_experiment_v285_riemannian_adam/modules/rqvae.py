...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v285 NOVEL: Riemannian SLERP commit (R36n e 几何变换变种 #4)
        # 公式: log_mid = (sin(0.5*omega) * log_r + sin(0.5*omega) * log_e) / sin(omega)
        #       res_mid = exp_0(log_mid, c)
        #       omega = angle(log_r, log_e) in tangent space
        # vs v282 midpoint: v282 = arithmetic mean of tangent vectors; SLERP = spherical
        # linear interpolation along geodesic on Poincaré ball.
        # 论文支撑: Álvarez et al. 2022 "Spherical Poincaré Embeddings" / qslerp.
        use_slerp_commit: bool = False,
        # v285 R1 (R36p): 层-wise SLERP — 仅在指定层启用, 其余用 baseline Möbius_sub.
        # 默认 = [True, False, False] (L0 SLERP, L1/L2 baseline).
        slerp_layer_mask: Optional[List[bool]] = None,
    ) -> None:
        self._config = locals()
        ...
        # v285 NOVEL: SLERP commit (R36n e 几何变换变种 #4)
        self.use_slerp_commit = bool(use_slerp_commit)
        if slerp_layer_mask is None:
            slerp_layer_mask = [False] * n_layers  # 默认全 False (不启用)
        assert len(slerp_layer_mask) == n_layers
        self.slerp_layer_mask = [bool(x) for x in slerp_layer_mask]
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001) -> RqVaeOutput:
        ...
        for li, layer in enumerate(self.layers):
            ...
            if self.gate_M2_intrinsic:
                ...
                elif self.use_slerp_commit and self.slerp_layer_mask[li]:
                    # v285 NOVEL: Riemannian SLERP commit (R36n e 几何变换变种 #4)
                    # 公式: omega = angle(log_r, log_e) in tangent space
                    #       log_mid = (sin(0.5*omega) * log_r + sin(0.5*omega) * log_e) / sin(omega)
                    #       res_mid = exp_0(log_mid, c)
                    # vs v282 midpoint: SLERP follows great-circle arc on Poincaré ball,
                    # preserves angle information (cos_omega). When log_r ∥ log_e (cos_omega=1),
                    # SLERP ≡ midpoint. When log_r ⊥ log_e (cos_omega=0), SLERP gives different point.
                    # 论文: Álvarez et al. 2022 "Spherical Poincaré Embeddings" (qslerp),
                    #       Ungar 2008 Gyrogroup (SLERP = special Möbius interpolation).
                    log_r = _logmap0_t(res, c_per)
                    log_e = _logmap0_t(emb, c_per)
                    norm_r = log_r.norm(dim=-1, keepdim=True).clamp(min=1e-9)
                    norm_e = log_e.norm(dim=-1, keepdim=True).clamp(min=1e-9)
                    cos_omega = (log_r * log_e).sum(dim=-1, keepdim=True) / (norm_r * norm_e)
                    cos_omega = cos_omega.clamp(-1.0 + 1e-7, 1.0 - 1e-7)
                    omega = torch.acos(cos_omega)
                    sin_omega = torch.sin(omega).clamp(min=1e-9)
                    # SLERP at t=0.5 (equal weights along great circle):
                    log_mid = (torch.sin(0.5 * omega) * log_r + torch.sin(0.5 * omega) * log_e) / sin_omega
                    res = _expmap0_t(log_mid, c_per)
                else:
