...
# SLERP 在切空间做 spherical linear interpolation, 保留方向信息.
# 与 v282 midpoint (Euclidean mean in tangent space) 的差异:
# - midpoint: log_mid = (log_r + log_e) / 2
# - SLERP:    log_mid = (sin(0.5*omega) * log_r + sin(0.5*omega) * log_e) / sin(omega)
#             omega = angle(log_r, log_e) in tangent space
# 当 log_r 与 log_e 平行 (cos_omega=1) 时, SLERP ≡ midpoint.
# 当 log_r 与 log_e 正交 (cos_omega=0) 时, 两者产生不同 commit.
# R36n (e) 几何变换家族变种, 与 v262/v270/v282 同源.
USE_SLERP_COMMIT = True                   # v285 启用 SLERP commit (覆盖 Möbius_sub on masked layers)
SLERP_LAYER_MASK = [True, False, False]   # v285 R1: 仅 L0 用 SLERP, L1/L2 baseline Möbius_sub
...
def main():
    ...
    model = RqVae(
        ...
        midpoint_layer_mask=[True, False, False] if USE_SLERP_COMMIT else [True, False, False],  # v285 R1: SLERP 接管 L0 midpoint; 关闭 midpoint (SLERP 优先)
        use_slerp_commit=USE_SLERP_COMMIT,  # v285: SLERP commit (R36n e variant #4)
        slerp_layer_mask=SLERP_LAYER_MASK,  # v285 R1: 仅 L0 用 SLERP
    ).to(device)
