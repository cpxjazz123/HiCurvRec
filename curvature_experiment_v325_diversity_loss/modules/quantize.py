...
class QuantizeOutput(NamedTuple):
    ...
    diversity_loss: Tensor = torch.zeros(())  # v325: -entropy(softmax(-dist/τ)) mean; 鼓励 codebook 均匀使用
...
class Quantize(nn.Module):
    def __init__(
        ...
        # v325 NOVEL: Codebook Diversity Loss (entropy maximization on soft assignment)
        # 机制: 每个 Quantize 层用 softmax(-dist/temperature) 算 soft assignment,
        #       最大化 entropy(soft_assignment) → 鼓励 codebook 使用均匀 → 防 R36p collapse
        # **非曲率变更**: 仅影响 loss 加权, 不动 manifold/commit/c; R36n 灰色地带, R36v3.6 字面 PASS
        # **论文支撑**: "Online Clustered Codebook" (ICML 2024) / BatchBalance / VQ-VAE 衍生工作
        use_diversity_loss: bool = False,    # v325: enable codebook diversity entropy loss
        diversity_temperature: float = 0.5,  # v325: softmax 温度 (低 → 更尖锐, 高 → 更平滑)
    ) -> None:
        super().__init__()
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v325: Codebook Diversity Loss (entropy maximization on soft assignment)
        # 用 dist 反向 → soft assignment 概率: p[b, k] = softmax(-dist[b, k] / τ)[k]
        # entropy per sample: H[b] = -Σ_k p[b, k] · log(p[b, k])  (max when uniform: log(K))
        # diversity_loss = -mean(H[b])  (maximize entropy = minimize -entropy)
        # 关键: soft assignment 梯度必须流过 dist 反向到 encoder/codebook (STE)
        diversity_loss = torch.zeros((), device=emb_out.device, dtype=emb_out.dtype)
        if self.use_diversity_loss and self.training and not self.hypervq:
            tau = max(self.diversity_temperature, 1e-6)
            # p = softmax(-dist / τ)  (距离越小 → 概率越大 → entropy 集中)
            log_p = F.log_softmax(-dist / tau, dim=-1)  # (B, K)
            p = log_p.exp()  # (B, K)
            # entropy per sample (B,) — 注意 K=256, log(K) ≈ 5.545
            entropy_per_sample = -(p * log_p).sum(dim=-1)  # (B,)
            diversity_loss = -entropy_per_sample.mean()  # 负 entropy (minimize)
            self._last_diversity_loss = diversity_loss.detach()
        else:
            self._last_diversity_loss = diversity_loss.detach()

        return QuantizeOutput(embeddings=emb_out, ids=ids, loss=loss, margin=margin, spread_loss=spread_loss, diversity_loss=diversity_loss)
