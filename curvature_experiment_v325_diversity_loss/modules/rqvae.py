...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # 用 softmax(-dist/τ) 算 soft assignment, 最大化 entropy(soft_assignment)
        # 鼓励 codebook 均匀使用, 防 R36p collapse (历史 v324 EMA R36p L1=66.4% FAIL 教训)
        # **非曲率变更**: R36v3.6 字面 PASS, 不动 manifold/commit/c
        diversity_loss_weight: float = 0.0,    # v325: 0=关闭; 0.01 推荐值
        diversity_temperature: float = 0.5,   # v325: softmax 温度 (0.5 推荐值)
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    # v325: Codebook Diversity Loss (entropy maximization)
                    use_diversity_loss=diversity_loss_weight > 0,
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v325: Codebook Diversity Loss — 累加所有 Quantize 层 diversity_loss, 加权到 total loss
        # 注意: diversity_loss 存在各 layer._last_diversity_loss (entropy minimization → 鼓励均匀使用)
        diversity_loss_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.diversity_loss_weight > 0:
            diversity_per_layer = [
                l._last_diversity_loss for l in self.layers if hasattr(l, "_last_diversity_loss")
            ]
            if diversity_per_layer:
                diversity_loss_total = torch.stack(diversity_per_layer).mean()
        loss = (
            (reconstuction_loss + rqvae_loss).mean()
            + self.margin_reg_weight * margin_loss
            + self.spread_loss_weight * spread_loss_total
            + self.diversity_loss_weight * diversity_loss_total
        )
