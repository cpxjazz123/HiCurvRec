...
# 机制: softmax(-dist/τ) 算 soft assignment, 最大化 entropy(soft_assignment) → 鼓励 codebook 均匀使用
# **非曲率变更**: R36n 灰色地带, R36v3.6 字面 PASS (Stage 1 端架构变更)
# **论文支撑**: "Online Clustered Codebook" (ICML 2024) + VQ-VAE 衍生工作
# **R36p 防御**: 直接对抗 cascade RQ-VAE L1/L2 collapse (历史 v324 EMA R36p L1=66.4% FAIL 教训)
# **τ 选择**: 0.5 比 1.0 更尖锐 (熵变化更显著, gradient 更有意义)
# **weight 选择**: 0.01 与 spread_loss_weight=0.005 同量级, 不主导 recon loss
USE_DIVERSITY_LOSS = True        # v325: enable codebook diversity entropy loss
DIVERSITY_LOSS_WEIGHT = 0.01     # v325: 正则权重 (与 spread_loss 同量级)
DIVERSITY_TEMPERATURE = 0.5      # v325: softmax 温度 (0.5 推荐值)
...
def main():
    ...
    model = RqVae(
        ...
        diversity_loss_weight=DIVERSITY_LOSS_WEIGHT if USE_DIVERSITY_LOSS else 0.0,
