...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v213 NOVEL: per-layer c_end ladder
        c_end_per_layer: Optional[List[float]] = None,       # v213: 各层独立 c_end
    ) -> None:
        self._config = locals()
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    c_start=c_start,
                    c_end=(c_end_per_layer[i] if c_end_per_layer is not None else c_end),  # v213: per-layer c_end ladder
    ...
    @torch.no_grad()
    def reset_dead_codes_all_layers(self, x: Tensor, per_layer_inputs: list = None) -> dict:
        """v213: 跨所有 Quantize 层执行 dead-code reset (R36n e/f codebook geometric update).

        每层独立 reset: 死码 = usage_count == 0, 用 per_layer_inputs[l] (该层 residual 输入) 重置.
        若 per_layer_inputs=None, 则各层都用同一个 x (encoder 输出). 所有 rank 各自调用
        (DDP 各 rank 各自 reset 自己看到的死码; 因 usage_count 是 buffer, 4 个 rank 累积相同,
        因为 input dist 相同 + ids 同步).

        Args:
            x: 备用默认 source (B, D)
            per_layer_inputs: list of (B, D) 或 None, 各层 reset 时用的 source

        Returns:
            dict: {layer_idx: n_dead_reset}
        """
        result = {}
        for li, layer in enumerate(self.layers):
            if hasattr(layer, "reset_dead_codes"):
                src = per_layer_inputs[li] if (per_layer_inputs is not None and li < len(per_layer_inputs)) else x
                n = layer.reset_dead_codes(src, None)
                result[li] = n
        return result
