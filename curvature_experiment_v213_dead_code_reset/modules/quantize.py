...
class Quantize(nn.Module):
    def __init__(
        ...
        # v213: dead-code usage counter (per-code 选中次数统计)
        # 用 buffer (非 parameter, 不参与 optimizer), 训练中累积
        self.register_buffer("usage_count", torch.zeros(n_embed, dtype=torch.long), persistent=False)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v213: 累积各 code 在本 batch 的 usage 计数 (供 dead_code_reset 决策)
        # 注意: 在 train 模式且 self.training 时累积, 评估时不累积
        if self.training:
            with torch.no_grad():
                ids_long = ids.long()
                batch_usage = torch.zeros(self.n_embed, dtype=torch.long, device=ids.device)
                batch_usage.scatter_add_(
                    0, ids_long, torch.ones_like(ids_long, dtype=torch.long)
                )
    ...
    @torch.no_grad()
    @torch.no_grad()
    def reset_dead_codes(self, x: Tensor, ids: Tensor, dead_threshold: int = 0) -> int:
        """v213: Dead-code reset (R36n e/f codebook geometric update).

        把 usage_count == 0 的 dead code 重新初始化为当前 batch 中随机 encoder outputs.
        论文支撑: VQ-VAE-2 (Razavi et al. 2019) "Generating Diverse High-Fidelity Images with VQ-VAE-2"
                  使用随机选样 reinit dead codes. 几何解读: 死码 = 嵌入空间死点, 重新置入数据流形.

        Args:
            x: 当前 batch 输入 latent (B, D) (已 detach)
            ids: 当前 batch 选中的 code ids (B,)
            dead_threshold: int, usage_count <= dead_threshold 视为 dead (默认 0)

        Returns:
            int: 被 reset 的 dead code 数
        """
        device = self.device
        K = self.n_embed
        # 1) 找 dead codes
        dead_mask = self.usage_count <= dead_threshold  # (K,)
        n_dead = int(dead_mask.sum().item())
        if n_dead == 0:
            return 0
        # 2) 用当前 batch 的 distinct encoder outputs 作为 reinit 候选
        # unique_ids = 当前 batch 用过的 codes (作为反例避免重置到已用 code)
        used_codes = torch.unique(ids)  # (n_used,)
        # 从 x 中随机选 n_dead 个 (有放回)
        n_pool = x.shape[0]
        if n_pool < n_dead:
            # batch 小于 dead 数, 全部用 (有放回)
            idx = torch.randint(0, n_pool, (n_dead,), device=device)
        else:
            idx = torch.randperm(n_pool, device=device)[:n_dead]
        new_codes = x[idx].clone()  # (n_dead, D)
        # 3) 替换 dead code 的 embedding
        dead_idx = torch.nonzero(dead_mask, as_tuple=False).squeeze(-1)  # (n_dead,)
        self.embedding.weight.data[dead_idx] = new_codes.to(self.embedding.weight.dtype)
        # 4) 重置 log_var (Mahalanobis) 到 init 值, 避免死码 var 异常
        import math as _math
        init_log_var = _math.log(_math.expm1(self.mahalanobis_init_var)) if self.mahalanobis_init_var > 0 else -10.0
        self.log_var.data[dead_idx] = init_log_var
        # 5) 清零 usage_count (避免下次 reset 立即重置新激活的 code)
