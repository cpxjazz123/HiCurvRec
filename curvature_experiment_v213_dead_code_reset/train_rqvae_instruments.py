...
USE_FIXED_CURVATURE = False               # v213: 关固定曲率, 让 per-layer c_end 生效
# === v213 NOVEL: per-layer c_end ladder + dead-code reset ===
# 8 rounds ladder/CDL R36p FAIL 后改走显式 codebook 干预. R36c Round 9.
# 机制: c_end_per_layer = [0.5, 0.6, 0.7] (R36n b) + 每 N 步重置 usage=0 的死码
# 死码重置 source = 当前 batch 该层 residual 随机采样 (几何流形自然位置)
# 论文支撑: VQ-VAE-2 (Razavi 2019) "Generating Diverse High-Fidelity Images"
USE_PER_LAYER_C_END = True
PER_LAYER_C_END = [0.5, 0.6, 0.7]    # L0 浅+扩张 → L1 中 → L2 深
DEAD_CODE_RESET_INTERVAL = 500         # 每 500 步重置死码
DEAD_CODE_RESET_WARMUP = 1000          # 前 1000 步不重置 (让 KMeans init 稳定)
...
def main():
    ...
    model = RqVae(
        ...
        # v213: per-layer c_end ladder
        c_end_per_layer=PER_LAYER_C_END if USE_PER_LAYER_C_END else None,
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v213: Dead-code reset — 每 DEAD_CODE_RESET_INTERVAL 步, 找 usage=0 的死码
            #       用当前 batch encoder 输出 (or residual) 重新初始化死码
            #       R36n e/f codebook geometric update
            if (
                DEAD_CODE_RESET_INTERVAL > 0
                and global_step_sync >= DEAD_CODE_RESET_WARMUP
                and (global_step_sync % DEAD_CODE_RESET_INTERVAL == 0)
            ):
                # 用当前 batch 的 encoder 输出作为 reset source (L0 源 = encoder(x))
                # L1/L2 用同源 (几何上各层 reset 都用 encoder 输出也可, 死码重置为 in-distribution 即可)
                # 简单实现: 各层都用 encoder(x) 作为 source (vs 严格用各层 residual)
                reset_src = x.detach()  # (B, embed_dim)
                if rank == 0 and global_step_sync % (DEAD_CODE_RESET_INTERVAL * 5) == 0:
                    print(f"[dead_code_reset] step={global_step_sync} (warmup passed)", flush=True)
                n_reset = model.module.reset_dead_codes_all_layers(x=reset_src, per_layer_inputs=None)
                if rank == 0 and n_reset and any(v > 0 for v in n_reset.values()):
                    total = sum(n_reset.values())
                    if global_step_sync % (DEAD_CODE_RESET_INTERVAL * 5) == 0:
                        print(f"[dead_code_reset] step={global_step_sync} reset {total} dead codes: {n_reset}", flush=True)
            global_step += 1
