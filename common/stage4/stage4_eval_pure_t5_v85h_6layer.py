...
CONFIG = dict(
    num_layers=6, num_decoder_layers=6, d_model=128, d_ff=1024,  # Issue #41 v85h 6-decoder-layer fork (2026-08-09): num_decoder_layers 6→4 匹配 v77 Stage3 训练 (v77 ckpt decoder blocks=[0,1,2,3], 53 decoder keys). 之前 6 默认会让 Stage4 eval 出现 26 missing keys (block.4.*), T5 评估用随机初始化 block 4 + HAB 状态不一致 → test_R@10 严重低估 (0.0957 vs v77 原 0.1080). v85p/v85j 6 layer ckpt 需要回退 fork 脚本 (R31).
...
def calculate_pos_index(preds, labels, maxk=20):
    # Issue #41 v85h fork (2026-08-09): 修复 BUG — 原 Stage4 eval 用 preds[i,j] (单 token) vs cur_label (list)
    # 永远不等 → R=0. 改用 train_pure_t5.py 的向量化正确逻辑: (B,maxk,seq_len)==(B,1,seq_len).all(dim=-1)
