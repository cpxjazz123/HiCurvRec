...
**日期**: 2026-07-31
**Issue**: #111 [方向C Gate3] — 用同一 Stage 3 checkpoint 验证 T5 是否真的消费层级 SID 几何 (而不是仅 token identity)
**任务**: 加载 task396b Stage 3 ckpt + 同一 batch + 5 组结构反事实 (logits diff + argmax match + gradient norm)
**结果**: ✅ **Gate 3 PASS** — 4 组结构破坏都产生稳定非零 logits 差异 + 有限非零 attention/ffn 梯度 + top-K 大幅变化
...
- **架构意义**: T5-mini 通过 `T5.embed_tokens(SID)` 把 SID 离散索引映射到 d_model=128 dense vector. embed_tokens 梯度 = 0 (frozen 验证 T5 训练阶段未更新), 但 attention/ffn 梯度非零 → 证明 T5 通过 attention path 真的消费了 SID 几何结构
...
- **verdict 路径**: `verdicts/task406_issue111_stage3_layered_geometry_counterfactual_v2.md`
...
1. **commit + push Issue #111 Gate 3 PASS verdict** ✅ (commit 47672ac, 已 push)
2. **close Issue #111** ✅ (per R16 + R20 + R21 comment)
3. **Issue #113 架构改造** (task426 in_progress): 实现真实三分量 product checkpoint schema, 是 #109/#110/#111 共同架构限制的解药
4. **Issue #114** (task427 pending): 在 Issue #113 基础上实现 minimal product-stereographic attention residual