...
Issue #181 owner 2026-08-01 派发方向B Gate4: 复用 #178 (Task #471) BoundedWeightedMixedCurvatureConditioner (α=softplus(α_logit).clamp(max=0.5), 三层 [κ,α,β,γ] 零中心) 完整 Stage 3 200 epoch + Stage 4 R@K 双复跑 (R@5/10/20, NDCG@5/10/20). 跟 Task #450 (方向C) + Task #472 (方向A) 平行, 实施差异 = 强制 α ≤ 0.5 有界残差 + 4 分量元数据 + 零中心.

R18 4 维度对比 vs Task #450 (方向C 200 epoch):
- **D1 spec**: Issue #181 方向B vs Issue #150 方向C — 同一 long train 协议, 不同 wrapper
- **D2 实施**: BoundedWeightedMixedCurvatureConditioner (α 有界 + 4 分量元数据 + 零中心) vs ZeroCenteredLayerNormAdapter (无 α clamp) → **不同**
- **D3 Gate 4 决策**: 方向B R@10 > 0.1020 vs 方向C R@10 > 0.1020 — 平行对照
...
- **wrapper**: `HG_Rec_with_BoundedWeightedMixedAdapter` (from #471)
  - BoundedWeightedMixedCurvatureConditioner: α = softplus(α_logit).clamp(max=0.5) 强制有界
  - 4 分量 [κ_l, alpha_l, beta_l, gamma_l] 编码 (3 layers × 4 dims = 12 dims) + 零中心
  - T5 主干冻结, 仅 `encoder.block.0.layer.0.layer_norm` + conditioner 解冻
- **GPU**: GPU 2 (R7 满足, GPU 0 task450, GPU 1 task472, GPU 3 留 Issue #161 backup)

## Gate 顺序 (per Issue #181 spec)

- **precheck**: 5 项 — ① SID SHA256 = #157/#158 ✅; ② wrapper 类加载成功; ③ α=0 init; ④ trainable/frozen 数合理; ⑤ Stage 3 训练开始
...
R11.5 兜底 4. 简单实用方案 = 严格按 Issue #181 spec:
...
- **verdict**: `verdicts/task473_issue181_direction_b_gate4_200ep_result.md`
...
- **train_trace**: `products/task473_issue181_direction_b_gate4_200ep/train_trace_200ep.json`
- **adapter_200ep.pt**: `products/task473_issue181_direction_b_gate4_200ep/adapter_200ep.pt`
- **eval_run1.json**: `products/task473_issue181_direction_b_gate4_200ep/eval_run1.json`
- **eval_run2.json**: `products/task473_issue181_direction_b_gate4_200ep/eval_run2.json`
- **script**: `scripts/task473_issue181_direction_b_gate4_200ep_stage3_stage4.py`
- **log**: `logs/task473_issue181_direction_b_gate4_200ep.log`
