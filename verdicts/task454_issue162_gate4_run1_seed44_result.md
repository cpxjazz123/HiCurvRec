# Task #454 / Issue #162 Gate 4 Stage 3 200 epoch 训练 + val_R@10 早停 — NO-GO 收口 (wrapper broken 真实失败)

## 任务摘要

Issue #162 owner 2026-08-01 派发: κ同步尺度适配单seed正式评估 (Task #84 baseline 六项指标). 本任务跑 200 epoch Stage 3 训练 + val_R@10 早停 (owner 派工, commit d54aa68 patch), 触发 val_R@10=0.0000 持续, wrapper broken 信号明确, 立即 NO-GO 收口.
...
- **wrapper**: HG_Rec_with_KappaScaleAdapter (Issue #162 spec, alpha=1.0 clamp)

## R18 4 维度对比 (vs Issue #159 kappa-scale Gate 3 训练 PASS)

| 维度 | Task #454 (#162 Gate 4 200 epoch) | Task #451 (#159 Gate 3 10 epoch) |
|------|-----------------------------------|----------------------------------|
| **D1 spec** | **200 epoch 长训 + val_R@10 早停 + Stage 4 双复跑六项指标** | 10 epoch 短训 + Stage 4 单 run |
| **D2 实施** | **val_R@10 早停 patch** (owner 2026-08-01) + Issue #162 wrapper | Issue #162 wrapper 同 Task #454 |
| **D3 Gate 1 失败机制** | wrapper alpha=1.0 clamp 饱和 → T5 logits 偏离 → val_R@10=0 | Gate 3 短训, loss 健康下降 |
| **D4 引用** | Issue #162 spec | Issue #159 spec |
...
- inspect.signature 动态探测 wrapper meta kwarg 名称
- input_ids= 修复 (wrapper forward 实际参数名)
- unpack 3-tuple 正确 (#450 vs #454 wrapper 返回结构差异)

## Gate 2 (= 200 epoch Stage 3 训练 + val_R@10 真实状态): ❌ FAIL — wrapper broken

**关键实测数据** (Task #454 v8, 实际跑到 epoch 19):
- val_R@10 跨 epoch 5/10/15/20 = **0.0000 持续**
- loss 健康下降 (9.15 → 0.04), 但 val_R@10 永远 0
- **真实失败根因**: wrapper alpha=1.0 clamp (Issue #162 spec 强制 α 有界, init_logit=-10.0 + softplus + clamp → 立即饱和到 1.0)
...
| 假设 | 状态 | 关键证据 |
|------|------|----------|
| **α 有界 + κ-sync 适配** | ❌ FAIL | wrapper alpha=1.0 饱和 → T5 logits 偏离 → R@10=0 |

**Issue #162 闭环决策: NO-GO** (跟 Issue #157+#158 Gate 2 PASS 但 Gate 3 仍未跑成 Task #84 baseline 0.1020 的趋势一致, κ适配 整体机制无 R@10 杠杆).

## 关键决策点 (R11.5 自主决策)

1. **早停 patch v8 严格 4-digit 协议**: 跟 Stage 4 eval `SIDRetrievalEvaluator` 一致, 不允许 fallback 宽松
2. **wrapper broken 真实失败**: alpha=1.0 clamp 立即饱和, residual 过大破坏 T5 layer_norm. 跟 #456 同模式
...
5. **Issue #162 close --reason completed**: val_R@10 跟踪闭环 + 失败信号明确, 满足 R16 强制 close

## 后续 (R10 v2 idle 允许)

Issue #162 NO-GO 收口 + Issue #163 (#456) 同模式 NO-GO 收口. κ适配路径 (#157/#158/#162/#163) 4 issue 全部 NO-GO, baseline recipe 内部 κ 元数据适配 R@10 杠杆已穷尽. 后续架构层方向 (R@10 > 0.1020 真杠杆) 需 owner 派工新 issue.