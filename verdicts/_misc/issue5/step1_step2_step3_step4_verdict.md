...
up: "[[index]]"
---
# Issue #5 [方向B Gate4修复] Step 1-4 闭环 verdict
...
**Bug 位置 (4 处)**:
- `taskB/stage4/taskB_stage4_resume.py:50` 原 `n_correct = sum(1 for p, t in zip(preds_list, targets_list) if p[:k] == t[:k])`
- `taskB/stage4/taskB_stage4_resume.py:57` 原 NDCG 同款
...
**修复前后对比 (同 ckpt `taskB_stage3_issue193_long_run/best_adapter.pt` @ epoch 50, autoregressive 4-step decode, 100 samples)**:
...
**修改文件**:
- `taskB/stage4/taskB_stage4_resume.py` (L49-75 改 compute_r_at_k + compute_ndcg_at_k; L161-167 改 main flow 传 [pred] 1-candidate list)
...
**Config 位置**: `taskB/stage3/taskB_stage3_issue193_long_run.py:78,525-531`
...
| 5 | best ckpt 路径 | `taskB/stage3/taskB_stage3_issue193_long_run/best_adapter.pt` @ epoch 50 (val_R@10 plateau) | `BEST_ADAPTER_CKPT_PATH` (L95), long-run 实际触发 `early_stop_triggered=True` |

**Val trace (方向B long-run, val_R@10 ∈ [0.055, 0.058] plateau, best @ epoch 50)**:
- (val_trace.json 详细记录, best epoch 由 val_R@10 max 确定)
...
**Canary 脚本**: `taskB/stage4/taskB_stage4_canary_issue5_step1.py`
...
**Verdict 文件**: `verdicts/issue5_step1_canary_result.json`
...
**结论**: 方向B Gate 4 实质性突破需要重训或换路径 (paper §6.7.9 已记录此结论), 修复后函数只是把"评估错误"修成"评估正确反映 4% R@10"。

---

## 闭环 4 件套 (R15)

- [x] verdict 落盘: `verdicts/issue5_step1_step2_step3_step4_verdict.md` (本文件) + `verdicts/issue5_step1_canary_result.json`
- [ ] commit: pending
- [ ] push: pending
