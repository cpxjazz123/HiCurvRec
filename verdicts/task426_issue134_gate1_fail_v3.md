# Task #426 / Issue #134 [方向A Gate1] hard-count EMA 双曲码本更新防单码字坍缩 — NO-GO 收口
...
- 关键数据:
  ...
  - `grad_finite_nz=False` (κ grad=0.0, encoder grad=6.45e-5 / 7.79e-5 / 2.59e-5 — κ 锁死导致 κ 无梯度, 不符合 5-step audit 的"κ grad 有限非零" 要求)
...
Issue #134 假设: hard argmin + geodesic EMA + count reg 能防单码字坍缩
实测结果: EMA 机制本身完整 (ema_drift=0.134 finite, reg_affects_loss=True, hard_argmin_ok=True, domain_ok=True), 但 5-step audit 第 3 项 (κ grad 有限非零) FAIL.

联立 #131/#132/#133 + #134 = **任何软/硬正则化路径 (Sinkhorn transport / per-item posterior / shared plan / hard EMA + count reg) 都被 R137 κ lock 与 5-step audit κ grad 要求冲突**. 下一方向必须明确"是否需要 κ grad" 二选一:
- 选项 A: 解除 κ lock → κ 可以动 → 上一轮 task199/task201/task203/task204 已证 κ 调节是低 ROI 杠杆 (R@10 ceiling 0.1053 锁死)
- 选项 B: 修改 5-step audit 兼容 R137 κ lock (per spec, audit 是硬要求, 不允许修改)

Issue #134 → Option B NO-GO. 下一步需要新的 spec 兼容 R137 κ lock.