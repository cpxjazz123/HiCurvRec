...
C_CYCLIC_MIN = 0.05                       # cyclic c_min (近欧氏, v299 fallback 当 per-layer=None)
C_CYCLIC_MAX = 0.7                        # cyclic c_max (双曲, v299 fallback 当 per-layer=None)
C_CYCLIC_PERIOD = 25_000                  # cyclic 周期 T (步数, 训练 100k 步 ≈ 4 周期)
# v299 NOVEL: per-layer amplitude-asymmetric cyclic (R36n a+b 联合方向)
# L0 baseline [0.05, 0.7], L1 略扩展 [0.1, 0.9], L2 深度双曲 [0.05, 1.5]
# 论文支撑: SGDR (Loshchilov & Hutter 2017) + multi-scale curriculum
C_CYCLIC_MIN_PER_LAYER = [0.05, 0.1, 0.05]   # v299: per-layer c_min, L0=baseline, L1=略高避免 0, L2=复位
C_CYCLIC_MAX_PER_LAYER = [0.7, 0.9, 1.5]     # v299: per-layer c_max, L0=baseline, L1=略扩展, L2=深度双曲
...
def main():
    ...
    model = RqVae(
        ...
        c_cyclic_min_per_layer=C_CYCLIC_MIN_PER_LAYER,  # v299: per-layer amplitude-asymmetric cyclic min
        c_cyclic_max_per_layer=C_CYCLIC_MAX_PER_LAYER,  # v299: per-layer amplitude-asymmetric cyclic max
