...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # 让不同层 c 范围不一样: L0=[0.05,0.7], L1=[0.1,0.9], L2=[0.05,1.5]
        # 论文支撑: SGDR (Loshchilov & Hutter 2017) + multi-scale curriculum
        c_cyclic_min_per_layer: Optional[List[float]] = None,
        c_cyclic_max_per_layer: Optional[List[float]] = None,
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    c_cyclic_min_per_layer=(
                        c_cyclic_min_per_layer[i] if c_cyclic_min_per_layer is not None else None
                    ),  # v299: per-layer amplitude-asymmetric cyclic
                    c_cyclic_max_per_layer=(
                        c_cyclic_max_per_layer[i] if c_cyclic_max_per_layer is not None else None
                    ),  # v299: per-layer amplitude-asymmetric cyclic
                    c_fixed=c_fixed,
