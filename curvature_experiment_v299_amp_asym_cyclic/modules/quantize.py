...
class Quantize(nn.Module):
    def __init__(
        ...
        # v299 NOVEL: amplitude-asymmetric cyclic — 三层 c_min/c_max 范围不同
        # 让每层曲率震荡幅度不同, L2 进入更深双曲区 (c_max 1.5 > 1.0 ball 边界)
        # 论文支撑: SGDR (Loshchilov & Hutter 2017) + per-layer capacity scaling (ResNet)
        c_cyclic_min_per_layer: Optional[List[float]] = None,  # v299: per-layer c_min, e.g. [0.05, 0.1, 0.05]
        c_cyclic_max_per_layer: Optional[List[float]] = None,  # v299: per-layer c_max, e.g. [0.7, 0.9, 1.5]
        ...
        # v299: per-layer amplitude
        self.c_cyclic_min_per_layer = c_cyclic_min_per_layer
        self.c_cyclic_max_per_layer = c_cyclic_max_per_layer
    ...
    def get_c(self) -> Tensor:
        ...
        if self.use_cyclic_curvature:
            ...
            # v299: per-layer amplitude — 三层不同 c_min/c_max 范围
            c_min_l = self.c_cyclic_min if self.c_cyclic_min_per_layer is None else self.c_cyclic_min_per_layer
            c_max_l = self.c_cyclic_max if self.c_cyclic_max_per_layer is None else self.c_cyclic_max_per_layer
            c = c_min_l + (c_max_l - c_min_l) * abs(math.sin(phase))
