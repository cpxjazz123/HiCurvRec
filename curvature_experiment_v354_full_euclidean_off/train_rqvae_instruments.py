...
USE_CYCLIC_CURVATURE = False              # v354 ABLATION: 全欧氏退化 — 关闭 cyclic c(t) (固定 c=1 即可)
...
USE_M3_TRANSPORT = False                  # v354 ABLATION: 关闭 M3 (RQ-VAE-Recommender 无跨层 transport)
# C29: 启用 M2 intrinsic Möbius 减法 (R37 rollback: M2 + M3 联合恶化 L0 collapse, test R@10 0.0991→0.0941)
USE_M2_INTRINSIC = False                  # v354 ABLATION: 关闭 M2 (res = res - emb 简单欧氏减法)
...
USE_GEODESIC_MIDPOINT_COMMIT = False      # v354 ABLATION: 关闭 midpoint (M2 关了, midpoint 也无需启用)
...
def main():
    ...
    model = RqVae(
        ...
        hyperbolic_distance=False, # v354 ABLATION: 关闭双曲距离 (用 L2 argmin, RQ-VAE-Recommender 等价)
