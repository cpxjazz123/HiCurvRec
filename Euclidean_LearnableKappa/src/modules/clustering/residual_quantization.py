...
class ResidualQuantization(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_poincare_dist = use_poincare_dist
        ...
        if use_hyperbolic:
            from src.components.hyperbolic import PoincareDistance, PoincareExpmap

            if use_expmap:
                self.expmap_layer = PoincareExpmap(
                    curvature=hyperbolic_curvature, learnable=learnable_curvature
                )
                from src.components.hyperbolic import PoincareLogmap

                self.logmap_layer = PoincareLogmap(
                    curvature=hyperbolic_curvature, learnable=learnable_curvature
                )
            for layer_idx, layer in enumerate(self.quantization_layer_list):
                if use_poincare_dist:
                    pd = PoincareDistance(
                        curvature=hyperbolic_curvature, learnable=learnable_curvature
                    )
                    layer.distance_function = pd
                    if hasattr(layer, "quantization_strategy"):
                        layer.quantization_strategy.distance_function = pd
                    self.register_module(
                        f"hyp_dist_{layer_idx}", pd
                    )
    ...
    def model_step(
        ...
        if self.use_expmap:
            encoded_embeddings = self.expmap_layer(encoded_embeddings)
        (
            cluster_ids,
            all_residuals,
        ...
        if (
            ...
            decoder_input = quantized_embeddings
            if self.use_expmap and hasattr(self, "logmap_layer"):
                decoder_input = self.logmap_layer(quantized_embeddings)
            reconstructed_embeddings = self.decoder(decoder_input)
