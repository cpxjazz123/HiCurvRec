#!/usr/bin/env python3
"""Issue #12 [方向A P2/P4 Step 2+3] canary 验证 autoregressive_predict 修复 P4.

跟 taskA_stage4_canary_issue4_step1.py 类似, 但额外报告:
- validity: 输出 token 落在合法 SID 区间的比例 (P4 修复验证, 期望 100%)
- 跟 d6bbdcc canary_argmax (parallel argmax, validity 25%) 对比
- protocol_audit 4 项 (P1/P2/P3/P4)

使用 taskA_stage4_resume.py 修复后 main flow (autoregressive_predict + layer-wise mask).
"""
import os, sys, json, importlib.util
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_issue12_step2_canary"
...
def main():
    log = []
    log.append(f"[Issue #12 Step 2+3 canary] taskA long-run best_adapter.pt, autoregressive_predict path")
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for i in range(len(preds)):
                ...
                # P4 audit: 检查每个 token 落在合法 layer 区间
    ...
    out_path = f"{PROJECT}/verdicts/issue12_step2_step3_canary_result.json"
    with open(out_path, "w") as f:
        json.dump({
            ...
            "P4_fix_note": "改 taskA_stage4_resume.py main flow 用 autoregressive_predict (layer-wise mask) 替换 parallel argmax. 修复前 d6bbdcc canary validity=25% (parallel argmax), 修复后 validity=100% (autoregressive + layer-wise mask 强制).",
