#!/usr/bin/env python3
"""Issue #190 [方向A Gate3协议对齐] 自回归逐步 argmax + 逐层合法SID约束 canary.

vs Issue #188 (task474) 的关键改进:
1. 自回归逐步 argmax: 4 个 decoder 顺序 forward, decoder_input_ids 严格遵循
   shift-right 约定 [PAD, t0_pred, t1_pred, t2_pred] for position 3 logits
2. 逐层合法SID范围约束: argmax 在 layer range 内取值, 跨层 token 禁止
3. P2 映射核实: vocab_size=1025 vs SID token 数 449 显式映射文档

Pass/Fail 判据: canary 真实 R@10 > 0 (非零即可) + 4 项检查表全 PASS.
"""
import os
import sys
import json
...
os.environ["TRITON_CACHE_DIR"] = f"/home/wlia0047/.triton/cache_issue190_canary_autoregressive"
...
_spec = importlib.util.spec_from_file_location(
    "t470", "/home/wlia0047/ar57/wenyu/GeneRec/taskA/stage3/_archive/taskA_stage3_kappa_scale_recontinue.py"
)
...
OUT_DIR = f"{PROJECT}/taskA/stage4/taskA_stage4_canary_autoregressive"
os.makedirs(OUT_DIR, exist_ok=True)
LOG_PATH = f"{PROJECT}/logs/issue190_canary_autoregressive.log"
...
def get_layer_ranges(codebook_size):
    """Compute per-layer valid SID token ranges (closed intervals [low, high]).
    Layer i uses cumulative+1 to cumulative+codebook_size (excluding padding 0).
    """
    cum = [0]
...
def main():
    seed_all(SEED)
    log = []
    log.append(f"\n=== Issue #190 [方向A canary] 自回归 + 逐层合法SID约束 ===")
    ...
    # P2 vocab mapping analysis
    layer_ranges = get_layer_ranges(CODEBOOK_SIZE)
    expected_vocab = sum(CODEBOOK_SIZE) + 2  # +2 for PAD=0 and EOS=1
    ...
    log.append(f"  SID token ranges (closed [low, high]): {layer_ranges}")
    ...
    forward_path_used = []
    layer_pred_distribution = [{}, {}, {}, {}]  # 4 layers
    ...
    verdict = {
        "task": "issue190_canary_autoregressive",
        "issue": 190,
        "canary_n": canary_n,
        "sid_sha256": sid_sha,
