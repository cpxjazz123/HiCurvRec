#!/usr/bin/env python3
"""Issue #12 [方向A Step 4] beam search K=20 eval — 让 6 指标互不恒等.

Per-sample 4-step beam search + layer-wise mask (跟 autoregressive_predict 一致).
K=20 candidates per sample → R@5/R@10/R@20 + NDCG@5/10/20 互不恒等.
"""
import os, sys, json, math, importlib.util
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_issue12_step4_beam"
...
def beam_search_predict(model_wrapper, history_tensor, attention_mask, layer_ranges, beam_size=20):
    ...
    with torch.no_grad():
        ...
        for b_idx in range(B):
            ...
            enc_h = encoder_outputs.last_hidden_state  # (1, L, D)
            enc_mask = mask_b
            ...
            for step in range(4):
                ...
                for tokens, score in beams:
                    # Build decoder_input_ids (1, 4) with [0] + tokens + pad
                    ...
                    logits = model_wrapper.t5.model.lm_head(dec_out.last_hidden_state[:, step, :])  # (1, vocab)
...
def main():
    log = []
    log.append(f"[Issue #12 Step 4 beam search] taskA long-run best_adapter.pt, BEAM_SIZE={BEAM_SIZE}")
    ...
    test_loader = DataLoader(test_ds, batch_size=8, shuffle=False, num_workers=0, collate_fn=collate_fn)  # batch=8 减少 per-sample encoder 开销
    ...
    if n_unique > 1:
        log.append(f"  ✅ 6 指标已差异化 (R@K 随 K 增大单调非降, NDCG 因位置折损有差异)")
    else:
    ...
    out_path = f"{PROJECT}/verdicts/issue12_step4_beam_search_result.json"
    with open(out_path, "w") as f:
        json.dump({
            "issue": 12,
            "step": "Step 4 beam search K=20",
            "ckpt": CKPT_PATH,
            ...
            "note": "per-sample beam search 4-step + layer-wise mask, 修复 #12 Step 4 (6 指标互不恒等)",
