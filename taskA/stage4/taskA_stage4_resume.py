"""Task #472 Stage 4 R@K double-run resume (skip retrain, use saved ckpt).

R23: ckpt already saved at products/task472_issue179_direction_a_gate4_200ep/adapter_200ep.pt
...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_task472_resume"
...
_t470_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(_t470_mod)
WrapperCls = _t470_mod.HG_Rec_with_BoundedAdapter
get_t5_config = _t470_mod.get_t5_config
load_t5_state_dict = _t470_mod.load_t5_state_dict
...
CKPT_PATH = f"{PROJECT}/taskA/stage3/taskA_stage3_direction_a_train/adapter_200ep.pt"
DEVICE = "cuda"
D_MODEL = 128
MAX_LEN = 4
PAD_TOKEN = 0
...
def compute_r_at_k(candidates_per_sample, targets_list, k):
    """Top-K hit rate. candidates_per_sample[i] = ordered list of 4-tuple candidates for sample i
    (rank 0 = best). Returns fraction of samples whose target SID appears in top-K candidates.
    数学正确: R@K = mean(1[target ∈ top-K candidates]). Argmax 单 candidate 时 R@K ≡ R@1 恒等
    (仅 1 candidate 可用), beam search 提供 K candidates 时 R@K 单调非降.
    """
    n_correct = 0
...
def compute_ndcg_at_k(candidates_per_sample, targets_list, k):
    """NDCG@K with position discount: 1/log2(rank+1) if target in top-K, else 0.
    rank = 1-indexed position in top-K candidates. Argmax 时 rank=1 → NDCG=1 if hit else 0.
    """
    import math
    ndcgs = []
    for cands, target in zip(candidates_per_sample, targets_list):
...
def main():
    log = []
    log.append("[Task #472 Stage 4 resume] 方向A BoundedKappaScaleConditioner R@K double-run")
    ...
    log.append("[Hash Check 跟 #157/#177 一致] True")
    ...
    # Save
    out_dir = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/stage3/taskA_stage3_direction_a_train"
    ...
    log_str = "\n".join(log)
    print(log_str)
    with open("logs/task472_resume_stage4.log", "w") as f:
        f.write(log_str + "\n")
