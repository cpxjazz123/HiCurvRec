...
V74_EVAL_CONFIG = {
    # ── 路径 (硬编码, v22.b Plan A 完整版 Stage2+Stage3 实时产物 — R37 vs v15 capmatch 锚点 0.1057) ──
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v22b_full_1000ep/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v22b_full_1000ep/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v22b_full_1000ep/eval",
    ...
    # ── 任务标签 (verdict 写入用) ──
    "tag": "v22b_branch_curvature_stage4_beam20",
}
...
def main():
    # R12 + Task #142: 启动时记录 git HEAD + product_dir + _STAGE4_PID (供 R37 attribution)
    try:
        ...
        head_sha = "unknown"
    product_dir = Path(V74_EVAL_CONFIG["product_dir"])
    product_dir.mkdir(parents=True, exist_ok=True)
    pid_path = product_dir / "_STAGE4_PID"
    pid_path.write_text(f"{os.getpid()}\nhead_sha={head_sha}\n")
    print(f"[taskA-stage4-launcher] cwd={REPO}", flush=True)
    print(f"[taskA-stage4-launcher] git_HEAD={head_sha}", flush=True)
    print(f"[taskA-stage4-launcher] _STAGE4_PID={pid_path}", flush=True)
