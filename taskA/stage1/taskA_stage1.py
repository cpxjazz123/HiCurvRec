...
R_MAX = 1.0  # v15+v74 baseline: sentence-t5 mean-pool F.normalize norm=1.0 (匹配 v15 capmatch Stage 2 训练输入 norm=1.0000)
# baseline 模式: 跳过 radius 调制, 直接输出 ||eu|| (≈1.0)
R_MODE = "fixed"
...
class HyperbolicEncoder(nn.Module):
    ...
    def forward(self, texts, device):
        """texts → (v, r, d).
        v: (B, e_dim) 欧氏基础向量, norm≈1.0 (v15+v74 baseline: sentence-t5 mean-pool F.normalize, 跳过 backbone+radius)
        r: (B,) 半径 ∈ (0, R_MAX) — baseline 时 r=1.0
        d: (B, e_dim) 单位方向
        """
        eu = self.encode_text(texts, device)  # (B, d_model) no_grad
        # v15+v74 baseline: F.normalize 强制 norm=1.0 (匹配 v15 capmatch Stage 2 训练时输入 norm=1.0000, std=0.0002, unique=11/9922)
        v = F.normalize(eu, p=2, dim=-1)  # (B, d_model), norm=1.0
        d = v  # baseline 时 d=v (unit vector, norm=1.0)
        r = torch.ones(eu.shape[0], device=eu.device)  # baseline 时 r=1.0
        return v, r, d
