...
UTIL_REVIVE_EVERY = 5  # 方案 D: 10→5, REVIVE 频率 2x (更快救活死码)
UTIL_HINGE_LAMBDA = 0.0  # 禁用 util hinge loss (实验证伪)
# 方案 D 改动: τ 周期性 re-calibrate 解决 τ 永远固定导致的 entropy 衰减 → collapse
MLR_RECALIBRATE_EVERY = 10  # 方案 D v2: 50→10, 应对 ep150 突发塌缩 (50 epoch 跟不上, τ 跟不上 κ drift)
MLR_RECALIBRATE_EMA_BETA = 0.5  # EMA 平滑: τ_new = β·τ_old + (1-β)·τ_recalibrated, 防跳跃
# 方案 D 改动: REVIVE warm-up (新码字 τ 临时×boost, 给竞争窗口)
REVIVE_WARMUP_EPOCHS = 5  # 新码字 warm-up 期间 (epoch 计数)
REVIVE_TAU_BOOST = 1.5  # warm-up 期间 τ 临时 / boost (即 τ 减小, softmax 更尖利于新码)
...
class HyperbolicHyperplaneMLR(KappaAwareVectorQuantization):
    ...
    def forward(self, x, use_sk=True):
        ...
        # 方案 D 改动 3b: REVIVE warm-up — 新码字在 warm-up 期间 τ 临时 / boost (更尖, 给新码竞争窗口)
        if REVIVE_WARMUP_EPOCHS > 0 and REVIVE_TAU_BOOST != 1.0:
            warm_until = getattr(self, "_revive_until_epoch", -1)
            # epoch 信息不可直接拿, 用一个简单的全局 epoch counter 间接标记
            # (这里 _revive_until_epoch 已是 epoch 整数, forward 时无 epoch 上下文 → 只在 REVIVE 当下 τ 减小一次)
            # 简化: REVIVE 触发瞬间强制 τ 临时改写, 由 epoch loop 控制何时回落
            cur_global_epoch = getattr(self, "_current_epoch_for_warmup", None)
            if cur_global_epoch is not None and cur_global_epoch < warm_until:
                tau = tau / REVIVE_TAU_BOOST
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for q in train_mm.vq_layers:
            q.anneal_tau(epoch)
            # 方案 D 改动 3b: 注入 epoch 到 vq_layer, 让 REVIVE warm-up 能 forward 时生效
            q._current_epoch_for_warmup = epoch
        ...
        if (is_main and UTIL_REVIVE_EVERY > 0
            ...
            with torch.no_grad():
                ...
                # 方案 D 改动 3b: REVIVE 后标记 _revive_until_epoch → forward τ 临时 ×boost
                if REVIVE_WARMUP_EPOCHS > 0 and REVIVE_TAU_BOOST != 1.0:
                    warm_until = epoch + REVIVE_WARMUP_EPOCHS
                    for l_idx in set(ll for ll, _ in revive_log):
                        train_mm.vq_layers[l_idx]._revive_until_epoch = warm_until

        # 方案 D 改动 1: τ 周期性 re-calibrate (解决 τ 永远固定 → entropy 衰减 → collapse)
        # 复用 calibration 子集 + bisect 找 H_target=0.85*log(K), EMA 平滑防跳跃
        if MLR_RECALIBRATE_EVERY > 0 and is_main and epoch > 0 \
                and epoch % MLR_RECALIBRATE_EVERY == 0 and epoch < args.epochs - 1:
            with torch.no_grad():
                recalib_log = []
                for li, q in enumerate(train_mm.vq_layers):
                    K_l = q.n_e
                    ...
                    # 复用 calibration 子集
                    n = min(MLR_CALIBRATION_SUBSET_SIZE, item_emb.shape[0])
                    gen = torch.Generator(device=item_emb.device).manual_seed(
                        MLR_CALIBRATION_SUBSET_SEED + li + epoch  # 加 epoch 让 subset 变化
                    )
                    subset_idx = torch.randperm(item_emb.shape[0], generator=gen,
                                                device=item_emb.device)[:n]
                    z_e = train_mm.encoder(item_emb[subset_idx].to(item_emb.device))  # (n, e_dim)
                    c_l = q.get_c()
                    c_geom = c_l.detach() if CURV_PRIOR else c_l
                    z_ball = proj_to_ball(expmap0(z_e, c_geom), c_geom)
                    mlr_logits = q._compute_signed_score(z_ball, c_l)  # (n, K_l)
                    ...
                    def _entropy(t):
                        d = F.softmax(mlr_logits / max(t, 1e-8), dim=-1)
                        return -(d * d.clamp_min(1e-12).log()).sum(-1).mean().item()

                    lo, hi = MLR_CALIBRATION_TAU_MIN, MLR_CALIBRATION_TAU_MAX
                    H_lo, H_hi = _entropy(lo), _entropy(hi)
                    if H_lo > target_H:
                        tau_new = lo; converged = False
                    elif H_hi < target_H:
                        tau_new = hi; converged = False
                    else:
                        tau_new = (lo + hi) / 2
                        for _it in range(MLR_CALIBRATION_BISECT_MAX_ITER):
                            H_new = _entropy(tau_new)
                            if abs(H_new - target_H) < MLR_CALIBRATION_TOLERANCE:
                                break
                            if H_new > target_H:
                                hi = tau_new
                            else:
                            ...
                            tau_new = (lo + hi) / 2
                        converged = True
                    # EMA 平滑: τ_new = β·τ_old + (1-β)·τ_target
                    tau_old = getattr(q, "_mlr_tau", tau_new)
                    tau_smoothed = MLR_RECALIBRATE_EMA_BETA * tau_old + \
                                   (1.0 - MLR_RECALIBRATE_EMA_BETA) * tau_new
                    q._mlr_tau = float(tau_smoothed)
                    recalib_log.append((li, tau_old, tau_smoothed, converged))
                print(f"[MLR RECALIB Ep{epoch}] " +
                      " ".join(f"L{l}:{o:.5f}→{s:.5f}({'OK' if c else 'NC'})"
                               for l, o, s, c in recalib_log),
                      flush=True)
