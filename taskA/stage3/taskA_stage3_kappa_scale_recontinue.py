...
R18 + Issue #177 spec 强制 (vs #159 closed NO-GO α unbounded 18.75):
- α clamp: softplus(α_logit).clamp(max=0.5) 强制 α ≤ 0.5 (有界残差)
- 复用 #157 冻结 SID SHA256 (commit 206ebb5)
- 复用 Task #84 frozen T5 ckpt
- T5 主干冻结, 仅 LN + conditioner 解冻
- 10 epoch short train (Issue #177 spec 强制)

Gate 3 验收 (per Issue #177 spec):
1. α=0 → max diff=0 (init identity)
2. 仅 LN + conditioner 解冻
3. 首步后 conditioner + LN 梯度 nonzero
4. 10 epoch 短训: loss 下降, α ≤ 0.5 全程, cond_grad/ln_grad nonzero 全程
5. save/load missing=0/unexpected=0, forward diff=0
6. SID hash = #157 SHA256 (2dab29229c36a9695a11d70b61d1b80fae95a08d00e3c3675e9bf899b709508a)
"""
import os
import sys
import json
...
# ============================================================================
# Config (per Issue #177 spec, 跟 #159 区分: 强制 α ≤ 0.5)
# ============================================================================
SEED = 42
...
ALPHA_MAX = 0.5  # Issue #177 spec: α 必须有界 (vs #159 α unbounded → 18.75)
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/stage3/taskA_stage3_kappa_scale_recontinue")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/_logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task470_issue177_gate3_a_recontinue.log"
...
class BoundedKappaScaleConditioner(nn.Module):
    """Issue #177: κ + sync codebook scale 元数据 → 每层零中心有界残差.
    ...
    防止 #159 (Task #451) α 暴涨到 18.75 的反例.
    """
    ...
    def forward(self, x_emb, sid_meta, kappa_meta, scale_meta):
        kappa_e = self.kappa_embed(kappa_meta.unsqueeze(-1))
...
class HG_Rec_with_BoundedAdapter(nn.Module):
    def __init__(self, t5_config, t5_state_dict, d_model=128, n_layers=3, sid_dim=4):
        ...
        self.adapter = BoundedKappaScaleConditioner(d_model=d_model, n_layers=n_layers, sid_dim=sid_dim)
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #470 Issue #177 Gate3续] κ+scale 元数据 T5 零中心有界残差 (vs #159 α 无界)")
    log_lines.append("=" * 70)
