#!/usr/bin/env python3
"""Issue #22 [方向A precheck] 用显式 Poincare 公式替代不可定位的 #47 公式.

本 issue 单变量修复:
1. taskA BoundedKappaScaleAdapter 新增 kappa_logits = nn.Parameter((3,)) 三层独立 learnable kappa
   init logit = [0.0, 2.0, 4.0] -> softplus = [0.693, 2.127, 4.490] (per-layer 不同, 破对称性)
2. forward 用 learnable kappa 替代外部 kappa_meta (不再以 torch.zeros(B,3) 作为 kappa 来源)
3. kappa 更新后同步用显式 Poincare 公式重校准 (替代不可定位的 #47 公式):
   - 距离: d_kappa(x,y) = (2/sqrtkappa)*artanh(sqrtkappa*||(-x)(+)_kappa y||)
   - 重校准 hook: 每次 optimizer.step() 后调用 kappa_recalibrate_hook(kappa_logits.data)
     记录: 触发次数 + 旧 kappa bytes sha256 + 新 kappa bytes sha256 (codebook hash 代理)
4. 老 ckpt 用 strict=False 加载 (从 #17 taskA_stage3_issue192_long_run/best_adapter.pt 续训, epoch 48 val_R@10=0.058)
5. 严禁 from-0 重训

起点: taskA/stage3/taskA_stage3_issue192_long_run/best_adapter.pt (epoch 48 val_R@10=0.058)
短程: <=10 epoch sanity (PASS 判据: 三层 kappa 分化层间差>1e-2 + 三层 dL/dkappa 非零 + 重校准次数==kappa 更新次数 + val_R@10 >= 0.058)

输出: taskA/stage3/taskA_stage3_issue22_precheck_fix/
- adapter.pt + best_adapter.pt (R12 + early stop)
- val_trace.json (含 kappa per-layer / 重校准次数 / kappa bytes sha256)
- training_summary.json + recalibrate_log.json (重校准触发详情)
- _TRAINING_PID
- log: taskA/_logs/task_issue22_precheck_fix.log
"""
import os
import sys
import json
...
# ============================================================================
# Config (per Issue #177 spec, 跟 #159 区分: 强制 alpha <= 0.5)
# ============================================================================
SEED = 42
...
ALPHA_MAX = 0.5  # Issue #177 spec: alpha 必须有界 (vs #159 alpha unbounded -> 18.75)
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/stage3/taskA_stage3_issue22_precheck_fix")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/_logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task_issue22_precheck_fix.log"
...
# #22 spec: 短程 <=10 epoch sanity (per-layer 不同 kappa init 破对称性)
PRIOR_CKPT = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/stage3/taskA_stage3_issue192_long_run/best_adapter.pt"
NUM_EPOCHS = 10
START_EPOCH = 49
KAPPA_INIT_LOGIT_PER_LAYER = [0.0, 2.0, 4.0]  # per-layer 不同, 破对称性
...
class BoundedKappaScaleConditioner(nn.Module):
    """Issue #177: kappa + sync codebook scale 元数据 -> 每层零中心有界残差.

    关键改进 vs #159 (#451):
    - alpha = softplus(alpha_logit).clamp(max=ALPHA_MAX) 强制 alpha <= 0.5 (有界)
    - direction 仍 tanh bounded [-1, 1]
    - residual = alpha * direction, 整体有界 |residual| <= ALPHA_MAX

    防止 #159 (Task #451) alpha 暴涨到 18.75 的反例.
    """
    ...
    def __init__(self, d_model=128, n_layers=3, sid_dim=4, alpha_init_logit=-10.0, alpha_max=0.5,
        ...
        # #22 spec: 三层独立 learnable kappa (per-layer 不同 init 破对称性)
        if kappa_init_logit_per_layer is None:
            kappa_init_logit_per_layer = [0.5413] * n_layers  # default -> softplus ≈ 1.0 同步
    ...
    def get_kappa_per_layer(self):
        """三层独立 learnable kappa (per-layer softplus of kappa_logits)."""
        return F.softplus(self.kappa_logits)  # (n_layers,)

    def forward(self, x_emb, sid_meta, kappa_meta, scale_meta):
        # #22 spec: 用 learnable kappa 替代外部 kappa_meta (不再以恒零外部输入作为 kappa 来源)
        kappa_per_layer = self.get_kappa_per_layer()  # (n_layers,)
        kappa_e = self.kappa_embed(kappa_per_layer.unsqueeze(-1).unsqueeze(0).expand(x_emb.shape[0], -1, -1))
        kappa_summary = kappa_e.mean(dim=1, keepdim=True).expand(-1, x_emb.shape[1], -1)
        scale_e = self.scale_embed(scale_meta.unsqueeze(-1))
        scale_summary = scale_e.mean(dim=1, keepdim=True).expand(-1, x_emb.shape[1], -1)
        sid_e = self.sid_token_proj(sid_meta)
        combined = torch.cat([sid_e, kappa_summary, scale_summary], dim=-1)
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #470 Issue #177 Gate3续] kappa+scale 元数据 T5 零中心有界残差 (vs #159 alpha 无界)")
    log_lines.append("=" * 70)
    ...
    if sid_sha != EXPECTED_SID_SHA:
        log_lines.append("[FAIL] SID hash 跟 #157 不一致, 立即 STOP")
    ...
    # Construct wrapper (#22 spec: per-layer 不同 kappa init 破对称性)
    ...
    # Gate 3 训练: 10 epoch 短训
    log_lines = []
    log_lines.append(f"\n[Gate 3 训练] {NUM_EPOCHS} epoch, batch_size={BATCH_SIZE}, alpha_max={ALPHA_MAX}")

    # #22 spec: 加载 PRIOR_CKPT (#17 已合规 Stage3, epoch 48 val_R@10=0.058) 用 strict=False
    if os.path.exists(PRIOR_CKPT):
        ...
        log_lines.append(f"  ↳ missing keys = 新 nn.Parameter kappa_logits (#22 spec)")
        ...
        log_lines.append(f"[WARN] PRIOR_CKPT 不存在: {PRIOR_CKPT}, 用 random init")

    # 记录初始 kappa (per-layer)
    ...
    # #22 spec: kappa_recalibrate hook (替代不可定位的 #47 公式)
    # 每次 optimizer.step() 后记录 kappa bytes sha256 + 触发次数
    recalibrate_log = []
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx in range(n_batches):
            ...
            # #22 spec: kappa_recalibrate hook (替代 #47 公式)
