...
R30 重构 (2026-08-05): 顶部 CONFIG 块硬编码所有超参 (env var 读取已清除).
历史实验变体 → 复制本脚本为新文件改常量, 不复用同一脚本 + env toggle.

Issue #43 [方向A v7] 分层距离区间负样本 + 假负样本保护:
  - 在 #37 KMeans cluster 分桶基础上, 替换为基于 sentence-t5 余弦距离的分层区间采样
  - L0 (far, 跨粗簇):   距离 ∈ [P85, P100]
  - L1 (mid, 类别边界): 距离 ∈ [P40, P75]
  - L2 (near, hard 但排除 top-20 假负): 距离 ∈ [P10, P40]
  - 阈值由训练集分位数确定并固定 (禁止验证集/测试集调参)
  - 假负样本保护: 每层 top-20 近邻 (cosine dist < P05) 直接排除, 不进负样本池
  - 仅替换 #37 负样本选择器, 其余配置保持 v5 (REC_LAYER_W=1:3:9, REL_STRUCT, CURV_AWARE,
    CURV_PRIOR, 自由 κ — 非 #41 锚定)

R18 4 维度路径对比 vs Issue #37:
  D1 spec: KMeans 簇分桶 (#37) vs 距离区间 + false-neg exclusion (#43)
  D2 实施: cluster label npz (#37) vs cosine dist 分位区间 + top-K exclusion mask (#43)
  D3 Gate 失败机制: cluster 内 hard/easy 混淆 (#37 R@10=0.1044 -0.0004 vs v5) vs 距离连续分桶
       边界 + false-neg 保护更精细 (#43 预期能接近 v5 0.1048)
  D4 引用文献: 同样 hard negative mining (#37 TNNLS 2023) + 增强 false-negative exclusion
       (contrastive learning 通用做法, e.g. SimCSE / DPR)
"""
...
# Issue #43 (v6-A 分层距离区间负样本 + false-neg exclusion): 距离分位区间 npz
#  (空 = 随机负样本). 由 taskA_stage2_precompute_neg_v7.py 预生成.
REC_LAYER_NEG_DIST_FILE = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/taskA_stage2_v7_issue43/rec_layer_neg_dist.npz"

# Issue #157: reload 一致性验证频率 (每 N 步验证一次, 默认 9 = 每 epoch)
RECAL_CHECK_EVERY = 9

# Issue #39 (v6-A/B 曲率 trust region + EMA) — #43 不用 (回 v5 自由 κ)
KAPPA_EMA_BETA = 0.0
KAPPA_TRUST_REGION = 0.0
KAPPA_TRUST_REGION_LAMBDA = 1.0
KAPPA_WARMUP_EPOCHS = 0

# Issue #41 (逐层锚定有界 κ) — #43 不用 (回 v5 自由 κ)
KAPPA_ANCHORS = []  # 空 = 不启用锚定, κ 自由学习
...
DEFAULT_GPU = 0
DEFAULT_PRODUCT_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/taskA_stage2_v7_issue43"
...
class KappaAwareVectorQuantization(nn.Module):
    ...
    def __init__(self, n_e, e_dim, beta=0.25, kmeans_init=True, kmeans_iters=10, sk_eps=0.0, sk_iters=3,
        ...
        if layer_idx < len(KAPPA_ANCHORS):
            ...
            # Issue #43: KAPPA_ANCHORS=[] → 锚点 0 + range=1.0, tanh 近似 [-1,1] → 自由 κ 行为
...
def compute_rec_loss(model, item_emb_all, batch_idx, nn_idx, tau=REC_TAU, neg_n=REC_NEG_N):
    ...
    # Issue #43 占位: 在层循环内按 distance-bucket pool 重采样 (REC_LAYER_NEG_DIST_FILE 非空时)
    # 预加载距离区间池 (per-layer (n_items, POOL_SIZE) int32)
    dist_pools = None
    _issue43_loaded = False
    if REC_LAYER_NEG_DIST_FILE and os.path.isfile(REC_LAYER_NEG_DIST_FILE):
        try:
            dd = np.load(REC_LAYER_NEG_DIST_FILE)
            dist_pools = [dd['l0_neg'], dd['l1_neg'], dd['l2_neg']]
            print(f"[Issue43] load dist pool: L0 shape={dist_pools[0].shape}, "
                  f"L1={dist_pools[1].shape}, L2={dist_pools[2].shape}", flush=True)
            _issue43_loaded = True
        except Exception as _e:
            print(f"[Issue43] load dist npz failed: {_e}, fallback to uniform neg", flush=True)
            dist_pools = None
    # 预采样所有层负样本 item_id, 提前加入 union (encoder forward 需要它们)
    neg_ids_per_layer = []  # list of (B, neg_n) int64 item_ids
    for li in range(N_HIERARCHIES):
        if dist_pools is not None and li < len(dist_pools):
            pool = dist_pools[li]  # (n_items, POOL_SIZE)
            ids = np.zeros((B, neg_n), dtype=np.int64)
            for i in range(B):
                anchor_id = int(batch_idx[i])
                # 池已是 [d_lo, d_hi] ∩ {非 top-20 假负 ∩ 非自身}, 直接采样 (允许重复)
                ids[i] = np.random.choice(pool[anchor_id], neg_n, replace=True)
            neg_ids_per_layer.append(ids)
        else:
            # fallback: batch 内随机 (原始 v5 行为)
            ids_local = cols + (cols >= np.arange(B)[:, None]).astype(np.int64)
            ids = batch_idx[ids_local]
            neg_ids_per_layer.append(ids)
    # union = batch ∪ pos ∪ ∪_l neg_ids_l (一次 encoder forward, z 无 grad)
    union = np.unique(np.concatenate([batch_idx, pos_idx] + [a.flatten() for a in neg_ids_per_layer]))
    ...
    for li, q in enumerate(mm.vq_layers):
        ...
        # Issue #43 分层距离区间负样本: 把预采样的 item_id 映射回 union 位置
        if dist_pools is not None and li < len(dist_pools):
            neg_ids_l = neg_ids_per_layer[li]  # (B, neg_n) item_ids
            neg_idx_use = np.zeros((B, neg_n), dtype=np.int64)
            missing = 0
            for i in range(B):
                for j in range(neg_n):
                    pos = u_map.get(int(neg_ids_l[i, j]), -1)
                    if pos == -1:
                        missing += 1
                        pos = int(batch_pos[i].item())  # fallback to anchor self (mask via d_n scale)
                    neg_idx_use[i, j] = pos
            if missing > 0 and li == 0:
                print(f"[Issue43] L{li} missing neg in union: {missing}/{B*neg_n}")
        else:
            neg_idx_use = neg_idx
        with torch.no_grad():
        ...
        # 参考点第 0 列 = 正邻居 (union 位置 p_pos), 其余列 = 负样本.
        # Issue #43: neg_idx_use 已经是 union 位置 (item_id 已映射过), 直接拼接不再 batch_pos[].
        if dist_pools is not None and li < len(dist_pools):
            neg_idx_t = torch.as_tensor(neg_idx_use, device=device, dtype=torch.long)
            ref_idx = torch.cat([p_pos.unsqueeze(1), neg_idx_t], dim=-1)  # (B, 1+neg_n)
        else:
            ref_idx = torch.cat([p_pos.unsqueeze(1), batch_pos[neg_idx_use]], dim=-1)
