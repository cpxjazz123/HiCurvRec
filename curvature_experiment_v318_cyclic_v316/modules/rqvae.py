...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        per_layer_target_std: list = None,    # v318 NEW: per-layer 异质 target_std (R36n b+f). None = 用 anisotropy_target_std 一致
        ...
        # v318 NEW: per-layer 异质 anisotropy_target_std (R36n b+f)
        if per_layer_target_std is not None:
            assert len(per_layer_target_std) == n_layers, f"per_layer_target_std length {len(per_layer_target_std)} != n_layers {n_layers}"
            self.per_layer_target_std = [float(x) for x in per_layer_target_std]
        else:
            self.per_layer_target_std = None
        ...
        for li, layer in enumerate(self.layers):
            ...
            # v318 NEW: per-layer 异质 anisotropy_target_std (R36n b+f)
            if self.per_layer_target_std is not None:
                layer.anisotropy_target_std = self.per_layer_target_std[li]
            else:
