...
ANISOTROPY_TARGET_STD = 2.0      # v316: 目标 std (Poincaré 单位, 球面直径 ≈ 5.0). 当 PER_LAYER_TARGET_STD=None 时, 三层均用此值.
ANISOTROPY_TEMP = 1.0            # v316: 温度缩放 ((cur_std - target)/temp)²

# v318 NEW: per-layer 异质 anisotropy_target_std (R36n b+f 联合)
# 动机: cascade 3 层 RQ-VAE 各层语义粒度不同 — L0 精细 (codebook 聚类细粒度细节),
#       L1/L2 粗粒度 (codebook 聚类语义层级). 异质 target_std 让每层代码本几何与该层任务粒度匹配.
# 设计: L0=1.5 (紧致, 精细聚类) / L1=2.0 (中等, 平衡) / L2=2.5 (分散, 粗粒度语义)
# 注意: L0 偏紧可避免 R36p collapse (精细细节需要紧凑 cell), L2 偏散可捕获 coarse 语义.
# 比较: vs v316_fixed (统一 target_std=2.0) baseline.
PER_LAYER_TARGET_STD = [1.5, 2.0, 2.5]   # v318 NEW: L0/L1/L2 异质 anisotropy target_std
...
def main():
    ...
    model = RqVae(
        ...
        per_layer_target_std=PER_LAYER_TARGET_STD,  # v318 NEW: per-layer 异质 anisotropy_target_std (R36n b+f)
    ).to(device)
