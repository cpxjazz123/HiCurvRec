...
USE_CYCLIC_CURVATURE = True               # v356 ABLATION: 启用 cyclic (与 v318 一致)
C_CYCLIC_MIN = 0.3
C_CYCLIC_MAX = 1.0
C_CYCLIC_PERIOD = 50_000
GRAD_CLIP_NORM = 1.0
USE_M3_TRANSPORT = False                  # v356 ABLATION: 关闭 M3 (测 midpoint+cyclic 联合贡献, 排除 M3)
USE_M2_INTRINSIC = True                   # v356 ABLATION: 启用 M2 (midpoint 需要)
USE_GEODESIC_MIDPOINT_COMMIT = True       # v356 ABLATION: 启用 midpoint commit
...
def main():
    ...
    model = RqVae(
        ...
        hyperbolic_distance=True,  # v356 ABLATION: 启用双曲距离 # v354 ABLATION: 关闭双曲距离 (用 L2 argmin, RQ-VAE-Recommender 等价)
