#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v20 Stage 3: v15 配方 + seed=45 (multi-seed averaging 第 4 个).

v19 7-way Borda ceiling 0.1027 (vs 6-way 0.1020, +0.0007).
v20: seed=45 → 8-way Borda 期望 ≥ 0.1033.
...
CONFIG = {"cuda_visible_devices": "0,1,2,3", "nproc_per_node": 4, "master_port": 29536}
V20_CONFIG = {
    "sid_npy": "/fs04/ar57/wenyu/GeneRec/taskA/_history/taskA_stage2_v15_capmatch_1000ep/sid_output.npy",
    "product_dir": "/fs04/ar57/wenyu/GeneRec/taskA/_history/v20_seed45_stage3",
...
# v18/v19 留下的 marker 状态 (EARLY_STOP=30, LR=1e-3) 已满足 v20 需求, 不再 patch.
# v20 唯一差异: SEED 42 → 45.
PATCH_SEED_OLD = "SEED = 42"
PATCH_SEED_NEW = "SEED = 45  # v20 (2026-08-09): multi-seed averaging 第 4 个 (42, 43, 44, 45)"
...
def main():
    ...
    print(f"[v20/stage3] cmd={' '.join(cmd)}")
    try:
