#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v30_scsb_stage3_from_v18 Stage 3: v18 base + SCSB curvature soft-bias on T5 logits.

Issue #107 (2026-08-10): Stage3 SCSB - Curvature Soft-Bias
  - 完全 v18 base (HAB + DDP 4-card + bf16 + 200 epoch)
  - SCSB patch: --scsb_enabled --scsb_alpha_init=1.0 --scsb_beta=0.1
  - SCSB 在 T5 lm_head 之后插入 α·β·log_softmax(-d_P(codeword, history_centroid))
  - 不改 T5 内部任何参数 (避开 #100/#105/#106 全部失败路径)
  - Stage3 ckpt 输出到 taskA/_history/v30_scsb_stage3/
...
CONFIG = {"cuda_visible_devices": "0,1,2,3", "nproc_per_node": 4, "master_port": 29930}
V30_CONFIG = {
    "sid_npy": "/fs04/ar57/wenyu/GeneRec/taskA/_history/taskA_stage2_v15_capmatch_1000ep/sid_output.npy",
    "product_dir": "/fs04/ar57/wenyu/GeneRec/taskA/_history/v30_scsb_stage3",
...
def main():
    Path(V30_CONFIG["product_dir"]).mkdir(parents=True, exist_ok=True)
    cmd = [
    ...
    print(f"[v30_scsb_stage3_from_v18/stage3] cmd={' '.join(cmd)}")
