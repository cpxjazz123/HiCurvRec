...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum")  # R44 baseline 自包含
...
V74_CONFIG = {
    ...
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum/stage2",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue128_stage3_lambda_curriculum/stage2/hrqvae_kappa_sync.ckpt",
...
# Issue #128 (2026-08-12): Stage3 HAB lambda curriculum schedule — 避开 valid 偏置.
# 根因: Issue #125/#127 训练全程 HAB lambda 恒定强度, 训练初期 T5 backbone 未学好就被
# HAB 几何信号引导 → 学到 valid 模式 overfit → test 退化 (0.1024→0.0973→0.0987).
# 修复: 前 30 epoch lambda=0 (HAB 不贡献, T5 backbone 自由学), 30-60 epoch 线性增长到
# 0.5*lambda_max (与 baseline lambda_init 一致), 60+ 稳定. 训练期 schedule 让 backbone
# 先学 valid 模式, 后期 HAB 慢慢引入做微调. 这是 R36 曲率框架允许的几何正则项强度调度.
HAB_LAMBDA_CURRICULUM_ENABLED = True  # Issue #128: 启用 curriculum, 关闭=沿用 #127 全程自由学习
HAB_LAMBDA_CURR_WARMUP_END = 30  # epoch 0-30: lambda_raw → 0 (HAB 冻结)
HAB_LAMBDA_CURR_RAMP_END = 60  # epoch 30-60: 线性 0 → 0.5*lambda_max
HAB_LAMBDA_CURR_FINAL = HAB_LAMBDA_MAX * 0.5  # epoch 60+: 与 baseline lambda_init 对齐
...
def main():
    ...
    if HAB_ENABLED and HAB_LAMBDA_LR_RATIO > 1.0:
        ...
        # Issue #128: 当 curriculum enabled 时, lambda_raw 不进 optimizer (frozen by schedule),
        # 只让 U/V (B_geo 低秩矩阵) 在 high-lr group 学习.
        if HAB_LAMBDA_CURRICULUM_ENABLED:
            hab_lambda_params = []  # freeze: curriculum 强制设值
        else:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Issue #128: HAB lambda_raw curriculum schedule (每个 epoch 开头强制覆盖 data)
        # - epoch 0-30 (warmup): lambda_raw = 0 (HAB 不贡献, T5 backbone 自由学 valid 模式)
        # - epoch 30-60 (ramp): 线性从 0 增长到 0.5*lambda_max (慢慢引入 HAB 几何)
        # - epoch 60+ (stable): lambda_raw = 0.5*lambda_max (与 baseline 一致)
        # lambda_raw 已从 optimizer 移除, 此处直接覆盖 .data 不被 optimizer 还原
        if HAB_LAMBDA_CURRICULUM_ENABLED and HAB_ENABLED:
            hab_curr_loop = model.module.hab_module if DDP_MODE else model.hab_module
            if epoch < HAB_LAMBDA_CURR_WARMUP_END:
                _curr_target = 0.0
            elif epoch < HAB_LAMBDA_CURR_RAMP_END:
                _curr_progress = (epoch - HAB_LAMBDA_CURR_WARMUP_END) / (HAB_LAMBDA_CURR_RAMP_END - HAB_LAMBDA_CURR_WARMUP_END)
                _curr_target = _curr_progress * HAB_LAMBDA_CURR_FINAL
            else:
                _curr_target = HAB_LAMBDA_CURR_FINAL
            with torch.no_grad():
                hab_curr_loop.lambda_raw.data.fill_(_curr_target)
        t0 = time.time()
