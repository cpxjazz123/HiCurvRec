"""Issue #138 全量曲率失败地图 — 复用 Issue #137 protocol-aligned Stage4 oracle 采全量 trace.

不改变任何 Stage1-4 模型、曲率公式、kappa、SID、tokenizer、checkpoint、generation config 或训练超参.
唯一允许的生成入口: Issue #137 已验证的 Stage4 `model.generate` / evaluate 路径.

Gate 1 (Issue #138 spec): 全量 R@10 必须复现 Issue #133 的 0.0962 (tolerance 0.5%),
否则 BLOCKED_PROTOCOL_DRIFT, 停止关联分析.
...
产物:
- stage4_full_oracle_manifest.json (ckpt/SID/tokenizer/data-index SHA256)
- raw_predictions_stage4_full.parquet (24,772 sample, per-sample target_sid + top-20 preds + best_rank + first_error + prefix_match + PAD status)
- eval_test.json (Issue #133 全量 R@10=0.0962 复现 check)
"""
import os
import sys
import json
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map/_lib")  # R44 baseline 自包含
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map")  # R44 baseline 自包含
...
import sys as _sys
_sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map")  # R44 baseline 自包含
...
# Issue #138 全量曲率失败地图 — 复用 Issue #137 protocol-aligned Stage4 oracle
V74_EVAL_CONFIG = {
    # 路径 (R44 baseline 自包含: 实时 Stage3 产物 — 与 Issue #137 同 ckpt/sid, sha256 一致)
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map/stage2/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map/stage2/eval",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue138_full_curvature_failure_map/stage2/hrqvae_kappa_sync.ckpt",
    # 任务标签
    "tag": "v138_full_oracle",
}
...
def main():
    ...
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            ...
            preds = preds[:, 1:]
            preds = preds.reshape(input_ids.shape[0], BEAM_SIZE, -1)  # (B, 20, 4)
            pos_index = calculate_pos_index(preds, labels, maxk=BEAM_SIZE)  # (B, 20) bool
            for k in TOP_K:
            ...
            for s in range(input_ids.shape[0]):
                ...
                # first-error position: 第一个 token mismatch (假设 beam=0 为 top-1 candidate)
                ...
                for t in range(4):
                    if top1_pred[t] != target[t]:
                        ...
                        break
                # prefix match length: 4 个 token 中前多少位 match
                prefix_len = 0
                for t in range(4):
                    if top1_pred[t] == target[t]:
                        prefix_len += 1
                        ...
                        break
                # PAD token status: top1 pred 是否含 PAD (=0)? Issue #136 L3 全 0
                top1_has_pad = int(0 in top1_pred)
                target_has_pad = int(0 in target)
                raw_rows.append({
                    "sample_idx": bi * BATCH_SIZE + s,
                    "target_sid": target,                       # list[int] (4 tokens, with offsets)
                    ...
                    "top1_pred": top1_pred,                     # list[int] (4 tokens)
                    "top1_l0": top1_pred[0],
                    "top1_l1": top1_pred[1],
                    "top1_l2": top1_pred[2],
                    "top1_l3": top1_pred[3],
                    "best_rank_top20": best_rank,               # 1..20, 21 = miss
                    "hit_at_5": int(best_rank <= 5),
                    "hit_at_10": int(best_rank <= 10),
                    "hit_at_20": int(best_rank <= 20),
                    "first_error_position": first_err,          # 1..4, 0=all match
                    "prefix_match_length": prefix_len,          # 0..4
                    "top1_has_pad": top1_has_pad,
                    "target_has_pad": target_has_pad,
                    # 完整 top-20 preds 列表 (用于后续 per-token rank 分析)
                    "pred_top20_sids": preds[s].cpu().tolist(), # (20, 4)
                    "pos_index_top20": pi,                      # (20,) bool
                })
            if (bi + 1) % 50 == 0:
                print(f"  {bi+1}/{len(loader)} batches done ({len(raw_rows)} samples collected)", flush=True)

    # Issue #138: 写 raw predictions parquet (Issue #138 spec 产品 2)
    import pandas as pd
    ...
    # Issue #138 spec Gate 1: 全量 R@10 必须复现 Issue #133 的 0.0962, 否则 BLOCKED_PROTOCOL_DRIFT
    issue133_full_R10 = 0.0962
    r10_drift = abs(result["R@10"] - issue133_full_R10)
    result["protocol_drift_check"] = {
        "issue133_full_R10": issue133_full_R10,
        "this_run_full_R10": result["R@10"],
        "abs_diff": round(r10_drift, 4),
        "blocked": bool(r10_drift > 0.005),  # 0.5% tolerance for DDP sampling variance
    }
    ...
    print(f"=== protocol_drift_check: {result['protocol_drift_check']}")
