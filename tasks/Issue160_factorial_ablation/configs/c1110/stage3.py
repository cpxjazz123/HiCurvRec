...
sys.path.insert(0, "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
sys.path.insert(0, "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110")  # R44 baseline 自包含
...
V74_CONFIG = {
    ...
    "sid_npy": "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110/stage2/sid_output.npy",
    "product_dir": "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110/stage2",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/fs04/ar57/wenyu/GeneRec/tasks/Issue160_factorial_ablation/configs/c1110/stage2/hrqvae_kappa_sync.ckpt",
...
# ── Issue #160 全因子消融 gate (R30 硬编码, 每格副本改此值) ──
GATE_M4_INJECTION = False  # M4: Stage3 curvature-state token injection
STAGE3_SINGLE_CARD = True  # #160 并行加速: True 时跳过 self-launch 单卡运行 (R42 例外, verdict 记录)
...
def main():
    ...
    model.to(device)
    # Issue #159/#160: 曲率状态注入 (在 DDP wrap 前, phi 零初始化 → gate-zero 严格等价)
    # GATE_M4 控制 M4 (curvature-state token injection) 开关
    curv_injector = None
    if GATE_M4_INJECTION:
        ...
        if is_main:
            log("[Issue #160] M4 OFF: 无 curvature-state 注入")
    ...
    # Issue #159/#160: 曲率状态注入 — M4 on 时用 CurvatureGenRecDataset
