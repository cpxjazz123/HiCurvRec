...
# -*- coding: utf-8 -*-
"""
Issue #142: Stage2 L0 mean-only anchor — 释放 codeword 曲率分化度 (基于 Issue #141 B 臂)
...
实施核心 (Issue #140 spec):
  ...
  - mean-only anchor: L_anchor = LAMBDA_ANCHOR * (mean_b(log c_0,b) - log c0_ref)^2
    Issue #141 发现 per-codeword anchor 抑制分化 (B 臂 std=0.0023 仅用范围 1%);
    mean-only 只锚整体均值防集体塌缩, 个体 c_0,b 自由分化
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue142_l0_mean_only_anchor/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae + hyperbolic_attention_bias + poincare_*

ITEM_EMB_PARQUET = "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue142_l0_mean_only_anchor/stage1/item_emb.parquet"  # R44 baseline 自包含: Stage1 实时产物
N_ITEMS = 9922
EMB_DIM = 768
N_HIERARCHIES = 3
...
LAMBDA_ANCHOR = 1.0                  # L_anchor = λ·(mean_b(log c_0,b) − log c0_ref)², 权重固定 (Issue #142: mean-only anchor, 只锚整体均值防集体漂移, 个体自由分化)
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue142_l0_mean_only_anchor/stage2")  # R44 baseline 自包含: 产物写本任务 stage2/
...
def train_step_with_sync_recalibration(model: KappaAwareHRQVAE, batch, batch_idx, nn_idx,
    ...
    # ── Issue #142: L_anchor = LAMBDA_ANCHOR * (mean_b(log c_0,b) − log c0_ref)² (mean-only anchor) ──
    # Issue #141 揭示 per-codeword anchor 抑制分化 (B 臂 std=0.0023, 仅用范围 1%).
    # mean-only: 只锚整体均值防集体塌缩到 c_min/c_max, 个体 c_0,b 自由分化.
    anchor_loss = torch.zeros((), device=total_loss.device)
    l0_stats = None
    for q in mm.vq_layers:
        if q.l0_branch:
            cs_b = q.get_c_l0()
            mean_log_c = torch.log(cs_b).mean()
            anchor_loss = anchor_loss + LAMBDA_ANCHOR * (mean_log_c - math.log(C0_REF)).pow(2)
            l0_stats = {
...
def run_gate1(item_emb: torch.Tensor, device) -> dict:
    ...
    anchor = sum(LAMBDA_ANCHOR * (torch.log(q.get_c_l0()).mean() - math.log(C0_REF)).pow(2)
