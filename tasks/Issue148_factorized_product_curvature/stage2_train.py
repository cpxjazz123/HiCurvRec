"""Issue #148 — Stage2 训练脚本 (A vs B factorized product manifold)

用法:
  python3 stage2_train.py --arm control    # A: 单 32D 双曲 (c_l 共享)
  python3 stage2_train.py --arm treatment  # B: 16D Euc + 16D Hyp 乘积 (c_l^H 共享)
...
TASK_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue148_factorized_product_curvature"
sys.path.insert(0, os.path.join(TASK_DIR, "_lib"))

from utils import EmbDataset, poincare_distance, expmap0, proj_to_ball
from factorized_product_quantizer import FactorizedProductHRQVAE
...
def main():
    ...
    print(f"[stage2/{arm}] item_emb: {item_emb.shape}, factorized={factorized}")

    model = FactorizedProductHRQVAE(
    ...
    for epoch in range(N_EPOCHS):
        ...
        if epoch % LOG_EVERY == 0 or epoch == N_EPOCHS - 1:
            print(f"  epoch {epoch:4d} | loss={avg_loss:.4f} | c={cs} | SID3={n_unique_3digit}")
    ...
    sid = np.concatenate(all_indices, axis=0)  # (9922, 3)
    ...
    n_unique_4digit = int(np.unique(sid[:, :3] * 1, axis=0).shape[0])  # 3 digits
