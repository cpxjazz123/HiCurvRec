...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment")  # R44 baseline 自包含
...
V74_CONFIG = {
    ...
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment/stage2",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue161_response_confidence_gate/treatment/stage2/hrqvae_kappa_sync.ckpt",
...
class CurvatureStateInjector(nn.Module):
    """Issue #161: response-confidence gated 曲率注入.
    gate = sigmoid(MLP(s)), s = [|dD/dc|, margin, boundary_distance, residual_norm].
    h_token = Emb(SID_l) + alpha * g_l,i * phi_l(c_l,i).
    alpha 零初始化 → 训练起点无注入 (与 Control 严格等价, gate-zero 退化).
    """
    def __init__(self, d_model, feat_dim=4, hidden=16, alpha_init=0.0):
        super().__init__()
        ...
        self._last_gate_mean = None  # 监控

    def forward(self, curvature, response):
        # curvature: (B, L), response: (B, L, feat_dim) 每 token 响应特征
        g = torch.sigmoid(self.gate_mlp(response))  # (B, L, 1)
        self._last_gate_mean = float(g.mean())
        delta = self.alpha * g * self.proj(curvature.unsqueeze(-1))
        return delta
    ...
    def curv_forward(self, input_ids, attention_mask=None, labels=None, curvature=None, response=None):
        ...
        if curvature is not None and curvature.shape[1] == input_ids.shape[1]:
            if response is not None:
                input_embeds = input_embeds + self.curv_injector(curvature, response)
            else:
