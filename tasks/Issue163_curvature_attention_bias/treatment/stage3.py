...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment")  # R44 baseline 自包含
...
V74_CONFIG = {
    # 路径 (R44 baseline 自包含: 实时 Stage2 产物)
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment/stage2",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue163_curvature_attention_bias/treatment/stage2/hrqvae_kappa_sync.ckpt",
...
def _collate_fn(batch, pad_token=0):
    ...
    if has_s:
        # Issue #161: 展平响应特征 (B, L, feat_dim) 与曲率对齐
        flat_s = torch.stack([
            torch.tensor([elem for sublist in item['history_s'] for elem in sublist], dtype=torch.float32)
            for item in batch
        ])  # (B, L*feat_dim)
        out['history_s'] = flat_s.view(flat_s.shape[0], -1, 4)  # (B, L, 4)
    return out
...
DATA_ROOT = "/home/wlia0047/ar57/wenyu/GeneRec/dataset"

# Issue #161: M4 (Stage3 注入) — control 关 (M2+M3 基础), treatment 开 (gated)
GATE_CURV_ATTN = True  # Issue #163 curvature attention bias
GATE_M4_INJECTION = True
STAGE3_SINGLE_CARD = True  # R44 baseline 自包含: 引用共享 dataset/ 顶层 (md5 一致 HG-Rec/dataset/Instruments/)
...
class CurvatureStateInjector(nn.Module):
    """Issue #161+#162: response-confidence gate + layer curvature mixer.
    gate = sigmoid(MLP(s)), s = [|dD/dc|, margin, boundary_distance, residual_norm].
    mixer: a_t = softmax(W[q_t, c0, c1, c2] / tau) → per-layer 注入权重 (L0/L1/L2).
    ...
    def forward(self, curvature, response, layer_ids=None):
        ...
        if layer_ids is not None:
            # per-sample mixer 输入: [q_pool, c_mean] — 用 response 前 3 维 (c 相关) + 平均
            c_feat = response[:, :, :3].mean(dim=1)  # (B, 3) 平均 c/boundary/dc
            q_pool = torch.full((response.shape[0], 2), 0.5, device=response.device)  # 简化 q_pool
...
def install_curvature_state(hg_rec, curv_injector):
    """Issue #159/#161: monkey-patch HG_Rec forward/generate 注入曲率状态.
    forward/generate 接受额外 curvature (B, L) 与 response (B, L, feat_dim).
    """
    device = next(hg_rec.parameters()).device
        ...
        if curvature is not None and curvature.shape[1] == input_ids.shape[1]:
            if response is not None:
                ...
                input_embeds = input_embeds + self.curv_injector(curvature,
                                                                 torch.zeros(curvature.shape[0], curvature.shape[1], 4, device=curvature.device))
        outputs = self.model(inputs_embeds=input_embeds,
                             attention_mask=attention_mask, labels=labels)
        return outputs.loss, outputs.logits
...
class CurvatureAttentionBias(nn.Module):
    """Issue #163: curvature-compatibility attention bias.
    B_c(i,j) = -softplus(w)^T Delta_c(i,j) + beta * sim(phi(c_i), phi(c_j))
    只读取曲率状态 (TOKEN_C_LUT), 禁读标签/流行度.
    零初始化 (w=0, beta=0) → lambda=0 时 attention 与 Control 严格等价.
    """
    def __init__(self, d_model, n_layers=3, tau=1.0, lambda_init=0.1):
        super().__init__()
        # 每层曲率差权重 (Δc 是 L0/L1/L2 三维 log 曲率差)
        ...
        self.tau = tau
        self._last_bias_mean = None

    def forward(self, token_c):
        # token_c: (B, L, 3) 每 token 的 log 曲率 (L0/L1/L2)
        # Δc(i,j) = |log c_i - log c_j| (B, L, L, 3)
        L = token_c.shape[1]
        c_i = token_c.unsqueeze(2)   # (B, L, 1, 3)
        c_j = token_c.unsqueeze(1)   # (B, 1, L, 3)
        dc = (c_i - c_j).abs()       # (B, L, L, 3)
        w = torch.softplus(self.w)   # (3,) 非负
        dist_term = -torch.einsum('bijl,l->bij', dc, w)  # (B, L, L)
        # similarity: sim(phi(c_i), phi(c_j)) = 归一化内积
        p_i = torch.nn.functional.normalize(self.phi(token_c), dim=-1)  # (B, L, d)
        sim = torch.einsum('bid,bjd->bij', p_i, p_i)  # (B, L, L)
        bias = dist_term + self.beta * sim
        self._last_bias_mean = float(bias.mean())
        return bias / self.tau


def install_curvature_attention(hg_rec, attn_bias_module, token_c_lut):
    """Issue #163: monkey-patch encoder.forward 在 attention_mask 加 curvature bias.
    token_c_lut: numpy (vocab_size, 3) 每 token 的 log 曲率 (L0/L1/L2).
    """
    import types
    device = next(hg_rec.parameters()).device
    attn_bias_module = attn_bias_module.to(device)
    lut = torch.as_tensor(token_c_lut, dtype=torch.float32).to(device)
    hg_rec.add_module("curv_attn_bias", attn_bias_module)
    orig_forward = hg_rec.model.encoder.forward

    def attn_encoder_forward(self, input_ids=None, attention_mask=None, inputs_embeds=None,
                             encoder_hidden_states=None, *args, **kwargs):
        if input_ids is not None:
            token_c = lut[input_ids]  # (B, L, 3)
            bias = attn_bias_module(token_c)  # (B, L, L)
            L = input_ids.shape[1]
            # 4D attention bias (B, 1, L, L), 加到 attention_mask (4D) 上
            b4 = bias.unsqueeze(1)  # (B, 1, L, L)
            am = attention_mask
            if am is not None and am.dim() == 2:
                am = am[:, None, None, :]
            if am is not None and am.dim() == 4:
                extended = am.expand(-1, -1, L, L).float()
                new_mask = extended + b4
            else:
                new_mask = b4
            return orig_forward(self, input_ids=input_ids, attention_mask=new_mask,
                                inputs_embeds=inputs_embeds,
                                encoder_hidden_states=encoder_hidden_states, *args, **kwargs)
        return orig_forward(self, input_ids=input_ids, attention_mask=attention_mask,
                            inputs_embeds=inputs_embeds,
                            encoder_hidden_states=encoder_hidden_states, *args, **kwargs)

    hg_rec.model.encoder.forward = types.MethodType(attn_encoder_forward, hg_rec.model.encoder)
    return hg_rec
...
def train(model, train_loader, optimizer, device, epoch, scheduler=None):
    ...
    for batch in train_loader:
        ...
        response = batch["history_s"].to(device) if "history_s" in batch else None
        optimizer.zero_grad()
        ...
        with autocast_ctx:
            ...
            if curvature is not None and response is not None:
                pf_out = model(input_ids=input_ids, attention_mask=attention_mask, labels=labels,
                               curvature=curvature, response=response)
...
def main():
    ...
    model.to(device)
    # Issue #161: 曲率状态注入 (M4 gate 控制)
    curv_injector = None
    if GATE_M4_INJECTION:
        curv_injector = CurvatureStateInjector(d_model=CONFIG["d_model"])
        model = install_curvature_state(model, curv_injector)
        if is_main:
            log(f"[Issue #161] gated injector: {sum(p.numel() for p in curv_injector.parameters())} params (alpha 零初始化)")
    else:
        if is_main:
            log("[Issue #161] M4 OFF: 无曲率注入 (M2+M3 基础)")
    # Issue #163: curvature-compatibility attention bias (treatment only)
    if GATE_CURV_ATTN:
        import numpy as _np
        curv_state = _np.load(os.path.join(os.path.dirname(SID_NPY), "curvature_state.npy"))  # (9922, 3)
        sid_arr = _np.load(SID_NPY)  # (9922, 4)
        # token → 曲率 LUT (1025, 3): 每 codeword token 在 SID 中的平均 log 曲率
        token_c_lut = _np.ones((1025, 3), dtype=_np.float32)
        cnt = _np.zeros(1025)
        for item_i in range(sid_arr.shape[0]):
            for li in range(3):
                tok = sid_arr[item_i, li] + 1  # token id = code+1 (L0:1-64, L1:65-192, L2:193-448)
                if tok < 1025:
                    token_c_lut[tok] = _np.log(curv_state[item_i, li] + 1e-6)
                    cnt[tok] += 1
        # 未出现 token 用全局均值
        for t in range(1025):
            if cnt[t] == 0:
                token_c_lut[t] = _np.log(curv_state.mean(axis=0) + 1e-6)
        attn_bias = CurvatureAttentionBias(d_model=CONFIG["d_model"], n_layers=3)
        model = install_curvature_attention(model, attn_bias, token_c_lut)
        if is_main:
            log(f"[Issue #163] curvature attention bias: {sum(p.numel() for p in attn_bias.parameters())} params (w/beta 零初始化)")
