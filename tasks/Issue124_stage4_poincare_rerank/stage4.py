...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
import sys as _sys
_sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank")  # R44 baseline 自包含
...
# Issue #124 (2026-08-12): Stage4 Poincaré Re-ranking — 在 beam search 生成的 top-K 上叠加
# Poincaré 几何距离重排 (issue238 spec). R36 曲率机制变更 (post-generation rerank), R35 单 ckpt + beam=20.
from _lib.poincare_rerank import (  # noqa: E402
    load_poincare_assets, compute_geo_score_batch,
)
...
V74_EVAL_CONFIG = {
    ...
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank/stage2/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank/stage2/eval",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue124_stage4_poincare_rerank/stage2/hrqvae_kappa_sync.ckpt",
    # 任务标签
...
# Issue #124 (2026-08-12): Stage4 Poincaré Re-ranking 启用 — issue238 spec 实施
# R36 曲率机制变更: post-generation rerank, R35 单 ckpt + beam=20 (num_beams 仍 = BEAM_SIZE)
# alpha=1.0 硬编码 (R30 禁 sweep), 与 issue238 spec 一致
POINCARE_RERANK_ENABLED = True  # Issue #124 启用 Stage4 几何重排
POINCARE_RERANK_ALPHA = 1.0  # Issue #124 硬编码常量 (issue238 spec), 不调参 (R30)
POINCARE_RERANK_LAYER = 0  # 用 L0 codebook (64 entries) 算历史+候选几何距离 (issue238 spec)
...
def main():
    ...
    t0 = time.time()
    # Issue #124 Stage4 Poincaré Re-ranking: 加载 L0 codebook + c (来自 Stage2 ckpt) — 一次性
    if POINCARE_RERANK_ENABLED:
        codebook_t, c_t = load_poincare_assets(CODEWORD_STAGE2_CKPT, layer=POINCARE_RERANK_LAYER)
        print(f"[Issue #124] Poincaré rerank enabled: layer={POINCARE_RERANK_LAYER} "
              f"c={c_t.tolist()} alpha={POINCARE_RERANK_ALPHA}", flush=True)
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            input_ids = batch["history"].to(DEVICE)
            attention_mask = batch["attention_mask"].to(DEVICE)
            labels = batch["target"].to(DEVICE)
            # Issue #124: return_dict_in_generate=True + output_scores=True 拿 beam scores
            if POINCARE_RERANK_ENABLED:
                gen_out = model.generate(
                    input_ids=input_ids, attention_mask=attention_mask,
                    num_beams=BEAM_SIZE, return_dict_in_generate=True, output_scores=True)
                preds = gen_out.sequences[:, 1:]
                preds = preds.reshape(input_ids.shape[0], BEAM_SIZE, -1)
                # sequences_scores: (B, BEAM_SIZE) beam log prob
                beam_scores = gen_out.sequences_scores.reshape(input_ids.shape[0], BEAM_SIZE)
                # 算 Poincaré geometry score (issue238 spec): R_geo = -d_P(history_emb, cand_emb)
                geo_scores = compute_geo_score_batch(
                    history_input_ids=input_ids, candidate_sid_seqs=preds,
                    codebook_t=codebook_t, c_t=c_t, device=DEVICE)  # (B, BEAM)
                # 重排: final_score = beam_log_prob + alpha * R_geo
                final_scores = beam_scores + POINCARE_RERANK_ALPHA * geo_scores
                sort_idx = final_scores.argsort(dim=-1, descending=True)  # (B, BEAM)
                preds = preds.gather(1, sort_idx.unsqueeze(-1).expand(-1, -1, preds.shape[-1]))
            else:
