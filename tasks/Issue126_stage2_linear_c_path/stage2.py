...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue126_stage2_linear_c_path/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae + hyperbolic_attention_bias + poincare_*

ITEM_EMB_PARQUET = "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue126_stage2_linear_c_path/stage1/item_emb.parquet"  # R44 baseline 自包含: Stage1 实时产物
N_ITEMS = 9922
EMB_DIM = 768
N_HIERARCHIES = 3
...
CURV_AWARE = os.environ.get("TASKA_STAGE2_CURV_AWARE", "1") == "1"  # Issue126 default True: 拆分 κ optimizer 避免几何冲击
...
CURV_PRIOR = os.environ.get("TASKA_STAGE2_CURV_PRIOR", "0") == "1"  # Issue126 强制 0 (走 c=1+κ+1e-3, 非 exp 路径)
...
REL_STRUCT = os.environ.get("TASKA_STAGE2_REL_STRUCT", "1") == "1"  # Issue126 default True: 启用结构损失让 κ 学到非零
...
PRODUCT_DIR = Path(os.environ.get("BASELINE_STAGE2_PRODUCT_DIR",
                                  "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue126_stage2_linear_c_path/stage2"))  # R44 baseline 自包含: 产物写 baseline/stage2/
...
class KappaAwareVectorQuantization(nn.Module):
    ...
    def get_c(self) -> torch.Tensor:
        """Issue #126 (2026-08-12): c=1+κ+1e-3 路径 (替代 c=exp(κ)).
        设计动机: Issue #125 per-head κ_h 学到 [0.7, 1.1] + c=exp(κ) 让 c 范围 [2.0, 3.0] 过强,
        per-head bias scale 达 -0.5, 触发 valid/test 偏置 + R37 退化.
        改用 c=1+κ+1e-3 (加法参数化): κ ∈ [-0.5, 0.5] → c ∈ [0.501, 1.501] 更可控, 无 exp 指数漂移.
        κ clamp 范围扩大到 [-0.5, 0.5] (比 Issue #125 默认 [-0.1, 0.5] 更宽松, 让负曲率路径可走).
        CURV_PRIOR env 强制忽略 — Issue #126 明确走加法路径 (R36 曲率机制变更)."""
        if self.fix_c:
            return torch.tensor(1.0, dtype=torch.float32, device=self.kappa.device)
        # Issue126: 强制 c=1+κ+1e-3 (不论 CURV_PRIOR env)
        kappa_clamped = self.kappa.clamp(min=-0.5, max=0.5)
