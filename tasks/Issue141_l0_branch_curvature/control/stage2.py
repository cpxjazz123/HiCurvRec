...
# -*- coding: utf-8 -*-
"""
Issue #141 [control/A] Stage2 L0 共享曲率 (A/B 对照 control 臂)
...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue141_l0_branch_curvature/control/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae + hyperbolic_attention_bias + poincare_*

ITEM_EMB_PARQUET = "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue141_l0_branch_curvature/control/stage1/item_emb.parquet"  # R44 baseline 自包含: Stage1 实时产物
N_ITEMS = 9922
EMB_DIM = 768
N_HIERARCHIES = 3
...
# ══════════════════════════════════════════════════════════════
# Issue #141: L0 共享曲率 (control/A 臂) — c_0 = c_min + (c_max-c_min)·sigmoid(θ_0)
# 与 treatment/B 臂 (codeword-specific c_0,b, 64 θ) 唯一差异: θ 数量 1 vs 64.
# 距离/归一化/anchor/训练配置/seed 与 B 完全一致 (单变量 A/B 对照).
# ══════════════════════════════════════════════════════════════
L0_BRANCH_CURVATURE = True          # L0 曲率机制启用 (共享曲率路径)
L0_SHARED_CURVATURE = True          # Issue #141 control: 共享单一曲率 (A 臂); treatment 用 False (64 codeword)
C_MIN = 0.5                          # c_0 下界 (固定写入配置, 训练中禁止修改)
C_MAX = 1.5                          # c_0 上界 (固定写入配置, 训练中禁止修改)
C0_REF = 1.0140550136566162          # L0 effective curvature 初始化锚点 (c_0 init = c0_ref)
LAMBDA_ANCHOR = 1.0                  # L_anchor = λ·mean_b(log(c_0)-log(c0_ref))², 权重固定 (防无界漂移)
D_NORM_EPS = 1e-8                    # d_norm 分母 eps
GATE1_DIST_TOL = 1e-5                # Gate 1 check: 距离逐元素容差
GATE1_SCALE_TOL = 0.05               # Gate 1 check: d_norm 尺度容差
BOUNDARY_HIT_EPS = 1e-4              # 边界命中判定: c_0 距 c_min/c_max < eps
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue141_l0_branch_curvature/control/stage2")  # R44 baseline 自包含: 产物写本任务 stage2/
...
class KappaAwareVectorQuantization(nn.Module):
    ...
    def __init__(self, n_e, e_dim, beta=0.25, kmeans_init=True, kmeans_iters=10, sk_eps=0.0, sk_iters=3,
        ...
        # Issue #140/#141: L0 曲率机制 (实例级开关, Gate 1 新旧对比用)
        ...
        if self.l0_branch:
            ...
            # Issue #141 control (A): 共享单一曲率 — 1 个 θ_0; treatment (B): 64 个 θ_b.
            # 两者 init 均使 c = C0_REF (起点等价, Gate 1 check 1 前提).
            if L0_SHARED_CURVATURE:
    ...
    def get_c_l0(self) -> torch.Tensor:
        """Issue #141: c_0 = C_MIN + (C_MAX-C_MIN)·sigmoid(θ_0) ∈ (C_MIN, C_MAX).
        control (A): θ_0 标量 → 广播到 (64,) — 所有 codeword 共享同一曲率;
        treatment (B): θ_b 64 个 → per-codeword. 返回形状统一 (K,) 供下游复用."""
        if not self.l0_branch:
            raise ValueError("get_c_l0 仅 L0 branch curvature 模式可用")
        c = C_MIN + (C_MAX - C_MIN) * torch.sigmoid(self.theta)
        if L0_SHARED_CURVATURE:
            return c.expand(self.n_e)  # (K,) 全同值 (共享曲率)
        return c
        ...
        if q.l0_branch:
            if q.theta.grad is None:
                ...
                if torch.isnan(g).any() or torch.isinf(g).any():
                    raise ValueError("Issue #141: theta grad NaN/Inf")
                raw_grad_theta = g.detach().cpu().reshape(-1).tolist()  # shared 标量 → [v]; per-codeword → 64 个
...
def run_gate1(item_emb: torch.Tensor, device) -> dict:
    ...
    # strict=False 允许新增 theta (未在 ckpt), 验证 theta 保持 THETA_INIT (shared: 标量; per-codeword: (64,))
    theta_check = new_model.vq_layers[0].theta.cpu()
    assert torch.allclose(theta_check, torch.full_like(theta_check, THETA_INIT)), "theta init broken"
    ...
    # ── check 2: 共享曲率下扰动唯一的 θ_0 → 所有 64 列距离同步变化 (A 臂语义) ──
    # (per-codeword 单列扰动隔离由 ab_initial_equivalence.py check5 在 B 臂验证)
    torch.manual_seed(2024)
    ...
    with torch.no_grad():
        pert_model.vq_layers[0].theta += 0.5  # 共享 θ_0 扰动
    ...
    all_cols_changed = len(changed_cols) == 64  # 共享曲率 → 所有列都变
    report["checks"]["check2_single_branch"] = {
        "shared_curvature": True,
        "perturbed_theta": "theta_0 (shared)",
        "changed_cols_count": len(changed_cols),
        "all_64_cols_changed": all_cols_changed,
    }
    c2_ok = all_cols_changed
    ...
    # spec: "所有曲率参数进入 optimizer, 梯度 finite" (非零不要求 — 未选中 codeword 的 θ 梯度为 0 是合法机制)
    n_nonzero = int((g.abs() > 0).sum().item()) if g is not None else 0
    n_theta = int(probe.vq_layers[0].theta.numel())
    report["checks"]["check3_theta_optimizer"] = {
        "n_theta": n_theta,
        "shared_curvature": L0_SHARED_CURVATURE,
...
def main():
    ...
    if is_main:
        ...
        # Issue #141 control (A): 共享曲率下 c_0 全列同值 (std=0) 是设计语义, 不算尺度捷径
        collapse_to_same = c_std_final < 1e-4
        collapse_to_boundary = n_boundary_final >= CODEBOOK_SIZES[0] * 0.5  # ≥32 个贴边
        # 尺度捷径: 全贴同一边界才判 shortcut (共享曲率 std=0 是 A 臂定义)
        scale_shortcut = bool(collapse_to_boundary)
        ...
        scale_audit = {
            ...
            "collapse_to_same_is_by_design": L0_SHARED_CURVATURE,  # control 共享曲率 std=0 是设计语义
