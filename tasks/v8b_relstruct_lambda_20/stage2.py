...
ITEM_EMB_NPY = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v8b_relstruct_lambda_20/item_emb_v15repro_u32.npy"  # Stage 1 实时产出 (v8 同 v7 输入分布 norm≈1.0000 std=0.0002)
...
REL_STRUCT = True
REL_STRUCT_TARGET = 0.3  # legacy 固定值 (v15 训练时实际 target, 实测 v15 epoch 100 κ=[0.33,1.94,1.83])
# Issue #228 v15 复现 (2026-08-09): 强制 DELTA=0 让 A=0 走 legacy 0.3 target.
#   之前 per-layer 反解 target=[0.24,0.42,0.55] 把 κ 拉低 (sqrt(c)*r < target 时 κ 应该涨但反向),
#   导致 epoch 100 κ 只能涨到 [0.14, 0.54, 0.55] 而非 v15 的 [0.33, 1.94, 1.83].
REL_STRUCT_DELTA = 0.0  # 强制走 legacy (line 696 if A<=0: return REL_STRUCT_TARGET)
# Issue #228 v7 修复 (2026-08-09): REL_STRUCT_LAMBDA 1.0 → 10.0 放大 10x 强制驱动 κ.
#   v6 调试发现: rec loss 较低 (~31 vs v15 ~38) → r_struct 小 (~0.17 vs v15 ~0.39) →
#   struct_term = √c·r - 0.3 在 c=[1.2, 1.3, 1.2] 时太小 → κ 不涨反衰减 (峰值 epoch 50 仅
#   [0.19, 0.71, 0.83] vs v15 [0.36, 2.00, 2.02]). 放大 λ 10x 让 struct_term² 梯度贡献
#   与 REC_LOSS 同量级 (~30), 让 κ 真正学到 v15 的 [0.30, 1.79, 1.48] 目标.
REL_STRUCT_LAMBDA = 20.0  # Issue #228 v8 (2026-08-09): 10.0 → 30.0 放大 3x, 强制 κ 推到 v15 水平 [0.30, 1.79, 1.48]
...
# Issue #228 v15 复现 (2026-08-09): 走 v15 真实训练路径 (commit 4460419 Issue #76 v10/v11).
#   v15 训练时 REL_STRUCT_LAMBDA=1.0 (非 LAMBDA_BALL=200),struct=√c·r 切空间量 (非 tanh ρ_ball),
#   target 默认 0.3 走 legacy (因 KAPPA_ANCHORS=[] 时 δ≤0). 实测 v15 epoch 100 κ=[0.331,1.944,1.828].
# 当前 Issue #76 重整用了 REL_STRUCT_ON_BALL=True + LAMBDA_BALL=200 + RHO_BALL_TARGET=[0.5,0.62,0.72],
#   导致 κ 涨速 5x (epoch 100 达 [2.03,3.75,4.00]), util_4digit 崩 0.001.
REL_STRUCT_ON_BALL = False  # v15 旧路径 (√c·r 切空间量)
RHO_BALL_TARGET = [0.50, 0.62, 0.72]  # 占位, 不生效 (REL_STRUCT_ON_BALL=False)
REL_STRUCT_LAMBDA_BALL = 1.0  # v15 旧值 (非 200)
REL_STRUCT_TARGET = 0.3  # v15 旧值 (legacy 固定 target)

# Issue #228 κ 路径修复 #2 (2026-08-09): KAPPA_ANCHORS=[0,0,0] + RANGE=2.0 用 anchor+tanh 形式
#   (具体值在下方 line 230 重新声明覆盖 line 178; 注释说明设计意图).
# 完整定义见 line 230 区域
...
# Issue #228 v15 复现 (2026-08-09): KAPPA_ANCHORS=[0,0,0] + KAPPA_ANCHOR_RANGE=2.0
#   走 anchor+tanh 路径 (KAPPA_ANCHORS 非空), init drift=0 → κ_eff=anchor+0=0 → c=1.0
#   上一轮 KAPPA_ANCHORS=[] 走 sigmoid 路径 → init κ_eff=-1+7·0.5=2.5 → c=12.18 (12x 偏差, 致 κ 涨过头).
#   现在 KAPPA_ANCHORS=[0,0,0] + RANGE=2.0 严格匹配 v15 epoch 0 init κ=[0.0007,0.001,0.001]≈0
#   且 anchor+tanh 形式梯度不被 sigmoid 压缩 (v15 epoch 50 κ=[0.357,1.995,2.017] 应能自然涨到).
KAPPA_ANCHORS = [0.0, 0.0, 0.0]  # v15 init κ=0 (匹配 epoch 0); 非 [] 走 anchor+tanh 路径
KAPPA_ANCHOR_RANGE = 2.0  # ±2 浮动覆盖 v15 epoch 100 κ=[0.33,1.94,1.83] 全范围
...
DEFAULT_GPU = 0
DEFAULT_PRODUCT_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v8b_relstruct_lambda_20"
...
class KappaAwareVectorQuantization(nn.Module):
    ...
    def __init__(self, n_e, e_dim, beta=0.25, kmeans_init=True, kmeans_iters=10, sk_eps=0.0, sk_iters=3,
        ...
        # Issue #228 κ_init 修复 (2026-08-09): KAPPA_ANCHORS=[0,0,0] 走 anchor+tanh 路径,
        #   init drift=0 → κ = anchor + tanh(0)·range = 0 + 0 = 0 ✓ 匹配 v15 epoch 0.
        #   之前的 sigmoid 路径 (KAPPA_ANCHORS=[]) drift init=-1.7918 已废 (改 anchor 路径).
