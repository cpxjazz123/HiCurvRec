...
OUTPUT_PARQUET = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/_data/Instruments/item_emb_v15repro_norm1p0.parquet")
OUTPUT_NPY = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v8b_relstruct_lambda_20/item_emb_v15repro_u32.npy")
TAG = "v15repro_norm1p0"
E_DIM = 768  # Issue #71 v82 (2026-08-07): 必须与 Stage2 EMB_DIM 对齐 (768), 不然 np.load shape mismatch
# R_MODE=fixed + R_MAX=1.0 输出 norm≈1.0,匹配 v15 capmatch Stage 2 训练输入 norm=1.0000 std=0.0002 unique=11/9922
R_MAX = 1.0  # 允许 R_MAX=1.0 (Stage 2 expmap0 用 arctanh 限制, 见 norm guard)
R_MODE = "fixed"  # norm=1.0 + 1e-4 扰动制造 std=0.0002 unique=11 分布
...
class HyperbolicEncoder(nn.Module):
    ...
    def forward(self, texts, device):
        """texts → (v, r, d).
        v: (B, e_dim) 欧氏基础向量, ||v||=r_i (per-item, ≈1.0 std=0.0002)
        r: (B,) 半径 ∈ (0, R_MAX) — fixed mode: r_i = 1.0 + 1e-4 * randn (std 0.0002)
        d: (B, e_dim) 单位方向
        """
        eu = self.encode_text(texts, device)  # (B, d_model) no_grad
        d = F.normalize(eu, p=2, dim=-1)  # unit vector direction
        # 加 1e-4 noise 到 radius (不破坏 direction), norm = 1.0 + 1e-4 * randn ≈ 1.0 std=0.0002
        # 模拟 v15 capmatch Stage 2 训练输入 norm=1.0000 std=0.0002 unique=11/9922
        r = 1.0 + 2e-4 * torch.randn(eu.shape[0], device=eu.device)  # std=2e-4 (放大一点让 unique>11)
        v = r.unsqueeze(-1) * d  # ||v||=r ≈ 1.0 std=0.0002
        return v, r, d
...
def main():
    ...
    if norms.max() > 1.0 + 1e-3:
        raise ValueError(f"v max_norm={norms.max():.4f} > 1.001, R_MAX={R_MAX} 过大")  # v15 训练输入 max=1.0005,放宽容差
    if all_r.max() >= R_MAX + 1e-3:
    ...
    # Issue #228 v8 (2026-08-09): 同时写 u32 npy 到 v8 目录, Stage 2 直接读 npy (避免 parquet→npy 中间转换)
    OUTPUT_NPY.parent.mkdir(parents=True, exist_ok=True)
    np.save(OUTPUT_NPY, all_v.astype(np.float32))
    log(f"[stage1-hyp] saved {OUTPUT_NPY} (u32 npy, shape {all_v.shape})")
