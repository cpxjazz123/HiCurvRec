...
class PrefixConditionedHRQVAE(nn.Module):
    def __init__(self, in_dim=768, num_emb_list=(64, 128, 256), e_dim=32,
        ...
        for i, n_e in enumerate(num_emb_list):
            ...
            # Issue #158: router 输入含上层曲率信号 (1 维) → L1: e_dim+1, L2: 2*e_dim+1
            if i == 1:
                in_dim_router = e_dim + 1  # L0 codeword + c_sig
            elif i == 2:
                in_dim_router = 2 * e_dim + 1  # L0 + L1 codeword + c_sig
            else:
    ...
    def _rq_forward(self, x, use_sk=True, intrinsic_subtraction=False, transport=True):
        ...
        - transport (Issue #158): 跨层曲率传递 —
            (a) c_{l+1} 的 router 输入拼接上一层有效曲率信号 (log c_l 归一化);
            (b) 层间几何显式转换: u_l = Log_0^{c_l}(h_l); h_{l+1} = Exp_0^{c_{l+1}}(u_l),
                使下层 residual 处于本层曲率空间 (曲率信息沿 L0→L1→L2 连续传递).
        transport=False 时无跨层曲率信号 (等 gate zero).
        """
