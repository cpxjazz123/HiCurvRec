...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
import sys as _sys
_sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval")  # R44 baseline 自包含
...
V74_EVAL_CONFIG = {
    ...
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage2/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage2/eval",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage2/hrqvae_kappa_sync.ckpt",
    # 任务标签
...
def main():
    ...
    if HAB_ENABLED:
        ...
        print(f"[Issue #125] per-head curvature enabled for eval: "
              ...
              flush=True)
        # Issue #130 (2026-08-12): 评估端 ablation — 强制 lambda_h_raw=0 让 per-head bias 失效.
        # 验证 #129 ckpt 在 per-head 缩放失效下的 test_R10, 揭示 per-head 学到的信号是否实际伤害 test.
        # κ_h 仍加载 ckpt 值 (per-head curvature 几何保留), 但 lambda 缩放 → 0.
        # 这是 Stage4 评估端变更, Stage1/2/3 沿用 Issue129 实时产物 (R40 豁免).
        with torch.no_grad():
            hab_module.lambda_h_raw.data.fill_(0.0)
            print(f"[Issue #130] per-head lambda_h_raw frozen to 0 for eval ablation "
                  f"(κ_h_ckpt loaded but bias scale=0)", flush=True)
