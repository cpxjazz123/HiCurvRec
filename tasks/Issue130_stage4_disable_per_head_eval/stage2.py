...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae + hyperbolic_attention_bias + poincare_*

ITEM_EMB_PARQUET = "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage1/item_emb.parquet"  # R44 baseline 自包含: Stage1 实时产物
N_ITEMS = 9922
EMB_DIM = 768
N_HIERARCHIES = 3
...
PRODUCT_DIR = Path(os.environ.get("BASELINE_STAGE2_PRODUCT_DIR",
                                  "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue130_stage4_disable_per_head_eval/stage2"))  # R44 baseline 自包含: 产物写 baseline/stage2/
...
class KappaAwareVectorQuantization(nn.Module):
    ...
    def get_c(self) -> torch.Tensor:
        ...
        if CURV_PRIOR:
            # Issue #129: c=exp(tanh(κ)) 自然约束曲率范围, 替代 #125/#127/#128 的 c=exp(κ)
            return torch.exp(torch.tanh(self.kappa))
