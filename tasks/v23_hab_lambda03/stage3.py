#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v23 Stage 3: v15 配方 + HAB lambda_max=0.3 (vs v15/v18-v22 λ=0.2) → 曲率机制 diversity.

R36: 不同 HAB lambda_max = 曲率机制变化 (vs 调参 sweep).
v23 期望: 与 v15/v18-v22 不同 optimizer trajectory + 不同 B_geo 强度 → 8-way Borda 加 diversity.
...
CONFIG = {"cuda_visible_devices": "0,1,2,3", "nproc_per_node": 4, "master_port": 29539}
V23_CONFIG = {
    "sid_npy": "/fs04/ar57/wenyu/GeneRec/taskA/_history/taskA_stage2_v15_capmatch_1000ep/sid_output.npy",
    "product_dir": "/fs04/ar57/wenyu/GeneRec/taskA/_history/v23_hab_lambda03_stage3",
    ...
    "hab_lambda_max": "0.3",  # 关键: 比 v15/v18-v22 0.2 强 50%
    ...
    "tag": "v23_hab_lambda03",
}

# 假设 v22 训练后 SEED 留 state 是 SEED=42 (v22 改回 SEED=42)
PATCH_SEED_OLD = "SEED = 42  # v22 (2026-08-09): 回 seed=42 (与 v15 同), 重点是 WD 0.01→0.005"
PATCH_SEED_NEW = "SEED = 42  # v23 (2026-08-09): 维持 seed=42, 重点是 HAB λ 0.2→0.3 曲率机制变化"
...
def main():
    ...
    print(f"[v23/stage3] cmd={' '.join(cmd)}")
    try:
