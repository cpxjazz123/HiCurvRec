...
def item2code(code_path, codebook_size):
    ...
    for index, code in enumerate(data):
        # C33 patch: HG-Rec 原版 = c + sum(codebook_size[0:i]) + 1
        # 但我们 C28 只有 3 层, 第 4 位是占位 PAD (HG-Rec 第 4 层 vocab=1 [只有 0])
        # 直接保留原值 (0), 不加 offset, 避免越界 vocab_size=769
        offsets = []
        for i, c in enumerate(code):
            if i >= len(codebook_size) or codebook_size[i] == 1:
                offsets.append(int(c))  # 第 4 层 PAD=0, 保持 0
            else:
