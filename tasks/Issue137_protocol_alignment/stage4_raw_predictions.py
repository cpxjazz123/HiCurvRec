...
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment/_lib")  # R44 baseline 自包含: HG_Rec/dataset/dataloader/utils/fsq_quantizer/hrqvae
sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment")  # R44 baseline 自包含: from _lib import hyperbolic_attention_bias
...
import sys as _sys
_sys.path.insert(0, "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment")  # R44 baseline 自包含
...
V74_EVAL_CONFIG = {
    ...
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment/stage2/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment/stage2/sid_output.npy",
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment/stage2/eval",
    "expected_sid_sha": "",
    ...
    "hab_stage2_ckpt": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue137_protocol_alignment/stage2/hrqvae_kappa_sync.ckpt",
    # 任务标签
...
def main():
    ...
    # Issue #137: raw predictions 收集 (per-sample preds/labels/pos_index) — 限 5000 sample 与 canary 对齐
    raw_predictions = []
    CANARY_MAX = 5000
    t0 = time.time()
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            ...
            # Issue #137: 收集 raw (只到 CANARY_MAX, 限前 5000 sample)
            cur_n = len(raw_predictions)  # bug fix: 之前用 len * batch_size 导致只有 1 batch 进
            if cur_n < CANARY_MAX:
                rem = CANARY_MAX - cur_n
                take = min(input_ids.shape[0], rem)
                batch_preds = preds[:take].cpu().tolist()
                batch_labels = labels[:take].cpu().tolist()
                batch_pos = pos_index[:take].cpu().tolist()
                for s in range(take):
                    raw_predictions.append({
                        "sample_idx": cur_n + s,
                        "pred_top20_sids": batch_preds[s],   # (20, 4)
                        "target_sid": batch_labels[s],         # (4,)
                        "pos_index_top20": batch_pos[s],       # (20,) bool
                    })
            if (bi + 1) % 50 == 0:
                print(f"  {bi+1}/{len(loader)} batches done", flush=True)

    # Issue #137: 写 raw predictions json (与 Issue #136 canary 对齐, 便于 alignment 对比)
    raw_json_path = PRODUCT_DIR / "raw_predictions_stage4.json"
    with open(raw_json_path, "w") as f:
        json.dump({
            ...
            "samples": raw_predictions,
        }, f, indent=2)
    print(f"[Issue #137] raw predictions -> {raw_json_path} ({len(raw_predictions)} samples, "
          f"{raw_json_path.stat().st_size/1024:.1f}KB)", flush=True)
