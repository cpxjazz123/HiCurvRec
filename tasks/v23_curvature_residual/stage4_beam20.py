#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v23 Stage 4 launcher — Stage2 branch curvature → Stage4 L1 token residual injection.

R30 严格合规:
  - 下方 V23_EVAL_CONFIG 字典 = 全部 v23 评估超参 + 路径, Python 顶部常量
  - 不暴露任何 argparse 接口
...
v23 评估硬编码 (与 Stage3 训练一致):
  - 加载 v23 Stage3 ckpt (本任务: tasks/v23_curvature_residual/HG_Rec_best.pth)
  - 复用 v22.b Stage2 SID (sha=332c948cce32...) + HAB Stage2 ckpt
  - v23 curvature residual: α_1 + f_1 从 ckpt load 训练末值 (train/eval 一致)
...
V23_EVAL_CONFIG = {
    # ── 路径 (硬编码, v23 Stage3 实时产物) ──
    "ckpt_path": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/v23_curvature_residual/HG_Rec_best.pth",
    "sid_npy": "/home/wlia0047/ar57/wenyu/GeneRec/taskA/_history/v22b_full_1000ep/sid_output.npy",  # 复用 v22.b SID
    "product_dir": "/home/wlia0047/ar57/wenyu/GeneRec/tasks/v23_curvature_residual/eval",
    ...
    # ── 任务标签 (verdict 写入用) ──
    "tag": "v23_curvature_residual_stage4_beam20",
}


def build_cmd() -> list:
    """构建 Stage4 评估命令 — 全部来自顶部 V23_EVAL_CONFIG 硬编码常量."""
    cmd = [PYTHON, "-u", "common/stage4/stage4_eval_pure_t5_v85p_4layer.py"]
    for k, v in V23_EVAL_CONFIG.items():
...
def main():
    ...
    try:
        ...
        head_sha = "unknown"
    product_dir = Path(V23_EVAL_CONFIG["product_dir"])
    product_dir.mkdir(parents=True, exist_ok=True)
    pid_path = product_dir / "_STAGE4_PID"
    pid_path.write_text(f"{os.getpid()}\nhead_sha={head_sha}\n")
    print(f"[v23-stage4-launcher] cwd={REPO}", flush=True)
    print(f"[v23-stage4-launcher] git_HEAD={head_sha}", flush=True)
    print(f"[v23-stage4-launcher] _STAGE4_PID={pid_path}", flush=True)
    print(f"[v23-stage4-launcher] ckpt={V23_EVAL_CONFIG['ckpt_path']}", flush=True)
    print(f"[v23-stage4-launcher] sid={V23_EVAL_CONFIG['sid_npy']}", flush=True)
    cmd = build_cmd()
    print(f"[v23-stage4-launcher] cmd={' '.join(cmd)}", flush=True)
