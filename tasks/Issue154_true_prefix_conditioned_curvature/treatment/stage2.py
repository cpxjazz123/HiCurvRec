...
def main():
    ...
    if rank == 0:
        ...
        print(f"[stage2/B] world_size={world} rank={rank} device={device} prefix_routing=True "
              f"(L1/L2 prefix-conditioned, #154 修复路由器) epochs={N_EPOCHS} global_batch={BATCH_SIZE}")
    ...
    if rank == 0:
        ...
        # ── Issue #154 Gate 2: 机制激活统计 (全量 9922, per-item c/delta 分布) ──
        gate2 = {"L0": None, "L1": None, "L2": None}
        with torch.no_grad():
            for li, q in enumerate(raw.vq_layers):
                if not q.prefix_routing:
                    gate2[f"L{li}"] = {"mode": "global", "c_global": q.get_c_global().item()}
                    continue
                # 全量 prefix embedding: 用训练完成的 encoder + L0 码本逐层生成
                z = raw.encoder(item_emb)
                prefix_emb = None
                # 用已保存 SID 的 L0/L1 前缀取 codeword
                prefix_codes = []
                for pl in range(li):
                    cb = raw.vq_layers[pl].embeddings.weight
                    pc = cb[sid_3digit[:, pl]]
                    prefix_codes.append(pc)
                if len(prefix_codes) == 1:
                    prefix_emb = prefix_codes[0]
                elif len(prefix_codes) > 1:
                    prefix_emb = torch.cat(prefix_codes, dim=-1)
                delta_all = q.router(prefix_emb)  # (9922,)
                theta_eff = q.theta + delta_all
                c_all = C_MIN + (C_MAX - C_MIN) * torch.sigmoid(theta_eff)
                d = delta_all.detach().cpu().numpy()
                c = c_all.detach().cpu().numpy()
                # 按前缀分组: L1 按 L0 前缀, L2 按 (L0,L1) 前缀
                if li == 1:
                    keys = sid_3digit[:, 0]
                else:
                    keys = sid_3digit[:, 0] * 1000 + sid_3digit[:, 1]
                groups = {}
                for k, cv in zip(keys, c):
                    groups.setdefault(int(k), []).append(float(cv))
                gmeans = {k: float(np.mean(v)) for k, v in groups.items()}
                gm = list(gmeans.values())
                gate2[f"L{li}"] = {
                    "mode": "prefix_routing",
                    ...
                    "delta_p50": float(np.median(d)),
                    "delta_p95": float(np.percentile(d, 95)),
                    "delta_max": float(np.max(d)),
                    "c_effective_mean": float(np.mean(c)),
                    "c_effective_std": float(np.std(c)),
                    "c_effective_min": float(np.min(c)),
                    "c_effective_p05": float(np.percentile(c, 5)),
                    "c_effective_p50": float(np.median(c)),
                    "c_effective_p95": float(np.percentile(c, 95)),
                    "c_effective_max": float(np.max(c)),
                    "fc1_weight_norm": float(q.router.fc1.weight.norm().item()),
                    "fc1_bias_norm": float(q.router.fc1.bias.norm().item()),
                    "fc2_weight_norm": float(q.router.fc2.weight.norm().item()),
                    "fc2_bias_norm": float(q.router.fc2.bias.norm().item()),
                    "boundary_hit_frac": float(np.mean((c <= C_MIN + 1e-4) | (c >= C_MAX - 1e-4))),
                    "n_groups": len(groups),
                    "group_c_mean_mean": float(np.mean(gm)),
                    "group_c_mean_std": float(np.std(gm)),
                    "group_c_mean_min": float(np.min(gm)),
                    "group_c_mean_max": float(np.max(gm)),
                }
                # prefix 置换 counterfactual: 固定 residual, 打乱 prefix 后 ≥10% 商品 |Δc|>1e-4
                perm = np.random.RandomState(42).permutation(N_ITEMS)
                delta_shuf = q.router(prefix_emb[perm])
                c_shuf = C_MIN + (C_MAX - C_MIN) * torch.sigmoid(q.theta + delta_shuf)
                dc = (c_all - c_shuf).abs().detach().cpu().numpy()
                gate2[f"L{li}"]["shuffle_fraction_dc_gt_1e-4"] = float(np.mean(dc > 1e-4))
        with open(out_dir / "gate2_mechanism.json", "w") as f:
            json.dump(gate2, f, indent=2, ensure_ascii=False)
        ...
        verdict = {
            "issue": "#154",
            "arm": "treatment",
            "source": "Issue #154 true prefix-conditioned curvature (Xavier fc1 + live-delta reg) + DDP 4 卡",
            "world_size": world,
            "n_epochs": N_EPOCHS,
