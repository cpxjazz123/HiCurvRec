...
# Issue #161 基础 (M2+M3, M1 off, 来自 #160 c0110)
GATE_M1_ROUTER = False
GATE_M2_INTRINSIC = False
GATE_M3_TRANSPORT = False  # S_C': 纯欧式 baseline (全机制 off)
...
def broadcast_kmeans_init(model: nn.Module, item_emb: torch.Tensor, rank: int, world: int) -> None:
    ...
    if rank == 0:
        model.train()
        # Issue #166: init 阶段强制 M2 off (欧式残差路径) → 两套 Stage2 初始 codebook 完全一致
