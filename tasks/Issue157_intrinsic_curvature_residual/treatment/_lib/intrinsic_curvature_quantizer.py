...
class PrefixConditionedHRQVAE(nn.Module):
    ...
    def _rq_forward(self, x, use_sk=True, intrinsic_subtraction=False):
        """逐层 RQ. intrinsic_subtraction=True (Issue #157): 用该层 per-item 曲率
        执行内在几何减法 (Möbius) 更新下一层 residual:
            h_r = Exp_0^c(r_l); h_e = Exp_0^c(e_l)
            h_next = (-h_e) ⊕_c h_r
            r_{l+1} = Log_0^c(h_next)
        intrinsic_subtraction=False 时退化为欧式相减 (与 Control 一致).
        """
        ...
        for q in self.vq_layers:
            ...
            if intrinsic_subtraction:
                # Issue #157: 同层 per-item c 贯穿距离 + residual 更新 (禁止两套曲率)
