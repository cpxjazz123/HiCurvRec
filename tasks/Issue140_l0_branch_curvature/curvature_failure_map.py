...
TASK_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/tasks/Issue140_l0_branch_curvature")
os.chdir(TASK_DIR)
...
def main():
    ...
    # ─── Gate 1: 全局重现 (Issue #138 spec; Issue #140 锚点 = Issue #139) ───
    eval_test = json.load(open(EVAL_TEST_JSON))
    R10_full = eval_test["R@10"]
    issue139_R10 = 0.0968  # Issue #140 锚点 (Issue #139 test R@10)
    drift = abs(R10_full - issue139_R10)
    drift_blocked = drift > 0.005
    print(f"  Gate 1: this_run R@10={R10_full:.4f} vs Issue #139 {issue139_R10:.4f} (diff={drift:.4f})",
    ...
    with open(verdict_md, "w") as f:
        ...
        f.write(f"**Stage4 full eval R@10**: {R10_full:.4f} (Issue #139 anchor {issue139_R10:.4f}, "
        ...
        f.write(f"- 与 Issue #139 anchor R@10={issue139_R10:.4f} 差 {drift:.4f} "
