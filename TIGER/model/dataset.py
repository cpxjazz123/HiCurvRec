...
class GenRecDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        for item in processed_data:
            item['history'] = [self.item_to_code.get(x, np.array([self.PAD_TOKEN]*3)) for x in item['history']]
            item['target'] = self.item_to_code.get(item['target'], np.array([self.PAD_TOKEN]*3))
        return processed_data
