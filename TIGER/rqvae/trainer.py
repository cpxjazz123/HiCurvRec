...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            # DDP 兼容: raw model 在 self.model.module 下
            compute_loss_fn = self.model.module.compute_loss if hasattr(self.model, 'module') else self.model.compute_loss
            loss, loss_recon = compute_loss_fn(out, rq_loss, xs=data)
    ...
    def _valid_epoch(self, valid_data):
        ...
        for batch_idx, data in enumerate(iter_data):
            num_sample += len(data)
            data = data.to(self.device)
            indices = self.model.get_indices(data) if not hasattr(self.model, 'module') else self.model.module.get_indices(data)
