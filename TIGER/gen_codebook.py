...
if __name__ == "__main__":
    ...
    R53 v3.8 (HG-Rec_amazon2023): 数据集硬编码 Amazon_2023_Instruments, ckpt_path / output_path
    与 train_hrqvae_kappa_single.py 产物路径一致 (codebook [256,256,256]).
    """
    dataset = "Amazon_2023_Instruments"
    # R51+ Stage 1 产物路径 (HRQ-VAE ckpt 由 train_hrqvae_kappa_single.py 训出)
    # 真实路径由 train_hrqvae_kappa_single.py 输出, 含时间戳子目录; 此处用占位符
    # 实际跑前必须手动改 ckpt_path 指向最新时间戳目录里的 best_collision_model.pth
    ckpt_path = f"./ckpt/{dataset}/LATEST_KAPPA_SINGLE/HG_Rec_HRQVAE_best.pth"
    output_path = f"./dataset/{dataset}/{dataset}_t5_rqvae_beta_0.250_codebook_[256,256,256]_sk_0.000_R51plus_seed42.npy"
