...
# === v60 G1 v2 Stage 3 auxiliary geodesic classifier (R36i Round 4 修复: cb normalize + ε=0.05) ===
G1_ENABLED = True
_G1_RQVAE_CKPT = "/home/wlia0047/hj82_scratch2/wenyu/rqvae_dataset/instruments/rqvae_out_m2m3/rqvae_final.pt"
_G1_C = 0.7
_G1_EPSILON = 0.05  # v60: 减弱 G1 让 baseline 主导 (v59=0.1 偏大, valid 退化)
_G1_BALL_DIM = 32
_g1_codebook = None
_g1_ball_proj = None


def _normalize_cb_to_ball(cb, c, max_norm_ratio=0.95):
    """v60 G1 v2 修复 (R36i Round 3 norm 必填): baseline RQ-VAE codebook 不在严格 Poincaré ball 内
    (L0 norm=1.48 > 1/sqrt(0.7)=1.195). normalize 到 ball 内 (radius * max_norm_ratio = 1.135)
    再算 poincare_dist, 否则 L0 distance 退化 (1-||x||² 是负数, arccosh 退化到 0).
    """
    ball_radius = (1.0 / c ** 0.5) * max_norm_ratio
    norms = cb.norm(dim=-1, keepdim=True).clamp(min=1e-7)
    cb_in_ball = cb * torch.clamp(ball_radius / norms, max=1.0)
    return cb_in_ball


def _g1_geo_classifier(hidden, model, c=_G1_C):
    """v60 G1 v2: auxiliary geodesic classifier + cb normalize, OOM-safe CPU 计算."""
    ball_proj = model.module.g1_ball_proj if hasattr(model, "module") else model.g1_ball_proj
    codebook_list = model.module.g1_codebook if hasattr(model, "module") else model.g1_codebook
    B, L, _ = hidden.shape
    # 全部移到 CPU 避免 GPU OOM
    ball_emb_raw = ball_proj(hidden.detach()).cpu().float()
    codebook_cpu = [cb.detach().cpu().float() for cb in codebook_list]
    D = ball_emb_raw.shape[-1]
    norm = ball_emb_raw.norm(dim=-1, keepdim=True).clamp(min=1e-7)
    sqrt_c = c ** 0.5
    ball_norm = torch.tanh(sqrt_c * norm / 2.0) / (sqrt_c * norm) * ball_emb_raw
    ball_norm_sq = (ball_norm ** 2).sum(dim=-1, keepdim=True)
    ball_norm_flat = ball_norm.reshape(B * L, D)
    ball_norm_sq_flat = ball_norm_sq.reshape(B * L, 1)
    geo_logits_chunks = []
    for start in range(0, B * L, 256):
        end = min(start + 256, B * L)
        ball_chunk = ball_norm_flat[start:end]
        ball_sq_chunk = ball_norm_sq_flat[start:end]
        layer_logits = []
        for li in range(3):
            cb = codebook_cpu[li]
            diff_norm_sq = ((ball_chunk.unsqueeze(1) - cb.unsqueeze(0)) ** 2).sum(dim=-1)
            cb_norm_sq = (cb ** 2).sum(dim=-1, keepdim=True).transpose(0, 1)
            eps = 1e-7
            denom = (1.0 - ball_sq_chunk) * (1.0 - cb_norm_sq) + eps
            arg = 1.0 + 2.0 * diff_norm_sq / denom
            arg = arg.clamp(min=1.0 + eps)
            poincare_dist = torch.arccosh(arg) / sqrt_c
            layer_logits.append(-poincare_dist)
        geo_logits_chunks.append(torch.cat(layer_logits, dim=-1))
    geo_logits = torch.cat(geo_logits_chunks, dim=0).reshape(B, L, -1)
    pad_col = torch.zeros(B, L, 1, dtype=geo_logits.dtype)
    geo_logits_full = torch.cat([geo_logits, pad_col], dim=-1)
    # 转回原 device (loss 计算需要)
    return geo_logits_full.to(hidden.device)
...
def _train_hgrec(
    ...
    # === v60 G1 v2 Stage 3 init: 加载 RQ-VAE codebook + 创建 ball_proj ===
    if G1_ENABLED:
        global _g1_codebook, _g1_ball_proj
        _rqvae_ckpt = torch.load(_G1_RQVAE_CKPT, map_location="cpu", weights_only=False)
        _model_state = _rqvae_ckpt["model"]
        _codebook_list = []
        for li in range(3):
            _cw = _model_state[f"layers.{li}.embedding.weight"]
            _cw_norm = _normalize_cb_to_ball(_cw, c=_G1_C, max_norm_ratio=0.95)
            _codebook_list.append(_cw_norm)
        _g1_codebook = nn.ParameterList([
            nn.Parameter(_cw.detach().clone(), requires_grad=False) for _cw in _codebook_list
        ])
        _g1_ball_proj = nn.Linear(hgrec_config["d_model"], _G1_BALL_DIM, bias=False)
        nn.init.xavier_uniform_(_g1_ball_proj.weight)
        _g1_ball_proj = _g1_ball_proj.to(next(model.parameters()).device)
        # 注册到 model 让 accelerator.prepare 自动 wrap (DDP 4 卡同步)
        model.g1_ball_proj = _g1_ball_proj
        model.g1_codebook = _g1_codebook
        if accelerator.is_main_process:
            print(f"[G1 v2 init] codebook list (3 layers, {sum(p.numel() for p in _g1_codebook)} params, "
                  f"normalized to ball radius {(1.0/_G1_C**0.5)*0.95:.4f}) + ball_proj (d_model→{_G1_BALL_DIM}) loaded", flush=True)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for batch in pbar:
            ...
            with accelerator.autocast():
                ...
                # === v60 G1 v2: auxiliary geodesic classifier on encoder hidden (mean pool → 1 sample) ===
                # detach 阻止梯度反传到 T5 encoder hidden (省 OOM),用 mean pool → 1 token 匹配 target L=1
                if G1_ENABLED:
                    dec_hs = enc_hs[-1].detach().mean(dim=1, keepdim=True)  # (B, 1, D)
                    geo_logits = _g1_geo_classifier(dec_hs, model, c=_G1_C)
                    # G1 mean-pool → 1 token/sample, target 也取第一个 SID
                    geo_ce = F.cross_entropy(
                        geo_logits.reshape(-1, geo_logits.size(-1)),
                        batch["target"][:, 0].reshape(-1),
                        ignore_index=0,
                    )
                    total_loss = total_loss + _G1_EPSILON * geo_ce
