    ...
    def do_eval(eval_dl, split_name: str):
        ...
        with torch.no_grad():
            for batch in eval_dl:
                ...
                # DDP exposes ``module`` on multi-GPU runs, but the
                # curvature_base probe is also useful on a single GPU.  Use
                # Accelerate's unwrap path so both launch modes share the
