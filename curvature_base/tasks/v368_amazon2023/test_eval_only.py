...
def _test_eval_hgrec(accelerator, device, BEST_CKPT_PATH):
    ...
    INSTRUMENTS_DIR = "/home/wlia0047/ar57/wenyu/GeneRec/dataset/Amazon_2023_Instruments"  # v318_amazon2023: 改指 2023 dataset
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if accelerator.is_main_process and not diag_done:
                print(f"[diag2] input_ids[0]={input_ids[0].tolist()}", flush=True)
                print(f"[diag2] labels[0]={labels[0].tolist()}", flush=True)
                print(f"[diag2] preds shape={tuple(preds.shape)}, sample0_beams_first_3={preds[:3].tolist()}", flush=True)
                diag_done = True
