...
USE_CYCLIC_CURVATURE = False              # v355 ABLATION: 关闭 cyclic (固定 c=1.0, 等于 v282)
C_CYCLIC_MIN = 0.3
C_CYCLIC_MAX = 1.0
C_CYCLIC_PERIOD = 50_000
GRAD_CLIP_NORM = 1.0
USE_M3_TRANSPORT = False                  # v355 ABLATION: 关闭 M3 (测 midpoint 单独贡献)
USE_M2_INTRINSIC = True                   # v355 ABLATION: 启用 M2 (midpoint 需要 M2 路径)
USE_GEODESIC_MIDPOINT_COMMIT = True       # v355 ABLATION: 启用 midpoint commit
...
def main():
    ...
    model = RqVae(
        ...
        hyperbolic_distance=True,  # v355 ABLATION: 启用双曲距离 (midpoint 需要) # v354 ABLATION: 关闭双曲距离 (用 L2 argmin, RQ-VAE-Recommender 等价)
