...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v284 NOVEL: per-layer 异质 commit 路径 (R36n b)
        # 3 层 cascade 用不同 commit: L0=midpoint (v282 winning), L1=sub (v262 winning), L2=direct (欧氏)
        # 每层 list[str] ∈ {"midpoint", "sub", "direct"}, 默认 None = 兼容 v282 路径
        # 论文: 不同 cascade 层几何需求不同 — 浅层 norm 大需强几何变换, 深层 norm 小几何退化为欧氏
        commit_path_per_layer: Optional[List[str]] = None,
    ) -> None:
        self._config = locals()
        ...
        # v284 NOVEL: per-layer 异质 commit 路径 (R36n b)
        if commit_path_per_layer is None:
            commit_path_per_layer = ["midpoint" if m else "sub" for m in midpoint_layer_mask]
        assert len(commit_path_per_layer) == n_layers
        for p in commit_path_per_layer:
            assert p in ("midpoint", "sub", "direct"), f"unknown commit_path {p}"
        self.commit_path_per_layer = list(commit_path_per_layer)
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001) -> RqVaeOutput:
        ...
        for li, layer in enumerate(self.layers):
            ...
            if self.gate_M2_intrinsic:
                ...
                # v284 NOVEL: per-layer 异质 commit 路径 (R36n b)
                # 三层 cascade 用不同 commit 路径, 优先级高于 v282 midpoint_layer_mask
                # path ∈ {"midpoint": Fréchet mean on hyperbolic, "sub": Möbius subtraction, "direct": Euclidean (跳过 M2)}
                path = self.commit_path_per_layer[li]
                if path == "midpoint":
                    # L0 / 任意层: Geodesic Midpoint commit (v282 winning path)
                    # 公式: res_mid = exp_0((log_0(res) + log_0(emb))/2, c)
                    ...
                    # L1 / 任意层: Möbius Subtraction (v262 winning path)
                    # 公式: res = exp_0(log_0(res) - log_0(emb), c)
                    # 论文: Ganea 2018 NeurIPS
                    h_r = _expmap0_t(res, c_per)
                    h_e = _expmap0_t(emb, c_per)
                    h_next = _mobius_add_t(-h_e, h_r, c_per)
                    res = _logmap0_t(h_next, c_per)
                elif path == "direct":
                    # L2 / 任意层: Euclidean direct commit (跳过 M2 几何变换)
                    # 直觉: 深层 norm 极小, 几何退化为欧氏, 不需要 Poincaré ball 变换
                    res = res - emb
                else:
                    raise ValueError(f"unknown commit_path {path}")
            else:
                # 兜底: gate_M2_intrinsic=False 时 fallback 到 Euclidean direct (与历史 baseline 一致)
                res = res - emb
