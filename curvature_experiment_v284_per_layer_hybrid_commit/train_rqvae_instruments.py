...
# v284 NOVEL: Per-Layer Hybrid Commit (R36n b) — 三层 cascade 用不同 commit 路径
# L0 = Geodesic Midpoint (v282 winning path) — 浅层输入 raw residual (norm 大, 双曲几何明确)
# L1 = Möbius Subtraction (v262 winning path) — 中层需要方向性变换
# L2 = Direct (欧氏, 无变换) — 深层 norm 极小, 几何退化为欧氏
# 论文: 不同 cascade 层几何需求不同 — Nickel & Kiela 2017 / Ungar 2008 / Fréchet mean
# 若 USE_PER_LAYER_HYBRID_COMMIT=True, 则 per-layer 路径覆盖 USE_GEODESIC_MIDPOINT_COMMIT
USE_PER_LAYER_HYBRID_COMMIT = True         # v284 启用 per-layer 异质 commit
COMMIT_PATH_PER_LAYER = ["midpoint", "sub", "direct"]  # v284 三层异质
...
def main():
    ...
    model = RqVae(
        ...
        commit_path_per_layer=COMMIT_PATH_PER_LAYER if USE_PER_LAYER_HYBRID_COMMIT else None,  # v284 NOVEL: per-layer 异质 commit
    ).to(device)
