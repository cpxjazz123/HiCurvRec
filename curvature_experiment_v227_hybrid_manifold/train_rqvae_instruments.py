...
# 动机: v226 全层 sphere anchor 证实 L0 (input norm≈0.04 << min=0.3) 有效 (+39% utility),
#       但 L1/L2 (input norm≈0.14 接近 min=0.2) 失败 (-62% utility, R37 REGRESS).
# 原因: L1/L2 input norm 接近 min, commit loss 反向 + anchor forward 拉扯 → collapse.
# v227: L0 sphere anchor (push 到球面) + L1/L2 retraction only (max_norm clip only, 无 min push)
#       L1/L2 让 codeword 在 input norm 自然分布 (类似 v225 retraction 避开拉扯).
# 预期: L0 utility ≥ 90% (sphere push-up), L1/L2 utility ≥ 90% (retraction only, v225 已验证).
USE_SPHERE_ANCHOR = True        # v227: 启用混合 manifold (L0 sphere anchor + L1/L2 retraction)
L0_ANCHOR_MIN, L0_ANCHOR_MAX = 0.3, 0.5   # L0: 强 sphere anchor (input norm<<min, push-up 有效)
L1_ANCHOR_MIN, L1_ANCHOR_MAX = 0.0, 0.3   # L1: 仅 max_norm clip (anchor_min_norm=0 禁用 min push)
L2_ANCHOR_MIN, L2_ANCHOR_MAX = 0.0, 0.3   # L2: 仅 max_norm clip (同 L1)
...
def main():
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v227: 球壳投影 (L0 sphere anchor + L1/L2 retraction)
