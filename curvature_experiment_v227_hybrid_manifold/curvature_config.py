...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "v227_hybrid_manifold"  # ← v227 NOVEL: v226 全层 sphere anchor 证实 L0 有效 (utility +39%) 但 L1/L2 失败 (R37 -62%). v227 改用 per-layer 异质 manifold: L0 sphere anchor (push 到球面) + L1/L2 retraction only (max_norm clip only, 无 min push). R36n 合规: (b) per-layer 异质 manifold + (c) manifold 几何替换 + (e) 几何变换. Stage 1 端纯几何变更. R36p v3.10 utility ≥ 90% 硬约束.
