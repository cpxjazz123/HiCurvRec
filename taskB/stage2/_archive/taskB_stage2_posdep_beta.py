#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #176 + #177 Stage 2 fork: 用 PosDepBetaHRQVAE 加载 β(x) 位置依赖训练 ckpt.

Mirror of scripts/task175_orc_locked_stage2_codebook.py with 2 changes:
  - import PosDepBetaHRQVAE instead of ORCLockedHRQVAE
  - 加载 ckpt 时也传 beta_mode / beta_base / beta_scale / beta_max

用法:
    python3 scripts/task176_177_posdep_beta_stage2_codebook.py \
        --ckpt_path /home/wlia0047/ar57/wenyu/GeneRec/products/task176/posdep_sigmoid/best_loss_model.pth \
        --output_path /home/wlia0047/ar57/wenyu/GeneRec/products/task176/t5mini_posdep_sigmoid/codes.npy \
        --beta_mode sigmoid \
...
def main():
    ...
    parser.add_argument("--beta_mode", choices=['sigmoid', 'conformal'], required=True)
    parser.add_argument("--beta_base", type=float, default=1.0)
    parser.add_argument("--beta_scale", type=float, default=10.0)
    ...
    log = logging.getLogger(f"task_{args.beta_mode}_stage2")
    ...
    log.info(f"  beta_mode={args.beta_mode}, beta_base={args.beta_base}, "
             f"beta_scale={args.beta_scale}, beta_max={args.beta_max}")
    ...
    model = PosDepBetaHRQVAE(
        ...
        beta=ckpt_args['beta'],
        kmeans_init=False,  # state_dict 已含 trained codebook
    ...
    # Verify κ per layer
    log.info(f"  Verify κ after load:")
    ...
    for d in tqdm(data_loader, desc=f"Task {args.beta_mode} Stage 2 forward"):
        d = d.to(device)
