#!/usr/bin/env python3
"""Task #475 / Issue #187 [方向B Gate3→Gate4] 协议对齐 canary 真实 argmax sanity check.

Issue #187 spec 强制:
- 禁止直接长跑 (no 200 epoch, no double-run)
- 仅小规模真实样本 Stage 4 argmax (默认 200)
- 4 项协议一致性 audit
- 复用 Issue #181 ckpt (task473 200 epoch 训练产物, 已存在, 禁止重训)
"""
import os
import sys
import json
...
os.environ["TRITON_CACHE_DIR"] = f"/home/wlia0047/.triton/cache_task475_issue187"
...
OUT_DIR = f"{PROJECT}/taskB/stage4/taskB_stage4_canary_argmax"
os.makedirs(OUT_DIR, exist_ok=True)
LOG_PATH = f"{PROJECT}/logs/task475_issue187_canary_stage4_argmax.log"
...
def main():
    seed_all(SEED)
    log = []
    log.append(f"\n=== Task #475 / Issue #187 [方向B canary] 协议对齐 sanity check ===")
    ...
    log.append(f"  SID match: {sid_match} (informational, same SID NPY as #176)")
    ...
    with torch.no_grad():
        for batch_start in range(0, canary_n, BATCH):
            ...
            residual, alpha = model_wrapper.adapter(
                ...
                torch.zeros(B, 3, 4, device=DEVICE),  # curvature_meta (B, 3, 4)
            )
    ...
    verdict = {
        "task": "task475_issue187_canary_stage4_argmax",
        "issue": 187,
        "canary_n": canary_n,
        "sid_sha256": sid_sha,
