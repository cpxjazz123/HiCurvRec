...
# ============================================================================
# Config (per Issue #18 spec option_B 提高 κ 初始化)
# 单变量: kappa_logits init 改 per-layer 不同 (破对称性)
#   旧 default: torch.full((3,), 0.5413) → softplus = [1.0, 1.0, 1.0] 同步
#   新 init:    torch.tensor([0.0, 2.0, 4.0]) → softplus = [0.693, 2.127, 4.490] 三层不同
# 严禁: 改其他架构 (mixing_logits init 不动, conditioner 不动, T5 frozen 不动, capacity 不动)
# ============================================================================
SEED = 42
DEVICE = "cuda:0"
...
NUM_EPOCHS = 60   # 短程 sanity 先 5 epoch, 通过则继续
LR_CONDITIONER = 1e-3
LR_LAYERNORM = 1e-4
...
# option_B 单变量: kappa_logits per-layer 不同 init
KAPPA_INIT_LOGIT_PER_LAYER = [0.0, 2.0, 4.0]  # 破对称性, 远离 default 0.5413
# mixing_logits init 保持 default (zeros → [1/3, 1/3, 1/3] 每层)
MIXING_INIT_PER_LAYER = None  # None = 用 default
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/stage3/taskB_stage3_issue18_option_b")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/_logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task_issue18_option_b.log"
...
def run_train():
    ...
    # option_B 单变量: 用 per-layer 不同 κ_init 覆盖 (破对称性)
    if KAPPA_INIT_LOGIT_PER_LAYER is not None:
        with torch.no_grad():
            model_wrapper.adapter.kappa_logits.data = torch.tensor(
                KAPPA_INIT_LOGIT_PER_LAYER, dtype=torch.float32, device=DEVICE
            )
            log_lines.append(f"[option_B] override kappa_logits → {KAPPA_INIT_LOGIT_PER_LAYER}")
            log_lines.append(f"[option_B] softplus(kappa_logits) = {model_wrapper.adapter.get_kappa_per_layer().detach().cpu().tolist()}")
            log_lines.append(f"[option_B] 期望三层 κ 不同 (破对称性), R23 κ_synced 应 False")
