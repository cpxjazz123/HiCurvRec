#!/usr/bin/env python3
"""Issue #193 [方向B Gate4] 几何混合分量单seed正式长训+Task84六项评估.

承接 Issue #191 commit 2368e44 (Gate3 PASS, canary R@10=0.025 + mixing non-degenerate).

关键改进 vs Issue #181 (closed NO-GO commit a5bbc98):
1. 续训: 从 #191 epoch=9 ckpt 续训到 epoch=199
2. Mid-train canary @ epoch=104 (50% point) + mixing 权重非退化复查
3. Stage 4 自回归解码 (vs Issue #181 decoder_input_ids=zeros bug)
4. 六项指标 R@5/10/20 + NDCG@5/10/20
5. 判据: test R@10 > 0.1020

R18 4 维度 vs Issue #181 (closed NO-GO):
- D1 spec: 不同 (新修复 #191 应用)
- D2 实施: 同 wrapper (BoundedWeightedMixedCurvatureConditioner), 同协议
- D3 Gate 1 失败机制: Gate 1 PASS (沿用 #158/#176)
- D4 引用文献: 同族

R7: GPU 1 (方向B), 单 seed = 42
"""
import os
import sys
import json
...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_issue193_long_run"
...
D_MODEL = 128
BATCH_SIZE = 32
START_EPOCH = 46  # 沿用本 long-run 已训 epoch=46 ckpt (vs #191 epoch=9)
NUM_EPOCHS = 199
LR_CONDITIONER = 1e-3
LR_LAYERNORM = 1e-4
ALPHA_MAX = 0.5
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/stage3/taskB_stage3_issue193_long_run")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/_logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task_issue193_long_run.log"
...
def autoregressive_predict(model_wrapper, history_tensor, attention_mask, layer_ranges):
    ...
    with torch.no_grad():
        ...
        curvature_meta = torch.zeros(B, 3, 4, device=device)
...
def run_val_eval(model_wrapper, layer_ranges, val_histories_t, val_targets_t, val_n=1000, batch_size=32):
    """Per-epoch val_R@10 eval. Returns val_r10 + per-layer in-range stats."""
    model_wrapper.eval()
...
def check_mixing_non_degenerate(model_wrapper):
    """方向B 专属: 三层 κ + mixing weights 当前值, 确认非退化."""
    curvature_embed = model_wrapper.adapter.curvature_embed
    weight_norms = curvature_embed.weight.norm(dim=1).detach().cpu().tolist()
    bias_values = curvature_embed.bias.detach().cpu().tolist()
    cond_w = model_wrapper.adapter.conditioner
    conditioner_norm_0 = cond_w[0].weight.norm().item()
    conditioner_norm_2 = cond_w[2].weight.norm().item()
    ...
    bias_nonzero = any(abs(b) > 1e-6 for b in bias_values)
    mixing_non_degenerate = weight_nonzero and bias_nonzero
    return {
        "weight_norms_min": min(weight_norms), "weight_norms_max": max(weight_norms),
        "weight_norms_mean": sum(weight_norms) / len(weight_norms),
        "bias_abs_max": max(abs(b) for b in bias_values),
        "alpha_logit": alpha_logit_val, "alpha_value": alpha_val,
        "weight_nonzero": bool(weight_nonzero),
        "bias_nonzero": bool(bias_nonzero),
        "mixing_non_degenerate": bool(mixing_non_degenerate),
        "conditioner_norm_0": conditioner_norm_0,
        "conditioner_norm_2": conditioner_norm_2,
    }
...
def run_canary(model_wrapper, layer_ranges, canary_n=200, batch_size=32):
    ...
    canary_pass = r10 > 0 and in_range_pct > 0.99 and mixing_audit["mixing_non_degenerate"]
    log_lines.append(f"  canary R@10={r10:.4f}, in-valid-range={in_range_count}/{total_tokens} ({in_range_pct*100:.1f}%)")
    log_lines.append(f"  mixing audit: {mixing_audit}")
    log_lines.append(f"  canary_pass={canary_pass}")
...
def run_stage4(model_wrapper, layer_ranges, batch_size=32):
    ...
    metrics = {
        ...
        "NDCG@5": compute_hit_at_k(preds_all, targets_all, 5),
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Issue #193 方向B Gate4 long-run] START_EPOCH={START_EPOCH}, NUM_EPOCHS={NUM_EPOCHS}")
    log_lines.append("=" * 70)
    ...
    # Train/val split (deterministic, last 10% as val)
    ...
    for epoch in range(START_EPOCH, NUM_EPOCHS):
        ...
        for batch_idx in range(n_batches):
            ...
            # kappa_mixing: kappa(3) + mixing 4 weights per Issue #193 wrapper
            kappa_mixing = torch.zeros(B, 3, 4, dtype=torch.float32, device=DEVICE)

            optimizer.zero_grad()
            output, _, alpha = model_wrapper(history_tensor_b, attention_mask=attention_mask_b,
                                              labels=target_tensor_b, sid_meta=sid_meta,
                                              curvature_meta=kappa_mixing)
        ...
        if epoch + 1 == CANARY_EPOCH:
            ...
            if not canary_pass:
                print(f"  [R23 KILL] canary FAIL @ epoch={epoch+1} R@10={canary_r10}", flush=True)
                with open(LOG_PATH, "a") as f:
