#!/usr/bin/env python3
"""Issue #23 [B Gate3] option_D  - conditioner  kappa  ( #20 ).

per #23 spec:
- :  #193 long-run  Stage3 ckpt (epoch 50, val_R@10=0.058)
-  #20 option_C : T5 encoder block 4-5  + LR_UNFROZEN_T5=5e-5 ( kappa/mixing ,  kappa )
-  option_D:  conditioner MLP ->  conditioner heads (L0/L1/L2  curvature_embed +  conditioner head +  residual )
- :  (kappa  /  / ) / seed/ / from-0  /  Gate3 
-  R23 NO-GO  kill: val_R@10=0  >=2 epoch /  /  kappa  /  conditioner head  0 / 

ckpt: taskB_stage3_issue193_long_run/best_adapter.pt (epoch 50 val_R@10=0.058) ( #20 )
: kappa_logits (3,) + mixing_logits (3,3) + per-layer curvature_embed (3x Linear(4, d_model)) + per-layer conditioner heads (3x MLP)  ckpt 
-> load_state_dict  strict=False,  params  init (kappa_init_logit=0.5413 -> softplus ~ 1.0, mixing=zeros -> softmax=[1/3,1/3,1/3], curvature_embed/cond_head  init)

: taskB/stage3/taskB_stage3_issue23_option_d/
- adapter.pt (per-epoch save, R12 )
- best_adapter.pt (best val_R@10 save)
- val_trace.json ( kappa per-layer / mixing per-layer /  conditioner head  / dL/dkappa / dL/dmix / )
- training_summary.json ( verdict)
- _TRAINING_PID
- log: taskB/_logs/task_issue23_option_d.log
"""
import os
import sys
import json
...
# ============================================================================
# Config (per Issue #23 spec option_D conditioner )
# :  conditioner MLP ->  conditioner heads
#  #20: T5 encoder block 4-5  (top N=2 of 6 layers)
# :  (kappa_logits init , mixing_logits init , , )
# ============================================================================
SEED = 42
DEVICE = "cuda:0"
...
D_MODEL = 128
BATCH_SIZE = 32
START_EPOCH = 51  #  best_adapter.pt (epoch 50) 
NUM_EPOCHS = 56   #  sanity  5 epoch (epoch 51-55), 
LR_CONDITIONER = 1e-3
LR_LAYERNORM = 1e-4
LR_NEW_PARAMS = 1e-3  # kappa_logits / mixing_logits 
LR_UNFROZEN_T5 = 5e-5  # T5  ( backbone , )

VAL_SPLIT_RATIO = 0.10
VAL_EVAL_N = 1000
EARLY_STOP_PATIENCE = 10
EARLY_STOP_MIN_DELTA = 1e-4  # min_delta  (per #20 spec )

# option_D :  conditioner MLP ->  conditioner heads
#  #20 option_C:  T5  N=2  (encoder block 4-5 of 6)
UNFREEZE_TOP_N = 2  #  N 
# mixing_logits init  default (zeros -> [1/3, 1/3, 1/3] )
KAPPA_INIT_LOGIT_PER_LAYER = None  #  default (0.5413 -> softplus ~ 1.0)
MIXING_INIT_PER_LAYER = None  #  default

# Issue #23  (per spec)
RECORD_PER_EPOCH_GRADS = True  #  dL/dkappa, dL/dmix,  +  conditioner head 
RECORD_MIXING_INDEPENDENCE = True  #  mixing  (per #20 spec )
RECORD_PER_HEAD_GRADS = True  #  conditioner head  (option_D )
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/stage3/taskB_stage3_issue23_option_d")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/taskB/_logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task_issue23_option_d.log"
...
def load_wrapper_cls():
    """Issue #23 option_D:  mod,  per-layer  conditioner + adapter.

     (vs shared conditioner MLP):
      - curvature_embed: shared Linear(4, d_model) -> nn.ModuleList[3 x Linear(4, d_model)] (per-layer )
      - conditioner head: shared MLP(d_model*2 -> d_model) -> nn.ModuleList[3 x MLP] (per-layer )
      - forward: curvature_meta (B, 3, 4) -> 3  curvature_embed -> (B, 3, d_model)
      - residual: 3  head  -> mean(dim=1)  sum -> (B, L, d_model)
      - :  Linear / MLP ()
    """
    # import HG_Rec for t5 backbone
    ...
    from HG_Rec import HG_Rec  # noqa: F401

    class BoundedWeightedMixedCurvatureConditionerPerLayer(nn.Module):
        """Issue #23 option_D:  curvature_embed +  conditioner head.

         #15 Step 1  kappa_logits (3,) + mixing_logits (3,3) learnable params,
         conditioner  (curvature_embed + MLP head)  per-layer .
        """

        def __init__(self, d_model=128, n_layers=3, sid_dim=4, alpha_init_logit=-10.0, alpha_max=0.5,
                     kappa_init_logit=0.5413):
            super().__init__()
            ...
            # per-layer  curvature_embed (3 x Linear(4, d_model)) - option_D 
            self.curvature_embed_per_layer = nn.ModuleList([
                nn.Linear(4, d_model) for _ in range(n_layers)
            ])

            # SID token  -> d_model (per-layer  sid_token_proj, )
            self.sid_token_proj = nn.Linear(sid_dim, d_model)

            # per-layer  conditioner head (3 x MLP) - option_D 
            self.conditioner_heads = nn.ModuleList([
                nn.Sequential(
            ...
            # alpha logit (init  -> alpha=0)
            self.alpha_logit = nn.Parameter(torch.tensor(alpha_init_logit, dtype=torch.float32))

            #  #15 Step 1: per-layer learnable kappa_logits (3,) + mixing_logits (3, 3)
        ...
        def build_curvature_meta(self, B):
            """ curvature_meta (B, 3, 4) from learnable per-layer kappa + mixing."""
            ...
            return torch.cat([kappa_l, mixing_l], dim=-1)  # (B, 3, 4)

        def forward(self, x_emb, sid_meta, curvature_meta):
            """per-layer  forward: curvature_meta (B,3,4) -> 3  curvature_embed -> 3  head."""
            B, L = x_emb.shape[0], x_emb.shape[1]
            sid_e = self.sid_token_proj(sid_meta)  # (B, L, d_model)

            #  curvature_embed + conditioner head
            head_outputs = []
            for layer_i in range(self.n_layers):
                # curvature_meta[:, layer_i, :] (B, 4) ->  embed -> (B, d_model)
                curv_e_l_i = self.curvature_embed_per_layer[layer_i](curvature_meta[:, layer_i, :])  # (B, d_model)
                curv_e_l_i = curv_e_l_i.unsqueeze(1).expand(-1, L, -1)  # (B, L, d_model)
                combined = torch.cat([sid_e, curv_e_l_i], dim=-1)  # (B, L, 2*d_model)
                direction_l_i = self.conditioner_heads[layer_i](combined)  # (B, L, d_model)
                head_outputs.append(direction_l_i)

            #  head  ->  (3 -> 1) -  dim 
            direction = torch.stack(head_outputs, dim=1).mean(dim=1)  # (B, L, d_model)
            alpha = F.softplus(self.alpha_logit).clamp(max=self.alpha_max)
            residual = alpha * direction
            return residual, alpha

    class HG_Rec_with_BoundedWeightedMixedAdapterPerLayer(nn.Module):
        def __init__(self, t5_config, t5_state_dict, d_model=128, n_layers=3, sid_dim=4):
            ...
            self.adapter = BoundedWeightedMixedCurvatureConditionerPerLayer(
    ...
    return HG_Rec_with_BoundedWeightedMixedAdapterPerLayer, get_t5_config


def autoregressive_predict(model_wrapper, history_tensor, attention_mask, layer_ranges):
    """Issue #191 :  4-forwards +  mask."""
...
def run_train():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Issue #23 B Gate3 option_D conditioner ] START_EPOCH={START_EPOCH}, NUM_EPOCHS={NUM_EPOCHS}")
    log_lines.append("=" * 70)
    ...
    if sid_sha != EXPECTED_SID_SHA:
        log_lines.append("[FAIL] SID hash ,  STOP")
    ...
    # Load prior ckpt with strict=False ( kappa_logits/mixing_logits  ckpt )
    ...
    log_lines.append(f"   missing keys =  nn.Parameter kappa_logits/mixing_logits (per #15 Step 1 , init )")
    ...
    log_lines.append(f"[Initial kappa per-layer] {model_wrapper.adapter.get_kappa_per_layer().detach().cpu().tolist()}")
    log_lines.append(f"[Initial mixing per-layer] {model_wrapper.adapter.get_mixing_per_layer().detach().cpu().tolist()}")

    #  #20 option_C:  T5 encoder  N=2  (block 4-5 of 6)
    #  #193 wrapper :  T5 frozen,  first_input_ln (encoder.block[0].layer[0].layer_norm) trainable
    ...
    for blk_idx in unfrozen_block_indices:
        for p in model_wrapper.t5.model.encoder.block[blk_idx].parameters():
            p.requires_grad = True
    log_lines.append(f"[option_D]  #20 option_C:  T5 encoder block indices = {unfrozen_block_indices} (top {UNFREEZE_TOP_N} of {n_total_blocks})")
    log_lines.append(f"[option_D] LR_UNFROZEN_T5 = {LR_UNFROZEN_T5}")
    log_lines.append(f"[option_D] conditioner : {len(model_wrapper.adapter.curvature_embed_per_layer)} x curvature_embed + {len(model_wrapper.adapter.conditioner_heads)} x conditioner head (per-layer )")
    ...
    log_lines.append(f"[mixing_independence]  mixing  = {mixing_init}")
    log_lines.append(f"[mixing_independence]  mixing  (init) = {not mixing_three_layers_identical}")
    log_lines.append(f"[mixing_independence] per #20 spec : ',  issue , '")
    ...
    for epoch in range(START_EPOCH, NUM_EPOCHS):
        ...
        # option_D :  conditioner head  (R23  head  != 0)
        cond_head_grad_sum = [0.0, 0.0, 0.0]
        n_batches = 0
        ...
        for batch_start in range(0, len(train_hist), BATCH_SIZE):
            ...
            for name, p in model_wrapper.named_parameters():
                if p.grad is not None:
                    ...
                    # option_D:  conditioner head idx  (curvature_embed_per_layer + conditioner_heads)
                    for layer_i in range(3):
                        if f"curvature_embed_per_layer.{layer_i}" in name:
                            cond_head_grad_sum[layer_i] += p.grad.abs().mean().item()
                        elif f"conditioner_heads.{layer_i}" in name:
                            cond_head_grad_sum[layer_i] += p.grad.abs().mean().item()
            n_batches += 1
        ...
        avg_cond_head_grad = [g / max(n_batches, 1) for g in cond_head_grad_sum]
        ...
        msg = (f"  [epoch {epoch+1}/{NUM_EPOCHS}] loss={avg_loss:.4f}, cond_grad={avg_cond_grad:.4e}, "
               ...
               f"cond_head_grad=[{avg_cond_head_grad[0]:.2e}, {avg_cond_head_grad[1]:.2e}, {avg_cond_head_grad[2]:.2e}], "
        ...
        val_trace.append({
            ...
            "grad_summary": {
                ...
                "cond_head_grad_per_layer": avg_cond_head_grad,  # option_D 
            },
            "loss": avg_loss,
        ...
        # R23  (per #23 spec - option_D :  conditioner head  != 0)
        r23_trigger = False
        r23_reasons = []
        # 1. val_R@10=0  >=2 epoch
        if len(val_trace) >= 2 and val_trace[-1]["val_r10"] == 0 and val_trace[-2]["val_r10"] == 0:
            r23_trigger = True
            r23_reasons.append("val_R@10=0  >=2 epoch")
        # 2.  / 
        if avg_kappa_grad == 0 or avg_kappa_grad > 100 or avg_mixing_grad == 0 or avg_mixing_grad > 100:
            r23_trigger = True
            r23_reasons.append(f"kappa_grad={avg_kappa_grad:.4e}  mixing_grad={avg_mixing_grad:.4e} ")
        # 3.  kappa  (option_D : , FAIL )
        if kappa_synced:
            r23_trigger = True
            r23_reasons.append(f" kappa  ({kappa_l}) - option_D ")
        # 4. mixing 
        if mixing_degenerate:
            r23_trigger = True
            r23_reasons.append(f"mixing  (max={mixing_max_per_layer})")
        # 5. option_D :  conditioner head  == 0 ()
        for layer_i in range(3):
            if avg_cond_head_grad[layer_i] < 1e-12:
                r23_trigger = True
                r23_reasons.append(f"conditioner head {layer_i}   0 ({avg_cond_head_grad[layer_i]:.2e}) - per-layer ")
    ...
    summary = {
        "issue": 23,
        "variable": "option_D conditioner  (3 x curvature_embed + 3 x conditioner head)",
        "inherited_from_#20": "T5 encoder block 4-5  + LR_UNFROZEN_T5=5e-5",
