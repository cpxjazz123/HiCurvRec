...
class Quantize(nn.Module):
    def __init__(
        ...
        # v287 NOVEL: Stochastic c perturbation (R36n n) — c 每 batch 加 N(0, sigma) 噪声
        # commit path 保持 v282 midpoint 不变, 仅在 c 上加 per-batch Gaussian 噪声.
        # 论文支撑: 同 v270 (Loshchilov & Hutter 2017 SGDR) + 噪声鲁棒优化 (Foret et al. ICLR 2021 SAM).
        # 与 v286 stochastic commit (R36n k) 的本质差异: v286 随机 commit 路径, v287 固定 commit 路径随机 c.
        # R36n (a) dynamic c variant #2 — 探索 c stochasticity 是否能突破 ceiling.
        stochastic_c_sigma: float = 0.0,    # v287: c 噪声标准差 (0=关闭)
        ...
        # v287: stochastic c perturbation
        self.stochastic_c_sigma = float(stochastic_c_sigma)
    ...
    def get_c_per_item(self, prefix_emb: Optional[Tensor] = None) -> Tensor:
        ...
        if self.use_cyclic_curvature:
            ...
            phase = math.pi * t / max(1, self.c_cyclic_period)
            c = self.c_cyclic_min + (self.c_cyclic_max - self.c_cyclic_min) * abs(math.sin(phase))
            c = c + getattr(self, "_current_c_noise", 0.0)  # v287: stochastic c perturbation
            return torch.tensor(c, device=self.theta.device, dtype=self.theta.dtype)
        if self.use_curriculum_curvature:
            t = min(1.0, self._curriculum_step / max(1, self.curriculum_steps))
            c = self.c_start + (self.c_end - self.c_start) * t
            ...
            return torch.tensor(c, device=self.theta.device, dtype=self.theta.dtype)
        if self.use_fixed_curvature:
            c = self.c_fixed + getattr(self, "_current_c_noise", 0.0)  # v287: stochastic c perturbation
            return torch.tensor(c, device=self.theta.device, dtype=self.theta.dtype)
        c_global = self.get_c()
        if not self.prefix_routing or prefix_emb is None:
            c = c_global + getattr(self, "_current_c_noise", 0.0)  # v287: stochastic c perturbation
            return c
