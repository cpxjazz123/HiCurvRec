...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        use_cosine_curvature: bool = False,  # v274: SGDR 余弦退火 (cos shape)
        c_cosine_min: float = 0.05,         # v274
        c_cosine_max: float = 0.7,          # v274
        c_cosine_period: int = 25_000,      # v274
    ) -> None:
        self._config = locals()
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    use_cosine_curvature=use_cosine_curvature,  # v274: SGDR 余弦退火
