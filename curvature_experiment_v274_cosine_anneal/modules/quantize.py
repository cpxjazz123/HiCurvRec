...
class Quantize(nn.Module):
    def __init__(
        ...
        use_cosine_curvature: bool = False,  # v274: SGDR 余弦退火 (cos shape vs cyclic |sin|)
        c_cosine_min: float = 0.05,         # v274: cosine c_min
        c_cosine_max: float = 0.7,          # v274: cosine c_max
        c_cosine_period: int = 25_000,      # v274: cosine 周期 T (训练 100k 步 ≈ 4 周期, SGDR 原文 Loshchilov & Hutter 2017)
        ...
        self.use_cosine_curvature = use_cosine_curvature
        self.c_cosine_min = float(c_cosine_min)
        self.c_cosine_max = float(c_cosine_max)
        self.c_cosine_period = int(c_cosine_period)
    ...
    def get_c(self) -> Tensor:
        ...
        if self.use_cosine_curvature:
            import math
            t = self._curriculum_step
            phase = math.pi * t / max(1, self.c_cosine_period)
            # SGDR 余弦退火 (Loshchilov & Hutter 2017): c(t) = c_min + 0.5*(c_max-c_min)*(1+cos(πt/T))
            # 与 v270 |sin| 不同: peak 在 t=0, trough 在 t=T (annealing); cos 比 |sin| 更平滑无尖峰.
            c = self.c_cosine_min + 0.5 * (self.c_cosine_max - self.c_cosine_min) * (1.0 + math.cos(phase))
            return torch.tensor(c, device=self.theta.device, dtype=self.theta.dtype)
