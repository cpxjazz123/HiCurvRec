...
USE_CYCLIC_CURVATURE = False              # v274 关闭 cyclic (用 cosine 替代)
C_CYCLIC_MIN = 0.05                        # 仍需定义 (RqVae() c_cyclic_min= 引用, 即使 USE_CYCLIC_CURVATURE=False)
C_CYCLIC_MAX = 0.7
C_CYCLIC_PERIOD = 25_000
USE_COSINE_CURVATURE = True               # v274 启用 SGDR 余弦退火 (Loshchilov & Hutter 2017)
C_COSINE_MIN = 0.05                       # v274: cosine c_min (与 v270 same)
C_COSINE_MAX = 0.7                        # v274: cosine c_max (与 v270 same)
C_COSINE_PERIOD = 25_000                  # v274: cosine 周期 T (训练 100k 步 ≈ 4 周期)
...
def main():
    ...
    model = RqVae(
        ...
        use_cosine_curvature=USE_COSINE_CURVATURE,  # v274: SGDR 余弦退火 (C28)
        c_cosine_min=C_COSINE_MIN,
        c_cosine_max=C_COSINE_MAX,
        c_cosine_period=C_COSINE_PERIOD,
    ).to(device)
