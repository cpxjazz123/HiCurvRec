...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v283 NOVEL: Hyperbolic Contrastive Commit Loss (R36n f 双曲几何损失)
        # 论文支撑: Nickel & Kiela 2017 "Poincaré Embeddings" (negative sampling on Poincaré ball) +
        #          Chami et al. ICML 2019 (Hyperbolic GCN with margin-based contrastive).
        # 机制: 在 commit loss 之外加 Poincaré distance margin contrastive loss.
        #   对每 (res, emb) anchor/positive pair, sample K negative codes from codebook,
        #   margin_loss = max(0, m + d_P(res, emb) - d_P(res, neg)).mean().
        # 与 v120 Mahalanobis (distance metric 替换) 区别: v283 是 loss-level 新增, 不改 distance metric.
        use_hyperbolic_contrastive: bool = False,  # v283: enable hyperbolic contrastive commit loss
        hyper_contrastive_margin: float = 0.5,     # v283: margin m (Poincaré ball radius ≈ 1.41 at c=1)
        hyper_contrastive_n_neg: int = 8,         # v283: K negative samples per anchor
        hyper_contrastive_weight: float = 0.05,   # v283: λ (vs commit_weight=1.0, 小权重)
    ) -> None:
        self._config = locals()
        ...
        self.use_hyperbolic_contrastive = bool(use_hyperbolic_contrastive)
        self.hyper_contrastive_margin = float(hyper_contrastive_margin)
        self.hyper_contrastive_n_neg = int(hyper_contrastive_n_neg)
        self.hyper_contrastive_weight = float(hyper_contrastive_weight)
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v283 NOVEL: Hyperbolic Contrastive Commit Loss (R36n f 双曲几何损失)
        # 论文: Nickel & Kiela 2017 "Poincaré Embeddings" (negative sampling on Poincaré ball)
        #       Chami et al. ICML 2019 (hyperbolic GCN with margin-based contrastive).
        # 机制: 对每层 (anchor = 选中 codeword, negative = K 随机其他 codebook entries),
        #   强制 d_P(anchor, neg) > m, 防止 codebook collapse 并推动双曲几何 spread.
        # 与 v120 Mahalanobis (distance metric 替换) 区别: v283 是 loss-level 新增.
        contrastive_loss_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.use_hyperbolic_contrastive:
            for li, layer in enumerate(self.layers):
                c_l = layer.get_c().view(-1, 1)  # (1,1) 当前层曲率
                # embs shape: (n_layers, D, B) — transpose to (B, D)
                emb_l = quantized.embeddings[li].transpose(0, 1).contiguous()  # (B, D)
                # 随机 K_neg negative samples from codebook
                codebook_w = layer.embedding.weight  # (K, D)
                K_code = codebook_w.shape[0]
                neg_idx = torch.randint(
                    0, K_code,
                    (emb_l.shape[0], self.hyper_contrastive_n_neg),
                    device=emb_l.device,
                )
                neg_emb = codebook_w[neg_idx]  # (B, K_neg, D)
                # d_P(anchor, neg_emb): 选中 codeword 离 negative 应远 > m
                d_neg = _poincare_distance_t(
                    emb_l.unsqueeze(1), neg_emb, c_l
                )  # (B, K_neg)
                # margin loss: relu(m - d_neg), 强制 d_neg > m
                margin_loss_l = F.relu(self.hyper_contrastive_margin - d_neg).mean()
                contrastive_loss_total = contrastive_loss_total + margin_loss_l
            contrastive_loss_total = contrastive_loss_total / len(self.layers)
        loss = (reconstuction_loss + rqvae_loss).mean() + self.margin_reg_weight * margin_loss + self.spread_loss_weight * spread_loss_total + self.hyper_contrastive_weight * contrastive_loss_total
