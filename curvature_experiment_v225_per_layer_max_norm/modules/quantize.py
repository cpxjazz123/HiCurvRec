...
class Quantize(nn.Module):
    def __init__(
        ...
        # 攻击 v213-v221 DDP L0 极早期坍缩通路: 即使 encoder 想把 codeword 拉出球, retraction 拉回.
        # 论文支撑: "Optimization on Manifolds" (Absil et al. 2008) retraction 概念
        #          + Poincaré Embeddings (Nickel & Kiela 2017) ball 约束
    ...
    @torch.no_grad()
    def retract_codebook(self) -> None:
        """v222: Riemannian Codebook Retraction (Euclidean projection to ball).

        每步 optimizer.step() 后调用, 强制 codebook 元素 norm ≤ (1/sqrt(c)) * ratio.
        这是 Euclidean retraction to ball (Absil et al. 2008): 最简的几何硬约束.
        攻击 v213-v221 DDP L0 极早期坍缩: 即使 encoder 想把 codeword 拉出球, retraction 拉回.

        用 Euclidean projection 而非 logmap+expmap 是因为:
        1. Numerical stability: logmap 在 ball 边缘数值不稳
        2. 计算效率: O(K*D) 比 logmap+expmap 少 1 个 norm 计算
        3. 数学上对 ball 内的 hard constraint 已经足够 (不需要 Riemannian metric 精确)
        """
        if not self.use_codebook_retraction:
            return
        c = self.get_c()
        ...
        scale = torch.where(
            norms > max_norm,
            max_norm / norms.clamp_min(1e-6),
            torch.ones_like(norms),
        )
        self.embedding.weight.data.mul_(scale)
