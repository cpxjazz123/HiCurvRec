...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def apply_retraction(self) -> None:
        """v222: 对所有 Quantize 层执行 Riemannian Codebook Retraction.
        训练脚本在每次 optimizer.step() 后调用 (与 DDP all-reduce 一致性兼容).
        """
        if not self.use_codebook_retraction:
            return
        for layer in self.layers:
            layer.retract_codebook()
