...
def train(
    ...
    # v84 R36c fix (复 v82/v83 经验): gin binding 失败 → hgrec_code_path fallback default = baseline v19 SIDs →
    # 训练端用错 SIDs → Stage 4 SIDs 不匹配 R@10=0. 强制覆盖为本实验 SIDS_NPY, 不依赖 gin binding
    from curvature_config import SIDS_NPY as _SIDS_NPY_V84
    hgrec_code_path = _SIDS_NPY_V84
