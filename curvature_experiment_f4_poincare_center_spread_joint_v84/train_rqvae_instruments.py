...
SPREAD_LOSS_WEIGHT = 0.001       # F3 v83: 正则权重 (低权重, 仅作引导, 不主导 recon loss)
SPREAD_LOSS_MARGIN = 2.0         # F3 v83: pairwise 距离阈值 (Poincaré 单位)
# === F2 v82 + F4 v84: Poincaré Center Loss — codebook 元素到原点距离 mean (compact) ===
# 推 codebook 向 Poincaré ball 原点 (norm → 0, 欧氏极限)
# 论文支撑: Poincaré Embeddings (Nickel & Kiela 2017) Eq.4 + Hyperbolic Embedding for Recommendation zero-norm prior
# F4 v84 双 loss 联合: center (compact) + spread (separate) 平衡 → 平衡点 = 某中间半径
USE_CENTER_LOSS = True           # F2/F4 v82/v84: enable center loss
CENTER_LOSS_WEIGHT = 0.001       # F4 v84: 与 spread 同权重 (双 loss 平衡), 低权重不主导 recon
# v84 双 loss 联合预测: spread 推分离, center 推向心 → 平衡点应在 ball 中段 (norm~0.5-0.8),
# 既不塌缩到原点 (collapse), 也不冲到边界 (boundary collapse). Stage 3 T5 期望获得更可分的 SID 表征.
...
def main():
    ...
    model = RqVae(
        ...
        center_loss_weight=CENTER_LOSS_WEIGHT if USE_CENTER_LOSS else 0.0,  # F2/F4 v82/v84: Center Loss 联合
