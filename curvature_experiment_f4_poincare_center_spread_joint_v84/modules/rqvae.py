...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # F2 v82 + F4 v84: Center Loss — 推 codebook 到 Poincaré 原点 (compact)
        # 论文支撑: Poincaré Embeddings (Nickel & Kiela 2017) Eq.4 + Hyperbolic Embedding for Recommendation
        center_loss_weight: float = 0.0,    # F2/F4 v82/v84: 0=关闭; 0.001 推荐值
        # F4 v84 联合机制: spread + center 同时作用 (compact + separate 平衡)
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    use_center_loss=center_loss_weight > 0,  # F2/F4 v82/v84: Center Loss 开关
                )
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # F2 v82 + F4 v84: Center Loss — 累加所有 Quantize 层 center_loss, 加权到 total loss
        # 与 spread_loss 互为反向: compact (向心) vs separate (离心). v84 双 loss 同时开启.
        center_loss_total = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.center_loss_weight > 0:
            center_per_layer = [
                l._last_center_loss for l in self.layers if hasattr(l, "_last_center_loss")
            ]
            if center_per_layer:
                center_loss_total = torch.stack(center_per_layer).mean()
        loss = (reconstuction_loss + rqvae_loss).mean() + self.margin_reg_weight * margin_loss + self.spread_loss_weight * spread_loss_total + self.center_loss_weight * center_loss_total
