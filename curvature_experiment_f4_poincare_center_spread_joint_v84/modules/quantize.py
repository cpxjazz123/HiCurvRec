...
class QuantizeOutput(NamedTuple):
    ...
    center_loss: Tensor  # F2 v82 + F4 v84: codebook 元素到原点 (Poincaré ball origin) 的距离 mean; 推 codebook 到原点, 与 spread 形成对比/互补
...
class Quantize(nn.Module):
    def __init__(
        ...
        # F2 v82 + F4 v84: Poincaré Center Loss — 推 codebook 到原点 (compact)
        # 论文支撑: Poincaré Embeddings (Nickel & Kiela 2017, NeurIPS) Eq.4,
        #          "Hyperbolic Embedding for Recommendation" 提到 zero-norm prior 防止边界 collapse.
        # 机制: L_center_l = mean_i poincare_dist(codebook_l[i], 0) = mean_i (2/sqrt(c)) · atanh(sqrt(c)·||codebook_l[i]||)
        #       推 codebook 元素 norm → 0 (欧氏极限), 即向 ball 原点 compact.
        use_center_loss: bool = False,     # F2/F4 v82/v84: enable center loss
    ) -> None:
        super().__init__()
        ...
        # F2 v82 + F4 v84: Center Loss 配置
        self.use_center_loss = bool(use_center_loss)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # F2 v82 + F4 v84: Center Loss — codebook 元素到 Poincaré 原点的距离 mean (compact)
        # L_center_l = mean_i poincare_dist(codebook_l[i], 0) = mean_i (2/sqrt(c)) · atanh(sqrt(c)·||codebook_l[i]||)
        # 推 codebook 元素 norm → 0 (球心), 与 spread 形成 compact↔separate 对比.
        # v84 双 loss 联合: spread 推分离, center 推向心 → 平衡点 = 某中间半径, 既不塌缩也不 boundary collapse
        center_loss = torch.zeros((), device=emb_out.device, dtype=emb_out.dtype)
        if self.use_center_loss and self.training and not self.hypervq:
            from modules.hyperbolic import _expmap0_t, _poincare_distance_t
            c = self.get_c()
            c_scalar = float(c.item()) if c.dim() == 0 else float(c.mean().item())
            c_t = torch.tensor(c_scalar, device=codebook.device, dtype=codebook.dtype)
            K = codebook.shape[0]
            # codebook 元素推到 Poincaré 球, 然后算到原点距离
            cb_h = _expmap0_t(codebook, c_t)  # (K, D)
            origin = torch.zeros_like(cb_h[:1])  # (1, D) 原点
            d_origin = _poincare_distance_t(
                cb_h.unsqueeze(1),  # (K, 1, D)
                origin.unsqueeze(0).expand(K, -1, -1),  # (K, 1, D)
                c_t,
            ).squeeze(-1).squeeze(-1)  # (K,)
            center_loss = d_origin.mean()
            self._last_center_loss = center_loss.detach()
        else:
            self._last_center_loss = center_loss.detach()

        return QuantizeOutput(embeddings=emb_out, ids=ids, loss=loss, margin=margin, spread_loss=spread_loss, center_loss=center_loss)
