...
class RqVae(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, batch: SeqBatch, gumbel_t: float) -> RqVaeComputedLosses:
        ...
        # v75 SimCIT-style contrastive loss (per-layer hyperbolic InfoNCE)
        # 论文: arXiv:2506.16683 — τ=0.1, batch=256 (我们 batch=2560), pure contrastive
        # 本实现: reconstruction + α·contrastive (α=0.1) — 比 pure contrastive 更安全,避免 cascade
        contrastive_loss_val = self._compute_simcit_contrastive(quantized, x)
        # Round 1: α=0.01 (vs Round 0 α=0.1) — 10x 更温和,避免 cascade
        loss = (reconstuction_loss + rqvae_loss).mean() + self.margin_reg_weight * margin_loss + 0.01 * contrastive_loss_val
    ...
    def _compute_simcit_contrastive(self, quantized: "RqVaeOutput", x: Tensor) -> Tensor:
        """v75 Round 1: SimCIT-style contrastive loss (arXiv:2506.16683) — cosine variant.

        关键修改 (vs Round 0):
        - 改用 cosine 距离 (SimCIT 论文原版用 cosine, 避免 arccosh 数值风险)
        - project_to_ball eps=1e-3 → eps=0.1 (norm≤0.9, 远离 singularity)
        - clamp 1-||x||² ≥ 1e-2 (避免 division by ~0)
        - 只对 L0 应用 (避免 3x cascade gradient pressure; Round 0 cascade 根因)
        - α 在 forward() 中改为 0.01 (10x 更温和)

        SimCIT InfoNCE:
          L = -Σ_b log( exp(s_b·c_b⁺/τ) / Σ_k exp(s_b·c_k/τ) )
        其中 s_b = cos(z_e, c_b) (similarity 而非 distance)

        兼容: hyperbolic_distance=False 时回退到 L2 (Euclidean fallback)
        """
        tau = 0.1
        z_e = self.encode(x)  # (B, D) — encoder output as query
        B = z_e.shape[0]
        if B == 0:
            return torch.tensor(0.0, device=z_e.device, dtype=z_e.dtype)

        # 只对 L0 (第一层) 应用 SimCIT,避免 cascade 3x gradient pressure
        # L1/L2 由 RQ-VAE 自身 VQ loss 处理
        li = 0
        layer = self.layers[li]
        codebook = layer.embedding.weight  # (K, D)
        sem_ids_li = quantized.sem_ids[:, li]  # (B,)

        if self.hyperbolic_distance:
            # Cosine variant: project to ball, compute cosine sim
            # eps=0.1 (norm ≤ 0.9), 远离 1-||x||² singularity
            def project_to_ball(x, eps=0.1):
                norm = x.norm(dim=-1, keepdim=True).clamp_min(1e-6)
                max_norm = 1.0 - eps
                return x * torch.clamp(max_norm / norm, max=1.0)

            q = project_to_ball(z_e)        # (B, D), norm ≤ 0.9
            k_all = project_to_ball(codebook)  # (K, D), norm ≤ 0.9

            # L2 normalize for cosine similarity
            q_n = F.normalize(q, dim=-1, eps=1e-6)
            k_n = F.normalize(k_all, dim=-1, eps=1e-6)

            # Similarity matrix (B, K) — bounded in [-1, 1]
            sim = q_n @ k_n.T  # (B, K)

            # InfoNCE: -log(exp(sim_pos/τ) / Σ_k exp(sim_k/τ))
            # paper-style with similarity (not -distance)
            logits = sim / tau
            log_prob = F.log_softmax(logits, dim=1)  # (B, K)
            loss_li = -log_prob[torch.arange(B, device=z_e.device), sem_ids_li].mean()
        else:
            # Euclidean fallback: cosine of raw vectors (no projection)
            q_n = F.normalize(z_e, dim=-1, eps=1e-6)
            k_n = F.normalize(codebook, dim=-1, eps=1e-6)
            sim = q_n @ k_n.T  # (B, K)
            logits = sim / tau
