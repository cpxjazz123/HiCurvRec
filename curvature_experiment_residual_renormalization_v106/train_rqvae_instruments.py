...
# === v106 ReSQ Residual Re-normalization — 根治 cascade collapse ===
# R36c v89/v103 诊断实证: cascade RQ-VAE L0 codes 24-32%, residual norm ≈0.14
# 在每层 Quantize 之前 normalize+rescale res 到固定 radius=0.5
# R36 合规: (n) 几何变换; 论文支撑: "Normalized Quantization" (2024 RecSys)
USE_RESNORM = True              # v106: 启用 Residual Re-normalization
RESNORM_RADIUS = 0.5            # v106: target norm (Poincaré 单位, 球半径 1.0 的 50%)
...
def main():
    ...
    model = RqVae(
        ...
        use_resnorm=USE_RESNORM,            # v106 ReSQ: Residual Re-normalization
        resnorm_radius=RESNORM_RADIUS,      # v106 ReSQ: target norm
    ).to(device)
