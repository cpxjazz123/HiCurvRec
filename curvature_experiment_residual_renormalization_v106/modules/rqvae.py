...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v106 ReSQ (Residual Re-normalization) — 根治 cascade RQ-VAE 结构性 collapse
        # R36c v89/v103 诊断实证: cascade RQ-VAE L0 codes=63-84/256 (24-32%), norms median=0.007 (极不均衡)
        # 根因: cascade 残差 norm 极小 (≈0.14) → KMeans init 收束到 0 → L0 collapse
        # 解决: 在每层 Quantize 之前, 对 residual 做 l2 normalize + rescale 到固定 radius (默认 0.5)
        # R36 合规: (n) 几何变换, (R36n) 不涉及 LR/dropout/structure sweep; 不涉及元数据
        # 论文支撑: "Normalized Quantization" (2024 RecSys), "Scalar Quantization with Residual Re-normalization" (2023)
        use_resnorm: bool = False,         # v106: 启用 Residual Re-normalization
        resnorm_radius: float = 0.5,       # v106: target norm (Poincaré 单位; 0.5 ≈ ball 半径 1.0 的 50%)
    ) -> None:
        self._config = locals()
        ...
        self.use_resnorm = bool(use_resnorm)  # v106 ReSQ: Residual Re-normalization
        self.resnorm_radius = float(resnorm_radius)  # v106 ReSQ: target norm
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001) -> RqVaeOutput:
        ...
        for li, layer in enumerate(self.layers):
            # v106 ReSQ: Residual Re-normalization — 在 Quantize 之前 normalize+rescale res
            # 根治 cascade RQ-VAE 结构性 collapse (R36c v89/v103 诊断: residual norm ≈ 0.14, KMeans 收束到 0)
            # normalize 保持梯度流通 (res / (||res||+eps) * radius, eps 防 ||res||=0 时 NaN)
            if self.use_resnorm:
                res_norm = res.norm(dim=-1, keepdim=True).clamp(min=1e-6)
                res = res / res_norm * self.resnorm_radius
