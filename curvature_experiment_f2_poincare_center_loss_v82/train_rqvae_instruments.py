...
# v82: F2 Poincaré Center Loss — Stage 1 端 loss 函数变化 (R36n 双曲几何损失合规)
# 不动 manifold 几何, 不依赖异质 manifold → R36g 升级不适用.
# L_center = mean( poincare_norm(codebook[k])² ) × weight
# weight=0.01 (低权重, 仅作为正则引导, 不主导 recon loss).
USE_CENTER_LOSS = True                    # v82: 启用 F2 Poincaré Center Loss
CENTER_LOSS_WEIGHT = 0.01                 # v82: center loss 权重 (低, 仅作正则)
CENTER_LOSS_C = 1.0                       # v82: center loss 曲率 (与 baseline c=1.0 一致)
