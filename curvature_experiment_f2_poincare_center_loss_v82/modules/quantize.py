...
class Quantize(nn.Module):
    def __init__(
        ...
        # v82: F2 Poincaré Center Loss (Stage 1 端 loss 函数变化, 非 manifold 异质)
        # 论文支撑: Poincaré Embeddings (Nickel & Kiela 2017) norm-based repulsion +
        #          Center Loss (Wen et al. ECCV 2016) 鼓励类内紧凑.
        # L_center = mean( poincare_norm(codebook[k])² )
        #   poincare_norm(x; c) = (1/sqrt(c)) * atanh(sqrt(c) * ||x||)
        # 不依赖异质 manifold → R36g 升级不适用; 不动 model 架构.
        use_center_loss: bool = False,
        center_loss_weight: float = 0.01,
        center_loss_c: float = 1.0,
    ) -> None:
        super().__init__()
        ...
        # v82: F2 Poincaré Center Loss
        self.use_center_loss = use_center_loss
        self.center_loss_weight = float(center_loss_weight)
        self.center_loss_c = float(center_loss_c)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        # v82: F2 Poincaré Center Loss — 鼓励 codebook 在 ball 内紧凑
        # poincare_norm(x; c) = (1/sqrt(c)) * atanh(sqrt(c) * ||x||)
        # L_center = mean( poincare_norm(codebook[k])² ) — R36n 双曲几何损失合规
        if self.use_center_loss and self.training:
            from modules.hyperbolic import _artanh
            cb = self.embedding.weight  # (K, D)
            sqrt_c = self.center_loss_c ** 0.5
            cb_norm = cb.norm(dim=-1).clamp(max=(1.0 / sqrt_c - 1e-5))
            mobius_norms = (1.0 / sqrt_c) * _artanh(sqrt_c * cb_norm)
            center_loss = (mobius_norms ** 2).mean()
