...
from curvature_config import USE_HGREC_ARCH, SAVE_DIR_ROOT as _SAVE_DIR_ROOT_DEFAULT, V102_INJECTION_MODE as _V102_INJECTION_MODE_CFG
...
# === v102: Curvature-Aware SID Embedding (Stage 3 端, R36 v3.6 explicit relaxation 一次) ===
# 冻结 Stage 1 (v87 baseline ckpt), 在 T5 input embedding 端注入 3-mode 几何信号.
# V102_INJECTION_MODE 从 curvature_config.py 导入 (3-mode 切换), 默认 ablation run 用 40 epoch 短程.
V102_INJECTION_MODE = _V102_INJECTION_MODE_CFG
V102_STAGE1_CKPT_PATH = "/home/wlia0047/ar57/wenyu/GeneRec/curvature_base/out/rqvae/instruments/rqvae_final.pt"
V102_CODEWORD_DIM = 32
V102_CODEBOOK_SIZE = 256
V102_N_CODEBOOK_LAYERS = 3
V102_GATE_INIT_LOGIT = -2.94  # σ(-2.94) ≈ 0.05
V102_TOKEN_OFFSETS = [1, 257, 513]  # PAD=0, L0:1-256, L1:257-512, L2:513-768


def _v102_load_stage1_codebooks_curvatures(ckpt_path: str, n_layers: int):
    """从 Stage 1 RQ-VAE ckpt 提取 codebooks (n_layers, K, d) + curvatures (n_layers,).

    假设 Stage 1 ckpt 结构: sd['model']['layers.{l}.embedding.weight'](K, d) + sd['model']['layers.{l}.theta']
    曲率 c = C_MIN + (C_MAX - C_MIN) * sigmoid(theta), C_MIN=0.3, C_MAX=2.0
    """
    sd = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    model_sd = sd["model"] if isinstance(sd, dict) and "model" in sd else sd

    codebooks = []
    curvatures = []
    for l in range(n_layers):
        cb_key = f"layers.{l}.embedding.weight"
        theta_key = f"layers.{l}.theta"
        if cb_key not in model_sd:
            raise KeyError(f"Cannot find codebook key {cb_key!r} in Stage 1 ckpt (sample keys: {list(model_sd.keys())[:6]})")
        cb = model_sd[cb_key].detach().cpu().float()
        codebooks.append(cb)
        # 曲率: 若 theta 存在则用 sigmoid 反推, 否则 fallback = 1.0
        if theta_key in model_sd:
            theta = float(model_sd[theta_key].detach().cpu().item())
            c_l = 0.3 + 1.7 / (1.0 + math.exp(-theta))  # sigmoid
        else:
            c_l = 1.0
        curvatures.append(c_l)
    codebooks_t = torch.stack(codebooks, dim=0)  # (n_layers, K, d)
    curvatures_t = torch.tensor(curvatures, dtype=torch.float32)
    return codebooks_t, curvatures_t
...
def _train_hgrec(
    ...
    hgrec_config = {
        ...
        "injection_mode": V102_INJECTION_MODE,
        "n_codebook_layers": V102_N_CODEBOOK_LAYERS,
        "codebook_size": V102_CODEBOOK_SIZE,
        "codeword_dim": V102_CODEWORD_DIM,
        "token_offsets": V102_TOKEN_OFFSETS,
        "gate_init_logit": V102_GATE_INIT_LOGIT,
    }
    if V102_INJECTION_MODE != "standard":
        # 冻结 Stage 1 (v87 baseline), 仅取 codebook + curvature 几何信号
        v102_codebooks, v102_curvatures = _v102_load_stage1_codebooks_curvatures(
            V102_STAGE1_CKPT_PATH, V102_N_CODEBOOK_LAYERS,
        )
        hgrec_config["stage1_codebooks"] = v102_codebooks
        hgrec_config["stage1_curvatures"] = v102_curvatures
        if accelerator.is_main_process:
            print(
                f"[v102 injection] mode={V102_INJECTION_MODE} "
                f"codebooks shape={tuple(v102_codebooks.shape)} "
                f"curvatures={v102_curvatures.tolist()} "
                f"gate_init=σ({V102_GATE_INIT_LOGIT:.2f})={1/(1+math.exp(-V102_GATE_INIT_LOGIT)):.3f}",
                flush=True,
            )
    else:
        if accelerator.is_main_process:
            print(f"[v102 injection] mode=standard (baseline T5, 无注入)", flush=True)
    ...
    # v102 ablation 阶段用短程预算 (~20% of full = 40 epoch, EARLY_STOP=15) 快速判断 3 组趋势;
    # 若 v102-C > v102-E > standard, 再切回 MAX_EPOCHS=200/EARLY_STOP=20 跑完整 pipeline.
    V102_SHORT_RUN = True
    if V102_SHORT_RUN:
        MAX_EPOCHS = 40
    ...
    # v102 short-run ablation 阶段: 跳过 test eval, 仅对比 valid NDCG@10 趋势; 选定 winner 后再跑 full pipeline
    if V102_SHORT_RUN:
        if accelerator.is_main_process:
            print(f"\n=== v102 SHORT_RUN 跳过 test eval (只看 valid NDCG@10 趋势) ===", flush=True)
            # 保存 best ckpt epoch 摘要
            out_json = _osp.join(save_dir_root, "ablation_summary.json")
            with open(out_json, "w") as f:
                _json.dump({
                    "v102_injection_mode": V102_INJECTION_MODE,
                    "best_ckpt_epoch": int(best_ckpt["epoch"]) if 'best_ckpt' in dir() else -1,
                    "best_ckpt_valid_ndcg10": float(best_metric),
                    "short_run_max_epochs": MAX_EPOCHS,
                    "short_run_early_stop": EARLY_STOP_PATIENCE,
                }, f, indent=2)
