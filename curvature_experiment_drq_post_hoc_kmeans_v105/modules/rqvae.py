...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # Stage 1a continuous VAE 模式 — 跳过所有 Quantize layer, 不更新 codebook,
        # encoder/decoder 仅受 reconstruction loss 驱动 (Poincaré distance², c=1).
        # Stage 1b post-hoc K-Means cascade 在 frozen latent (B, D=32) 上跑
        # (R41c random_state=42, sklearn KMeans).
        # 合规: R36n (manifold geometry change), R36h ceiling 验证是否被打破
        # (训练方法论变更而非几何变更, 新维度).
        skip_quantization: bool = False,   # v105: 跳过 RQ-VAE 量化层 (Stage 1a continuous VAE)
    ) -> None:
        self._config = locals()
        ...
        self.skip_quantization = bool(skip_quantization)  # v105 DRQ: Stage 1a continuous VAE
    ...
    def get_semantic_ids(self, x: Tensor, gumbel_t: float = 0.001) -> RqVaeOutput:
        ...
        # v105 DRQ Stage 1a: continuous VAE — 跳过所有 Quantize layer
        # 直接 encoder → 构造 dummy RqVaeOutput (embs (B,D,1), 0 quantize_loss, dummy sem_ids)
        # forward 后续 decode(embs.sum(-1)) = decode(encoder_output), 零 quantize_loss
        if self.skip_quantization:
            z = self.encode(x)  # (B, D)
            z_3d = z.unsqueeze(-1)  # (B, D, 1) 模拟 n_layers=1
            B = z.shape[0]
            # dummy sem_ids: 全 0 (n_layers=3 列) — Stage 1a 不需要真实 SID
            dummy_sids = torch.zeros(B, self.n_layers, dtype=torch.long, device=z.device)
            return RqVaeOutput(
                embeddings=z_3d,
                residuals=z_3d,        # residuals = embs (Stage 1a 无 residual 概念)
                sem_ids=dummy_sids,
                quantize_loss=torch.zeros((), device=z.device, dtype=z.dtype),
                margins=[torch.zeros(B, device=z.device, dtype=z.dtype)] * self.n_layers,
            )
