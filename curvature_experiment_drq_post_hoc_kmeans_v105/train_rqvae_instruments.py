...
# === v105 DRQ Stage 1a: continuous VAE (跳过 RQ-VAE 量化层) ===
# 论文支撑: arxiv 2606.01844 (Shopee 2026) — Decoupled Residual Quantization
# Stage 1a: encoder/decoder 只受 reconstruction loss 驱动 (Poincaré distance², c=1)
# Stage 1b: 在 frozen latent (B, D=32) 上跑 sklearn KMeans cascade (R41c random_state=42)
USE_SKIP_QUANTIZATION = True   # v105: 跳过 Quantize layer, encoder/decoder only
...
def main():
    ...
    model = RqVae(
        ...
        skip_quantization=USE_SKIP_QUANTIZATION,  # v105 DRQ Stage 1a: continuous VAE
    ).to(device)
