...
OUT_DIR = "/home/wlia0047/hj82_scratch2/wenyu/rqvae_dataset/instruments/rqvae_out_v67_b1_per_layer"
...
def main():
    ...
    # === R51+ 6 项约束 (Stage 1 RQ-VAE 训练端, v67 沿用 v65 同口径) ===
    import os as _os
    ...
    def _v67_worker_init_fn(wid):
        import numpy as _np
        import random as _random
        _np.random.seed(SEED + wid)
        _random.seed(SEED + wid)

    if rank == 0:
        os.makedirs(OUT_DIR, exist_ok=True)
        print(f"=== RQ-VAE Musical_Instruments DDP (world_size={world_size}) ===", flush=True)
        print(f"[R51+ seed] Stage 1 RQ-VAE (rank={rank}): PYTHONHASHSEED/CUBLAS/cudnn/float32_matmul_precision deterministic", flush=True)
    ...
    train_loader = DataLoader(
        ...
        worker_init_fn=_v67_worker_init_fn,
    )
    ...
    # === v67 B1 Per-Layer Schedule: 启用 per-layer 独立 schedule (R36b 满足, c 训练中动态变化) ===
    # L0: cosine cyclic [0.05, 0.7]  (与 v65 D1 同款, 周期循环)
    # L1: linear warmup [0.5, 1.0]  (单调增, 训练全程动态)
    # L2: step decay [1.0 → 0.7]   (单调减, step=curriculum_steps/2 时切换)
    from modules.quantize_v67_config import (
        V67_B1_ENABLED, V67_B1_C_START_PER_LAYER, V67_B1_C_END_PER_LAYER,
        V67_B1_SCHEDULE_PER_LAYER, V67_B1_STEP_DECAY_HALF,
    )
    if V67_B1_ENABLED:
        for li, q in enumerate(model.layers):
            q.enable_v67_b1(
                layer_idx=li,
                c_start=V67_B1_C_START_PER_LAYER[li],
                c_end=V67_B1_C_END_PER_LAYER[li],
                schedule_type=V67_B1_SCHEDULE_PER_LAYER[li],
                step_decay_half=V67_B1_STEP_DECAY_HALF,
            )
        if rank == 0:
            print(f"[v67 B1] enabled per-layer schedule:", flush=True)
            for li in range(N_LAYERS):
                print(f"  L{li}: {V67_B1_SCHEDULE_PER_LAYER[li]} c_start={V67_B1_C_START_PER_LAYER[li]} c_end={V67_B1_C_END_PER_LAYER[li]}", flush=True)
