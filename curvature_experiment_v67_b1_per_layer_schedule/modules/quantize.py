...
class Quantize(nn.Module):
    def __init__(
        ...
        # v67 B1: Per-Layer Schedule (default 未启用, 由 train 端 enable_v67_b1() 启用)
        # 3 层独立 schedule: L0 cosine_cyclic, L1 linear_warmup, L2 step_decay (见 quantize_v67_config)
        self._v67_b1_enabled = False
        self._v67_b1_layer_idx = -1
        self._v67_b1_c_start = None
        self._v67_b1_c_end = None
        self._v67_b1_schedule_type = None
        self._v67_b1_step_decay_half = 0.5
    ...
    def get_c(self) -> Tensor:
        ...
        v67 B1: Per-Layer Schedule — 优先于 C27, 按 _v67_b1_layer_idx 选对应 schedule.
        """
        if self._v67_b1_enabled:
            import math as _math
            step = self._curriculum_step
            cycle_steps = max(1, self.curriculum_steps)
            t = min(1.0, step / cycle_steps)
            s_type = self._v67_b1_schedule_type
            cs = self._v67_b1_c_start
            ce = self._v67_b1_c_end
            if s_type == "cosine_cyclic":
                # 与 v65 D1 一致: cosine warm restart, 但 v67 周期 = cycle_steps (无嵌套 cycle)
                cycles = step // cycle_steps
                t_local = (step % cycle_steps) / cycle_steps
                c = cs + (ce - cs) * 0.5 * (1.0 + _math.cos(_math.pi * t_local))
                _ = cycles
            elif s_type == "linear_warmup":
                c = cs + (ce - cs) * t
            elif s_type == "step_decay":
                # step_decay: t < half → c_start (high c), t >= half → c_end (low c)
                # 本例 c_start=1.0, c_end=0.7: 训练前半 c=1.0, 后半 c=0.7
                half = self._v67_b1_step_decay_half
                if t < half:
                    c = cs
                else:
                    # 平滑过渡: t=half→c_start, t=1→c_end (线性插值)
                    c = cs + (ce - cs) * (t - half) / max(1e-9, 1.0 - half)
            else:
                raise ValueError(f"v67 B1 unknown schedule type: {s_type}")
            return torch.tensor(float(c), device=self.theta.device, dtype=self.theta.dtype)
    ...
    def enable_v67_b1(self, layer_idx: int, c_start: float, c_end: float,
                      schedule_type: str, step_decay_half: float = 0.5) -> None:
        """v67 B1 Per-Layer Schedule: 启用 per-layer 独立曲率 schedule.
        Args:
            layer_idx: 该层在 cascade RQ-VAE 中的索引 (0/1/2)
            c_start: 该层 schedule 起点 c (per-layer 不同)
            c_end: 该层 schedule 终点 c (per-layer 不同)
            schedule_type: "cosine_cyclic" / "linear_warmup" / "step_decay"
            step_decay_half: step_decay 切换点 t (0~1), 默认 0.5
        启用后, get_c() 优先返回 per-layer schedule c (覆盖 C27 + C26).
        """
        self._v67_b1_enabled = True
        self._v67_b1_layer_idx = int(layer_idx)
        self._v67_b1_c_start = float(c_start)
        self._v67_b1_c_end = float(c_end)
        self._v67_b1_schedule_type = str(schedule_type)
        self._v67_b1_step_decay_half = float(step_decay_half)
    ...
    def get_c_per_item(self, prefix_emb: Optional[Tensor] = None) -> Tensor:
        ...
        v67 B1: Per-Layer Schedule — get_c_per_item 也走 B1 (B1 是 schedule, per-item 路由失效, 与 C27 同模式).
        """
        if self._v67_b1_enabled:
            # B1 是 schedule, 不依赖 prefix_emb; 返回全局 schedule c (与 C27 一致)
            return self.get_c()
        if self.use_curriculum_curvature:
            t = min(1.0, self._curriculum_step / max(1, self.curriculum_steps))
            c = self.c_start + (self.c_end - self.c_start) * t
