...
class ResidualQuantization(LightningModule):
    ...
    def _instantiate_quantization_layer_list(
        ...
        if quantization_layer_list is not None:
            # Ensure sub-Modules are registered with the parent Module.
            # nn.ModuleList triggers parameter auto-registration; a plain Python
            # list (or OmegaConf ListConfig) does not, so model.parameters()
            # would otherwise return empty.
            return nn.ModuleList(list(quantization_layer_list))
        else:
            if n_layers is None:
                raise ValueError(
    ...
    def forward(
        ...
        # Stash the latest cluster_ids for external callbacks (e.g. Dead Code Revival).
        # Detached to avoid leaking computation graph into long-lived buffers.
        self.last_cluster_ids = cluster_ids.detach()
        self.last_embeddings = embeddings.detach()
        # Stash per-layer residual-input pool for precise Revival replacement:
        # layer_idx=0 receives `embeddings`; subsequent layers receive
        # `embeddings - sum_{i<l} centroid[cluster_id_i]`. By stashing these
        # per-layer inputs, the Revival callback can replace dead centroids with
        # samples from the *correct* subspace instead of always using the encoder
        # output. Without this, layer 1/2 centroids get reset with magnitude that
        # does not match the residual subspace, harming quantization.
        if self.track_residuals and all_residuals is not None:
            # `all_residuals` is stacked with `dim=-1`, so its shape is
            # `(batch_size, n_features, n_layers)`. To get the residual AFTER
            # layer k was subtracted (i.e. shape `(batch_size, n_features)`),
            # we must index the LAST dim: `all_residuals[:, :, k]`.
            # Indexing `all_residuals[k]` would incorrectly slice the BATCH dim.
            try:
                layer_inputs_pool = [embeddings]
                for k in range(len(self.quantization_layer_list) - 1):
                    # all_residuals[:, :, k] is residual AFTER layer k is subtracted.
                    layer_inputs_pool.append(all_residuals[:, :, k])
                self.last_layer_inputs = [
                    t.detach() for t in layer_inputs_pool
                ]
            except Exception:
                self.last_layer_inputs = [embeddings.detach()]
        else:
            # track_residuals disabled — fallback to encoder output for all layers.
            self.last_layer_inputs = [embeddings.detach()] * len(
                self.quantization_layer_list
            )
