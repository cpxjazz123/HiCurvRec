...
# v226: Sphere Anchor — 强制 codeword norm 落在 [min, max] 球壳内, 解 R36p v3.10 utility ≥ 90%
# 动机: v222-v225 retraction (max_norm clip) 4 轮 R36p FAIL, 因 L0 input norm ≈ 0.04 << max_norm=0.5
#       max_norm clip 不触发, codeword 永久卡在 norm 0.04 → KMeans 收束 → collapse.
# v226: 加入 min_norm 下限, 形成球壳 [min, max] 强制 push codeword 到几何流形.
# Per-layer 配置 (基于 v222-v225 单卡 smoke norm 实测):
#   L0 input norm ≈ 0.04 → min=0.3 max=0.5 (球壳宽度 0.2, 强制 spread)
#   L1 input norm ≈ 0.14 → min=0.2 max=0.3
#   L2 input norm ≈ 0.10 → min=0.15 max=0.25
USE_SPHERE_ANCHOR = True        # v226: 启用球壳投影
L0_ANCHOR_MIN, L0_ANCHOR_MAX = 0.3, 0.5
L1_ANCHOR_MIN, L1_ANCHOR_MAX = 0.2, 0.3
L2_ANCHOR_MIN, L2_ANCHOR_MAX = 0.15, 0.25
...
def main():
    ...
    model = RqVae(
        ...
        anchor_min_per_layer=[L0_ANCHOR_MIN, L1_ANCHOR_MIN, L2_ANCHOR_MIN],  # v226: per-layer min norm
        anchor_max_per_layer=[L0_ANCHOR_MAX, L1_ANCHOR_MAX, L2_ANCHOR_MAX],  # v226: per-layer max norm
    ).to(device)
    ...
    while not done:
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            # v226: 球壳投影 (强制 codeword norm ∈ [min, max], 解决 L0 collapse)
