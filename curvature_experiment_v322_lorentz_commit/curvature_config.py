...
# === 机制标识 (R52 + R53 联动) ===
MECHANISM_NAME = "lorentz_commit_v322"  # ← v322 NOVEL (R36n c 全新子方向): v316_fixed backbone (anisotropy_loss + spread_loss 真正反向传播) + Lorentz manifold commit path. 把 Stage 1 commit 路径从 Poincaré 距离 d_P=2/√c·artanh(√c·‖(-x)⊕y‖) 替换为 Lorentz distance d_L=(1/√c)·acosh(-c·⟨x,y⟩_L). 数值优势: acosh 而非 artanh, 无 norm 除法, 梯度更平滑, manifold constraint 自动满足 (无需 project to ball). 论文支撑: Law et al. NeurIPS 2019 "Lorentzian Distance Learning for Hyperbolic Representations". 比较 baseline = v316_fixed (test_R@10=0.11606). v286 product manifold 已 R37 FAIL, 但纯 Lorentz commit 没单独试过 — v322 是 R36n (c) Poincaré→Lorentz 直接替换.
