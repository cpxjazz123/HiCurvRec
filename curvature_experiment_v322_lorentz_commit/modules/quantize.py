...
class Quantize(nn.Module):
    def __init__(
        ...
        # v322 NOVEL: Lorentz manifold commit (R36n c 全新子方向).
        # 用 Lorentz distance d_L = (1/√c) acosh(-c·⟨x,y⟩_L) 替代 Poincaré distance.
        # 数值优势: acosh 而非 artanh (无除法 norm, 梯度更平滑, 不需 project to ball).
        # 论文: Law et al. NeurIPS 2019 "Lorentzian Distance Learning for Hyperbolic Representations".
        use_lorentz_commit: bool = False,
        ...
        self.use_lorentz_commit = use_lorentz_commit  # v322
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        if self.hypervq:
            ...
            if self.use_lorentz_commit:
                # v322 NOVEL: Lorentz manifold commit path (R36n c 全新子方向)
                # 替代 Poincaré 距离: 把 latent + codebook 都映射到 Lorentz L^D_c manifold,
                # 算 Lorentzian (Minkowski) 距离 d_L = (1/√c) acosh(-c·⟨x,y⟩_L).
                # 论文: Law et al. NeurIPS 2019 Lorentzian Distance Learning.
                # 数值优势: acosh 而非 artanh, 梯度平滑, 无需 project to ball (manifold constraint 自动).
                from modules.hyperbolic import (
                    _euclidean_to_lorentz_t, _lorentz_distance_t,
                )
                c = self.get_c_per_item(prefix_emb)  # (1,) 或 (B,)
                c_exp = c.view(-1, 1, 1)             # (1,1,1) 或 (B,1,1)
                B = x.shape[0]
                K = codebook.shape[0]
                # Euclidean → Lorentz (D → D+1)
                latent_l = _euclidean_to_lorentz_t(x.unsqueeze(1), c_exp)             # (B,1,D+1)
                cb_exp0 = codebook.unsqueeze(0).expand(B, K, -1)                    # (B,K,D)
                codebook_l = _euclidean_to_lorentz_t(cb_exp0, c_exp)                # (B,K,D+1)
                # Lorentzian 距离 (B,K,1) → squeeze (B,K)
                d_lorentz = _lorentz_distance_t(
                    latent_l.expand(B, K, -1), codebook_l, c_exp
                ).squeeze(-1)
                if self.use_mcdq:
                    # C23 兼容: MCDQ 混合距离 = (1-α)·d_L + α·d_E (Lorentz + Euclidean)
                    d_euclid = (
                    ...
                    dist = (1.0 - alpha) * d_l_norm + alpha * d_e_norm
                else:
                    dist = d_lorentz
            elif self.hyperbolic_distance:
        ...
        if self.sk_eps > 0 and (self.hyperbolic_distance or self.use_lorentz_commit):
            # HG-Rec 防坍缩: Sinkhorn-Knopp 均衡分配 (balanced assignment, 强制 code 均衡使用)
            # v322: 也支持 Lorentz distance (R36n c 兼容)
