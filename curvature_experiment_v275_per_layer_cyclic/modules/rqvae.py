...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v275 NOVEL: per-layer heterogeneous c_max 阶梯 [L0, L1, L2]
        # R36b per-layer 维度 + R36b cyclic 组合: 每层有自己的 c_max, 共享 c_min 和 period
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    c_cyclic_max_per_layer=c_cyclic_max_per_layer,  # v275: per-layer 异质 c_max 阶梯
                    layer_index=i,  # v275: 让 Quantize 知道自己在第几层 (per-layer 选 c_max 用)
                    c_fixed=c_fixed,
