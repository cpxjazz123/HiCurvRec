...
class Quantize(nn.Module):
    def __init__(
        ...
        c_cyclic_max: float = 0.7,          # v270: cyclic c_max (默认单一值; v275 用 per-layer 异质列表覆盖)
        c_cyclic_period: int = 25_000,      # v270: cyclic 周期 T (步数, 训练期 ~4 个完整周期)
        # v275 NOVEL: per-layer heterogeneous c_max. None=关闭, 否则按 layer index 选 [L0, L1, L2]
        c_cyclic_max_per_layer: Optional[List[float]] = None,  # v275: per-layer 异质 c_max 阶梯 [0.5, 0.6, 0.7]
        layer_index: int = 0,  # v275: RqVae 创建时传 i, Quantize 按 layer_index 选 c_max
        ...
        # v275 NOVEL: per-layer heterogeneous c_max 阶梯 (R36b per-layer 维度 + R36b cyclic 组合)
        # RqVae 创建时按 layer_index 传入 c_cyclic_max, 让 L0/L1/L2 的 c_max 各不相同
        self.c_cyclic_max_per_layer = c_cyclic_max_per_layer  # None=关闭 (走 c_cyclic_max), 否则用 list[index]
        self.layer_index = int(layer_index)  # RqVae 创建时设置
    ...
    def get_c(self) -> Tensor:
        ...
        if self.use_cyclic_curvature:
            ...
            # v275: per-layer 异质 c_max (R36b per-layer + cyclic 组合)
            if self.c_cyclic_max_per_layer is not None:
                c_max_l = float(self.c_cyclic_max_per_layer[min(self.layer_index, len(self.c_cyclic_max_per_layer) - 1)])
            else:
                c_max_l = self.c_cyclic_max
            c = self.c_cyclic_min + (c_max_l - self.c_cyclic_min) * abs(math.sin(phase))
    ...
    def get_c_per_item(self, prefix_emb: Optional[Tensor] = None) -> Tensor:
        ...
        v275: cyclic 模式下 c_max 按 layer_index 选自 per_layer 列表 (per-layer 异质).
        """
        if self.use_cyclic_curvature:
            import math
            t = self._curriculum_step
