...
MAX_GLOBAL_STEPS = 50_000        # v94 smoke: 50k steps (节省 GPU, parameterization 是结构性约束, 不需要 curriculum 完整 ramp)
...
USE_FIXED_CURVATURE = False                # v94: 关固定 c, per-layer learnable 替代
C_FIXED = 1.0                             # 固定曲率值 (HG-Rec)
# C27: Curriculum Curvature Schedule (ICML 2025)
USE_CURRICULUM_CURVATURE = False           # v94: 关 curriculum, per-layer learnable 替代
...
# === v94: Constrained Hyperbolic Codebook Parameterization ===
# 核心创新: 不允许 optimizer 把 codebook 拉回近欧氏区域.
# ρ_{ℓk} = RHO_MIN + (RHO_MAX - RHO_MIN) · σ(s_{ℓk}) 结构硬约束 ∈ [RHO_MIN, RHO_MAX]
# e_{ℓk} = (ρ_{ℓk} / √c_ℓ) · u_{ℓk} / |u_{ℓk}| (方向自由, ρ 锁定)
# c_ℓ = C_MIN + (C_MAX - C_MIN) · σ(c_param_ℓ) per-layer learnable
# R36n 合规 (per-item/per-layer 异质曲率). 不加 D, 不加 A loss (parameterization 替代)
USE_HARD_RADIAL_PARAM = True              # v94: 启用硬约束 codebook parameterization
RHO_MIN = 0.20                            # v94: codeword 距中心下限
RHO_MAX = 0.70                            # v94: codeword 距中心上限
USE_PER_LAYER_LEARNABLE_C = True          # v94: 每层 c 独立可学 (替代 curriculum)
C_MIN = 0.05                              # v94: per-layer c 下限 (近欧氏起点)
C_MAX = 2.0                               # v94: per-layer c 上限 (允许更激进双曲)
...
def main():
    ...
    model = RqVae(
        ...
        margin_reg_weight=0.0 if (USE_FIXED_CURVATURE or USE_HARD_RADIAL_PARAM) else MARGIN_REG_WEIGHT,  # v94: 关 C5
    ...
    # === v94: Constrained Hyperbolic Codebook Parameterization ===
    # 替换 layer.embedding 为 CodebookParam, ρ ∈ [RHO_MIN, RHO_MAX] 硬约束 + per-layer learnable c_ℓ
    if USE_HARD_RADIAL_PARAM:
        import types
        class CodebookParam(nn.Module):
            """v94 hard radial parameterization:

            e_{ℓk} = (ρ_{ℓk} / √c_ℓ) · u_{ℓk} / |u_{ℓk}|
            ρ_{ℓk} = RHO_MIN + (RHO_MAX - RHO_MIN) · σ(s_{ℓk}) ∈ [RHO_MIN, RHO_MAX]
            c_ℓ = C_MIN + (C_MAX - C_MIN) · σ(c_param_ℓ) ∈ [C_MIN, C_MAX]

            weight property: 每次访问重新合成 in-graph (梯度流通到 _s, _u, _c_param).
            forward(ids): 模拟 nn.Embedding(ids) 接口 (RqVae get_item_embeddings 用).
            get_c(): 返回 per-layer learnable c_ℓ.
            """
            def __init__(self, K, D, init_weight, RHO_MIN, RHO_MAX, C_MIN, C_MAX):
                super().__init__()
                self.K = K
                self.D = D
                self._s = nn.Parameter(torch.zeros(K))         # init σ(0)=0.5 → ρ=midpoint
                self._u = nn.Parameter(init_weight.clone())     # init direction = KMeans init
                self._c_param = nn.Parameter(torch.zeros(1))   # init σ(0)=0.5 → c=midpoint
            ...
            @property
            def weight(self):
                u_norm = F.normalize(self._u, dim=-1, eps=1e-12)
                rho = self._rho_min + (self._rho_max - self._rho_min) * torch.sigmoid(self._s)
                c = self._c_min + (self._c_max - self._c_min) * torch.sigmoid(self._c_param)
                return (rho.unsqueeze(-1) * u_norm) / (c ** 0.5)  # (K, D)
            ...
            def get_c(self):
                return self._c_min + (self._c_max - self._c_min) * torch.sigmoid(self._c_param)

        @torch.no_grad
        def v94_kmeans_init(self, x):
            """v94: KMeans init 写 _u 而非 weight (CodebookParam 路径)."""
            if isinstance(self.embedding, CodebookParam):
                kmeans_init_(self.embedding._u, x=x)
            else:
                kmeans_init_(self.embedding.weight, x=x)
            self.kmeans_initted = True

        def v94_get_c(self):
            return self.embedding.get_c() if isinstance(self.embedding, CodebookParam) else self.theta

        def v94_get_c_per_item(self, prefix_emb=None):
            return self.embedding.get_c() if isinstance(self.embedding, CodebookParam) else self.theta

        for _li, layer in enumerate(model.layers):
            K, D = layer.embedding.weight.shape
            _dev = layer.embedding.weight.device
            init_weight = layer.embedding.weight.detach().clone()
            if 'weight' in layer.embedding._parameters:
                del layer.embedding._parameters['weight']
            layer.embedding = CodebookParam(K, D, init_weight, RHO_MIN, RHO_MAX, C_MIN, C_MAX).to(_dev)
            layer._kmeans_init = types.MethodType(v94_kmeans_init, layer)
            layer.get_c = types.MethodType(v94_get_c, layer)
            layer.get_c_per_item = types.MethodType(v94_get_c_per_item, layer)
            if rank == 0:
                _init_rho = RHO_MIN + (RHO_MAX - RHO_MIN) * 0.5
                _init_c = C_MIN + (C_MAX - C_MIN) * 0.5
                print(f"[v94] L{_li}: CodebookParam (K={K}, D={D}, init_ρ={_init_rho:.3f}, init_c={_init_c:.3f})", flush=True)
