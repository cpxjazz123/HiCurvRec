...
def train(
    ...
    model = EncoderDecoderRetrievalModel(
        ...
        # v168: Stage 3 T5 端 G1 geometry-routed attention (HAB) — 共享 v161 Stage 1 ckpt + curv_state/response
        # R36n (e) 几何变换: HAB 在 T5 self-attention 加 Poincaré distance bias (Stage 2 RQ-VAE codebook 几何预计算)
        # C7: c_perturb_raw init 0 → 起点与 frozen baseline 严格等价; Stage 3 反向微调有效曲率 (R36)
        curv_state_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "out/rqvae/instruments/curvature_state.npy"),
        response_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "out/rqvae/instruments/response.npy"),
        hab_rqvae_ckpt=os.path.join(os.path.dirname(os.path.abspath(__file__)), "out/rqvae/instruments/rqvae_final.pt"),
    )
    if False:
