...
class RqVae(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        # v120 baseline bug: log_var 永远不训练 (QuantizeLoss 用 Poincaré dist)
        # v129 修复: 在每层 Quantize forward 内加入 Mahalanobis commit loss term
        mahalanobis_commit_weight: float = 0.0,  # v129: 0.0=disabled (v120 behavior), 0.1=enabled
    ) -> None:
        self._config = locals()
        ...
        self.mahalanobis_commit_weight = float(mahalanobis_commit_weight)  # v129
        ...
        self.layers = nn.ModuleList(
            modules=[
                Quantize(
                    ...
                    mahalanobis_commit_weight=mahalanobis_commit_weight,  # v129: log_var 训练 bug fix
                )
