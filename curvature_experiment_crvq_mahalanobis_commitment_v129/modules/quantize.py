...
class Quantize(nn.Module):
    def __init__(
        ...
        # v129 MAHALANOBIS COMMITMENT LOSS FIX: 让 log_var 真正参与训练
        # 论文支撑: C-RVQ arxiv 2505.12143 May 2025 (commitment loss 用 Mahalanobis dist²)
        # v120 baseline bug: QuantizeLoss 用 Poincaré dist 做 commitment, log_var 没有 gradient
        # v129 修复: 在 forward 内加入 mahalanobis_commit_loss 项, 让 log_var 通过 (x - codebook[ids])² / var[ids] 路径获取 gradient
        mahalanobis_commit_weight: float = 0.0,  # v129 NOVEL: 0.0 = disabled (v120 behavior), 0.1 = enabled
    ) -> None:
        super().__init__()
        ...
        # v129: Mahalanobis commitment loss weight
        self.mahalanobis_commit_weight = float(mahalanobis_commit_weight)
    ...
    def forward(self, x, temperature, prefix_emb=None) -> QuantizeOutput:
        ...
        if self.training:
            ...
            # v129 MAHALANOBIS COMMITMENT LOSS FIX: 让 log_var 真正参与训练
            # 论文支撑: C-RVQ arxiv 2505.12143 May 2025
            # 机制: 在 train 阶段额外加 Mahalanobis dist² commit loss 项
            #       L_mahan = mean_B Σ_d (x_d - codebook[ids]_d)² / var[ids]_d
            #       gradient 流向: codebook (via codebook[ids]), log_var (via var[ids]), x (encoder)
            # 注意: 该项只有在 MAHALANOBIS distance_mode 下才有意义 (其他 distance 用 var 物理上不对)
            if self.training and self.mahalanobis_commit_weight > 0 and self.distance_mode == QuantizeDistance.MAHALANOBIS:
                # 用与 forward 一致的 var 计算路径
                mahan_var = torch.nn.functional.softplus(self.log_var) + 1e-6
                mahan_var = mahan_var.clamp(min=0.1, max=10.0)
                chosen_codeword = self.embedding(ids)  # (B, D) 梯度→embedding
                mahan_diff = x - chosen_codeword  # (B, D) 梯度→x + embedding
                # var[ids] (B, D) 梯度→log_var
                mahan_dist_sq = (mahan_diff ** 2 / mahan_var[ids]).sum(-1)  # (B,)
                loss = loss + self.mahalanobis_commit_weight * mahan_dist_sq.mean()
