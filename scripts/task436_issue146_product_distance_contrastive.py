...
R18 强制: 不改 hard SID + 新增 product-distance pairwise contrastive calibration loss.
d_mix = α · d_anchor + β · d_fixed_hyp + γ · d_eucl.
contrastive_loss = mean relu(margin - d_mix_pos + d_mix_neg).
梯度通过 d_mix → mixing/κ, hard SID 完全隔离.

Precheck 强制 (Issue #146 spec):
1. autograd trace: contrastive_loss → d_mix → mixing/κ
2. hard SID 不参与该梯度路径

Gate 1: 1000-step no-aux control + product-calibration 对照.
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task436_issue146_product_distance_contrastive")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task436_issue146_product_distance_contrastive.log"
...
class ProductDistanceModel(nn.Module):
    ...
    def contrastive_loss(self, d_mix, batch_idx):
        """Issue #146 contrastive: positive pull, negative push via margin.
        同 batch 随机选 NUM_PAIRS_PER_BATCH 对 (positive, negative) 对比 d_mix.
        """
        B = d_mix.shape[0]
        # 选 positive 对 (item_i, item_j 共享 same-anchor 最近 codebook)
        # 简化: anchor 选 z_e_i 最近的 codebook 索引, positive = 同 batch 的 z_e_j 最近同一 codebook
        # negative = 跟 anchor 不同 codebook 的 z_e
        # 用 d_mix 取 argmin 当 anchor assign
        anchor_assign = d_mix.argmin(dim=-1)  # (B,)
        idx_perm = torch.randperm(B, device=d_mix.device)
        # 简化 contrastive: 同一个 anchor assign 的两个 item 当 positive, 不同 assign 当 negative
        # 为简化, 取 batch 内 pair (i, j): j 跟 i 同 assign → positive, j 跟 i 不同 assign → negative
        n_pairs = min(NUM_PAIRS_PER_BATCH, B // 2)
        i_idx = torch.randperm(B, device=d_mix.device)[:n_pairs]
        # 找一个跟 i 同 assign 的 j, 没有就用 random
        pos_dists = []
        neg_dists = []
        for i in i_idx.tolist():
            assign_i = anchor_assign[i].item()
            same_mask = (anchor_assign == assign_i)
            same_mask[i] = False  # 排除自己
            diff_mask = (anchor_assign != assign_i)
            same_idx = torch.where(same_mask)[0]
            diff_idx = torch.where(diff_mask)[0]
            if len(same_idx) > 0 and len(diff_idx) > 0:
                j = same_idx[torch.randint(len(same_idx), (1,)).item()].item()
                k = diff_idx[torch.randint(len(diff_idx), (1,)).item()].item()
                # Issue #146 fix: 用 tensor indexing 不用 .item() 保留 grad_fn
                pos_dists.append(d_mix[i, anchor_assign[i]])
                neg_dists.append(d_mix[k, anchor_assign[i]])
        if len(pos_dists) == 0:
            # Issue #146 fix: 即使没有 positive pair, 也返回 requires_grad tensor (用 d_mix.mean() * 0 保留 grad_fn)
            contrastive = d_mix.mean() * 0.0
            return contrastive, 0.0, 0.0
        # Issue #146 fix: 用 torch.stack 保留 grad_fn (旧版 .item() detach 路径)
        pos_t = torch.stack(pos_dists)
        neg_t = torch.stack(neg_dists)
        # contrastive: relu(margin - pos + neg), mean
        loss = F.relu(MARGIN - pos_t + neg_t).mean()
        return loss, pos_t.mean().item(), neg_t.mean().item()
    ...
    def forward_layer(self, x, layer_idx, use_aux=False):
        ...
        if use_aux:
            ...
            contrastive, pos_mean, neg_mean = self.contrastive_loss(d_mix, None)
