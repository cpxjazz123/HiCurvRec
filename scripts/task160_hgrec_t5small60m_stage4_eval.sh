#!/bin/bash
# Task #160 Stage 4 — T5-small 60M test eval
# 2026-07-24 (mirror task84 stage4_eval, T5-small 60M config + code_default SID)
# 输出: verdicts/task160_t5small_60m_metrics.json
...
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task160_t5small_60m_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #160 Stage 4] T5-small 60M test eval launched at $(date) =====" | tee $LOG_FILE
echo "Recipe: T5-small 60M + code_default SID" | tee -a $LOG_FILE
...
BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task160/ckpt_hgrec/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
...
result = {
    'task': 'task160_t5small_60m',
    'recipe': 'T5-small 60M + code_default SID (β=0.25, [32,64,256], sk=0.5)',
    't5_config': '6 enc + 6 dec, d_model=512, d_ff=2048, 8 heads (~60M params)',
...
echo "===== [Task #160 Stage 4] Test eval completed at $(date), exit code: $EXIT_CODE =====" | tee -a $LOG_FILE
