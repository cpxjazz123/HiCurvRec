#!/bin/bash
# Task #300 / Issue #29 Gate 1 — Stage 1 100 epoch 训练 (per-layer K_l)
# 2026-07-29
#
# 背景: Issue #29 §Gate 0 PASS (scripts/task300_issue29_gate0_per_layer_k.py)
#       Gate 1 = Stage 1 100 epoch 训练 per-layer K_l=[128,64,32]
#       使用 baseline train_hrqvae.py (Issue #29 沿用 baseline HRQVAE 构造)
#       R11.3 决策: baseline train_hrqvae.py argparse 不支持 --c_k_range_list /
#       --gumbel_tau_l / --loss_mult_codebook, 这些参数只在 HG-Rec/model/hrqvae.py argparse
#       存在. Issue #29 Gate 1 简化为 per-layer K_l 单一变量, c_k 沿用 baseline c_k (1.0).
#
# GATE_DECLARATION:
...
#
# R7: GPU 0 空闲 (4×L40S, 当前 0% util 0 MB used)
...
echo "PID=$TRAIN_PID (GPU 0, per-layer K_l=[128,64,32], 100 epoch)"
echo "Log: $LOG_FILE"
echo "Expected runtime: ~1h"
