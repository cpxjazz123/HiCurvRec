#!/usr/bin/env python3
"""Task #478 — Issue #59 Stage 1 重训练 #55 fixed (GPU 3).

基于 Issue #58 审计 + #59 修复, 重跑 Issue #55 Stage 1 训练:
  - 使用 FreeCurvVectorQuantizationMixedCurvWithScaleFixed (α_l + scale_l 都可微)
  - RiemannianAdamWFixed 用正确 (1-κ‖x‖²)²/4 公式
  - recipe 跟 #157 一致 (κ=1.0, lr=1e-3, 1000 epoch)
  - 验证: α_l/scale_l 不再全程停在 0.5/1.0, Stage 1 loss > 0.01
...
from model.hrqvae_issue55_56_fixed import (
    FreeCurvVectorQuantizationMixedCurvWithScaleFixed,
    RiemannianAdamWFixed,
)
...
def replace_vq_with_mixed_curv_scale_fixed(model: FreeCurvHRQVAE, kappa_fixed: float,
                                           scale_init: float) -> None:
    """替换 hrq.vq_layers 为 FreeCurvVectorQuantizationMixedCurvWithScaleFixed."""
    ...
    new_layers = []
    for n_e in n_e_list:
        vq = FreeCurvVectorQuantizationMixedCurvWithScaleFixed(
        ...
        # Issue #59 fix: 移到正确 device
        vq = vq.to(next(model.parameters()).device)
        new_layers.append(vq)
    model.hrq.vq_layers = torch.nn.ModuleList(new_layers)
    print(f"  Replaced vq_layers with {len(new_layers)} MixedCurvWithScaleFixed layers "
          f"(κ={kappa_fixed}, scale_init={scale_init})")
...
def train(args):
    ...
    print("=" * 70)
    print(f"Task #478 — Issue #59 Stage 1 重训练 #55 Fixed")
    print(f"  kappa_fixed = {args.kappa_fixed}")
    print(f"  scale_init  = {args.scale_init}")
    ...
    replace_vq_with_mixed_curv_scale_fixed(model, args.kappa_fixed, args.scale_init)

    # Split params: codebook params 用 RiemannianAdamWFixed, 其他用 AdamW
    codebook_params = []
    other_params = []
    for vq in model.hrq.vq_layers:
        codebook_params.append(vq.embeddings.weight)
        # α_l_raw 和 scale_l 也用 Riemannian (它们是流形参数)
        if hasattr(vq, 'alpha_l_raw'):
            codebook_params.append(vq.alpha_l_raw)
        if hasattr(vq, 'scale_l'):
            codebook_params.append(vq.scale_l)
    other_params = [p for n, p in model.named_parameters()
                    if not any(p is cp for cp in codebook_params)]

    optimizer_codebook = RiemannianAdamWFixed(
        codebook_params, lr=args.lr, weight_decay=0.01, kappa=args.kappa_fixed
    )
    optimizer_other = torch.optim.AdamW(other_params, lr=args.lr, weight_decay=0.01)
    print(f"  Split optimizers: codebook={len(codebook_params)} Riemannian, "
          f"other={len(other_params)} AdamW")
    ...
    # Output dir
    save_dir = REPO / "products/task478/ckpt/Instruments"
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for vq in model.hrq.vq_layers:
            ...
            if hasattr(vq, 'scale_l'):
                scale_l_values.append(vq.scale_l.item())
        mean_alpha = float(np.mean(alpha_l_values))
        mean_scale = float(np.mean(scale_l_values)) if scale_l_values else 1.0

        history.append({
            "epoch": epoch,
            "loss": epoch_loss,
            "mean_alpha_l": mean_alpha,
            "mean_scale_l": mean_scale,
        })
    ...
    print(f"     Initial: alpha_l=0.5 (sigmoid(0)), scale_l={args.scale_init}")
    print(f"     Final: mean_alpha_l={history[-1]['mean_alpha_l']:.4f}, "
          f"mean_scale_l={history[-1]['mean_scale_l']:.4f}")
    print(f"     ⚠️  若 alpha_l≈0.5 且 scale_l≈{args.scale_init} → Bug #1+#2 修复未生效 (NO-GO)")
    print(f"     ✅ 若两者都偏离初始值 → 修复有效 (GO Stage 2)")
