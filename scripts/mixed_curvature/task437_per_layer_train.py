#!/usr/bin/env python3
"""task56_per_layer_train.py — Task #468: 支持 per-layer h_offset/h_dim 的 Mixed-Curvature 训练

扩展 task56_h_offset_train.py 的单 h_offset/h_dim 为 per-layer 配置，
支持 H_E_E_E / H_H_E_E / H_H_H_H 三种变体，验证多层 H 是否更激进 brand-cluster。

设计:
  - 每个 layer 有独立的 (h_offset, h_dim)
  - 默认: l1=(0,32), l2=(0,0), l3=(0,0) → H_E_E_E (与 task56 P0.3 baseline 一致)
  - H_H_E_E: l1=(768,32), l2=(768,32), l3=(768,0) → H 在 L1+L2 brand 范围
  - H_H_H_H: l1=(768,32), l2=(768,32), l3=(768,32) → H 在 L1+L2+L3 brand 范围

用法:
  # H_E_E_E baseline
  python3 task56_per_layer_train.py --l1_h_offset=768 --l1_h_dim=32 --l2_h_dim=0 --l3_h_dim=0

  # H_H_E_E
  python3 task56_per_layer_train.py --l1_h_offset=768 --l1_h_dim=32 --l2_h_offset=768 --l2_h_dim=32 --l3_h_dim=0

  # H_H_H_H
  python3 task56_per_layer_train.py --l1_h_offset=768 --l1_h_dim=32 --l2_h_offset=768 --l2_h_dim=32 --l3_h_offset=768 --l3_h_dim=32
"""
import os
import sys
import math
...
class PerLayerMixedCurvatureRQVAE(nn.Module):
    """Mixed-Curvature RQ-VAE with per-layer (h_offset, h_dim) configuration."""

    def __init__(self, dim=INPUT_DIM, n_clusters=N_CLUSTERS,
                 h_offsets=(0, 0, 0), h_dims=(32, 32, 32),
                 init_radii=(0.25, 0.25, 0.25),
                 max_norm=MAX_NORM,
                 init_log_w=(0.0, 0.0)):
        super().__init__()
        assert len(h_offsets) == NUM_LAYERS
        assert len(h_dims) == NUM_LAYERS
        for l, (h_off, h_d) in enumerate(zip(h_offsets, h_dims)):
            assert h_off >= 0
            assert h_off + h_d <= dim, f"L{l+1} h_offset ({h_off}) + h_dim ({h_d}) must <= dim ({dim})"
        self.dim = dim
        self.h_offsets = list(h_offsets)
        ...
        # Per-layer softplus-bounded radius (only used when h_dim > 0 for that layer)
        ...
        # w_H, w_E learnable per-layer (log space), softmax normalized
        # If layer has h_dim=0, we still keep weights but w_H won't be used
        self.log_w_raw = nn.Parameter(
            torch.tensor(init_log_w, dtype=torch.float32).repeat(NUM_LAYERS, 1),
            requires_grad=True,
        )

        # Codebooks (one per layer)
    ...
    @property
    def weights(self):
        return F.softmax(self.log_w_raw, dim=-1)
    ...
    def _compute_distance(self, x, C, layer_idx):
        """Per-sub-space distance with layer-specific H offset/dim."""
        h_offset = self.h_offsets[layer_idx]
        h_dim = self.h_dims[layer_idx]
        w_H, w_E = self.weights[layer_idx, 0], self.weights[layer_idx, 1]
        ...
        if h_offset > 0:
            ...
            x_e_pre = x[:, :0]
            C_e_pre = C[:, :0]
        if h_offset + h_dim < self.dim:
            x_e_post = x[:, h_offset + h_dim:]
            C_e_post = C[:, h_offset + h_dim:]
        else:
            x_e_post = x[:, :0]
            C_e_post = C[:, :0]
        ...
        # H subspace (only if h_dim > 0 for this layer)
        if h_dim > 0:
            x_h = x[:, h_offset:h_offset + h_dim]
            C_h = C[:, h_offset:h_offset + h_dim]
            ...
            # No H subspace for this layer, pure Euclidean
...
def init_codebooks(model: PerLayerMixedCurvatureRQVAE, x_euclidean: torch.Tensor, device='cuda'):
    """K-means init for codebooks on encoded z (task58 路径)."""
    ...
    print(f'[init] running k-means on z={z_sub.shape} subset...', flush=True)
...
def main():
    ...
    # Per-layer H configuration
    ap.add_argument('--l1_h_offset', type=int, default=768)
    ap.add_argument('--l1_h_dim', type=int, default=32)
    ap.add_argument('--l2_h_offset', type=int, default=768)
    ap.add_argument('--l2_h_dim', type=int, default=32)
    ap.add_argument('--l3_h_offset', type=int, default=768)
    ap.add_argument('--l3_h_dim', type=int, default=32)
    # Training
    ...
    ap.add_argument('--task-tag', type=str, default='task57_perlayer')
    args = ap.parse_args()

    DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
    torch.manual_seed(args.seed)

    h_offsets = (args.l1_h_offset, args.l2_h_offset, args.l3_h_offset)
    h_dims = (args.l1_h_dim, args.l2_h_dim, args.l3_h_dim)
    # Variant tag (compact): HHEE → hhh-eee 二值标记
    variant = ''.join(['H' if d > 0 else 'E' for d in h_dims])
    variant += f'_o{args.l1_h_offset}d{args.l1_h_dim}'
    task_tag = f'{args.task_tag}_{variant}'
    print(f'[task57] device={DEVICE}, variant={variant}, h_offsets={h_offsets}, h_dims={h_dims}', flush=True)
    print(f'[task57] task_tag={task_tag}', flush=True)

    out_dir = f'{LOG_ROOT}/{task_tag}'
    Path(out_dir).mkdir(parents=True, exist_ok=True)

    # Load concat embedding + z-score
    print(f'[task57] loading {CONCAT_EMB}', flush=True)
    full = torch.load(CONCAT_EMB, map_location='cpu', weights_only=False).float()
    print(f'[task57] full shape={tuple(full.shape)}', flush=True)
    full_mean = full.mean(dim=0, keepdim=True)
    full_std = full.std(dim=0, keepdim=True).clamp(min=1e-6)
    full = (full - full_mean) / full_std
    print(f'[task57] z-score normalized', flush=True)
    ...
    torch.save({
        ...
        'h_offsets': list(h_offsets),
        'h_dims': list(h_dims),
        'variant': variant,
        'task_tag': task_tag,
    }, ckpt_path)
    print(f'[task57] saved ckpt to {ckpt_path}', flush=True)
