#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #304 / D6 ablation Arm A — Stage 1 训练 wrapper (r_l only)

背景 (Task #304 D6 ablation):
  Arm A 设计: r_l alone (s_l=baseline) + per-layer c_k range
...
实施方式 (R11.4 + R11.5):
  - 沿用 task301/task303 训练 wrapper 模式
  - per-layer c_k range 训练前 sample + 注入
"""
import argparse
import os
import sys
...
def apply_per_layer_codebook_transforms(model: HRQVAE,
    ...
    for li, q in enumerate(model.hrq.vq_layers):
        ...
        print(f'[Task #304 Arm A] Layer {li}: r_l={r}, s_l={s}, eff_norm={eff.norm().item():.4f}, '
...
def apply_per_layer_c_k_range(model: HRQVAE, c_k_range_list: List[tuple], seed: int = 42):
    ...
    for li, q in enumerate(model.hrq.vq_layers):
        ...
        q.c = c
        print(f'[Task #304 Arm A] Layer {li}: c_k={c:.4f} (sampled from [{cmin}, {cmax}])')


def parse_args():
    parser = argparse.ArgumentParser(description='Task #304 / D6 Arm A Stage 1 training')
    ...
    parser.add_argument('--ckpt_dir', type=str, default='./ckpt/Instruments_d6_arm_a')
    parser.add_argument('--seed', type=int, default=42)

    parser.add_argument('--radius_list', type=float, nargs='+', default=[0.1, 1.0, 10.0],
                        help='per-layer 半径缩放. Arm A 跟 Issue #30 相同')
    parser.add_argument('--scale_list', type=float, nargs='+', default=[1.0, 1.0, 1.0],
                        help='per-layer scale factor. Arm A = baseline (取消 Issue #30 [2,2,2])')
    parser.add_argument('--c_k_range_list', type=str, default='1.0:5.0,0.5:20.0,0.5:20.0',
                        help='per-layer c_k range (跟 Issue #30 相同). 格式 "low1:high1,...".')
...
def main():
    ...
    print(f'[Task #304 Arm A] radius_list = {args.radius_list}')
    print(f'[Task #304 Arm A] scale_list = {args.scale_list}')
    print(f'[Task #304 Arm A] c_k_range_list = {c_k_range_list}')

    data = EmbDataset(args.data_path)
    print(f'[Task #304 Arm A] Dataset: {len(data)} items, dim={data.dim}')
    ...
    print(f'[Task #304 Arm A] Apply per-layer codebook transforms: r_l={args.radius_list}, s_l={args.scale_list}')
    ...
    print(f'[Task #304 Arm A] Apply per-layer c_k range: {c_k_range_list}')
    ...
    print(f'[Task #304 Arm A] Best Loss = {best_loss}, Best Collision Rate = {best_collision_rate}')
