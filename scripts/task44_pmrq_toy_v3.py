#!/usr/bin/env python3
"""Task #44 — PM-RQ Toy 实验 (Phase 1, 第三轮: d_total commitment loss).

第二轮 STE 诊断:
  - fusion_logits 仍然纹丝不动 (C: [0.333,0.333,0.333] 全程不变; D: [0.5,0.5] 全程不变)
  - κ_sphere 从 +0.8446 跑到 -8.99 (进入了强双曲,dist_kappa 路径通)
  - B=0.275, C=2.394, D=0.895 (C/D 都退化)

Root cause: fusion_logits 只影响 d_total, 而 d_total 仅用于 argmin(idx), 不参与 recon loss.
  STE 修复的是码字 c 的梯度, 不是 d_total → fusion_logits 的梯度.

第三轮修复: 把 d_total[idx] 直接加入 loss (commitment surrogate).
  这样 d_total → fusion_logits 的梯度链完全打开.
  同时用 d_total 在 batch 内做 normalize (而非 raw d_s/d_e/d_h),让 fusion_logits 的 3 个输出有可比 scale.

第三轮新增:
  1. recon_loss = ||final residual||²
  2. commitment_loss = d_total.gather(idx).mean()  ← 关键: 打开 fusion_logits 梯度路径
  3. 平衡权重: commitment_loss * 1.0

实验组:
  B. MCKG-standard (fused_64d normalized, 标准 RQ-VAE)  [不变]
  C. PM-RQ 3 段 (核心方法, d_total commitment)
  D. PM-RQ 2 段消融 (sphere + euclid)
"""
import json
import sys
...
NORM_CAP = 5.0
STE_TEMP = 5.0
COMMIT_W = 1.0  # commitment loss 权重
...
class PMRQv3(nn.Module):
    ...
    def forward_layer(self, r_s, r_e, r_h=None):
        """STE + in-batch d_total normalize + commitment-friendly."""
        # 1. 算各段距离 (raw)
        ...
        return idx, (c_s, c_e, c_h if self.use_hyperbolic else None), (r_s_next, r_e_next, r_h_next if self.use_hyperbolic else None), d_total

    def forward(self, r_s, r_e, r_h=None):
        all_idx, all_codes, all_d_total = [], [], []
        ...
        return all_idx, all_codes, (r_s, r_e, r_h), all_d_total
...
def train_pmrq(model, sub_s, sub_e, sub_h, n_steps=N_STEPS, batch_size=BATCH_SIZE, lr=LR, use_hyperbolic=True):
    ...
    trajectory = {'step': [], 'kappa_s': [], 'kappa_h': [],
                  'w_s': [], 'w_e': [], 'w_h': [],
                  'recon_mse': [], 'commit_loss': [], 'total_loss': []}
    ...
    for step in range(n_steps):
        ...
        all_idx, all_codes, residuals, all_d_total = model(r_s, r_e, r_h)
        r_s_final, r_e_final, r_h_final = residuals

        # recon loss: sum of squared final residual norms
        recon = (r_s_final ** 2).sum(dim=-1).mean() + (r_e_final ** 2).sum(dim=-1).mean()
        if use_hyperbolic:
            recon = recon + (r_h_final ** 2).sum(dim=-1).mean()

        # commitment surrogate: 选中的码字在 d_total 距离上的 loss, 梯度通到 fusion_logits
        commit = sum(dt.gather(1, ai.unsqueeze(1)).squeeze(1).mean()
                     for dt, ai in zip(all_d_total, all_idx))

        loss = recon + COMMIT_W * commit
        ...
        if step % 200 == 0 or step == n_steps - 1:
            ...
            trajectory['recon_mse'].append(recon.item())
            trajectory['commit_loss'].append(commit.item())
            trajectory['total_loss'].append(loss.item())
            print(f'  step {step}: recon={recon.item():.4f}, commit={commit.item():.4f}, total={loss.item():.4f}, '
                  f'κ_s={model.kappa_sphere.item():+.4f}, κ_h={model.kappa_hyperbolic.item() if use_hyperbolic else 0.0:+.4f}, '
                  f'w=[{w[0]:.3f}, {w[1]:.3f}, {w[2] if use_hyperbolic else 0.0:.3f}]')
    return trajectory
...
def main():
    print('========== Task #44 Phase 1 PM-RQ Toy (第三轮: d_total commitment) ==========')
    print(f'GPU: {DEVICE}')
    ...
    torch.save(model_b.state_dict(), OUT_DIR / 'model_B_standard_v3.pt')

    print('\n========== C 组: PM-RQ 3 段 (commitment) ==========')
    model_c = PMRQv3(K=K_CODEBOOK, dim=64, n_layers=N_LAYERS, use_hyperbolic=True).to(DEVICE)
    ...
    results['C_pmrq_3seg'] = {'trajectory': traj_c, 'final_loss': traj_c['total_loss'][-1]}
    torch.save(model_c.state_dict(), OUT_DIR / 'model_C_pmrq_3seg_v3.pt')

    print('\n========== D 组: PM-RQ 2 段消融 ==========')
    model_d = PMRQv3(K=K_CODEBOOK, dim=64, n_layers=N_LAYERS, use_hyperbolic=False).to(DEVICE)
    ...
    results['D_pmrq_2seg'] = {'trajectory': traj_d, 'final_loss': traj_d['total_loss'][-1]}
    torch.save(model_d.state_dict(), OUT_DIR / 'model_D_pmrq_2seg_v3.pt')
    ...
    print(f'D 训练后 κ_s: {model_d.kappa_sphere.item():+.4f}')

    summary = {
        'task': 'Task #44 Phase 1 PM-RQ Toy v3 (d_total commitment)',
        'config': {
    ...
    out = OUT_DIR / 'task44_phase1_v3_summary.json'
