#!/bin/bash
# Task #174 Stage 3 — T5-mini 9.18M + κ-Stereo + M=2 + 门控融合 D 臂 SID
...
export TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task174_s3

LOG_DIR=/home/wlia0047/ar57/wenyu/GeneRec/logs/task174
TS=$(date +%b-%d-%Y_%H-%M-%S | sed 's/^.../\L&/')
LOG_FILE=$LOG_DIR/stage3_train_${TS}.log

PROD_DIR=/home/wlia0047/ar57/wenyu/GeneRec/products/task174/t5mini_mckg_gating/${TS}
mkdir -p $PROD_DIR
mkdir -p $LOG_DIR

echo "===== [Task #174 Stage 3] T5-mini 9.18M + κ-Stereo + M=2 + gating D 臂 launched at $(date) =====" | tee $LOG_FILE
echo "Code path: _t5_rqvae_phase_b_mckg_gating.npy (from #174 Stage 2)" | tee -a $LOG_FILE
echo "Output: $PROD_DIR" | tee -a $LOG_FILE
echo "Target: test R@10 > 0.1058 (Stop hook 条件 C3)" | tee -a $LOG_FILE

CODE_FILE=/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/Instruments_t5_rqvae_phase_b_mckg_gating.npy
if [ ! -f "$CODE_FILE" ]; then
...
TRAIN_PID=$!
echo $TRAIN_PID > /home/wlia0047/ar57/wenyu/GeneRec/products/task174/_TRAINING_PID
echo "PID: $TRAIN_PID" | tee -a $LOG_FILE
echo "Tail log: tail -f $LOG_FILE" | tee -a $LOG_FILE

cat > /home/wlia0047/ar57/wenyu/GeneRec/products/task174/_STAGE4_TRIGGER.sh <<EOF2
#!/bin/bash
...
EOF2
chmod +x /home/wlia0047/ar57/wenyu/GeneRec/products/task174/_STAGE4_TRIGGER.sh
nohup bash /home/wlia0047/ar57/wenyu/GeneRec/products/task174/_STAGE4_TRIGGER.sh > /dev/null 2>&1 &
