#!/usr/bin/env bash
# Step 3 v11: angular_dim=8, radial_dim=26. 8D hyp 角带宽 √(8/64)≈0.354 (baseline 50%).
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_v11_angdim8
...
COMMON="\
  ...
  --beta 0.5 \
  --product_manifold --angular_dim 8 --radial_dim 26 \
...
echo "==== Step 3 v11: angular_dim=8 + radial_dim=26 (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
DISABLE_USAGE_KILL=1 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_v11 \
PYTHONUNBUFFERED=1 \
