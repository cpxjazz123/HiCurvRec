...
from model.HG_Rec import HG_Rec  # R11.3 FIX: HG-Rec 仓库 model/HG_Rec.py (大写), 上游 fork 写小写, Linux 文件系统大小写敏感
from model.HG_Rec_issue57 import HG_Rec_Issue57  # Issue #57 Gate 0 wrapper
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='Yelp', help='The name of dataset')
    parser.add_argument('--dataset_path', type=str, default='./dataset/', help='Path to the dataset')
    parser.add_argument('--codebook_size', type=int, nargs='+', default=[64,128,256,1], help='emb num of every vq')
    parser.add_argument('--code_path', type=str, default='_t5_hrqvae_poincare.npy', help='Path to the item-to-code mapping file (R11.3: matches Stage 2 output)')
    ...
    # Issue #57 Gate 0: SID token embedding init mode
    parser.add_argument('--sid_embedding_init', type=str, default='random',
                        choices=['random', 'hyperbolic'],
                        help='Issue #57 Gate 0: SID token init (random=T5 default, hyperbolic=expmap0(c·x))')
    parser.add_argument('--hyp_c', type=float, default=0.74,
                        help='Issue #57 Gate 0: κ for expmap0 (Ollivier mean from #70)')
    ...
    # Issue #57 Gate 0: Use Issue #57 wrapper for sid_embedding_init flag
    ...
    for epoch in range(config['num_epochs']):
        ...
        if avg_ndcgs['NDCG@20'] > best_ndcg:
            ...
            # Save the best model
            _ckpt_path = os.path.join(ckpt_path,"HG_Rec_best.pth")
            # R12 FIX: 删除旧 best ckpt (保留最新) — 禁止多 epoch ckpt 累积
            import glob as _glob
            for _old in _glob.glob(os.path.join(ckpt_path, "HG_Rec_epoch_*.pth")) + _glob.glob(os.path.join(ckpt_path, "HG_Rec_best.pth")):
