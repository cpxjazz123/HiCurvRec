#!/usr/bin/env bash
# Step 3 v7: w_div=100 + w_angular=10 + sk_eps=0.03 (Sinkhorn 平滑) + kill 禁用.
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_50ep_sinkhorn_wangular
...
COMMON="\
  ...
  --sk_epsilons 0.03 0.03 0.03 --sk_iters 50 \
  --beta 0.5 \
...
echo "==== Step 3 v7: sk_eps=0.03 + w_div=100 + w_angular=10 (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
DISABLE_USAGE_KILL=1 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_v7 \
PYTHONUNBUFFERED=1 \
