#!/usr/bin/env python3
"""Task #284 Stage 2 — fork of task194_stage2_codebook.py for FreeCurvHRQVAE (task89).

用法:
  python3 scripts/task284_stage2_free_curv.py \
      --ckpt_path <path/to/best_loss_model.pth> \
      --output_path <path/to/Instruments_t5_hrqvae_kappa_decouple_arm<X>_k0256.npy> \
      --device cuda:0 \
      --max_sinkhorn_iters 30

跟 task194_stage2_codebook.py 区别:
- 加载 FreeCurvHRQVAE (task89) 而非 HRQVAE (task84)
- FreeCurvHRQVAE 多 M / kappa_max / soft_vq_enabled 字段
- 兼容 ckpt['args'] 可能是 dict (vars(args)) 而非 Namespace
"""
import argparse
import collections
import logging
...
def main():
    ...
    model = FreeCurvHRQVAE(in_dim=data.dim,
                            num_emb_list=num_emb_list,
                            e_dim=e_dim,
                            M=_get_arg('M', 1),
                            kappa_max=_get_arg('kappa_max', 2.0),
                            ...
                            soft_vq_enabled=_get_arg('soft_vq_enabled', False),
                            soft_vq_tau_start=_get_arg('soft_vq_tau_start', 1.0),
                            soft_vq_tau_end=_get_arg('soft_vq_tau_end', 0.01),
                            soft_vq_anneal_type=_get_arg('soft_vq_anneal_type', 'linear'))
