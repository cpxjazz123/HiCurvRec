#!/usr/bin/env bash
# Task #84 baseline ckpt Stage 4 eval @ beam=100 — test baseline ceiling with K=100
# baseline #84 @ beam=20 → R@10=0.1020 (anchored)
# Test K=100 amplification on baseline
# GPU 1 (R7 空闲)
...
CKPT_PATH=$(ls -td $REPO/products/task84/ckpt_hgrec/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
if [ -z "$CKPT_PATH" ]; then
    echo "ERROR: no ckpt found at $REPO/products/task84/ckpt_hgrec/Instruments/*/HG_Rec_best.pth"
    exit 1
fi
echo "Using ckpt: $CKPT_PATH (size: $(stat -c %s $CKPT_PATH))"

LOG_DIR=$REPO/logs/task84
mkdir -p $LOG_DIR

TS=$(date +%Y%m%d_%H%M%S)
LOG="$LOG_DIR/stage4_eval_beam100_${TS}.log"
echo "===== [Task #84 baseline / beam=100 ceiling test] launched at $TS =====" | tee "$LOG"
echo "code_path = _t5_rqvae_code_default.npy (baseline default)" | tee -a "$LOG"
...
print(f'[TASK84 baseline beam=100] missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
out = {
    'task': 'task84_baseline_ckpt_beam100_ceiling_test',
    'ckpt_path': '${CKPT_PATH}',
    'code_path': '_t5_rqvae_code_default.npy',
    'beam_size': 100,
    'config': config,
...
out_path = os.path.join(metrics_dir, 'task84_baseline_beam100_metrics.json')
with open(out_path, 'w') as f:
    json.dump(out, f, indent=2, default=float)
print(f'[TASK84 baseline beam=100] metrics written to {out_path}')
print(f'[TASK84 baseline beam=100] elapsed={elapsed:.1f}s')
print(f'[TASK84 baseline beam=100] Recall@5/10/20 = {[avg_recalls["Recall@5"], avg_recalls["Recall@10"], avg_recalls["Recall@20"]]}')
print(f'[TASK84 baseline beam=100] NDCG@5/10/20 = {[avg_ndcgs["NDCG@5"], avg_ndcgs["NDCG@10"], avg_ndcgs["NDCG@20"]]}')
PYEOF
echo "===== [Task #84 baseline beam=100] completed at $(date) =====" | tee -a "$LOG"
