...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task447_issue156_affine_truncated_softmax")
...
# ──────────────────────────────────────────────────────────────
# AffineTruncatedSimplexKappaModel (Issue #156: 仿射截断 softmax, 替换 hard clamp)
# ──────────────────────────────────────────────────────────────
class TruncatedSimplexKappaModel(nn.Module):
    ...
    def affine_truncated_softmax(self, logits: torch.Tensor) -> torch.Tensor:
        """Issue #156 spec: 无饱和仿射截断 softmax.
        alpha_i = 0.1 + 0.7 * softmax(logits)_i
        每项严格 (0.1, 0.8), 对 logits 可导, 无 clamp, grad path 完好.
        数学: sum_i alpha_i = K*0.1 + 0.7*sum_i softmax_i = K*0.1 + 0.7 = 1.0 (K=3 时)
        """
        softmax_out = F.softmax(logits, dim=-1)  # (B, K) ∈ (0, 1) 严格
        return 0.1 + 0.7 * softmax_out  # 仿射截断, 每项严格 (0.1, 0.8)
    ...
    def forward(self, z_e: torch.Tensor) -> Dict:
        ...
        # per-sample weight MLP + 仿射截断 softmax (Issue #156 spec 强制)
        for l in range(N_HIERARCHIES):
            logits = self.weight_mlps[l](z_e)  # (B, 3)
            weight = self.affine_truncated_softmax(logits)  # (B, 3) ∈ (0.1, 0.8) 严格
            results[f"weight_l{l}"] = weight
            results[f"logits_l{l}"] = logits  # Issue #156 spec: 记录 mixing-logits 用于 grad audit
...
def precheck(model: TruncatedSimplexKappaModel, sample_emb: torch.Tensor) -> Dict:
    print("\n=== Precheck 5 项验证 (Issue #156 spec 强制) ===", flush=True)
    model.train()
    ...
    # (3) 仿射截断softmax bounds (Issue #156 spec 强制: alpha ∈ (0.1, 0.8) 严格, sum=1, 无 clamp)
    ...
    print(f"(3) 仿射截断softmax bounds [w_min={model.w_min}, w_max={model.w_max}]: min={w0.min().item():.3f}, max={w0.max().item():.3f}, sum={w0.sum(dim=-1).mean().item():.3f} → {'PASS' if bounds_ok else 'FAIL'}")
...
def train_step(model: TruncatedSimplexKappaModel, batch: torch.Tensor, opt: torch.optim.Optimizer,
    ...
    # Issue #156 spec: 在 opt.zero_grad()/step() 前记录 raw κ + mixing-logit grad + step 前后 delta
    kappa_before = model.kappa.detach().clone()
    logits_before = [results[f"logits_l{l}"].detach().clone() for l in range(N_HIERARCHIES)]

    opt.zero_grad()
    total_loss.backward()

    # 关键: 在 opt.step() 之前读 raw grad (Issue #156 spec 强制)
    ...
    # mixing-logit grad (每个 weight_mlp 的所有参数 max abs)
    raw_grad_logits = [max(p.grad.abs().max().item() if p.grad is not None else 0.0 for p in mlp.parameters())
    ...
    # logits delta 需要从 forward 重新算 (因为 opt.step 改了 MLP, 重新 forward logits 会变)
    with torch.no_grad():
        logits_after_list = [model.weight_mlps[l](batch) for l in range(N_HIERARCHIES)]
    logits_delta = [logits_after_list[l].sub(logits_before[l]).abs().mean().item() for l in range(N_HIERARCHIES)]

    metrics = {
        "loss": total_loss.item(),
        "aux_loss": aux_loss.item(),
        "contrast": contrast.item(),
        "raw_grad_kappa_before_step": raw_grad_kappa,  # Issue #156 spec 强制
        "raw_grad_logits_before_step": raw_grad_logits,  # Issue #156 spec 强制
        "kappa_before": kappa_before.tolist(),
        "kappa_after": kappa_after.tolist(),
        "kappa_delta": kappa_delta,  # Issue #156 spec 强制
        "logits_delta_mean": logits_delta,  # Issue #156 spec 强制
        "infeasible": False,
    }
    for l in range(N_HIERARCHIES):
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Task #447 / Issue #156 [方向B Gate1] 无饱和仿射截断 softmax + 双边配额")
    print(f"Steps={args.steps}, seed={args.seed}, GPU={args.gpu}")
    print(f"Codebook sizes (L0/L1/L2): {CODEBOOK_SIZES}")
    print(f"Weight bounds [w_min={W_MIN}, w_max={W_MAX}], 仿射截断 softmax, bilateral quota")
    print(f"{'='*60}\n")
    ...
    with open(PRODUCT_DIR / "affine_truncated_proof.json", "w") as f:
    ...
    # ── Gate 1 决策 (Issue #156 spec: raw κ + mixing-logit grad before step + step delta) ──
    print(f"\n{'='*60}\nGATE 1 决策 (Issue #156 spec)\n{'='*60}")
    final = bi_trace[-1]
    ...
    # Issue #156 spec 强制: alpha 严格 [0.1, 0.8] (仿射截断无 clamp, 应严格内部)
    weight_bounds_ok = all(final[f"weight_min_l{l}"] >= W_MIN + 1e-5 and final[f"weight_max_l{l}"] <= W_MAX - 1e-5
                           for l in range(N_HIERARCHIES))
    comp_contrib_ok = all(sum(1 for c in [final[f"comp{l}_l{l}"] for l in range(N_HIERARCHIES)]) >= 2 for l in range(N_HIERARCHIES))
    # Issue #156 spec 强制: raw κ grad + mixing-logit grad before step finite nonzero
    raw_kappa_grad_ok = all(g > 1e-8 for g in final["raw_grad_kappa_before_step"])
    raw_logits_grad_ok = all(g > 1e-8 for g in final["raw_grad_logits_before_step"])
    # Issue #156 spec 强制: step 前后 delta 非零
    kappa_delta_ok = all(d > 1e-8 for d in final["kappa_delta"])
    logits_delta_ok = all(d > 1e-10 for d in final["logits_delta_mean"])
    no_nan = not (math.isnan(final["loss"]) or math.isinf(final["loss"]))

    gate1_pass = (util_ok and max_load_ok and min_load_ok and weight_bounds_ok and comp_contrib_ok
                  and raw_kappa_grad_ok and raw_logits_grad_ok and kappa_delta_ok and logits_delta_ok and no_nan)
    print(f"  util>=90%: {util_ok}")
    print(f"  max_load<5%: {max_load_ok}")
    print(f"  min_load>=1/K: {min_load_ok}")
    print(f"  weight bounds [{W_MIN}, {W_MAX}] (strict interior): {weight_bounds_ok}")
    ...
    print(f"  raw mixing-logit grad before step finite nonzero: {raw_logits_grad_ok}")
    print(f"  κ step delta nonzero: {kappa_delta_ok}")
    print(f"  mixing-logit step delta nonzero: {logits_delta_ok}")
    print(f"  no NaN/Inf: {no_nan}")
    print(f"\n>>> GATE 1 决策 (Issue #156 spec): {'✅ PASS' if gate1_pass else '❌ FAIL'} <<<\n")

    config = {
        "issue": "#156",
        "task": "#447",
        "spec": "Issue #156 无饱和仿射截断 softmax (alpha = 0.1 + 0.7 * softmax(logits)) + bilateral quota",
    ...
    verdict = {
        ...
        "final_raw_grad_logits_before_step": final["raw_grad_logits_before_step"],
        "final_kappa_delta": final["kappa_delta"],
        "final_logits_delta_mean": final["logits_delta_mean"],
    ...
    print(f"\n产物落地: {PRODUCT_DIR}")
    print(f"  config.json + precheck.json + affine_truncated_proof.json + train_curve.json + verdict.json")
