...
# 目的: 验证 K14 beam=50 vs K=100 (task307) sweep.
# Anchors:
#   Issue #30 GO @ beam=20 → R@10=0.1022
#   Issue #30 GO @ beam=100 → R@10=0.1045 (task307)
#   task194_k0256 @ beam=20 → R@10=0.1053 (overall anchor)
# R11.5: GPU 1 空闲
...
LOG_DIR=$REPO/logs/task309b
mkdir -p $LOG_DIR

TS=$(date +%Y%m%d_%H%M%S)
LOG="$LOG_DIR/stage4_eval_beam50_${TS}.log"
echo "===== [Task #309b / Issue #30 GO ckpt beam=50] launched at $TS =====" | tee "$LOG"
...
# T5-mini config (Issue #30 baseline architecture)
config = {
...
print(f'[TASK309b Issue #30 beam=50] missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
out = {
    'task': 'task309b_issue30_ckpt_beam50_stage4_eval',
    'ckpt_path': '${CKPT_PATH}',
    'code_path': '_t5_hrqvae_issue30_per_layer_transforms.npy',
    'beam_size': 50,
    'config': config,
...
out_path = os.path.join(metrics_dir, 'task309b_issue30_beam50_metrics.json')
with open(out_path, 'w') as f:
    json.dump(out, f, indent=2, default=float)
print(f'[TASK309b Issue #30 beam=50] metrics written to {out_path}')
print(f'[TASK309b Issue #30 beam=50] elapsed={elapsed:.1f}s')
print(f'[TASK309b Issue #30 beam=50] Recall@5/10/20 = {[avg_recalls["Recall@5"], avg_recalls["Recall@10"], avg_recalls["Recall@20"]]}')
print(f'[TASK309b Issue #30 beam=50] NDCG@5/10/20 = {[avg_ndcgs["NDCG@5"], avg_ndcgs["NDCG@10"], avg_ndcgs["NDCG@20"]]}')
PYEOF
echo "===== [Task #309b Issue #30 beam=50] completed at $(date) =====" | tee -a "$LOG"
