#!/bin/bash
# Task #157 Stage 4 — T5-base 220M test eval
# 2026-07-24 (镜像 task84 stage4_eval, 但 T5-base config + code_default SID)
# R11.3: 复用 scripts/task84_hgrec_stage3_train.py fork 的 evaluate 函数
# 输出: verdicts/task157_t5_base_metrics.json (Test Recall@5/10/20 + NDCG@5/10/20)
...
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task157_t5_base_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #157 Stage 4] T5-base 220M test eval launched at $(date) =====" | tee $LOG_FILE
echo "Recipe: T5-base 220M + code-default SID" | tee -a $LOG_FILE
echo "GPU 0" | tee -a $LOG_FILE
echo "Output JSON: $RESULT_JSON" | tee -a $LOG_FILE

BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task157/ckpt_hgrec/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
...
# T5-base config (mirror Task #157 Stage 3)
config = {
    'batch_size': 256,
    'infer_size': 96,
    'lr': 1e-4,
...
result = {
    'task': 'task157_t5_base_capacity_unlock',
    'recipe': 'T5-base 220M + code-default SID (β=0.25, [32,64,256], sk=0.5)',
    't5_config': '12 enc + 12 dec, d_model=768, d_ff=3072, 12 heads (~220M params)',
    'best_ckpt': '$BEST_CKPT',
    'beam_size': 20,
...
echo "===== [Task #157 Stage 4] Test eval completed at $(date), exit code: $EXIT_CODE =====" | tee -a $LOG_FILE
