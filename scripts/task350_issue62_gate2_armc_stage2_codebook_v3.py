"""Task #350 Issue #62 Gate 2 v3 — NO HypPre wrapper (per-layer transforms only)

R11.5 决策: v1 + v2 都 fail (3-digit collision 99.99%). v3 测 per-layer transforms (#30)
是否能在没有 HypPre wrapper 的情况下让 Stage 2 inference 多样化. 如果 v3 collision ≤ 0.20,
则继续用 no-HypPre 的 SID 训练 T5 (虽然训练时用了 HypPre, 但 per-layer transforms
可能是主要 leverage, HypPre 是次要). 如果 v3 也 fail, Issue #62 整体 NO-GO 收口.
"""
import collections
import glob
import logging
...
def main():
    ...
    log = logging.getLogger("task350_issue62_gate2_armc_v3")
    ...
    # R11.5 v3: baseline HRQVAE (NO HypPre wrapper)
    ...
    log.info(f"  baseline HRQVAE load_state_dict strict=False: missing={len(missing)}, unexpected={len(unexpected)}")
    log.info(f"  v3 = NO HypPre wrapper (per-layer transforms via ckpt weights)")
    ...
    log.info("=" * 60)
    log.info("Issue #62 Gate 2 verification (v3 NO HypPre wrapper):")
