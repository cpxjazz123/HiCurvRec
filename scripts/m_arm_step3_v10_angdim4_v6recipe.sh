#!/usr/bin/env bash
# Step 3 v10: angular_dim=4 + radial_dim=30, 但 recipe 回到 v6 (β=0.5 + bs=256 + w_div=100 + w_angular=10).
# 假设: 4D + v6 recipe 看 collision 是否比 v6 (95.68%) 更低.
# 排除 v9 失败因素 (v9 是 v8 recipe + 4D → w_angular spike 打爆).
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_v10_angdim4_v6recipe
LOG="${OUTDIR}/train.log"
mkdir -p "$OUTDIR"

COMMON="\
  --data_path $DATA \
  --lr 1e-3 --batch_size 256 --epochs 100 \
...
echo "==== Step 3 v10: angular_dim=4 + v6 recipe (β=0.5, bs=256, w_div=100) (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
DISABLE_USAGE_KILL=1 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_v10 \
PYTHONUNBUFFERED=1 \
