"""
Task #336 — Issue #49 Stage 4 — Test set evaluation for Arm B (κ=-0.12).

Standalone test eval loading best checkpoint from
products/task340/arm_minus/t5_out/Instruments/<ts>/HG_Rec_best.pth
"""
...
def main():
    config = {
        ...
        'code_path': '_t5_rqvae_arm_minus.npy',
        'topk_list': [5, 10, 20],
        'beam_size': 20,
    }

    # Find best checkpoint
    ckpt_dir = BASE / 'products' / 'task340' / 'arm_minus' / 't5_out' / 'Instruments'
    ckpt_list = sorted(ckpt_dir.glob('*/HG_Rec_best.pth'))
    if not ckpt_list:
        print("❌ No best checkpoint found")
        sys.exit(1)
    best_ckpt = str(ckpt_list[-1])
    print(f'[Stage 4] Loading: {best_ckpt}')
    ...
    # Evaluate on TEST (beam=20 only, per user request)
    beam_size = 20
    print(f'\n===== Beam size = {beam_size} =====')
    ...
    result_json = BASE / f'verdicts/task336_issue49_stage4_beam{beam_size}.json'
    with open(result_json, 'w') as f:
        json.dump(result, f, indent=2)
    print(f'[Stage 4] Results saved: {result_json}')

    print('\n✅ Stage 4 complete for Issue #49 Arm B')
