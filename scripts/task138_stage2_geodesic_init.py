"""Task #138 Stage 2 — FreeCurvHRQVAE codebook generation (A-arm, geodesic_kmeans init).

Loads Task #138 A-arm best_loss_model.pth (FreeCurvHRQVAE, M=1, κ_max=0.5,
trained with --geodesic_kmeans --re_kmeans_every 50 --dead_code_reset_every 100),
generates 4-digit SID codes, resolves duplicates via append digit,
saves as Instruments_t5_hrqvae_geodesic_init.npy.

R11.4 dry-run: writes fork, NOT launch. Launch waits for A-arm 1000 epoch completion
(estimated ~2.5h after C-arm exit, total ~3h from now).
"""
import collections
import json
import logging
...
def main():
    """Stage 2 — generate codebook from Task #138 A-arm best_loss_model.pth.

    R11.3: ckpt_path = Task #138 arm_A_M1 best_loss_model.pth (geodesic_init variant).
    R11.3: output_path = Instruments_t5_hrqvae_geodesic_init.npy (avoid clobbering
            Task #137 baseline codebooks).
    R11.3: κ_max=0.5 → use post-fix FreeCurvHRQVAE (R137 unified operator).
    """
    dataset = "Instruments"
    ckpt_base = "/home/wlia0047/ar57/wenyu/GeneRec/products/task138/train/arm_A_M1"
    ckpt_path = os.path.join(ckpt_base, "best_loss_model.pth")
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            f"No best_loss_model.pth at {ckpt_path} — Stage 1 not complete. "
            f"Wait for A-arm training to finish."
        )
    # Distinct output name (avoid clobber task137 baseline + task84 baseline)
    output_path = (
        f"/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/{dataset}/"
        f"{dataset}_t5_hrqvae_geodesic_init.npy"
    )
    ...
    log.info(f"  geodesic_kmeans={args.get('geodesic_kmeans', '?')}, "
             f"re_kmeans_every={args.get('re_kmeans_every', '?')}, "
             f"dead_code_reset_every={args.get('dead_code_reset_every', '?')}")
    ...
    # Sinkhorn resolution loop (mirror task84/task137 stage2 behavior)
    tt = 0
    while True:
    ...
    log.info(
        f"  collision rate: "
        f"{(len(all_indices_str) - len(set(all_indices_str.tolist()))) / len(all_indices_str):.4f}"
    )
    ...
    # Append dedup digit (same as Task #84 / #137)
    ...
    meta = {
        'task': 'Task #138 A-arm Stage 2 (geodesic_kmeans init)',
        'ckpt_path': ckpt_path,
        'output_path': output_path,
        'M': args['M'],
        'kappa_max': args['kappa_max'],
        'geodesic_kmeans': args.get('geodesic_kmeans', None),
        're_kmeans_every': args.get('re_kmeans_every', None),
        'dead_code_reset_every': args.get('dead_code_reset_every', None),
        'final_kappa_per_layer': [
            vq.kappa_m().detach().cpu().tolist() for vq in model.hrq.vq_layers
        ],
