#!/bin/bash
# Task #172 Stage 4 — T5-mini 9.18M + κ-Stereo + κ_max=4.0 SID test eval
...
export TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task172_s4

LOG_DIR=/home/wlia0047/ar57/wenyu/GeneRec/logs/task172
TS=$(date +%b-%d-%Y_%H-%M-%S | sed 's/^.../\L&/')
LOG_FILE=$LOG_DIR/stage4_eval_${TS}.log
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task172_kappa_max4_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #172 Stage 4] T5-mini 9.18M + κ-Stereo + κ_max=4.0 test eval launched at $(date) =====" | tee $LOG_FILE

BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task172/t5mini_kappa_max4/*/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
if [ -z "$BEST_CKPT" ] || [ ! -f "$BEST_CKPT" ]; then
    echo "❌ Stage 3 best ckpt MISSING for #172" | tee -a $LOG_FILE
    exit 1
fi
...
model = HG_Rec(config)
print(f'[Task #172 Stage 4] Loading best ckpt...', flush=True)
model.load_state_dict(torch.load('$BEST_CKPT', map_location='cpu'))
model.to(device)
print(f'[Task #172 Stage 4] Model loaded on {device}', flush=True)
print(f'[Task #172 Stage 4] Total params: {sum(p.numel() for p in model.parameters()):,}', flush=True)
...
print(f'[Task #172 Stage 4] Test dataset size: {len(test_dataset)}', flush=True)

avg_recalls, avg_ndcgs = evaluate(model, test_dataloader, config['topk_list'], config['beam_size'], device)

result = {
    'task': 'task172_t5mini_kappa_max4',
    'recipe': 'T5-mini 9.18M + κ-Stereographic + κ_max=4.0 (vs baseline 2.0) SID + 200 epoch',
    'best_ckpt': '$BEST_CKPT',
    'beam_size': 20,
...
print(f'[Task #172 Stage 4] Saved: $RESULT_JSON', flush=True)
print(f'[Task #172 Stage 4] Test recalls: {avg_recalls}', flush=True)
print(f'[Task #172 Stage 4] Test NDCGs: {avg_ndcgs}', flush=True)
PYTHON_EOF

EXIT_CODE=${PIPESTATUS[0]}
echo "===== [Task #172 Stage 4] exit: $EXIT_CODE at $(date) =====" | tee -a $LOG_FILE
[ $EXIT_CODE -ne 0 ] && exit 1

python3 -c "
import json
d = json.load(open('$RESULT_JSON'))
print('=== Task #172 Test Results (T5-mini 9.18M + κ-Stereo + κ_max=4.0) ===')
...
print()
print('vs #166 (T5-mini 纯 κ-Stereo + κ_max=2.0): R@10 = 0.1019 (-3.7% vs baseline)')
