#!/bin/bash
# Task #312 Issue #35 Gate 3 — Stage 3 T5-mini 200 epoch 训练
# 2026-07-30
#
# 背景: Issue #35 Gate 1+2 PASS. Stage 3 = T5-mini 200 epoch 训练, code_path = issue35_rl_identity_sl22.
#       Stage 4 = load best ckpt 评估 test R@10.
# R11.3: 沿用 task84_hgrec_stage3_train.py 模式 (跟 task301/302 一致)
# R7: GPU 0 (R7 空闲, task309 T5-small 已完成)
# R12: save_limit=1 (Stage 3 ckpt fork)
# R14: Issue #35 hard-stop at Gate 3 fail
...
echo "===== [Task #312 Issue #35 Gate 3 Stage 3] r_l identity + s_l=[2,2,2] launched at $(date) =====" | tee $LOG_FILE
...
echo "===== [Task #312 Issue #35 Gate 3] Training PID: $TRAIN_PID =====" | tee -a $LOG_FILE
