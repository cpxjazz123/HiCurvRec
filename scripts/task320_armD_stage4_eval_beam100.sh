#!/usr/bin/env bash
# Task #320 Arm D Stage 4 eval @ beam=100
...
# Find latest ckpt for arm D
CKPT_PATH=$(ls -t $REPO/products/task320/armD/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
if [ -z "$CKPT_PATH" ] || [ ! -f "$CKPT_PATH" ]; then
    echo "ERROR: no ckpt found in $REPO/products/task320/armD/"
    exit 1
fi
...
echo "===== [Task #320 Arm D beam=100] launched at $TS =====" | tee "$LOG"

GPU=${1:-0}
echo "GPU $GPU" | tee -a "$LOG"

CUDA_VISIBLE_DEVICES=$GPU "$PYTHON_BIN" $REPO/scripts/task320_arm_stage4_eval.py \
    --arm D --ckpt_path "$CKPT_PATH" --beam_size 100 --gpu 0 --task_id 320 2>&1 | tee -a "$LOG"
echo "===== [Task #320 Arm D beam=100] completed at $(date) =====" | tee -a "$LOG"
