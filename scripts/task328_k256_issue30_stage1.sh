#!/bin/bash
# Task #328 / K=256 anchor + Issue #30 per-layer Codebook Transforms — Stage 1
...
SAVE_PATH=$REPO/products/task328/stage1_k256_issue30
mkdir -p $SAVE_PATH

TS=$(date +%Y%m%d_%H%M%S)
LOG_FILE=$LOG_DIR/stage1_k256_issue30_${TS}.log

echo "[$(date)] Launching Task #328 K=256 + Issue #30 Stage 1"
...
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task328_k256_issue30
...
TRAIN_PID=$!
echo "$TRAIN_PID" > $REPO/products/task328/_TRAINING_PID_STAGE1
echo "[$(date)] K=256 + Issue #30 Stage 1 PID=$TRAIN_PID, GPU=1"
echo ""
echo "===== Task #328 / K=256 + Issue #30 Stage 1 Launched ====="
