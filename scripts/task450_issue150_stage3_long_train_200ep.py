...
VAL_PARQUET = "/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/val.parquet"  # owner 2026-08-01 派工: train 切 10% 作 val
...
def main():
    ...
    train_trace = []
    epoch0_loss = None
    best_val_r10 = -1.0  # owner 2026-08-01 派工: 早停 best tracker
    patience_counter = 0  # owner 2026-08-01 派工: patience=5
    early_stop_triggered = False  # owner 2026-08-01 派工
            ...
            try:
                ...
                with torch.no_grad():
                    for _es_i in range(0, _es_n, BATCH_SIZE):
                        ...
                        # meta tensors (按 wrapper signature 探测传)
                        ...
                        # try each subset of compatible meta kwargs
                        _es_out = None
                        _es_logits = None
                        for _es_keys in [_es_compat_kw, {}]:  # first with metas, then bare
                            _es_call_kw = dict(_es_kwargs)
                            _es_call_kw.update(_es_keys)
                            try:
                                _es_loss, _es_logits, _es_alpha = model_wrapper(**_es_call_kw)
                                break
                            except TypeError:
                                continue
                        if _es_logits is None:
                            raise RuntimeError(f"EarlyStop: no compatible forward call, params={list(_es_sig.parameters)}")
                        # _es_logits is (B, 4, V) from wrapper output
                        # Stage 4 eval 协议: argmax → 4-digit SID pred, 跟 target 4-digit 全 match 算 R@K
                        _es_preds = _es_logits.argmax(dim=-1)  # (B, 4)
