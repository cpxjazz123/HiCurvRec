#!/usr/bin/env bash
# Step 3 闭环: 50 epoch radius spread + EMA 中心化 + LR_LOG_R=10.0
# 配置 = Step 2 + EMA 中心化修复 (utils.py 已加).
# 预期: 半径 std > 0.05 (LR 足够), 方向多样 (50 epoch 自然形成), Step 3 通过.
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_50ep_ema
LOG="${OUTDIR}/train.log"
mkdir -p "$OUTDIR"

# 跟 Step 2 完全一致 (radius spread + LR_LOG_R=10), 只改 EMA 默认开 (utils.py 已加).
COMMON="\
  --data_path $DATA \
...
echo "==== Step 3 闭环: 50 epoch EMA + radius spread (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_ema \
PYTHONUNBUFFERED=1 \
