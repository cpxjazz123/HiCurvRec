...
Owner 2026-08-01 派发 Issue #179: 复用 #177 (Task #470) BoundedKappaScaleConditioner (α=softplus(α_logit).clamp(max=0.5))
完整 Stage 3 200 epoch + Stage 4 R@K 双复跑 (R@5/10/20, NDCG@5/10/20).

R18 4 维度 vs Task #450 (Issue #150 方向C 200 epoch):
- D1 spec: 方向A Gate4 vs 方向C Gate3+4 (长训协议相同, wrapper 不同)
- D2 实施: BoundedKappaScaleConditioner (α clamp 0.5 + κ+scale 元数据) vs ZeroCenteredLayerNormAdapter (无 adapter)
- D3 Gate 4 决策: 方向A target reached vs 方向C pending — 平行对照
- D4 引用: 同一族 + Issue #179 spec 强制复用 #177 wrapper

R12 强制: 训练结束 → 删旧 adapter_200ep.pt → 存新 ckpt
R7: GPU 1 (R7 满足, GPU 0 task450, 2/3 留给 task473)
R137: TRITON_CACHE_DIR=~/.triton/cache_task472
"""
import sys
import os
import json
...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_task472"
os.makedirs(os.environ["TRITON_CACHE_DIR"], exist_ok=True)

# ============================================================================
# Config (per Issue #179 spec, 200 epoch 等同方向C task450)
# ============================================================================
SEED = 42
DEVICE = "cuda:0"  # CUDA_VISIBLE_DEVICES=1 remaps GPU 1 → cuda:0
...
EXPECTED_SID_SHA = "2dab29229c36a9695a11d70b61d1b80fae95a08d00e3c3675e9bf899b709508a"  # #157/#177 一致

PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task472_issue179_direction_a_gate4_200ep")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
TRAINING_PID_FILE = PRODUCT_DIR / "_TRAINING_PID"
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task472_issue179_direction_a_gate4_200ep.log"
...
# ============================================================================
# Wrapper 类 (跟 task470 同架构, 直接 import 复用)
# ============================================================================
import importlib.util as _ilu
_t470_spec = _ilu.spec_from_file_location(
    "task470_module",
    "/home/wlia0047/ar57/wenyu/GeneRec/scripts/task470_issue177_gate3_a_recontinue.py",
)
_t470_mod = _ilu.module_from_spec(_t470_spec)
_t470_spec.loader.exec_module(_t470_mod)
WrapperCls = _t470_mod.HG_Rec_with_BoundedAdapter
verify_sid_token_range = _t470_mod.verify_sid_token_range
get_t5_config = _t470_mod.get_t5_config
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #472 Issue #179 方向A Gate4] BoundedKappaScaleConditioner 完整 Stage3 200 epoch + Stage 4")
    log_lines.append(f"[R18] 4 维度 vs Task #450: 方向A wrapper vs 方向C zero-centered LN adapter")
    log_lines.append("=" * 70)
    ...
    log_lines.append(f"[Hash Check 跟 #157/#177 一致] {sid_sha == EXPECTED_SID_SHA}")
    ...
    config = {
        ...
        "wrapper_class": "BoundedKappaScaleConditioner (from #470/#177)",
        "issue_spec": "Issue #179",
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Early Stopping (owner 2026-08-01 派工)
    ...
    with open(TRAIN_TRACE_PATH, "w") as f:
        json.dump({"train_trace": train_trace, "best_epoch": best_epoch,
                   ...
                   "issue": "Issue #179", "task_id": 472}, f, indent=2)

    # Load best ckpt (per R12)
    if ADAPTER_CKPT_PATH.exists():
    ...
    def run_eval(run_id):
        ...
        for batch_idx in range(n_test_batches):
            ...
            sid_meta = torch.stack([digit_values / 1025.0, layer_idx / 4.0, pos_in_history, padding_flag], dim=-1)
            kappa_meta = torch.zeros(B, 3, dtype=torch.float32, device=DEVICE)
            scale_meta = torch.ones(B, 3, dtype=torch.float32, device=DEVICE)  # 训练时 init=1, Stage 4 同训练协议
