#!/bin/bash
# Task #163 Stage 4 — T5-mini 5.5M + ORC-init free-curv test eval
# Fork of task162 stage4, target = _t5_rqvae_orc_init.npy
...
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task163_orc_init_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #163 Stage 4] T5-mini 5.5M + ORC-init free-curv test eval launched at $(date) =====" | tee $LOG_FILE
echo "Recipe: T5-mini 5.5M (d_model=128, 4+4 layers, 4 heads × d_kv=32) + ORC-init free-curv SID (θ_init=[-0.3161,-0.7997,-0.7997])" | tee -a $LOG_FILE
echo "GPU 2" | tee -a $LOG_FILE
echo "Output JSON: $RESULT_JSON" | tee -a $LOG_FILE

export RESULT_JSON

BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task163/ckpt_hgrec/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
...
result = {
    'task': 'task163_orc_init',
    'recipe': 'T5-mini 5.5M (d_model=128) + ORC-init free-curv SID (θ_init=[-0.3161,-0.7997,-0.7997], β=0.5, [32,64,256], sk=0)',
    ...
    'theta_init': [-0.3161, -0.7997, -0.7997],
    'kappa_max': 2.0,
}
...
echo "===== [Task #163 Stage 4] Test eval completed at $(date), exit code: $EXIT_CODE =====" | tee -a $LOG_FILE