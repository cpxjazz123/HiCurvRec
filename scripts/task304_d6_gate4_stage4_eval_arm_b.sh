#!/bin/bash
# Task #304 / D6 ablation Arm B — Gate 4 Stage 4 eval (s_l only SID)
# 2026-07-30
#
# 背景: Task #304 Arm B Gate 1+2+3 PASS
#       Gate 4 = Stage 4 评估 Test R@10 > 0.1020
# R7: GPU 2 (R7 空闲)
...
export TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task304_arm_b_gate4

LOG_DIR=$REPO/logs/task304
TS=$(date +%b-%d-%Y_%H-%M-%S | sed 's/^.../\L&/')
LOG_FILE=$LOG_DIR/stage4_eval_arm_b_${TS}.log
RESULT_JSON=$REPO/verdicts/task304_d6_stage4_arm_b_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #304 D6 Arm B Gate 4 Stage 4] s_l only SID eval launched at $(date) =====" | tee $LOG_FILE
echo "code_path = _t5_hrqvae_d6_arm_b_s_only.npy" | tee -a $LOG_FILE
echo "GPU 2 (R7 空闲)" | tee -a $LOG_FILE
echo "Output JSON: $RESULT_JSON" | tee -a $LOG_FILE

BEST_CKPT=$(ls -t $REPO/products/task304/ckpt_hgrec_arm_b/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
...
print(f'[Stage 4 D6 Arm B] Test dataset size: {len(test_dataset)}, codebook={config[\"codebook_size\"]}')

avg_recalls, avg_ndcgs = evaluate(model, test_dataloader, config['topk_list'], config['beam_size'], device)

result = {
    'task': 'task304_d6_arm_b_s_only',
    'recipe': 'per-layer r_l=[1,1,1] + s_l=[2,2,2] + c_k_range (D6 Arm B s_l only)',
    'best_ckpt': '$BEST_CKPT',
    'beam_size': 20,
...
print(f'[Stage 4 D6 Arm B] Saved: $RESULT_JSON')
print(f'[Stage 4 D6 Arm B] Test recalls: {avg_recalls}')
print(f'[Stage 4 D6 Arm B] Test NDCGs: {avg_ndcgs}')
" 2>&1 | tee -a $LOG_FILE

EXIT_CODE=${PIPESTATUS[0]}
echo "===== [Task #304 D6 Arm B Gate 4 Stage 4] Test eval completed at $(date), exit code: $EXIT_CODE =====" | tee -a $LOG_FILE