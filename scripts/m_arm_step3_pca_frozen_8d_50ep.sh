#!/usr/bin/env bash
# 方向 H 扩展: PCA 钉方向 + 冻结 + hyp_dim=8 (跟 v11 一致). 看 8D 上能不能 collision ≤ 12% AND 5cond PASS.
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_50ep_wdiv100_pca_frozen_8d_${TS}
...
echo "==== 方向 H 扩展: PCA-init + freeze + hyp_dim=8 (GPU=$GPU) ===="
echo "outdir → $OUTDIR"
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
...
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_pca_frozen_8d_${TS} \
PYTHONUNBUFFERED=1 \
