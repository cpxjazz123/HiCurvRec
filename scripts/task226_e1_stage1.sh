#!/usr/bin/env bash
# Task #226 E1 Stage 1 — NormCap only (sanity baseline for c_k spread)
# GPU: 1 (per user 2026-07-26 编排)
set -euo pipefail
...
SAVE_DIR=/home/wlia0047/ar57/wenyu/GeneRec/products/task226/e1_normcap
LOG_DIR=/home/wlia0047/ar57/wenyu/GeneRec/logs/task226
LOG_FILE=$LOG_DIR/e1_stage1_${TS}.log
PID_FILE=$SAVE_DIR/_TRAINING_PID

mkdir -p "$SAVE_DIR" "$LOG_DIR"
echo "[task226] launching E1 Stage 1 (NormCap only) on cuda:1 ..."
...
python3 -u train_hrqvae.py \
    ...
    --c_k_update_mode fixed --distance_mode sphere \
    --save_limit 5 --device cuda:1 \
    --ckpt_dir "$SAVE_DIR" > "$LOG_FILE" 2>&1 &

TRAIN_PID=$!
echo "$TRAIN_PID" > "$PID_FILE"
echo "[task226] E1 PID=$TRAIN_PID, log=$LOG_FILE"
