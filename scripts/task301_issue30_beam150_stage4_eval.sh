...
LOG="$LOG_DIR/stage4_eval_beam150_${TS}.log"
echo "===== [Task #301 / Issue #30 ckpt @ beam=150 ceiling test] launched at $TS =====" | tee "$LOG"
...
print(f'[TASK301 Issue #30 beam=150] missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
out = {
    'task': 'task301_issue30_ckpt_beam150_ceiling_test',
    'ckpt_path': '${CKPT_PATH}',
    'code_path': '_t5_hrqvae_issue30_per_layer_transforms.npy',
    'beam_size': 150,
    'config': config,
...
out_path = os.path.join(metrics_dir, 'task301_issue30_beam150_metrics.json')
with open(out_path, 'w') as f:
    json.dump(out, f, indent=2, default=float)
print(f'[TASK301 Issue #30 beam=150] metrics written to {out_path}')
print(f'[TASK301 Issue #30 beam=150] elapsed={elapsed:.1f}s')
print(f'[TASK301 Issue #30 beam=150] Recall@5/10/20 = {[avg_recalls["Recall@5"], avg_recalls["Recall@10"], avg_recalls["Recall@20"]]}')
print(f'[TASK301 Issue #30 beam=150] NDCG@5/10/20 = {[avg_ndcgs["NDCG@5"], avg_ndcgs["NDCG@10"], avg_ndcgs["NDCG@20"]]}')
PYEOF
echo "===== [Task #301 Issue #30 beam=150] completed at $(date) =====" | tee -a "$LOG"
