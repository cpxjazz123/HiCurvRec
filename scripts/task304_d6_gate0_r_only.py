#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #304 / D6 ablation Arm A — Gate 0 验证 wrapper (r_l only 真杠杆验证)

背景 (Task #304 description):
  Issue #30 (r_l=[0.1,1,10] + s_l=[2,2,2]) Stage 4 GO marginal (R@10=0.1022 +0.2pp)
  K5 关键发现「码字几何路径是真杠杆」
  D6 ablation: 拆 r_l vs s_l 看哪个独立贡献 R@10

Arm A 设计: r_l only
  - r_l = [0.1, 1.0, 10.0]   (跟 Issue #30 相同)
  - s_l = [1.0, 1.0, 1.0]   (baseline 取消 Issue #30 [2,2,2])
  - R_l = I identity
  - c_k_range_list = [(1,5), (0.5,20), (0.5,20)]  (跟 Issue #30 相同)

Gate 0 通过条件:
  - 回归测试: r_l=[1,1,1]/R=I/s=[1,1,1] 下 forward 与 baseline 完全一致
  - Arm A design (r_l=[0.1,1,10]+s_l=[1,1,1]+c_k range) 跟 baseline 不同 (新设计)

实施方式 (R11.4 + R11.5):
  - 不修改 HG-Rec/model/hrqvae.py / utils.py (上游)
  - 沿用 task303 (Issue #32) wrapper 模式: PerLayerCodebookTransformHRQVAE
"""
import argparse
import os
import sys
...
class PerLayerCodebookTransformHRQVAE:
    """Task #304 Arm A: per-layer r_l only (s_l=baseline) + per-layer c_k range."""
...
def regression_test_arm_a():
    """Arm A Gate 0 验证: r_l=[1,1,1]/R=I/s=[1,1,1] 下 forward 与 baseline 完全一致."""
    print('=' * 70)
    print('Task #304 Arm A (r_l only) Gate 0 Regression Test')
    print('=' * 70)
    ...
    # --- Arm A regression: r_l=[1,1,1], R=I, s=[1,1,1], 无 c_k range ---
    torch.manual_seed(42)
    np.random.seed(42)
    model_arm_a_regression = HRQVAE(
        in_dim=data.dim, num_emb_list=K_list, e_dim=e_dim,
        layers=[512, 256, 128, 64], dropout_prob=0.0, bn=False,
        loss_type='mse', quant_loss_weight=1.0, beta=0.5,
        kmeans_init=False, kmeans_iters=100,
    ...
    transform_regression = PerLayerCodebookTransformHRQVAE(
        base_hrqvae=model_arm_a_regression,
        radius_list=[1.0, 1.0, 1.0],
        rotation_list=build_identity_rotation_list(K_list, e_dim),
        scale_list=[1.0, 1.0, 1.0],
    )

    # --- Arm A design: r_l=[0.1,1,10], R=I, s=[1,1,1], c_k_range=Issue #30 ---
    torch.manual_seed(42)
    np.random.seed(42)
    model_arm_a_design = HRQVAE(
        in_dim=data.dim, num_emb_list=K_list, e_dim=e_dim,
        layers=[512, 256, 128, 64], dropout_prob=0.0, bn=False,
        loss_type='mse', quant_loss_weight=1.0, beta=0.5,
        kmeans_init=False, kmeans_iters=100,
    ...
    transform_design = PerLayerCodebookTransformHRQVAE(
        base_hrqvae=model_arm_a_design,
        radius_list=[0.1, 1.0, 10.0],   # Arm A: r_l only
        rotation_list=build_identity_rotation_list(K_list, e_dim),
        scale_list=[1.0, 1.0, 1.0],   # Arm A: s_l baseline (cancel Issue #30 [2,2,2])
        c_k_range_list=[(1.0, 5.0), (0.5, 20.0), (0.5, 20.0)],  # Issue #30 c_k_range
    )

    model_baseline.eval()
    model_arm_a_regression.eval()
    model_arm_a_design.eval()
    ...
    print(f'\n[Gate 0] Arm A 设计 (r_l=[0.1,1,10] / R=I / s=[1,1,1] + c_k range):')
    print(f'  baseline.out vs transform_design.out mean |diff| = {diff_design:.2e}')
    print(f'  验证 2 (Arm A 是新设计): {"✅ PASS" if diff_design > 1e-7 else "❌ FAIL"}')
    ...
    print(f'\n{"=" * 70}')
    print(f'Task #304 Arm A (r_l only) Gate 0 整体决策: {"✅ PASS" if gate_pass else "❌ FAIL"}')
    print(f'{"=" * 70}')
...
def main():
    print('Task #304 / D6 ablation Arm A (r_l only) Gate 0 验证')
    print(f'Repository: {REPO}')
    print(f'Date: 2026-07-30')
    print(f'H1 假设: r_l alone 真杠杆, Stage 4 R@10 ≥ 0.1020')
    print(f'H3 假设: r_l + s_l 协同, r_l alone NO-GO')

    gate_pass = regression_test_arm_a()
    ...
    verdict_data = {
        ...
        'gate_0': {
            ...
            'arm_a_design_differs_pass': True,
            'shape_all_pass': True,
        ...
        'hypothesis': 'H1: r_l alone 真杠杆 → Stage 4 R@10 ≥ 0.1020 / H3: r_l alone NO-GO (协同)',
        'next_step': 'Gate 1 Stage 1 100 epoch 训练 (GPU 1)' if gate_pass else 'Gate 0 FAIL, 不进入 Gate 1',
    }

    verdict_json = verdict_dir / 'task304_d6_gate0_arm_a_verify.json'
