#!/usr/bin/env python3
"""Task #464 / Issue #171 [方向A Gate2] 零中心有界残差 + κ 同步重校准 短程审计.

R18 4 维度对比 #169 (closed NO-GO 724c38c):
- D1: 真实 Stage 1 export + κ/scale 三层独立learnable + 3 epoch sanity
- D2: task462 模板 + 加 (a) 零中心 bounded residual y = layer_norm(x + tanh(w)*dx)
       + (b) κ 更新后同步重校准 codebook + distance
- D3: α 1 epoch 涨 2524× 是 #169 失败根因 → 零中心 + tanh + 同步重校准应该是根因解药
- D4: arXiv:2405.13979 + DOI:10.1016/j.neunet.2026.109172 (task-geometry decoupling)

Issue #171 spec 关键约束:
- 保留三层独立 learnable κ (L0 K64 / L1 K128 / L2 K256)
- 加零中心有界残差门控 (per-layer)
- κ 更新后同步重校准 codebook 尺度 + 距离 + 统一公式 (#47 复用)
- 真实 Stage 1 κ/scale metadata 注入
...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_task464"
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task464_issue171_stage2_zero_centered_bounded_residual_kappa_recalibration")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task464_issue171_stage2_zero_centered_bounded_residual_kappa_recalibration.log"
...
class ZeroCenteredBoundedResidualAdapter(nn.Module):
    """Issue #171 spec 关键机制:
    1. 零中心: layer_norm 让 residual 起点 = 0
    2. 有界: tanh 饱和在 ±RESIDUAL_BOUND
    3. per-layer learnable κ_l
    4. κ 更新后同步重校准 codebook 尺度 + 距离
    """
    def __init__(self, d_model=128, n_layers=3, sid_dim=4,
                 alpha_init_logit=-10.0, kappa_init_logit=0.0, residual_bound=0.5):
        super().__init__()
    ...
    def synchronize_kappa(self, codebooks):
        ...
        c_l = kappa_l
        return c_l.detach().cpu().numpy()


class HG_Rec_with_ZeroCenteredBoundedResidual(nn.Module):
    def __init__(self, t5_config, t5_state_dict, d_model=128, n_layers=3, sid_dim=4,
        ...
        self.adapter = ZeroCenteredBoundedResidualAdapter(
            d_model=d_model, n_layers=n_layers, sid_dim=sid_dim,
            alpha_init_logit=alpha_init_logit, kappa_init_logit=kappa_init_logit,
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #464 Issue #171 Gate 2] 零中心有界残差 + κ 同步重校准 短程 sanity")
    log_lines.append("=" * 70)
    ...
    config = {
        ...
        "alpha_init_logit": ALPHA_INIT_LOGIT, "kappa_init_logit": KAPPA_INIT_LOGIT,
        "residual_bound": RESIDUAL_BOUND, "d_model": D_MODEL, "n_layers": 3,
        "triton_cache_dir": os.environ["TRITON_CACHE_DIR"],
        "task_id": 464, "issue": "Issue #171",
        "real_stage1_export": True, "three_epoch_sanity": True,
        "mechanism": "zero_centered_bounded_residual_kappa_recalibration",
    }
    ...
    log_lines.append(f"  [Gate 1] 真实 Stage 1 export: {gate1_pass}")
    ...
    if not gate1_pass:
        ...
        with open(VERDICT_PATH, "w") as f:
            json.dump({"gate1_pass": False, "reason": "stage1_export_failed",
                       "codebook_hashes": cb_hashes, "task_id": 464,
                       "issue": "Issue #171"}, f, indent=2)
        return
    ...
    log_lines.append(f"  Real metadata: shape={real_metadata.shape}, hash={real_metadata_sha[:16]}...")
    ...
    model_wrapper = HG_Rec_with_ZeroCenteredBoundedResidual(
    ...
    log_lines.append(f"[Precheck] init κ_l (softplus): {init_kappa_l.tolist()}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        log_lines.append(f"  [epoch {epoch}] loss={avg_loss:.4f}, cond_grad={avg_cond_grad:.4e}, ln_grad={avg_ln_grad:.4e}, kappa_grad={avg_kappa_grad:.4e}, kappa_l={[f'{x:.4f}' for x in kappa_l_val]}, residual_w={residual_w_val:.4e}, val_R@10={val_r10:.4f}, nan_inf={nan_inf_detected}")
    ...
    log_lines.append("  (5) cond_grad epoch trace: " + str(["{:.4e}".format(t["avg_cond_grad_norm"]) for t in train_trace]))
    log_lines.append("  (6) ln_grad epoch trace: " + str(["{:.4e}".format(t["avg_ln_grad_norm"]) for t in train_trace]))
    log_lines.append("  (7) kappa_grad epoch trace: " + str(["{:.4e}".format(t["avg_kappa_grad_norm"]) for t in train_trace]))
    log_lines.append(f"  (8) nan_inf all False: {all(not t['nan_inf'] for t in train_trace)}")
    log_lines.append(f"  (9) sid_range all_in_range: {sid_range['all_in_range']}")
    log_lines.append(f"  (10) save/load missing={len(missing_keys)}/unexpected={len(unexpected_keys)}")
    log_lines.append(f"  (11) forward diff: {forward_diff:.2e}")
    ...
    with open(TRAIN_TRACE_PATH, "w") as f:
        json.dump({"train_trace": train_trace, "epoch0_loss": epoch0_loss,
                   ...
                   "kappa_l_per_layer_diff": kappa_l_per_layer_diff}, f, indent=2)
    ...
    with open(VERDICT_PATH, "w") as f:
        json.dump({
            ...
            "r23_kill": r23_kill, "task_id": 464, "issue": "Issue #171",
            "kappa_l_history": kappa_l_history, "residual_w_history": residual_w_history,
