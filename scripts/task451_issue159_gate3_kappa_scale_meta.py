...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_task451"
...
D_MODEL = 128
BATCH_SIZE = 32
NUM_EPOCHS = 10  # Gate 3 spec 10 epoch 短训
LR_CONDITIONER = 1e-3
LR_LAYERNORM = 1e-4
ALPHA_INIT = 0.0
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task451_issue159_gate3_kappa_scale_meta")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task451_issue159_gate3_kappa_scale_meta.log"
...
class KappaScaleConditioner(nn.Module):
    """Issue #159: κ + sync codebook scale 元数据 → 每层有界零中心残差.
...
class HG_Rec_with_KappaScaleAdapter(nn.Module):
    """Issue #159 wrapper: 包装 T5, 加入 κ-scale conditioner + T5 input LayerNorm (解冻)."""
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, sid_meta=None, kappa_meta=None, scale_meta=None):
        """Issue #159 forward: 拿到 input embedding (跟 #150 同路径) + κ-scale residual + LayerNorm."""
...
def gradient_proof(model_wrapper, sid_meta, kappa_meta, scale_meta, history_tensor, attention_mask, target_tensor):
    ...
    loss = loss + 1e-4 * (model_wrapper.adapter.alpha_logit ** 2)  # 强制梯度 path
    loss.backward()
    grad_summary = {}
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #451 Issue #159 Gate3] κ-scale 元数据 + T5 零中心几何适配 (R18, 跟 #150/#157 区分)")
    log_lines.append("=" * 70)
    ...
    if not precheck_pass:
        log_lines.append("[STOP] Precheck FAIL, 不做训练 (Issue #159 spec 强制)")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx in range(n_batches):
            ...
            # κ + scale 元数据 (Issue #159 spec: 来自 #157 的同步重校准结果)
