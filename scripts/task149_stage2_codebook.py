"""Task #149 Stage 2 — fork of task144_stage2_codebook.py with Task #149 paths.

Loads FreeCurvHRQVAE trained ckpt from products/task149/train/main_heterokappa/
→ produces (N, 4) SID .npy for downstream Stage 3 T5 training.

Distinct from Task #144 stage 2 because:
  - Task #149 ckpt was trained with theta_init_list = [-0.3, 0, 0.3] (3 diverse inits)
  - Final κ final values: L0=-0.127, L1=0.0, L2=+0.163 (vs Task #144 Arm A all 0)
"""
import argparse
import collections
import json
...
def main():
    ...
    ckpt_path = "/home/wlia0047/ar57/wenyu/GeneRec/products/task149/train/main_heterokappa/best_loss_model.pth"
    output_path = "/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/Instruments_t5_hrqvae_heterokappa.npy"
    diag_path = output_path.replace(".npy", "_diagnostic.json")

    device = torch.device(args.device)
    log = logging.getLogger(f"task149_stage2")
    ...
    log.info(f"  theta_init={ckpt_args.get('theta_init', '?')}, theta_init_list={ckpt_args.get('theta_init_list', 'N/A')}")

    data = EmbDataset(ckpt_args['data_path'])
    log.info(f"  Data: {len(data)} items, dim={data.dim}")

    # Read the soft_vq params (Task #145 lineage) — pass through even if disabled
    soft_vq_kwargs = dict(
        soft_vq_enabled=ckpt_args.get('soft_vq_enabled', False),
        soft_vq_tau_start=ckpt_args.get('soft_vq_tau_start', 1.0),
        soft_vq_tau_end=ckpt_args.get('soft_vq_tau_end', 0.01),
        soft_vq_anneal_type=ckpt_args.get('soft_vq_anneal_type', 'linear'),
    )
    ...
    # Task #149: re-apply per-layer θ_init from theta_init_list (default θ=0 init may overwrite ckpt)
    theta_init_list = ckpt_args.get('theta_init_list', None)
    if theta_init_list is not None:
        with torch.no_grad():
            for li, vq in enumerate(model.hrq.vq_layers):
                vq.theta_m.data = torch.full(
                    (ckpt_args['M'],), theta_init_list[li], dtype=torch.float32
                )
        log.info(f"  Re-applied per-layer θ_m init: {theta_init_list}")

    model.load_state_dict(state_dict)
    model = model.to(device)
    model.eval()

    # Per-layer κ_m diagnostic (post-load)
    for li, vq in enumerate(model.hrq.vq_layers):
    ...
    for d in tqdm(data_loader, desc=f"Task #149 Stage 2 forward"):
        d = d.to(device)
    ...
    diag = {
        "task": "task149_heterogeneous_kappa",
        "ckpt_path": ckpt_path,
        ...
        "kappa_init_list": ckpt_args.get('theta_init_list', None),
        "sinkhorn_iters": tt,
