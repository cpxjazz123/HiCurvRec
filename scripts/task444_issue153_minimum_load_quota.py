...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task444_issue153_minimum_load_quota")
...
def precheck(model: BilateralQuotaKappaModel, sample_emb: torch.Tensor) -> Dict:
    ...
    print("\n=== Precheck 4 项验证 (Issue #153 spec 强制) ===", flush=True)
    model.train()
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Task #444 / Issue #153 [方向A Gate1] 最小占用配额的全覆盖硬双曲分配")
    ...
    # ── Precheck ──
    print(f"{'='*60}\nPHASE 1: PRECHECK (Issue #153 spec 强制)\n{'='*60}")
    ...
    gate1_pass = util_ok and max_load_ok and min_load_ok and kappa_grad_ok and no_nan
    ...
    print(f"\n产物落地: {PRODUCT_DIR}")
    print(f"  config.json + precheck.json + quota_graph_proof.json + train_curve.json + verdict.json")
