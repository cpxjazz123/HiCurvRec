...
新假设 (vs #127 失败): 训练期对稳定 hyperbolic cost matrix 执行**批内 Sinkhorn 平衡运输分配**,
其列边际固定为均匀、行边际为样本质量; 硬 argmin SID 仍仅用于记录/导出.
κ 的梯度来自运输损失与重构损失, κ 更新后继续按 #47 同步 scale/codebook.

不做: global κ / fixed-only / 欧氏绕过 / 改 K / 温度 sweep.
"""
import sys
import json
import math
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task423_issue131_sinkhorn_balanced_transport")
...
def sinkhorn_balanced_transport(cost_matrix, row_marginal=None, col_marginal=None,
    ...
    """Stable Sinkhorn-Knopp balanced transport on cost_matrix (B, K).

    Returns:
        plan: (B, K) transport plan
        col_residual: max deviation of col marginal from target
        row_residual: max deviation of row marginal from target
        iters: actual iterations used
    """
    B, K = cost_matrix.shape
    if row_marginal is None:
        row_marginal = torch.ones(B, device=cost_matrix.device) / B
    if col_marginal is None:
        col_marginal = torch.ones(K, device=cost_matrix.device) / K  # uniform column marginal
    # Kernel from cost (smaller cost → larger kernel)
    log_K = -cost_matrix  # log kernel ∝ -cost
    ...
    for it in range(max_iters):
        # u-update: scale rows to match row_marginal
        v_old = v.clone()
        u = row_marginal / (K_kernel @ v + 1e-30)
        # v-update: scale cols to match col_marginal
...
class SinkhornBalancedHRQVAE(nn.Module):
    """Stable hyperbolic cost + Sinkhorn balanced transport + hard argmin SID (record only)."""
...
class ArgminControlHRQVAE(nn.Module):
    """Stable hyperbolic cost + argmin SID (control, no transport)."""
...
def compute_layer_metrics(model, X, layer_idx, batch_size, device):
    ...
    return {"util": util, "max_load": max_load, "entropy_normalized": entropy / entropy_max,
            "n_used": n_used, "K": K}


def five_step_sinkhorn_audit(model, X, batch_size, device):
    """5-step audit per Issue #131 §Gate1 1:
    1. Sinkhorn 行/列残差, 域裕度, κ梯度, loss 全 finite
    2. κ 微扰改变 transport plan + loss
    3. hard assignments 不作为梯度唯一来源
    4. round-trip OK
    5. NaN/Inf 监测
    """
    # Baseline
    ...
    # κ perturbation (no rescale hook, audit κ dependency)
    for l in range(len(model.num_emb_list)):
        model.kappa_l_raw[l].data += 2.0  # 大幅扰动打破 Sinkhorn 收敛, 触发 plan 重分配
    ...
    # If plan_change_max < 1e-6, check cost matrix changed (Sinkhorn on uniform marginals may be insensitive to micro cost)
    cost_changed = (out_perturb["out_list"][0]["cost"] - out_baseline["out_list"][0]["cost"]).abs().max() > 1e-6

    # Domain margin OK (use baseline κ margin, since perturb only diagnoses plan sensitivity, not domain failure)
    domain_ok = domain_margin_baseline < 1.0 - EPS

    # Sinkhorn residuals finite
    sinkhorn_residual_ok = (np.isfinite(row_res_perturb) and np.isfinite(col_res_perturb) and
    ...
    # Hard assignment recorded but NOT sole gradient source
    # (transport plan is gradient source → check plan has grad-fn)
    plan_requires_grad = plan_perturb.requires_grad

    audit_pass = all([domain_ok, sinkhorn_residual_ok, cost_changed, loss_changed,
                       grad_finite_nz, plan_requires_grad])
    ...
    return {
        ...
        "sinkhorn_residual_ok": bool(sinkhorn_residual_ok),
        "row_res_baseline": row_res_baseline, "col_res_baseline": col_res_baseline,
...
def main():
    ...
    np.random.seed(SEED)
    print("=" * 70)
    print(f"[Task #423 Issue #131 Gate 1] Sinkhorn balanced transport on stable hyperbolic cost")
    print("=" * 70)
    ...
    # 5-step audit on main config
    print("\n[5-step sinkhorn audit] Main config...", flush=True)
    e_dim_actual = X.shape[1]
    audit_model = SinkhornBalancedHRQVAE(
        num_emb_list=NUM_EMB_LIST, e_dim=e_dim_actual,
        kappa_min=KAPPA_MIN, kappa_max=KAPPA_MAX, beta=BETA, temperature=TEMPERATURE,
    ).to(DEVICE)
    audit = five_step_sinkhorn_audit(audit_model, X, BATCH_SIZE, DEVICE)
    audit_pass = audit.get("audit_pass", False)
    print(f"  domain_ok: {audit.get('domain_ok')}")
    print(f"  sinkhorn_residual_ok: {audit.get('sinkhorn_residual_ok')}")
    ...
    print(f"  cost_changed: {audit.get('cost_changed')}")
    ...
    if not audit_pass:
        verdict = {"task": "task423_issue131_sinkhorn_balanced_transport", "issue": 131,
    ...
    print("\n[Main] 30 epoch Sinkhorn-balanced training...", flush=True)
    main_model, main_metrics, main_best_epoch, main_best_avg_util = train_one_config(
        SinkhornBalancedHRQVAE, X, BATCH_SIZE, NUM_EPOCHS, DEVICE, SEED, "Main-Sinkhorn",
    )

    print("\n[Control] 30 epoch argmin-only control...", flush=True)
    control_model, control_metrics, control_best_epoch, control_best_avg_util = train_one_config(
        ArgminControlHRQVAE, X, BATCH_SIZE, NUM_EPOCHS, DEVICE, SEED, "Ctrl-Argmin",
    )
    ...
    verdict = {
        "task": "task423_issue131_sinkhorn_balanced_transport", "issue": 131,
        "audit": audit, "audit_pass": audit_pass,
        "main": {
