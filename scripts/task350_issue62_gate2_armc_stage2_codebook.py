...
Issue #62 Arm C ckpt → (N, 4) SID .npy 给 Stage 3 T5-mini.

R11.5 决策: 沿用 Issue #30 Gate 2 sinkhorn_5_iter 协议 (task298 review PASS).
Stage 2 输出文件名: Instruments_t5_hrqvae_issue62_gate1_armc.npy (跟 Issue #30 同位置)
"""
import collections
import glob
import logging
...
def main():
    dataset = "Instruments"
    ckpt_base = "/home/wlia0047/ar57/wenyu/GeneRec/products/task350/hrqvae_issue62_gate1_armc"
    ...
    output_path = f"/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/{dataset}/{dataset}_t5_hrqvae_issue62_gate1_armc.npy"
    device = torch.device("cuda:0")

    log = logging.getLogger("task350_issue62_gate2_armc")
    ...
    # R11.5: Issue #62 wrapper 在 init 时已 transform .embeddings.weight 一次
    # ckpt 中 weights 即变换后的. baseline HRQVAE + strict=False 兼容
    ...
    # Sinkhorn 5 iter (Issue #62 Gate 2 same as Issue #30)
    MAX_SK_ITERS = 5
    tt = 0
    while True:
    ...
    # Issue #62 Gate 2 pass conditions (mirror Issue #30)
    log.info("=" * 60)
    log.info("Issue #62 Gate 2 verification:")
