...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task452_issue160_gate3_weighted_mixed_curvature_meta")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task452_issue160_gate3_weighted_mixed_curvature_meta.log"
...
class WeightedMixedCurvatureConditioner(nn.Module):
    """Issue #160: per-layer [κ_l, alpha_l, beta_l, gamma_l] 三分量加权混合曲率 → 零中心几何残差.
...
class HG_Rec_with_WeightedMixedAdapter(nn.Module):
    """Issue #160 wrapper: 包装 T5, 加入三分量 conditioner + T5 input LayerNorm (解冻)."""
...
def gradient_proof(model_wrapper, sid_meta, curvature_meta, history_tensor, attention_mask, target_tensor):
    ...
    loss = loss + 1e-4 * (model_wrapper.adapter.alpha_logit ** 2)
    loss.backward()
    grad_summary = {}
...
def main():
    ...
    log_lines = []
    log_lines.append("=" * 70)
    log_lines.append(f"[Task #452 Issue #160 Gate3] 三分量加权混合曲率元数据 + T5 逐层几何适配 (R18, 跟 #150/#158/#159 区分)")
    log_lines.append("=" * 70)
    ...
    if not precheck_pass:
        log_lines.append("[STOP] Precheck FAIL, 不做训练 (Issue #160 spec 强制)")
