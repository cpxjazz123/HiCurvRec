#!/usr/bin/env bash
# Task #304 Arm A (D6 ablation r_l only, s_l identity) Stage 4 re-eval @ beam=100
# 目的: 验证 K14 (beam=100) 是否放大 task304 Arm A (s_l=[1,1,1] identity) — Issue #35 r_l alone 真杠杆验证
# Anchors:
#   task304 Arm A @ beam=20 → R@10=0.0990 (below baseline 0.1020)
#   task309b Issue #30 @ beam=100 → R@10=0.1041 (+2.1pp vs baseline)
# R11.5: GPU 1 空闲
...
echo "===== [Task #304 Arm A D6 ablation / beam=100 re-eval] launched at $TS =====" | tee "$LOG"
...
print(f'[TASK304 Arm A beam=100] missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
out = {
    'task': 'task304_arm_a_d6_r_only_beam100',
    'ckpt_path': '${CKPT_PATH}',
    'code_path': '_t5_hrqvae_d6_arm_a_r_only.npy',
    'beam_size': 100,
    'config': config,
...
out_path = os.path.join(metrics_dir, 'task304_arm_a_d6_beam100_metrics.json')
with open(out_path, 'w') as f:
    json.dump(out, f, indent=2, default=float)
print(f'[TASK304 Arm A D6 beam=100] metrics written to {out_path}')
print(f'[TASK304 Arm A D6 beam=100] elapsed={elapsed:.1f}s')
print(f'[TASK304 Arm A D6 beam=100] Recall@5/10/20 = {[avg_recalls["Recall@5"], avg_recalls["Recall@10"], avg_recalls["Recall@20"]]}')
print(f'[TASK304 Arm A D6 beam=100] NDCG@5/10/20 = {[avg_ndcgs["NDCG@5"], avg_ndcgs["NDCG@10"], avg_ndcgs["NDCG@20"]]}')
PYEOF
echo "===== [Task #304 Arm A D6 beam=100] completed at $(date) =====" | tee -a "$LOG"
