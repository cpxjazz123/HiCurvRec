#!/usr/bin/env python3
"""Task #453 / Issue #161 [方向C Gate4] 第二份独立 200 epoch run (seed=43, GPU 1) + Stage 4 R@K 双复跑.

Issue #161 spec Gate 4 强制:
- 外部产生两次独立正式run (Task #450 seed=42 GPU 0 / Task #453 seed=43 GPU 1)
- 完整六项指标: R@5/10/20 + NDCG@5/10/20 (baseline R@10=0.1020)
- Target reached: test R@10 > 0.1020
- 不得更换 SID / 数据切分 / evaluator / baseline; 不得使用 10 epoch sanity 结果冒充最终指标

R18 4 维度 vs Task #450: seed 不同 → 初始化不同 → ckpt SHA256 不同
...
os.environ["TRITON_CACHE_DIR"] = "/home/wlia0047/.triton/cache_task453"
...
PRODUCT_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/products/task453_issue161_gate4_run2_seed43")
PRODUCT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = Path("/home/wlia0047/ar57/wenyu/GeneRec/logs")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_PATH = LOG_DIR / "task453_issue161_gate4_run2_seed43.log"
