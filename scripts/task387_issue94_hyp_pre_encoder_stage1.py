#!/usr/bin/env python3
"""
Task #387 / Issue #94 [方向A Gate1] HypPreEncoder + κ-Stereographic 距离公式准入验证 (R22 + R19, GPU 0)

FreeCurvHRQVAEWithHypPre 50 epoch Stage 1 训练:
- 接入 HypPreEncoder (task334 PASS) BEFORE encoder (输入 768d → expmap0 → encoder)
- 跟 task334 HRQVAEWithHypPre 同架构, 但 base 是 FreeCurvHRQVAE (不是 baseline HRQVAE)
- 50 epoch + per-step trace
- 验证 step1 不再 collapse_50%, final util ≥90%, collision ≤0.20

Issue #94 Gate1 PASS 条件:
  - step1 max_load < 50% (跟 task384 #91 FAIL 的 step1 max_load=82.56% 对比)
  - final L0/L1/L2 util ≥ 90% (跟 task384 1.56%/0.78%/0.39% 对比)
  - SID collision rate ≤ 0.20
"""
import os, sys, time, json, torch, numpy as np, pandas as pd
from pathlib import Path

sys.path.insert(0, '/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec')
sys.path.insert(0, '/home/wlia0047/ar57/wenyu/GeneRec/scripts')
os.environ['CUDA_VISIBLE_DEVICES'] = '0'
DEVICE = "cuda:0"

from model.hrqvae_free_curv import FreeCurvHRQVAE
from task334_issue43_gate2a_hyp_pre_encoder import HypPreEncoder, expmap0

# === 0. config ===
TASK_NAME = "task387_issue94_hyp_pre_encoder_stage1"
SEED = 42
NUM_EMB_LIST = [64, 128, 256]
E_DIM = 32
M = 3
KAPPA_MAX = 2.0
EPOCHS = 50
LOG_EVERY_STEPS = 50
BATCH = 64
LR = 1e-3
DATASET_PATH = "/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/item_emb.parquet"
OUT_DIR = Path(f"/home/wlia0047/ar57/wenyu/GeneRec/products/{TASK_NAME}")
OUT_DIR.mkdir(parents=True, exist_ok=True)
TRACE_FILE = OUT_DIR / "trace_per_step.jsonl"
CKPT_PATH = OUT_DIR / "hyp_pre_encoder_stage1_ckpt.pt"

torch.manual_seed(SEED); np.random.seed(SEED)
print("=" * 70)
print(f"Task #387 / Issue #94 [方向A Gate1] HypPreEncoder κ-Stereographic Stage 1")
print(f"Device: {DEVICE}, Epochs: {EPOCHS}, K: {NUM_EMB_LIST}, HypPre c=0.74 (Issue #43 Task #70)")
print("=" * 70)

# === 1. 数据 ===
df = pd.read_parquet(DATASET_PATH)
emb_col = 'embedding' if 'embedding' in df.columns else 'emb'
X = np.stack([np.asarray(e, dtype=np.float32) for e in df[emb_col]])
X = X / (np.linalg.norm(X, axis=-1, keepdims=True) + 1e-8) * 0.85
X_t = torch.tensor(X, dtype=torch.float32).to(DEVICE)
print(f"[数据] shape={X_t.shape}")

# === 2. FreeCurvHRQVAEWithHypPre wrapper (复刻 task334 但 base 是 FreeCurvHRQVAE) ===
class FreeCurvHRQVAEWithHypPre(torch.nn.Module):
    def __init__(self, base_hrqvae, c=0.74, enabled=True):
        super().__init__()
        self.base = base_hrqvae
        self.hyp_pre = HypPreEncoder(c=c, enabled=enabled)

    def forward(self, x, use_sk=False):
        # R-FIX: kmeans_init 在 first forward 时调用 init_emb, 必须 train mode + full dataset
        # 先触发 init_emb (我们 wrapper 不能改 base 的 forward, 所以手动调用一次)
        x_hyp = self.hyp_pre(x)
        return self.base(x_hyp, use_sk=use_sk)

base = FreeCurvHRQVAE(
    in_dim=X.shape[1], num_emb_list=NUM_EMB_LIST, e_dim=E_DIM, M=M,
    kappa_max=KAPPA_MAX, layers=[512, 256, 128],
    loss_type='poincare', quant_loss_weight=1.0, beta=0.25,
    kmeans_init=True, kmeans_iters=10,  # kmeans init + HypPreEncoder 联合修复
    sk_eps=[0.0, 0.0, 0.0], sk_iters=100,
).to(DEVICE)
model = FreeCurvHRQVAEWithHypPre(base, c=0.74, enabled=True).to(DEVICE)
print(f"[模型] {sum(p.numel() for p in model.parameters()):,} params (FreeCurvHRQVAE + HypPreEncoder)")

# R-FIX: 触发 kmeans_init on full dataset (n=9922 ≥ max K=256) in train mode
print("[R-FIX] 触发 kmeans_init on full dataset (train mode, HypPreEncoder enabled)")
with torch.no_grad():
    model.train()
    _ = model(X_t, use_sk=False)  # 触发 init_emb
print(f"[R-FIX] kmeans_init 完毕")

# === 3. 训练 + per-step trace ===
optim = torch.optim.Adam(model.parameters(), lr=LR)
N = X_t.shape[0]
steps_per_epoch = (N + BATCH - 1) // BATCH
print(f"[steps_per_epoch] {steps_per_epoch}, total_steps={steps_per_epoch * EPOCHS}")

TRACE_FILE.unlink(missing_ok=True)
collapse_step_50pct = [None, None, None]

def compute_trace(model, X_t, step):
    model.eval()
    with torch.no_grad():
        z_e = model.base.encoder(model.hyp_pre(X_t))
        traces = []
        for l_id, layer in enumerate(model.base.hrq.vq_layers):
            latent = z_e.view(-1, layer.e_dim)
            cb = layer.embeddings.weight
            d = layer._per_component_dist_sq(latent, cb)
            indices = torch.argmin(d, dim=-1)
            n_unique = len(set(indices.cpu().numpy().tolist()))
            util = n_unique / cb.shape[0]
            counts = torch.bincount(indices, minlength=cb.shape[0])
            max_load = counts.max().item() / N
            cb_norm = cb.norm(dim=-1).mean().item()
            traces.append({
                "layer_id": l_id,
                "cb_norm": cb_norm,
                "util": util,
                "max_load": max_load,
                "n_unique": n_unique,
            })
            if max_load > 0.5 and collapse_step_50pct[l_id] is None:
                collapse_step_50pct[l_id] = step
    model.train()
    return traces

start_time = time.time()
for ep in range(EPOCHS):
    print(f"\n[ep {ep}/{EPOCHS}]")
    model.train()
    perm = torch.randperm(N)
    for i in range(0, N, BATCH):
        idx = perm[i:i+BATCH]
        x_b = X_t[idx]
        optim.zero_grad()
        out, qloss, _ = model(x_b, use_sk=False)
        recon_loss = torch.nn.functional.mse_loss(out, x_b)
        loss = recon_loss + qloss
        loss.backward()
        optim.step()
        step = ep * steps_per_epoch + (i // BATCH) + 1
        if step % LOG_EVERY_STEPS == 0 or step == 1:
            traces = compute_trace(model, X_t, step)
            for l_tr in traces:
                l_tr['step'] = step
                l_tr['epoch'] = ep
                l_tr['loss'] = loss.item()
                with open(TRACE_FILE, 'a') as f:
                    f.write(json.dumps(l_tr, default=str) + '\n')
            elapsed = time.time() - start_time
            utils_str = ' '.join(f"L{l_tr['layer_id']}=util{l_tr['util']*100:.1f}%maxload{l_tr['max_load']*100:.1f}%" for l_tr in traces)
            print(f"  step {step}/{steps_per_epoch * EPOCHS} ep{ep} loss={loss.item():.4f} {utils_str} elapsed={elapsed:.0f}s")

# R12: ckpt save
if CKPT_PATH.exists():
    CKPT_PATH.unlink()
torch.save(model.state_dict(), CKPT_PATH)
print(f"\n[R12 ckpt] saved to {CKPT_PATH}")

# === 4. 总结 + final eval ===
print(f"\n[坍缩点定位]")
for l_id in range(M):
    print(f"  L{l_id}: collapse_50% step = {collapse_step_50pct[l_id]}")

model.eval()
with torch.no_grad():
    final_utils = []
    final_collisions = []
    z_e = model.base.encoder(model.hyp_pre(X_t))
    residual = z_e
    for layer_id, layer in enumerate(model.base.hrq.vq_layers):
        z_q, _, _ = layer(residual, use_sk=False)
        residual = residual - z_q
    for layer_id, layer in enumerate(model.base.hrq.vq_layers):
        z_q_full = model.base.encoder(model.hyp_pre(X_t))
        d = layer._per_component_dist_sq(z_q_full.view(-1, layer.e_dim), layer.embeddings.weight)
        idx = torch.argmin(d, dim=-1)
        n_unique = len(set(idx.cpu().numpy().tolist()))
        final_utils.append(n_unique / layer.embeddings.weight.shape[0])

# === 5. evidence ===
all_traces = []
with open(TRACE_FILE, 'r') as f:
    for line in f:
        all_traces.append(json.loads(line))

evidence = {
    "task_id": "task387",
    "issue": "#94",
    "epochs": EPOCHS,
    "fix_applied": "HypPreEncoder + κ-Stereographic (c=0.74 from Issue #43 Task #70) + kmeans_init=True",
    "collapse_step_50pct": collapse_step_50pct,
    "final_util_per_layer": final_utils,
    "trace_records": len(all_traces),
    "verdict_path": "verdicts/task387_issue94_hyp_pre_encoder_stage1_v2.md",
}
with open(OUT_DIR / "evidence_package.json", "w") as f:
    json.dump(evidence, f, indent=2, default=str)
print(f"\n[证据] saved to {OUT_DIR / 'evidence_package.json'}")

# === 6. Issue #94 Gate 1 PASS 判定 ===
step1_traces = [t for t in all_traces if t['step'] == 1]
max_load_step1 = max(t['max_load'] for t in step1_traces) if step1_traces else 0.0
util_all_pass = all(u >= 0.9 for u in final_utils)
checks = [
    ("step1 max_load < 50% (vs #91 FAIL 82.56%)", max_load_step1 < 0.5),
    ("L0 final util >= 90% (vs #91 FAIL 1.56%)", final_utils[0] >= 0.9),
    ("L1 final util >= 90% (vs #91 FAIL 0.78%)", final_utils[1] >= 0.9),
    ("L2 final util >= 90% (vs #91 FAIL 0.39%)", final_utils[2] >= 0.9),
    ("ckpt_saved", CKPT_PATH.exists()),
    ("no_nan_inf", all(np.isfinite(t['loss']) and np.isfinite(t['util']) for t in all_traces)),
    ("trace_records >= 100", len(all_traces) >= 100),
]
n_pass = sum(1 for _, p in checks if p)
print(f"\n[sanity] PASS: {n_pass}/{len(checks)}")
for name, ok in checks:
    print(f"  [{'PASS' if ok else 'FAIL'}] {name}")

print(f"\n[Issue #94 Gate 1 决策]")
print(f"  Gate 1: {'✅ PASS' if n_pass == len(checks) else '❌ FAIL'}")
print(f"  step1 max_load = {max_load_step1*100:.2f}% (目标 <50%, 跟 #91 82.56% 对比)")
print(f"  final_util L0/L1/L2 = {final_utils[0]*100:.2f}% / {final_utils[1]*100:.2f}% / {final_utils[2]*100:.2f}%")