#!/usr/bin/env bash
# Step 3 v12: angular_dim=16, radial_dim=18. 16D hyp 角带宽 √(16/64)=0.5 (baseline 70%, 最可能达成).
# 其他 v10 配置: β=0.5 + bs=256 + w_div=100 + w_angular=10 + 100 epoch.
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_v12_angdim16
...
COMMON="\
  ...
  --beta 0.5 \
  --product_manifold --angular_dim 16 --radial_dim 18 \
...
echo "==== Step 3 v12: angular_dim=16 + radial_dim=18 (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
DISABLE_USAGE_KILL=1 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_v12 \
PYTHONUNBUFFERED=1 \
