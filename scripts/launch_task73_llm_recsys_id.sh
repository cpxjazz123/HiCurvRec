#!/bin/bash
# Task #73 — LLM-RecSys-ID training for SID + CID baselines on Musical_Instruments
# Uses transformers 4.51.3 + tokenizers 0.21.4 via PYTHONPATH override (genrec_env has 5.x)
# Run on GPU 2 (ETEGRec on GPU 0, SASRec retrain on GPU 1)
set -e
...
# === CID (Collaborative Indexing) ===
LOG_FILE="$LOG_DIR/task73_llm_recsys_id_cid.log"
echo "===== [Task #73] CID started at $(date) =====" | tee "$LOG_FILE"
...
python -u main.py \
    ...
    --gpu '0' \
    --logging_step 1000 \
    --logging_dir 'log/task73_instruments_CID.log' \
    --model_dir '/home/wlia0047/ar57/wenyu/GeneRec/products/task73/llm_recsys_id_cid.pt' \
...
echo "===== [Task #73] CID completed at $(date) =====" | tee -a "$LOG_FILE"

# === SID (Sequential Indexing) ===
LOG_FILE="$LOG_DIR/task73_llm_recsys_id_sid.log"
echo "===== [Task #73] SID started at $(date) =====" | tee "$LOG_FILE"
