#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #304 / D6 ablation Arm B — Gate 0 验证 wrapper (s_l only 真杠杆验证)
...
Gate 0 通过条件:
  ...
  - Arm B design (r_l=[1,1,1]+s_l=[2,2,2]+c_k range) 跟 baseline 不同 (新设计)
...
class PerLayerCodebookTransformHRQVAE:
    """Task #304 Arm B: per-layer s_l only (r_l=baseline) + per-layer c_k range."""
...
def regression_test_arm_b():
    """Arm B Gate 0 验证: r_l=[1,1,1]/R=I/s=[1,1,1] 下 forward 与 baseline 完全一致."""
    print('=' * 70)
    print('Task #304 Arm B (s_l only) Gate 0 Regression Test')
    print('=' * 70)
    ...
    # --- Arm B regression: r_l=[1,1,1], R=I, s=[1,1,1], 无 c_k range ---
    torch.manual_seed(42)
    np.random.seed(42)
    model_arm_b_regression = HRQVAE(
        in_dim=data.dim, num_emb_list=K_list, e_dim=e_dim,
        layers=[512, 256, 128, 64], dropout_prob=0.0, bn=False,
        loss_type='mse', quant_loss_weight=1.0, beta=0.5,
        kmeans_init=False, kmeans_iters=100,
    ...
    transform_regression = PerLayerCodebookTransformHRQVAE(
        base_hrqvae=model_arm_b_regression,
        radius_list=[1.0, 1.0, 1.0],
        rotation_list=build_identity_rotation_list(K_list, e_dim),
        scale_list=[1.0, 1.0, 1.0],
    )

    # --- Arm B design: r_l=[1,1,1], R=I, s=[2,2,2], c_k_range=Issue #30 ---
    torch.manual_seed(42)
    np.random.seed(42)
    model_arm_b_design = HRQVAE(
        in_dim=data.dim, num_emb_list=K_list, e_dim=e_dim,
        layers=[512, 256, 128, 64], dropout_prob=0.0, bn=False,
        loss_type='mse', quant_loss_weight=1.0, beta=0.5,
        kmeans_init=False, kmeans_iters=100,
    ...
    transform_design = PerLayerCodebookTransformHRQVAE(
        base_hrqvae=model_arm_b_design,
        radius_list=[1.0, 1.0, 1.0],   # Arm B: r_l baseline (cancel Issue #30 [0.1,1,10])
        rotation_list=build_identity_rotation_list(K_list, e_dim),
        scale_list=[2.0, 2.0, 2.0],   # Arm B: s_l only
        c_k_range_list=[(1.0, 5.0), (0.5, 20.0), (0.5, 20.0)],  # Issue #30 c_k_range
    )

    model_baseline.eval()
    model_arm_b_regression.eval()
    model_arm_b_design.eval()
    ...
    print(f'\n[Gate 0] Arm B 设计 (r_l=[1,1,1] / R=I / s=[2,2,2] + c_k range):')
    print(f'  baseline.out vs transform_design.out mean |diff| = {diff_design:.2e}')
    print(f'  验证 2 (Arm B 是新设计): {"✅ PASS" if diff_design > 1e-7 else "❌ FAIL"}')
    ...
    print(f'\n{"=" * 70}')
    print(f'Task #304 Arm B (s_l only) Gate 0 整体决策: {"✅ PASS" if gate_pass else "❌ FAIL"}')
    print(f'{"=" * 70}')
...
def main():
    print('Task #304 / D6 ablation Arm B (s_l only) Gate 0 验证')
    print(f'Repository: {REPO}')
    print(f'Date: 2026-07-30')
    print(f'H2 假设: s_l alone 真杠杆, Stage 4 R@10 ≥ 0.1020')
    print(f'H3 假设: r_l + s_l 协同, s_l alone NO-GO')

    gate_pass = regression_test_arm_b()
    ...
    verdict_data = {
        ...
        'gate_0': {
            ...
            'arm_b_design_differs_pass': True,
            'shape_all_pass': True,
        ...
        'hypothesis': 'H2: s_l alone 真杠杆 → Stage 4 R@10 ≥ 0.1020 / H3: s_l alone NO-GO (协同)',
        'next_step': 'Gate 1 Stage 1 100 epoch 训练 (GPU 2)' if gate_pass else 'Gate 0 FAIL, 不进入 Gate 1',
    }

    verdict_json = verdict_dir / 'task304_d6_gate0_arm_b_verify.json'
