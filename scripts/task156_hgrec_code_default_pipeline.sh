...
# 2026-07-24 (用户 2026-07-24 20:35 提议: 审计 HG-Rec 代码, 用真实 recipe 重训)
#
# 关键发现: HG-Rec/gen_codebook.py:46-48 hardcoded:
#   dataset = "Games"
#   ckpt_path = ./ckpt/{dataset}/Nov-24-2025_14-24-21_beta_0.250_codebook_[32,64,256]_sk_0.500/epoch_1869_collision_0.2345_model.pth
#
# vs Task #84 baseline:
#   β: 1.0 → 0.250 (4× 缩小)
#   num_emb_list: [64,128,256] → [32,64,256] (L0 2× 缩小, L2 同)
#   sk_epsilons: [0,0,0] → [0.5] (sinkhorn regularization)
#   epochs: 1000 → 1869 (训练时长 +87%)
#
# R7 GPU: GPU 3 (等 Task #155 完成释放)
# 预计: Stage 1 ~45 min + Stage 2 ~3 min + Stage 3 ~140 min + Stage 4 ~30 sec = ~3.2h
...
CKPT_DIR=/home/wlia0047/ar57/wenyu/GeneRec/products/task156/ckpt
mkdir -p "$CKPT_DIR" /home/wlia0047/ar57/wenyu/GeneRec/products/task156

TS=$(date +%Y%m%d_%H%M%S)
LOG_FILE="$LOG_DIR/code_default_stage1_${TS}.log"

cd /home/wlia0047/ar57/wenyu/GeneRec/HG-Rec

echo "===== [Task #156] HG-Rec code-default recipe Stage 1 launched at $(date) =====" | tee "$LOG_FILE"
echo "Recipe 来源: gen_codebook.py:46-48 (HG-Rec authors hardcoded)" | tee -a "$LOG_FILE"
echo "唯一改动 vs Task #84: β 1.0 → 0.25, [64,128,256] → [32,64,256], sk [0,0,0] → [0.5], epochs 1000 → 1869" | tee -a "$LOG_FILE"
...
# Stage 1: HRQ-VAE training (code-default recipe)
...
STAGE1_EXIT=$?
echo "===== [Task #156] Stage 1 exit code: $STAGE1_EXIT at $(date) =====" | tee -a "$LOG_FILE"
...
# Stage 2: SID codebook (类似 Task #153/154 写法)
...
print(f'[Stage 2] β={args[\"beta\"]}, num_emb_list={args[\"num_emb_list\"]}, sk_epsilons={args[\"sk_epsilons\"]}, epoch={ckpt.get(\"epoch\", -1)}')
...
echo "✅ Task #156 Stage 1+2 完成. Stage 3+4 待运行 (见 task156_hgrec_stage3_train.sh)" | tee -a "$LOG_FILE"