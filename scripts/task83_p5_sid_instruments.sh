#!/bin/bash
# Task #83 — P5-SID 训练 (paper Table 2 baseline #13, R@10 paper=0.0438)
# 基于 LLM-RecSys-ID Wenyueh 原作者 repo + instruments remapped_sequential indexing
# GPU 2 单卡
...
LOG_FILE="$LOG_DIR/task83_p5_sid_${TS}.log"

mkdir -p "$LOG_DIR"

echo "===== [Task #83] P5-SID training launched at $(date) =====" | tee "$LOG_FILE"
...
echo "Target: R@10 ≥ 0.0438 (paper Musical_Instruments P5-SID)" | tee -a "$LOG_FILE"
...
python3 main.py \
    ...
    --logging_dir /home/wlia0047/ar57/wenyu/GeneRec/logs/task83_p5_sid_${TS}.log \
...
echo $TRAIN_PID > /home/wlia0047/ar57/wenyu/GeneRec/products/task83/_TRAINING_PID
echo "[$(date +%H:%M:%S)] task83 training PID: $TRAIN_PID" | tee -a "$LOG_FILE"
wait $TRAIN_PID
echo "===== [Task #83] P5-SID training completed at $(date) =====" | tee -a "$LOG_FILE"
rm -f /home/wlia0047/ar57/wenyu/GeneRec/products/task83/_TRAINING_PID
