#!/bin/bash
# Task #171 Stage 4 — T5-mini 9.18M + κ-Stereo + dead_code_reset_every=10 SID test eval
...
export TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task171_s4

LOG_DIR=/home/wlia0047/ar57/wenyu/GeneRec/logs/task171
TS=$(date +%b-%d-%Y_%H-%M-%S | sed 's/^.../\L&/')
LOG_FILE=$LOG_DIR/stage4_eval_${TS}.log
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task171_kappa_dead_code_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #171 Stage 4] T5-mini 9.18M + κ-Stereo + dead_code_reset test eval launched at $(date) =====" | tee $LOG_FILE

BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task171/t5mini_kappa_dead_code/*/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
if [ -z "$BEST_CKPT" ] || [ ! -f "$BEST_CKPT" ]; then
    echo "❌ Stage 3 best ckpt MISSING for #171" | tee -a $LOG_FILE
    exit 1
fi
...
model = HG_Rec(config)
print(f'[Task #171 Stage 4] Loading best ckpt...', flush=True)
model.load_state_dict(torch.load('$BEST_CKPT', map_location='cpu'))
model.to(device)
print(f'[Task #171 Stage 4] Model loaded on {device}', flush=True)
print(f'[Task #171 Stage 4] Total params: {sum(p.numel() for p in model.parameters()):,}', flush=True)
...
print(f'[Task #171 Stage 4] Test dataset size: {len(test_dataset)}', flush=True)

avg_recalls, avg_ndcgs = evaluate(model, test_dataloader, config['topk_list'], config['beam_size'], device)

result = {
    'task': 'task171_t5mini_kappa_dead_code',
    'recipe': 'T5-mini 9.18M + κ-Stereographic + dead_code_reset_every=10 SID + 200 epoch',
    'best_ckpt': '$BEST_CKPT',
    'beam_size': 20,
...
print(f'[Task #171 Stage 4] Saved: $RESULT_JSON', flush=True)
print(f'[Task #171 Stage 4] Test recalls: {avg_recalls}', flush=True)
print(f'[Task #171 Stage 4] Test NDCGs: {avg_ndcgs}', flush=True)
PYTHON_EOF

EXIT_CODE=${PIPESTATUS[0]}
echo "===== [Task #171 Stage 4] exit: $EXIT_CODE at $(date) =====" | tee -a $LOG_FILE
[ $EXIT_CODE -ne 0 ] && exit 1

python3 -c "
import json
d = json.load(open('$RESULT_JSON'))
print('=== Task #171 Test Results (T5-mini 9.18M + κ-Stereo + dead_code_reset) ===')
