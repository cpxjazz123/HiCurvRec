#!/usr/bin/env bash
# Step 3 闭环 v2: 50 epoch EMA + radius spread + w_div=1.0 (强制方向多样性).
# 配置 = Step 2 + EMA (utils.py) + 新增 --w_div 1.0 (防止 encoder 坍缩).
# 预期: encoder 方向多样, 半径 std > 0.05, 50 epoch 跑完不触发 util<20% kill.
...
OUTDIR=$REPO/products/m_arm/m_radius_spread_step3_50ep_wdiv
...
COMMON="\
  ...
  --w_ent 0.1 --w_div 1.0 --anti_collapse dead_revive \
  --r_spread_list 0.3 0.3 0.3 \
  --eval_step 5 --num_workers 0 \
  --device cuda:0 \
  --ckpt_dir $OUTDIR"

echo "==== Step 3 v2: 50 epoch EMA + w_div=1.0 (GPU=$GPU) ===="
echo "log → $LOG"
cd "$REPO"
LR_LOG_R=10.0 \
CUDA_VISIBLE_DEVICES=$GPU \
TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_radius_spread_step3_wdiv \
PYTHONUNBUFFERED=1 \
