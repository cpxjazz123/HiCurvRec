#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #223 — Stage 2 SID inference fork.

Mirror of scripts/task211_C1_stage2_codebook.py with 2 changes:
  - ckpt_path: task222/hrqvae_pck_replay/.../best_collision_model.pth (Task #222 ep29 healthy ckpt)
  - output_path: Instruments/Instruments_t5_rqvae_task223_pck.npy
  - pass assignment_mode / c_k_min / c_k_max / c_k_seed kwargs to HRQVAE instantiation
"""
...
def main():
    ...
    log = logging.getLogger("task223_stage2")
    ...
    log.info(f"  assignment_mode={ckpt_args.get('assignment_mode', 'shared')}, "
             f"c_k=[{ckpt_args.get('c_k_min', '?')}, {ckpt_args.get('c_k_max', '?')}]")
    ...
    # HRQVAE instantiation — 透传 per_codeword_kappa 系列参数
    model = HRQVAE(
        in_dim=data.dim,
    ...
    for d in tqdm(data_loader, desc="Task #223 Stage 2 forward"):
        d = d.to(device)
    ...
    diag = {
        "arm": "Task #223 (Per-Codeword κ healthy ckpt)",
        "task": "task223_stage2_sid_inference",
