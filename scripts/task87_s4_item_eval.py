"""Task #87 Stage 4 item-level R@5/R@10/NDCG@5/NDCG@10 evaluator.

Identical algorithm to scripts/task388v4_s4_item_eval.py but bound to the
GeneRec repo layout: REPO_ROOT, configs/, TOYS_DATA_DIR, paths.root_dir, etc.

Usage:
    python scripts/task87_s4_item_eval.py \\
        --variant task87_tiger_baseline \\
        --constrained_pt <top-10 SID predictions tensor> \\
        --sid <sid_dedup.pt (4, 11924)> \\
        --ckpt <best TIGER ckpt> \\
...
def evaluate(variant: str, constrained_pt: str, sid_path: str, ckpt_path: str) -> dict:
    ...
    assert pred.dim() == 3 and pred.shape[2] == NUM_HIERARCHIES, f"bad shape {pred.shape}"
