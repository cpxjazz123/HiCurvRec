#!/bin/bash
# Task #201 Stage 1 臂 B — exp_global with θ_init=log(10) (用户拍板选项 A)
#
# 跟 #199 臂 B 唯一区别: theta_init=2.3026 (c_init=10.0) 而非 #199 默认 0.0 (c_init=1.0).
# 验证用户预测: θ_init=log(10) 后 θ 学起来仍是 > log(10) (c > 10) 而非 #199 学到 0.8 (向下).
#
...
# 变量 (vs #199 B): theta_init=log(10)=2.3026
# GPU 1
...
GPU=1
SAVE_DIR=$REPO/products/task201/stage1_arm_B_exp_global_init_log10
LOG_FILE=$REPO/logs/task201/stage1_arm_B.log
rm -f $LOG_FILE

THETA_INIT=$(python3 -c "import math; print(math.log(10))")
echo "[$(date)] === #201 臂 B: exp_global, θ_init=log(10)=$THETA_INIT (c_init=10.0), GPU=$GPU, epochs=1000 ===" | tee -a $LOG_FILE

CUDA_VISIBLE_DEVICES=$GPU TRITON_CACHE_DIR=/home/wlia0047/.triton/cache_task201_arm_B \
python3 -u $REPO/HG-Rec/train_hrqvae.py \
    ...
    > $LOG_FILE 2>&1 &
TRAIN_PID=$!
echo $TRAIN_PID > $REPO/products/task201/_TRAINING_PID_arm_B
