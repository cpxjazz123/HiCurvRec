#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Task #178 Stage 2 fork: 用 vanilla HRQVAE 加载 fixed baseline ckpt (Phase 0 修复后).

Mirror of scripts/task175_orc_locked_stage2_codebook.py:
  - import HRQVAE (vanilla) instead of ORCLockedHRQVAE (κ-locked variant)
  - num_emb_list = [64, 128, 256] (论文 Table 6 Instruments 原配)
  - beta = 0.5 (论文 Table 6)
  - sk_epsilons = [0.003, 0.003, 0.003] (Phase 0.2 打开)
  - 4 层 → 后处理用 3rd digit 做 4th-digit dedup (HG-Rec baseline pattern)
"""
...
def main():
    ...
    parser.add_argument("--bn", type=bool, default=True,
                        help="Match Stage 1 ckpt: argparse type=bool quirk makes `--bn False` "
                             "actually True, so default True + Stage 1 actual config")
    parser.add_argument("--data_path", type=str,
                        default="/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/item_emb.parquet")
    args = parser.parse_args()

    device = torch.device(args.device)
    log = logging.getLogger("task178_stage2")
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(message)s')

    log.info(f"Loading ckpt: {args.ckpt_path}")
    # NOTE: weights_only=False because ckpt may be full dict
    ...
    # Build HRQVAE (vanilla, paper Table 6 config)
    model = HRQVAE(
        in_dim=data.dim,
        ...
        curvature_list=None,  # upstream default [1.0, 1.0, 1.0]
    )
    ...
    for d in tqdm(data_loader, desc="Task #178 Stage 2 forward"):
        d = d.to(device)
    ...
    for value in all_indices_dict.values():
        ...
        codes.append(code)
    codes_array = np.array(codes)  # shape (N, n_layers) = (N, 3) for num_emb_list=[64,128,256]
    ...
    if len(duplicates) > 0:
        ...
        # HG-Rec baseline: append a 4th column for dedup (overwrite L3 dummy)
        if codes_array.shape[1] == 3:
            # num_emb_list=[64,128,256] gives 3 cols, need to extend to 4 for HG_Rec
    ...
    for li in range(n_total_cols):
        ...
        log.info(f"  Layer {li} utilization: {len(unique_codes_li)} / {cap} "
                 f"({100 * len(unique_codes_li) / (args.num_emb_list[li] if li < n_layers else 1):.1f}%)")
