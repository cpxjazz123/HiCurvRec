#!/bin/bash
# Task #162 Stage 2 — paper-faithful free-curv SID inference (CPU)
# Fork of task137_stage2_codebook_free_curv.py logic (with R137 fix)
# Stage 1 ckpt: products/task162/rqvae_free_curv/<ts>/best_loss_model.pth
# 输出: HG-Rec/dataset/Instruments/Instruments_t5_rqvae_paper_free_curv.npy (N, 4)
...
STAGE1_DIR=$(ls -td /home/wlia0047/ar57/wenyu/GeneRec/products/task162/rqvae_free_curv/*/ | head -1)
...
LOG_FILE=$LOG_DIR/stage2_inference_${TS}.log

OUT_DIR=/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments
mkdir -p $LOG_DIR $OUT_DIR

OUT_NPY=$OUT_DIR/Instruments_t5_rqvae_paper_free_curv.npy

echo "===== [Task #162 Stage 2] paper-faithful free-curv SID inference launched at $(date) =====" | tee $LOG_FILE
...
meta = {
    'task': 'Task #162 paper-faithful free-curv Stage 2',
    'ckpt_path': ckpt_path,
...
echo "===== [Task #162 Stage 2] paper-free-curv SID inference completed at $(date), exit=$EXIT_CODE =====" | tee -a $LOG_FILE
