#!/bin/bash
# Task #161 Stage 4 — T5-mini 9.18M d_kv fix test eval
# Fork of task159 stage4_eval, target = products/task161/...
# 输出: verdicts/task161_t5mini_12m_metrics.json
...
RESULT_JSON=/home/wlia0047/ar57/wenyu/GeneRec/verdicts/task161_t5mini_12m_metrics.json

mkdir -p $LOG_DIR

echo "===== [Task #161 Stage 4] T5-mini 9.18M d_kv fix test eval launched at $(date) =====" | tee $LOG_FILE
echo "Recipe: T5-mini 9.18M d_kv fix + code_default SID" | tee -a $LOG_FILE
echo "GPU 1" | tee -a $LOG_FILE
echo "Output JSON: $RESULT_JSON" | tee -a $LOG_FILE

export RESULT_JSON

BEST_CKPT=$(ls -t /home/wlia0047/ar57/wenyu/GeneRec/products/task161/ckpt_hgrec/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
...
result = {
    'task': 'task161_t5mini_dkv_fix',
    'recipe': 'T5-mini 9.18M d_kv fix (heads 6→4) + code_default SID (β=0.25, [32,64,256], sk=0.5)',
    't5_config': '4 enc + 4 dec, d_model=256, d_ff=1024, 4 heads × d_kv=64 (4×64=256 strict)',
...
echo "===== [Task #161 Stage 4] Test eval completed at $(date), exit code: $EXIT_CODE =====" | tee -a $LOG_FILE
