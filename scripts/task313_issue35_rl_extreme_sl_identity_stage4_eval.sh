#!/usr/bin/env bash
# Task #313 — Stage 4 eval on r_l=[0.1,1,10] + s_l identity ckpt + beam=50
# 目的: 验证 r_l 是不是真杠杆 (vs task312 反向 orthogonal ablation)
# K14 最优: beam_size = 50 (Stage 4 inference protocol 层唯一杠杆)
# R11.5: 等 Stage 3 完成后启动 (Stage 3 ETA ~90min)
...
CKPT_PATH=$(ls -td $REPO/products/task313/ckpt_hgrec_issue35_rl_extreme_sl_identity/Instruments/*/HG_Rec_best.pth 2>/dev/null | head -1)
if [ -z "$CKPT_PATH" ]; then
    echo "ERROR: no ckpt found at $REPO/products/task313/ckpt_hgrec_issue35_rl_extreme_sl_identity/Instruments/*/HG_Rec_best.pth"
...
echo "===== [Task #313 / Issue #35 Gate 4 Stage 4 eval] beam=50 launched at $TS =====" | tee "$LOG"
echo "code_path = _t5_hrqvae_issue35_rl_extreme_sl_identity.npy" | tee -a "$LOG"
echo "ckpt = $CKPT_PATH" | tee -a "$LOG"
echo "GPU 2 (R7 空闲, task309 GPU 1 / task312 GPU 0 / task313 Stage 3 GPU 3)" | tee -a "$LOG"
...
print(f'[TASK313] missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
out = {
    'task': 'task313_issue35_rl_extreme_sl_identity_stage4_eval',
    'ckpt_path': '${CKPT_PATH}',
    'code_path': '_t5_hrqvae_issue35_rl_extreme_sl_identity.npy',
    'beam_size': 50,
    'config': config,
...
out_path = os.path.join(metrics_dir, 'task313_rl_extreme_sl_identity_metrics.json')
with open(out_path, 'w') as f:
    json.dump(out, f, indent=2, default=float)
print(f'[TASK313] metrics written to {out_path}')
print(f'[TASK313] elapsed={elapsed:.1f}s')
print(f'[TASK313] Recall@5/10/20 = {[avg_recalls["Recall@5"], avg_recalls["Recall@10"], avg_recalls["Recall@20"]]}')
print(f'[TASK313] NDCG@5/10/20 = {[avg_ndcgs["NDCG@5"], avg_ndcgs["NDCG@10"], avg_ndcgs["NDCG@20"]]}')
PYEOF
echo "===== [Task #313 Stage 4 eval] completed at $(date) =====" | tee -a "$LOG"