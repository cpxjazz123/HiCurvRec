"""
task6_constrained_eval.py — 评估 task4 已产出的 3 个 constrained inference tensor。

不需要 model.generate()，直接 load (19412, 10, 4) constrained tensor，
把它的 SID 序列当作 generated_ids，与 ground-truth 4-digit SID 对比。

Usage（每个算法运行一次）:
    cd /fs04/ar57/wenyu/GeneRec/GRID
    python /home/wlia0047/ar57/wenyu/GeneRec/scripts/task6_constrained_eval.py \\
        --algo rqvae \\
        --constrained_pt logs/inference/runs/2026-07-07/21-47-35/pickle/merged_predictions_tensor.pt \\
        ...
        --out_json /home/wlia0047/ar57/wenyu/GeneRec/task6_eval_rqvae.json
"""
...
@torch.no_grad()
def evaluate_constrained(algo: str, constrained_pt: str, sid_path: str, ckpt_path: str) -> dict:
    """Load constrained inference tensor, align with dataloader ground-truth, eval.

    Alignment strategy: USER_ID DIRECT. The merge function
    `merge_list_of_keyed_tensors_to_single_tensor` writes
    `output_tensor[user_id] = predictions`, so the tensor's row i is the
    prediction for the user whose id equals i. We index by user_id from the
    dataloader directly (no uid_to_row indirection).
    """
    # 1. Load constrained prediction tensor (shape (N, 10, 4))
    ...
    assert pred.dim() == 3 and pred.shape[2] == 4, f"bad shape {pred.shape}"
    ...
    # Synthetic rank-based score (top-1 highest); evaluator only uses it for
    # `preds.reshape(-1)`, so any strictly decreasing score preserves order.
    ...
    for batch_idx, batch in enumerate(predict_loader):
        ...
        kept_labels_t = labels_2d[per_row_indices]  # (K, 4)
