"""Task #144 Stage 2 — fork task84_hgrec_stage2_codebook.py for both arms.
把 FreeCurvHRQVAE trained ckpt → (N, 4) SID .npy 给 Stage 3 T5 训练.

Usage:
    python3 scripts/task144_stage2_codebook.py --arm A
    python3 scripts/task144_stage2_codebook.py --arm B
"""
import argparse
import collections
import glob
...
def main():
    ...
    arm_dir = "arm_A_phaseA_only" if args.arm == "A" else "arm_B_decouple"
    ckpt_path = f"/home/wlia0047/ar57/wenyu/GeneRec/products/task144/train/{arm_dir}/best_loss_model.pth"
    output_path = f"/home/wlia0047/ar57/wenyu/GeneRec/HG-Rec/dataset/Instruments/Instruments_t5_hrqvae_kappa_decouple_arm_{args.arm}.npy"

    device = torch.device(args.device)
    log = logging.getLogger(f"task144_stage2_arm_{args.arm}")
    ...
    for d in tqdm(data_loader, desc=f"Arm {args.arm} Stage 2 forward"):
        d = d.to(device)
    ...
    # Save diagnostic JSON next to output
